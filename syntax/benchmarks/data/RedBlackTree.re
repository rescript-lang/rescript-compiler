    ...
    if (successor.color === Red) {
      ...
      while (!break.contents) {
        ...
        | Some(successorParent) =>
          ...
          if (successorParent.color === Black
              && (
                sibling === None
                || (
...
let makeWith = (array, ~compare) => {
  ...
  array->(
           Js.Array2.forEach(((value, height)) =>
...
type nonrec oldNewVisible('value) = {
...
let onChangedVisible =
  ...
  let old = oldNewVisible.new_;
  let new_ = oldNewVisible.old;
  new_
