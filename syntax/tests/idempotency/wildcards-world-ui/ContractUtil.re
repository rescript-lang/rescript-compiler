let getProviderOrSigner =
    (library: Web3.web3Library, account: option(Web3.ethAddress)) => {
  switch (account) {
...
type arrayOfValues;
type sha3Result;
[@bs.send] external sha3ToString: (sha3Result, string) => string = "toString";
[@bs.module "ethereumjs-abi"]
external soliditySHA3: (array(string), arrayOfValues) => sha3Result =
  "soliditySHA3";
...
let constructMetaTransactionMessage: (string, string, string, string) => string =
  (nonce, chainId, functionSignature, contractAddress) => {
    "0x"
    ++ soliditySHA3(
