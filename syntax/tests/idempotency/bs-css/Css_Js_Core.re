...
let contentRules = xs =>
  D("content", xs->Belt.Array.map(string_of_content)->join(" "));
...
let countersSet = xs =>
  D("counter-set", xs->Belt.Array.map(string_of_counter_set)->join(" "));
...
let fontFamilies = xs =>
  D("fontFamily", xs->Belt.Array.map(FontFamilyName.toString)->join(", "));
...
let transforms = x =>
  D("transform", x->Belt.Array.map(Transform.toString)->join(" "));
...
let filter = x =>
  D("filter", x->Belt.Array.map(string_of_filter)->join(" "));
...
let boxShadows = x =>
  D("boxShadow", x->Belt.Array.map(Shadow.toString)->join(", "));
...
let textShadows = x =>
  D("textShadow", x->Belt.Array.map(Shadow.toString)->join(", "));
...
let transitionList = x =>
  D("transition", x->Belt.Array.map(Transition.toString)->join(", "));
...
let animations = x =>
  D("animation", x->Belt.Array.map(Animation.toString)->join(", "));
