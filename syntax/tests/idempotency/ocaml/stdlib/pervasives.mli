...
external exp : float -> float = "exp" [@@unboxed] [@@noalloc]
(** Exponential. *)

external log : float -> float = "log" [@@unboxed] [@@noalloc]
(** Natural logarithm. *)
...
external expm1 : float -> float = "caml_expm1"
  [@@unboxed] [@@noalloc]
...
external log1p : float -> float = "caml_log1p"
  [@@unboxed] [@@noalloc]
...
external cos : float -> float = "cos" [@@unboxed] [@@noalloc]
(** Cosine.  Argument is in radians. *)

external sin : float -> float = "sin" [@@unboxed] [@@noalloc]
(** Sine.  Argument is in radians. *)

external tan : float -> float = "tan" [@@unboxed] [@@noalloc]
...
external hypot : float -> float -> float = "caml_hypot"
  [@@unboxed] [@@noalloc]
...
external ldexp : (float [@unboxed]) -> (int [@untagged]) -> (float [@unboxed]) =
  "caml_ldexp_float_unboxed" [@@noalloc]
