...
Luckily, OCaml allows customized multiple-line string support, BuckleScript *reserves* the delimiter `js` and `j`(`j` is unused yet). 
...
Inside `js` delimiter, the escape convention is like JavaScript
...
[NOTE]
======
In general, `bs.uncurry` is recommended, and compiler will do lots of optimizations to resolve the `curry` to `uncurry` calling convention in the compile time. However, there are some cases the compiler optimizer could not do it, in that case, it will be converted runtime. 
...
There are two cases, we might want to do a name mangling for JS object method name.

First, in OCaml, some names are keywords, we want to add an underscore to make it not a syntax
error. 
...
convert nullable value to `option` which is a boxed value, but the compiler will 
...
When return type is `unit`: the compiler will append its return value 
with an OCaml `unit` literal to make sure it does return `unit`, its main purpose
is to make user consume FFI in idiomatic OCaml code, the cost is *very very small* and 
the compiler will do smart optimizations to remove it when the returned value is not used (mostly likely)
 
When return type is `bool`, the compiler will coerce its return value from 
JS boolean to OCaml boolean. The cost is also *very small* and compiler will remove
such coercison when it is not needed. Note even if your external FFI does return OCaml `bool` or `unit`, 
such implicit coercion will *cause no harm*.

`identity` will make sure that compiler will do nothing about returned value, it 
