...
Suppose we have a JS file which exports two properties: `height` and `width`:
...
#### Bindings to create simple JS object literal
...
#### A more flexible way to create JS object
...
#### JS property getter/setter and method application
...
-------------------------------------------------------------
<1> property get should not come with any argument, this will be guaranteed by the type checker
