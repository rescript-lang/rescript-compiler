...
* Create a directory called `hello` and create `bsconfig.json`, `package.json`
  as below:
...
User can  also run `watch` mode see changes instantly while editing.
...
* Create a directory `fib` and created files, `bsconfig.json`, `package.json`
+
[source,js]
.bsconfig.json
----
