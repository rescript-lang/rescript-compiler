...
You can download the example [here](https://github.com/bloomberg/bucklescript-addons/tree/master/examples/node-http-server):


As you can see, there is no name mangling in the generated code, the function `string_of_int` is from a normal OCaml module `Pervasives`, `Pervasives.string_of_int` is directly callable from other JavaScript code.

The attribute `[@bs]` allows BuckleScript to support JS's uncurried calling convention in addition to OCaml's curried calling convention.



## Disclaimer

It is in an *beta* stage, but we encourage you to try it and share your feedback.
...
1. Clone the bucklescript repo
  ...
  Note that you have to clone this project with the `--recursive` option,
...
2. Build the patched OCaml Compiler
  ...
  directory; check if `ocamlc.opt` and `ocamlopt.opt` are there, and then temporarily
...
3. Build BuckleScript Compiler
  ...
  Now that you have a binary called `bsc` in the `jscomp/bin` directory,
  put it in your `PATH`.
...
used, since we currently make this JS backend a plugin instead, but
it shows that it is easy to assemble a whole compler using OCaml
compiler libraries and based upon that, we can add more compilation flags for a
JS backend.

[stdlib](jscomp/stdlib) is copied from ocaml's [stdlib](ocaml/stdlib) to have it compiled with
the new JS compiler.

[test](jscomp/test) copies and adapts some modules from ocaml's [testsuite](ocaml/testsuite).
...
It tries to insert 1,000,000 keys into an immutable map and then query it.
