...
(** Sorting and merging lists.
    ...
    sorting functions in {!Array} and {!List} should be used instead. The new
    functions are faster and use less memory. *)
...
(** Sort an array in increasing order according to an ordering predicate. The
    predicate should return [true] if its first argument is less than or equal
    to its second argument. The array is sorted in place. *)
...
(** Merge two lists according to the given predicate. Assuming the two argument
    ...
    containing the elements from the two lists. The behavior is undefined if
    the two argument lists were not sorted. *)
