...
let fill ar ofs len x =
  if ofs < 0 || len < 0 || ofs + len > length ar then
...
module Make (H : Hashtbl.HashedType) : S with type data = H.t = struct
  ...
  let fold f t init =
    let rec fold_bucket i b accu =
      if i >= length b then accu
        ...
        | None -> fold_bucket (i + 1) b accu in
  ...
  let rec resize t =
    ...
    if newlen > oldlen then (
      ...
      let add_weak ob oh oi =
        ...
        let h = oh.(oi) in
        add_aux newt setter None h (get_index newt h) in
  ...
  and add_aux t setter d h index =
    ...
    let rec loop i =
      ...
      else if check bucket i then loop (i + 1)
      else (setter bucket i d ; hashes.(i) <- h) in
    loop 0
    ...
    let rec loop i =
      if i >= sz then ifnotfound h index
        ...
        | Some v when H.equal v d -> (
          match get bucket i with Some v -> v | None -> loop (i + 1) )
        | _ -> loop (i + 1)
      else loop (i + 1) in
    loop 0
  ...
  let find_opt t d =
    ...
    let rec loop i =
      if i >= sz then None
        ...
        match get_copy bucket i with
        | Some v when H.equal v d -> (
          match get bucket i with Some _ as v -> v | None -> loop (i + 1) )
        | _ -> loop (i + 1)
      else loop (i + 1) in
    loop 0
