...
external execvpe_c : string -> string array -> string array -> 'a
  = "unix_execvpe"
...
let execvpe_ml name args env =
  ...
  let rec scan_dir eacces = function
    | [] ->
        (* No matching file was found (if [eacces = false]) or a matching file
           was found but we got a "permission denied" error while trying to
           execute it (if [eacces = true]). Raise the error appropriate to each
           case. *)
        raise
    ...
    | dir :: rem -> (
        ...
        with Unix_error (err, _, _) as exn -> (
          match err with
          (* The following errors are treated as nonfatal, meaning that we will
             ignore them and continue searching in the path. Among those
             errors, EACCES is recorded specially so as to produce the correct
             exception in the end. To determine which errors are nonfatal, we
             looked at the execvpe() sources in Glibc and in OpenBSD. *)
  ...
  if String.contains name '/' then
    ...
    |> String.split_on_char ':' |> scan_dir false

(* [unsafe_getenv] and not [getenv] to be consistent with [execvp], which looks
   up the PATH environment variable whether SUID or not. *)
...
external unsafe_recv :
  file_descr -> bytes -> int -> int -> msg_flag list -> int = "unix_recv"
...
external unsafe_send :
  file_descr -> bytes -> int -> int -> msg_flag list -> int = "unix_send"
...
let getaddrinfo_emulation node service opts =
  ...
  let ports =
    ...
    | Some ty -> if service = "" then [(ty, 0)] else [] in
...
let system cmd =
  match fork () with
  | 0 -> (
    try execv "/bin/sh" [|"/bin/sh"; "-c"; cmd|] with _ -> sys_exit 127 )
  | id -> snd (waitpid_non_intr id)

(* Duplicate [fd] if needed to make sure it isn't one of the standard
   descriptors (stdin, stdout, stderr). Note that this function always leaves
   the standard descriptors open, the caller must take care of closing them if
   needed. The "cloexec" mode doesn't matter, because the descriptor returned
   by [dup] will be closed before the [exec], and because no other thread is
   running concurrently (we are in the child process of a fork). *)
...
let perform_redirections new_stdin new_stdout new_stderr =
  ...
  (* The three dup2 close the original stdin, stdout, stderr, which are the
     descriptors possibly left open by file_descr_not_standard *)
...
let open_process_in cmd =
  ...
  ( try open_proc cmd None (Process_in inchan) stdin in_write stderr
...
let open_process_out cmd =
  ...
  ( try open_proc cmd None (Process_out outchan) out_read stdout stderr
...
let open_process cmd =
  ...
  let inchan = in_channel_of_descr in_read in
  let outchan = out_channel_of_descr out_write in
  ( try open_proc cmd None (Process (inchan, outchan)) out_read in_write stderr
    with e ->
      close out_read ;
      close out_write ;
      close in_read ;
...
let close_process_out outchan =
  ...
  (* The application may have closed [outchan] already to signal end-of-input
     to the process. *)
...
let close_process_full (inchan, outchan, errchan) =
  let pid =
    find_proc_id "close_process_full" (Process_full (inchan, outchan, errchan))
  in
  close_in inchan ;
  (try close_out outchan with Sys_error _ -> ()) ;
  close_in errchan ;
...
let establish_server server_fun sockaddr =
  ...
  while true do
    ...
    | 0 ->
        ...
        (* Do not close inchan nor outchan, as the server_fun could have done
           it already, and we are about to exit anyway (PR#3794) *)
        exit 0
    | id ->
        close s ;
