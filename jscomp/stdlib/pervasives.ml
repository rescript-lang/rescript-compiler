...
let ( ^ ) s1 s2 =
  ...
  string_blit s1 0 s 0 l1 ; string_blit s2 0 s l1 l2 ; bytes_unsafe_to_string s
...
let output oc s ofs len =
  if ofs < 0 || len < 0 || ofs > bytes_length s - len then invalid_arg "output"
  else unsafe_output oc s ofs len

let output_substring oc s ofs len =
  if ofs < 0 || len < 0 || ofs > string_length s - len then
...
let input ic s ofs len =
  if ofs < 0 || len < 0 || ofs > bytes_length s - len then invalid_arg "input"
...
let really_input ic s ofs len =
  if ofs < 0 || len < 0 || ofs > bytes_length s - len then
    invalid_arg "really_input"
  else unsafe_really_input ic s ofs len

let really_input_string ic len =
  let s = bytes_create len in
  really_input ic s 0 len ; bytes_unsafe_to_string s
