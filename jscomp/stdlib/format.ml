...
(* A pretty-printing facility and definition of formatters for 'parallel' (i.e.
   unrelated or independent) pretty-printing on multiple out channels. *)

(* The pretty-printing engine internal data structures. *)

(* A devoted type for sizes to avoid confusion between sizes and mere integers. *)
type size

external size_of_int : int -> size = "%identity"
external int_of_size : size -> int = "%identity"

(* The pretty-printing boxes definition: a pretty-printing box is either -
   hbox: horizontal box (no line splitting) - vbox: vertical box (every break
   hint splits the line) - hvbox: horizontal/vertical box (the box behaves as
   an horizontal box if it fits on the current line, otherwise the box behaves
   as a vertical box) - hovbox: horizontal or vertical compacting box (the box
   is compacting material, printing as much material as possible on every
   lines) - box: horizontal or vertical compacting box with enhanced box
   structure (the box behaves as an horizontal or vertical box but break hints
   split the line if splitting would move to the left) *)
...
(* The pretty-printing tokens definition: are either text to print or pretty
   printing elements that drive indentation and line splitting. *)
type pp_token =
...
(* The pretty-printer queue definition: pretty-printing material is not written
   in the output as soon as emitted; instead, the material is simply recorded
   in the pretty-printer queue, until the enclosing box has a known computed
   size and proper splitting decisions can be made.

   To define the pretty-printer queue, we first define polymorphic queues, then
   pretty-printer queue elements. *)

(* The pretty-printer queue: polymorphic queue definition. *)
type 'a queue_elem = Nil | Cons of {head: 'a; mutable tail: 'a queue_elem}
type 'a queue = {mutable insert: 'a queue_elem; mutable body: 'a queue_elem}

(* The pretty-printer queue: queue element definition. The pretty-printer queue
   contains formatting elements to be printed. Each formatting element is a
   tuple (size, token, length), where - length is the declared length of the
   token, - size is effective size of the token when it is printed (size is set
   when the size of the box is known, so that size of break hints are
   definitive). *)
...
(* The pretty-printer scanning stack: scanning element definition. Each element
   is (left_total, queue element) where left_total is the value of
   pp_left_total when the element has been enqueued. *)
...
(* The pretty-printer formatting stack: the formatting stack contains the
   description of all the currently active boxes; the pretty-printer formatting
   stack is used to split the lines while printing tokens. *)
...
(* The formatter definition. Each formatter value is a pretty-printer instance
...
(* Pp_infinity: large value for default tokens size.
   ...
   Pp_infinity is documented as being greater than 1e10; to avoid confusion
   about the word 'greater', we choose pp_infinity greater than 1e10 + 1; for
   correct handling of tests in the algorithm, pp_infinity must be even one
   more than 1e10 + 1; let's stand on the safe side by choosing 1.e10+10.

   Pp_infinity could probably be 1073741823 that is 2^30 - 1, that is the
   minimal upper bound for integers; now that max_int is defined, this limit
   could also be defined as max_int - 1.

   However, before setting pp_infinity to something around max_int, we must
   carefully double-check all the integer arithmetic operations that involve
   pp_infinity, since any overflow would wreck havoc the pretty-printing
   algorithm's invariants. Given that this arithmetic correctness check is
   difficult and error prone and given that 1e10 + 1 is in practice large
   enough, there is no need to attempt to set pp_infinity to the theoretically
   maximum limit. It is not worth the burden ! *)
...
let format_pp_token state size = function
  ...
  | Pp_tbreak (n, off) -> (
      ...
      | Pp_tbox tabs :: _ ->
          ...
          if offset >= 0 then break_same_line state (offset + n)
  ...
  | Pp_break (n, off) -> (
    ...
    | Format_elem (ty, width) :: _ -> (
      match ty with
      | Pp_hovbox ->
          if size > state.pp_space_left then break_new_line state off width
          else break_same_line state n
      | Pp_box ->
          (* Have the line just been broken here ? *)
          if state.pp_is_new_line then break_same_line state n
          else if size > state.pp_space_left then
...
(* Print if token size is known else printing is delayed. Size is known when
   not negative. Printing is delayed when the text waiting in the queue
   requires more room to format than exists on the current line.
...
let make_queue_elem size tok len = {elem_size= size; token= tok; length= len}
...
(* Routines for scan stack determine size of boxes. *)
...
(* Setting the size of boxes on scan stack: if ty = true then size of break is
   set else size of box is set; in each case pp_scan_stack is popped.

   Note: Pattern matching on scan stack is exhaustive, since scan_stack is
   never empty. Pattern matching on token in scan stack is also exhaustive,
   since scan_push is used on breaks and opening of boxes. *)
let set_size state ty =
  match state.pp_scan_stack with
  | Scan_elem
      (left_tot, ({elem_size= size; token= tok; length= _} as queue_elem))
    :: t -> (
      let size = int_of_size size in
      (* test if scan stack contains any data that is not obsolete. *)
      if left_tot < state.pp_left_total then clear_scan_stack state
      else
        match tok with
        ...
        | Pp_begin (_, _) ->
            if not ty then (
              queue_elem.elem_size <- size_of_int (state.pp_right_total + size) ;
              state.pp_scan_stack <- t )
        | Pp_text _ | Pp_stab | Pp_tbegin _ | Pp_tend | Pp_end | Pp_newline
         |Pp_if_newline | Pp_open_tag _ | Pp_close_tag ->
            () (* scan_push is only used for breaks and boxes. *) )
  | [] -> ()

(* scan_stack is never empty. *)

(* Push a token on pretty-printer scanning stack. If b is true set_size is
   called. *)
...
(* To open a new box : the user may set the depth bound pp_max_boxes any text
   nested deeper is printed as the ellipsis string. *)
let pp_open_box_gen state indent br_ty =
  ...
  else if state.pp_curr_depth = state.pp_max_boxes then
...
let pp_close_tag state () =
  if state.pp_mark_tags then
    pp_enqueue state {elem_size= size_of_int 0; token= Pp_close_tag; length= 0} ;
  if state.pp_print_tags then
...
let clear_tag_stack state =
  List.iter (fun _ -> pp_close_tag state ()) state.pp_tag_stack
...
(* Printing break hints: A break hint indicates where a box may be broken. If
   line is broken then offset is added to the indentation of the current box
   else (the value of) width blanks are printed. *)
...
(* Print a space : a space is a break hint that prints a single space if the
   break does not split the line; a cut is a break hint that prints nothing if
   the break does not split the line. *)
...
let pp_set_tab state () =
  if state.pp_curr_depth < state.pp_max_boxes then
    let elem = make_queue_elem (size_of_int 0) Pp_stab 0 in
    enqueue_advance state elem
...
(* Initially, we have : pp_max_indent = pp_margin - pp_min_space_left, and
...
let pp_set_margin state n =
  if n >= 1 then (
    ...
    let new_max_indent =
      ...
      if state.pp_max_indent <= state.pp_margin then state.pp_max_indent
      else
...
let pp_set_formatter_out_functions state
    {out_string= f; out_flush= g; out_newline= h; out_spaces= i; out_indent= j}
    =
...
let rec display_blanks state n =
  if n > 0 then
    if n <= 80 then state.pp_out_string blank_line 0 n
    else (
...
(* Building a formatter given its basic output functions. Other fields get
...
let pp_make_formatter f g h i j =
  ...
  let sys_scan_stack = Scan_elem (1, sys_tok) :: scan_stack_bottom in
  let pp_margin = 78 and pp_min_space_left = 10 in
...
let formatter_of_out_functions out_funs =
  pp_make_formatter out_funs.out_string out_funs.out_flush out_funs.out_newline
    out_funs.out_spaces out_funs.out_indent

(* Make a formatter with default functions to output spaces, indentation, and
   new lines. *)
...
(* Allocating buffer for pretty-printing purposes. Default buffer size is
   pp_buffer_size or 512. *)
...
(* Predefined formatters standard formatter to print to [Pervasives.stdout],
   [Pervasives.stderr], and {!stdbuf}. *)
...
(* [flush_buffer_formatter buf ppf] flushes formatter [ppf], then returns the
   contents of buffer [buf] that is reset. Formatter [ppf] is supposed to print
   to buffer [buf], otherwise this function is not really useful. *)
...
let formatter_of_symbolic_output_buffer sob =
  let symbolic_flush sob () = add_symbolic_output_item sob Output_flush
  ...
  and symbolic_spaces sob n = add_symbolic_output_item sob (Output_spaces n)
  and symbolic_indent sob n = add_symbolic_output_item sob (Output_indent n) in
...
(* Basic functions on the 'standard' formatter (the formatter that prints to
   [Pervasives.stdout]). *)
...
and set_formatter_out_functions = pp_set_formatter_out_functions std_formatter
and get_formatter_out_functions = pp_get_formatter_out_functions std_formatter
and set_formatter_output_functions =
  pp_set_formatter_output_functions std_formatter
and get_formatter_output_functions =
  pp_get_formatter_output_functions std_formatter
and set_formatter_tag_functions = pp_set_formatter_tag_functions std_formatter
and get_formatter_tag_functions = pp_get_formatter_tag_functions std_formatter
...
let compute_tag output tag_acc =
  ...
  if len < 2 then Buffer.contents buf else Buffer.sub buf 1 (len - 2)
...
let output_formatting_lit ppf fmting_lit =
...
let rec output_acc ppf acc =
  ...
  | Acc_string_literal (p, s) | Acc_data_string (p, s) ->
      output_acc ppf p ; pp_print_string ppf s
  | Acc_char_literal (p, c) | Acc_data_char (p, c) ->
      output_acc ppf p ; pp_print_char ppf c
...
let rec strput_acc ppf acc =
  ...
  | Acc_formatting_gen (p, Acc_open_box acc') ->
      ...
      pp_open_box_gen ppf indent bty
  | Acc_string_literal (p, s) | Acc_data_string (p, s) ->
      strput_acc ppf p ; pp_print_string ppf s
  | Acc_char_literal (p, c) | Acc_data_char (p, c) ->
      strput_acc ppf p ; pp_print_char ppf c
  | Acc_delay (p, f) ->
      strput_acc ppf p ;
...
let kfprintf k ppf (Format (fmt, _)) =
  make_printf (fun ppf acc -> output_acc ppf acc ; k ppf) ppf End_of_acc fmt

and ikfprintf k ppf (Format (fmt, _)) = make_iprintf k ppf fmt
...
let pp_set_all_formatter_output_functions state ~out:f ~flush:g ~newline:h
    ~spaces:i =
...
(* Deprecated : error prone function, do not use it. This function is neither
   compositional nor incremental, since it flushes the pretty-printer queue at
   each call. To get the same functionality, define a formatter of your own
   writing to the buffer argument, as in let ppf = formatter_of_buffer b then
   use {!fprintf ppf} as usual. *)
