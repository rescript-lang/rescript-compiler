...
and ('a, 'b) bucketlist =
  | Empty
  | Cons of
      {mutable key: 'a; mutable data: 'b; mutable next: ('a, 'b) bucketlist}

(* The sign of initial_size encodes the fact that a traversal is ongoing or
   not.
  ...
  if nsize < Sys.max_array_length then (
    ...
    let rec insert_bucket = function
      ...
      | Cons {key; data; next} as cell ->
          ...
          ndata_tail.(nidx) <- cell ; insert_bucket next in
      ...
      if compare key k1 = 0 then Some d1
        ...
        | Cons {key= k2; data= d2; next= next2} -> (
            if compare key k2 = 0 then Some d2
              ...
              | Empty -> None
              | Cons {key= k3; data= d3; next= next3} ->
                  if compare key k3 = 0 then Some d3
                  else find_rec_opt key next3 ) )
...
let replace h key data =
  let i = key_index h key in
  let l = h.data.(i) in
  if replace_bucket key data l then (
...
let mem h key =
  let rec mem_in_bucket = function
    | Empty -> false
    | Cons {key= k; next} -> compare k key = 0 || mem_in_bucket next in
  mem_in_bucket h.data.(key_index h key)

let iter f h =
  let rec do_bucket = function
    | Empty -> ()
    | Cons {key; data; next} -> f key data ; do_bucket next in
  ...
  try
    ...
    if not old_trav then flip_ongoing_traversal h
  with exn when not old_trav -> flip_ongoing_traversal h ; raise exn
...
let fold f h init =
  let rec do_bucket b accu =
    match b with
    | Empty -> accu
    | Cons {key; data; next} -> do_bucket next (f key data accu) in
  let old_trav = ongoing_traversal h in
  if not old_trav then flip_ongoing_traversal h ;
  try
    let d = h.data in
...
module MakeSeeded (H : SeededHashedType) : SeededS with type key = H.t = struct
  ...
  let mem h key =
    let rec mem_in_bucket = function
      | Empty -> false
      | Cons {key= k; next} -> H.equal k key || mem_in_bucket next in
    mem_in_bucket h.data.(key_index h key)
