...
let parse_and_expand_argv_dynamic_aux allow_expand current argv speclist
  ...
  let convert_error error =
    (* convert an internal error to a Bad/Help exception *or* add the program
       name as a prefix and the usage message as a suffix to an user-raised Bad
       exception. *)
    let b = Buffer.create 200 in
    let progname =
        ...
        if String.length s >= 1 && s.[0] = '-' then
          let action, follow =
            ...
            with Not_found -> (
              ...
              with Not_found -> raise (Stop (Unknown s)) ) in
          let no_arg () =
            match follow with
            | None -> ()
          ...
          let get_arg () =
            ...
            | Some arg -> arg in
          let consume_arg () =
            match follow with None -> incr current | Some _ -> () in
          let rec treat_action = function
            ...
            | Symbol (symb, f) ->
                let arg = get_arg () in
                if List.mem arg symb then (f arg ; consume_arg ())
                else
                  raise
                    (Stop
            ...
            | Expand f ->
                ...
                argv := Array.concat [before; newarg; after] in
          treat_action action
        else anonfun s
      with
...
let read_aux trim sep file =
  ...
  let rec read () =
    try
      ...
      if c = sep then (stash () ; read ())
      else (Buffer.add_char buf c ; read ())
    with End_of_file -> if Buffer.length buf > 0 then stash () in
  read () ;
  close_in ic ;
