...
(** 32-bit integers.
    ...
    integers. Unlike the built-in [int] type, the type [int32] is guaranteed to
    be exactly 32-bit wide on all platforms. All arithmetic operations over
    [int32] are taken modulo 2{^ 32}.
    ...
    slower than those on [int]. Use [int32] only when the application requires
...
(** Integer remainder. If [y] is not zero, the result of [Int32.rem x y]
    ...
    [x = Int32.add (Int32.mul (Int32.div x y) y) (Int32.rem x y)]. If [y = 0],
...
(** [Int32.shift_right_logical x y] shifts [x] to the right by [y] bits. This
...
(** Convert the given 32-bit integer (type [int32]) to an integer (type [int]).
    On 32-bit platforms, the 32-bit integer is taken modulo 2{^ 31}, i.e. the
...
(** Convert the given string to a 32-bit integer. The string is read in decimal
    ...
    [\[0, 2*Int32.max_int+1\]]. If the input exceeds {!Int32.max_int} it is
    ...
    ignored. Raise [Failure "Int32.of_string"] if the given string is not a
    valid representation of an integer, or if the integer represented exceeds
    the range of integers representable in type [int32]. *)
