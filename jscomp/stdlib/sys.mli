...
(** System interface.
    ...
    Every function in this module raises [Sys_error] with an informative
    message when the underlying system call signal an error. *)
...
(** Rename a file. [rename oldpath newpath] renames the file called [oldpath],
    ...
    (permissions, owner, etc) of [newpath] can either be preserved or be
    replaced by those of [oldpath]. @since 4.06 concerning the "replace
...
(** Return the names of all files present in the given directory. Names
    ...
    in Unix) are not returned. Each string in the result is a file name rather
...
val os_type : string
(** Operating system currently executing the OCaml program. One
    of
...
val int_size : int
(** Size of an int. It is 31 bits (resp. 63 bits) when using the OCaml compiler
    on a 32 bits (resp. 64 bits) platform. It may differ for other compilers,
    e.g. it is 32 bits when compiling to JavaScript.
    @since 4.03.0 *)
...
(** [ocaml_version] is the version of OCaml. It is a string of the form
    ["major.minor\[.patchlevel\]\[+additional-info\]"], where [major], [minor],
    and [patchlevel] are integers, and [additional-info] is an arbitrary
    string. The [\[.patchlevel\]] and [\[+additional-info\]] parts may be
    absent. *)

val enable_runtime_warnings : bool -> unit
(** Control whether the OCaml runtime system can emit warnings on stderr.
    Currently, the only supported warning is triggered when a channel created
    by [open_*] functions is finalized without being closed. Runtime warnings
