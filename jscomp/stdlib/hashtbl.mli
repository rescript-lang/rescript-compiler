...
(** [Hashtbl.create n] creates a new, empty hash table, with initial size [n].
    ...
    A hash table that is created with [~random:false] uses a fixed hash
    function ({!Hashtbl.hash}) to distribute keys among buckets. As a
    ...
    elements of the hash table using {!Hashtbl.fold} or {!Hashtbl.iter} is no
    ...
    calling {!Hashtbl.randomize} or by setting the [R] flag in the
    [OCAMLRUNPARAM] environment variable.
    @before 4.00.0
    the [random] parameter was not present and all hash tables were created in
    non-randomized mode. *)
...
(** [Hashtbl.add tbl x y] adds a binding of [x] to [y] in table [tbl]. Previous
    ...
    performing {!Hashtbl.remove} [tbl x], the previous binding for [x], if any,
    is restored. (Same behavior as with association lists.) *)
...
(** [Hashtbl.replace tbl x y] replaces the current binding of [x] in [tbl] by a
    binding of [x] to [y]. If [x] is unbound in [tbl], a binding of [x] to [y]
    is added to [tbl]. This is functionally equivalent to {!Hashtbl.remove} 
...
(** [Hashtbl.iter f tbl] applies [f] to all bindings in table [tbl]. [f]
    ...
    the bindings are enumerated is reproducible between successive runs of the
...
(** [Hashtbl.filter_map_inplace f tbl] applies [f] to all bindings in table
    ...
    returns [None], the binding is discarded. If it returns [Some new_val], the
...
(** After a call to [Hashtbl.randomize()], hash tables are created in
    randomized mode by default: {!Hashtbl.create} returns randomized hash
    tables, unless the [~random:false] optional parameter is given. The same
    effect can be achieved by setting the [R] parameter in the [OCAMLRUNPARAM]
    ...
    to the non-randomized default behavior of {!Hashtbl.create}. This is
...
(** The functorial interface allows the use of specific comparison and hash
    functions, either for performance/security concerns, or because keys are
    ...
    This creates a new module [IntHashtbl], with a new type ['a IntHashtbl.t]
    of tables from [int] to ['a]. In this example, [h] contains [string] values
    so its type is [string IntHashtbl.t].
...
module type HashedType = sig
  ...
  (** A hashing function on keys. It must be such that if two keys are equal
      ...
      [hash]. Examples: suitable ([equal], [hash]) pairs for arbitrary key
      types
      include
      {ul
...
(** Functor building an implementation of the hashtable structure. The functor
    ...
    but use the hashing and equality functions specified in the functor
    argument [H] instead of generic equality and hashing. Since the hash
    function is not seeded, the [create] operation of the result structure
...
module type SeededHashedType = sig
  ...
  (** A seeded hashing function on keys. The first argument is the seed. It
      must be the case that if [equal x y] is true, then
      [hash seed x = hash seed y] for any value of [seed]. A suitable choice
      for [hash] is the function {!Hashtbl.seeded_hash} below. *)
end
...
(** Functor building an implementation of the hashtable structure. The functor
    [Hashtbl.MakeSeeded] returns a structure containing a type [key] of keys
    ...
    if randomization is globally on (see {!Hashtbl.randomize}).
    @since 4.00.0 *)
...
(** [Hashtbl.hash x] associates a nonnegative integer to any value of any type.
    It is guaranteed that if [x = y] or [Pervasives.compare x y = 0], then
...
(** [Hashtbl.hash_param meaningful total x] computes a hash value for [x], with
    ...
    {!Hashtbl.hash} and {!Hashtbl.seeded_hash} take [meaningful = 10] and
    [total = 100]. *)
