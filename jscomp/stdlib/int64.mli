...
(** 64-bit integers.
    ...
    integers. Unlike the built-in [int] type, the type [int64] is guaranteed to
    be exactly 64-bit wide on all platforms. All arithmetic operations over
    [int64] are taken modulo 2{^ 64}
    ...
    slower than those on [int]. Use [int64] only when the application requires
...
(** Integer remainder. If [y] is not zero, the result of [Int64.rem x y]
    ...
    [x = Int64.add (Int64.mul (Int64.div x y) y) (Int64.rem x y)]. If [y = 0],
...
(** [Int64.shift_right_logical x y] shifts [x] to the right by [y] bits. This
    is a logical shift: zeroes are inserted in the vacated bits regardless of
...
(** Convert the given 64-bit integer (type [int64]) to an integer (type [int]).
    On 64-bit platforms, the 64-bit integer is taken modulo 2{^ 63}, i.e. the
    high-order bit is lost during the conversion. On 32-bit platforms, the
    64-bit integer is taken modulo 2{^ 31}, i.e. the top 33 bits are lost
...
(** Convert the given 64-bit integer (type [int64]) to a 32-bit integer (type
    [int32]). The 64-bit integer is taken modulo 2{^ 32}, i.e. the top 32 bits
...
(** Convert the given string to a 64-bit integer. The string is read in decimal
    ...
    [\[0, 2*Int64.max_int+1\]]. If the input exceeds {!Int64.max_int} it is
    converted to the signed integer
    [Int64.min_int + input - Int64.max_int - 1].

    The [_] (underscore) character can appear anywhere in the string and is
    ignored. Raise [Failure "Int64.of_string"] if the given string is not a
    valid representation of an integer, or if the integer represented exceeds
    the range of integers representable in type [int64]. *)
...
(** Do not use this deprecated function. Instead, used {!Printf.sprintf} with a
    [%L...] format. *)
