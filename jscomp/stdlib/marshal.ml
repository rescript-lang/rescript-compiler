...
(* The functions below use byte sequences as input, never using any mutation.
   It makes sense to use non-mutated [bytes] rather than [string], because we
   really work with sequences of bytes, not a text representation. *)
...
let data_size buff ofs =
  if ofs < 0 || ofs > Bytes.length buff - header_size then
...
let from_bytes buff ofs =
  if ofs < 0 || ofs > Bytes.length buff - header_size then
    ...
    if ofs > Bytes.length buff - (header_size + len) then
      invalid_arg "Marshal.from_bytes"
    else from_bytes_unsafe buff ofs

let from_string buff ofs =
  (* Bytes.unsafe_of_string is safe here, as the produced byte sequence is
     never mutated *)
