...
let char_of_iconv iconv =
  match iconv with
  | Int_d | Int_pd | Int_sd -> 'd'
  | Int_i | Int_pi | Int_si -> 'i'
  | Int_x | Int_Cx -> 'x'
  | Int_X | Int_CX -> 'X'
  | Int_o | Int_Co -> 'o'
  | Int_u -> 'u'
...
let char_of_fconv fconv =
  match fconv with
  | Float_f | Float_pf | Float_sf -> 'f'
  | Float_e | Float_pe | Float_se -> 'e'
  | Float_E | Float_pE | Float_sE -> 'E'
  | Float_g | Float_pg | Float_sg -> 'g'
  | Float_G | Float_pG | Float_sG -> 'G'
  | Float_F -> 'F'
  | Float_h | Float_ph | Float_sh -> 'h'
  | Float_H | Float_pH | Float_sH -> 'H'
...
let bprint_char_set buf char_set =
  ...
  and print_char buf i =
    ...
    | '@' -> buffer_add_char buf '%' ; buffer_add_char buf '@'
    | c -> buffer_add_char buf c in
  buffer_add_char buf '[' ;
  print_start
    ( if is_in_char_set char_set '\000' then (
      buffer_add_char buf '^' ; rev_char_set char_set )
    else char_set ) ;
  buffer_add_char buf ']'
...
let bprint_fconv_flag buf fconv =
  match fconv with
  | Float_pf | Float_pe | Float_pE | Float_pg | Float_pG | Float_ph | Float_pH
    ->
      buffer_add_char buf '+'
  | Float_sf | Float_se | Float_sE | Float_sg | Float_sG | Float_sh | Float_sH
    ->
      buffer_add_char buf ' '
  ...
  let rec fmtiter : type a b c d e f. (a, b, c, d, e, f) fmt -> bool -> unit =
   fun fmt ign_flag ->
    ...
    | Reader rest ->
        ...
        fmtiter rest false
    | Flush rest -> buffer_add_string buf "%!" ; fmtiter rest ign_flag
...
(* Precondition: we assume that the two fmtty_rel arguments have equal values
   ...
   | Foo x, Foo y -> (* case where indeed both values start with constructor
   Foo *) | Foo _, _ | _, Foo _ -> (* different head constructors: broken
 ...
 fun fmt fmtty ->
  ...
  | Char fmt_rest, Char_ty fmtty_rest ->
      let (Fmt_fmtty_EBB (fmt', fmtty')) =
        type_format_gen fmt_rest fmtty_rest in
  ...
  | ( Format_subst (pad_opt, sub_fmtty, fmt_rest)
    , Format_subst_ty (sub_fmtty1, _sub_fmtty2, fmtty_rest) ) ->
      ...
      let (Fmt_fmtty_EBB (fmt', fmtty')) =
        type_format_gen fmt_rest (erase_rel fmtty_rest) in
...
(* This implementation of `recast` is a bit disappointing. The invariant
   ...
   parameters, but they are not at all exchangeable, and putting one in place
   of the other must result in a dynamic failure.
...
let convert_float fconv prec x =
  ...
  | _ -> (
      ...
      if fconv <> Float_F then str
      else
        let len = String.length str in
        let rec is_valid i =
          if i = len then false
          else
            match str.[i] with '.' | 'e' | 'E' -> true | _ -> is_valid (i + 1)
        in
...
let rec make_printf :
    type a b c d e f.
    (b -> (b, c) acc -> f) -> b -> (b, c) acc -> (a, b, c, d, e, f) fmt -> a =
 fun k o acc fmt ->
  match fmt with
  | Char rest ->
      fun c ->
  ...
  | Caml_char rest ->
      fun c ->
        ...
        make_printf k o new_acc rest
  | String (pad, rest) -> make_padding k o acc rest pad (fun str -> str)
  | Caml_string (pad, rest) ->
  ...
  | Bool (pad, rest) -> make_padding k o acc rest pad string_of_bool
  | Alpha rest ->
      fun f x -> make_printf k o (Acc_delay (acc, fun o -> f o x)) rest
  | Theta rest -> fun f -> make_printf k o (Acc_delay (acc, f)) rest
  | Custom (arity, f, rest) -> make_custom k o acc rest arity (f ())
  | Reader _ ->
      ...
      (* Indeed, since printf and co. take a format4 as argument, the 'd and 'e
         ...
         constructor which touch 'd and 'e type parameters of the format type,
         it adds an (->) to the 'd parameters. Consequently, a format4 cannot
         contain a Reader node, except in the sub-format associated to an
         %{...%}. It's not a problem because make_printf do not call itself
         recursively on the sub-format associated to %{...%}. *)
      assert false
  | Flush rest -> make_printf k o (Acc_flush acc) rest
  | String_literal (str, rest) ->
  ...
  | Format_subst (_, fmtty, rest) ->
      fun (Format (fmt, _)) ->
        make_printf k o acc (concat_fmt (recast fmt fmtty) rest)
  ...
  | Scan_next_char rest ->
      fun c ->
        let new_acc = Acc_data_char (acc, c) in
        make_printf k o new_acc rest
  | Ignored_param (ign, rest) -> make_ignored_param k o acc ign rest
  | Formatting_lit (fmting_lit, rest) ->
      make_printf k o (Acc_formatting_lit (acc, fmting_lit)) rest
  | Formatting_gen (Open_tag (Format (fmt', _)), rest) ->
      let k' koc kacc =
        make_printf k koc (Acc_formatting_gen (acc, Acc_open_tag kacc)) rest
      in
      make_printf k' o End_of_acc fmt'
  | Formatting_gen (Open_box (Format (fmt', _)), rest) ->
      let k' koc kacc =
        make_printf k koc (Acc_formatting_gen (acc, Acc_open_box kacc)) rest
      in
      make_printf k' o End_of_acc fmt'
  | End_of_format -> k o acc
...
and make_padding :
 ...
 fun k o acc fmt pad trans ->
  ...
  | Lit_padding (padty, width) ->
      fun x ->
        let new_acc =
          Acc_data_string (acc, fix_padding padty width (trans x)) in
        make_printf k o new_acc fmt
  | Arg_padding padty ->
      fun w x ->
...
and make_int_padding_precision :
    ...
    -> int_conv
    -> x =
 fun k o acc fmt pad prec trans iconv ->
  match (pad, prec) with
...
and make_float_padding_precision :
    ...
    -> float_conv
    -> x =
 fun k o acc fmt pad prec fconv ->
  match (pad, prec) with
  | No_padding, No_precision ->
      fun x ->
...
and make_custom :
    ...
    -> x
    -> y =
 fun k o acc rest arity f ->
  match arity with
  | Custom_zero -> make_printf k o (Acc_data_string (acc, f)) rest
  | Custom_succ arity -> fun x -> make_custom k o acc rest arity (f x)

let const x _ = x

let rec make_iprintf :
    type a b c d e f. (b -> f) -> b -> (a, b, c, d, e, f) fmt -> a =
 fun k o fmt ->
  match fmt with
...
let failwith_message (Format (fmt, _)) =
  ...
  let k () acc =
    strput_acc buf acc ;
    failwith (Buffer.contents buf) in
  ...
  if str = "" then (0, Pp_box)
    ...
    and parse_int i j =
      if j = len then j
        ...
        match str.[j] with '0' .. '9' | '-' -> parse_int i (j + 1) | _ -> j
    in
...
let fmt_ebb_of_string ?legacy_behavior str =
  ...
  let legacy_behavior =
    ...
    (* When this flag is enabled, the format parser tries to behave as the
       <4.02 implementations, in particular it ignores most benine nonsensical
       ...
       ' ' (if the number is positive, pad with a space) does not make sense,
       but the legacy (< 4.02) implementation was happy to just ignore the
       space. *) in
  (* Raise [Failure] with a friendly error message. *)
  let invalid_format_message str_ind msg =
    failwith_message "invalid format %S: at character number %d, %s" str
      str_ind msg in
  ...
  let invalid_nonnull_char_width str_ind =
    invalid_format_message str_ind
      "non-zero widths are unsupported for %c conversions" in
  (* Raise [Failure] with a friendly error message about an option dependency
     problem. *)
  let invalid_format_without str_ind c s =
    failwith_message
      "invalid format %S: at character number %d, '%c' without %s" str str_ind
      c s in
    ...
    | _ -> (
      ...
      | Zeros ->
          (* a '0' padding indication not followed by anything should be
             interpreted as a Right padding of width 0. This is used by
             scanning conversions %0s and %0c *)
          parse_after_padding pct_ind str_ind end_ind minus plus hash space ign
            (Lit_padding (Right, 0))
      | Right ->
          parse_after_padding pct_ind str_ind end_ind minus plus hash space ign
   ...
   fun pct_ind str_ind end_ind minus plus hash space ign pad ->
    ...
    | _ ->
        if legacy_behavior then
          (* note that legacy implementation did not ignore '.' without a
             number (as it does for padding indications), but interprets it as
             '.0' *)
  ...
  and parse_after_precision :
   ...
   fun pct_ind str_ind end_ind minus plus hash space ign pad prec ->
    ...
    (* in legacy mode, some formats (%s and %S) accept a weird mix of padding
       ...
       example, in %.10s the precision is implicitly understood as padding
       %10s, but the left-padding component may be specified either as a left
       padding or a negative precision: %-.3s and %.-3s are equivalent to %-3s *)
    match pad with
    | No_padding -> (
   ...
   fun pct_ind str_ind end_ind plus hash space ign pad prec padprec symb ->
    ...
    let get_int_pad () =
      (* %5.3d is accepted and meaningful: pad to length 5 with spaces, but
         ...
         zeros... To add insult to the injury, the legacy implementation
         ignores the 0-padding indication and does the 5 padding with spaces
         instead. We reuse this interpretation for compatibility, but
         statically reject this format when the legacy mode is disabled, to
         protect strict users from this corner case. *)
    ...
    let check_no_0 symb (type a b) (pad : (a, b) padding) =
      ...
      | Arg_padding Zeros ->
          if legacy_behavior then Arg_padding Right
          else incompatible_flag pct_ind str_ind symb "0" in
    (* Get padding as a pad_option (see "%_", "%{", "%(" and "%["). (no need
       for legacy mode tweaking, those were rejected by the legacy parser as
       well) *)
    let opt_of_pad c (type a b) (pad : (a, b) padding) =
      ...
      | Arg_padding _ -> incompatible_flag pct_ind str_ind c "'*'" in
    ...
    let get_prec_opt () =
      ...
      | Arg_precision -> incompatible_flag pct_ind str_ind '_' "'*'" in
    let fmt_result =
      match symb with
      ...
      | 'f' | 'e' | 'E' | 'g' | 'G' | 'F' | 'h' | 'H' ->
          let fconv =
            compute_float_conv pct_ind str_ind (get_plus ()) (get_space ())
              symb in
          let (Fmt_EBB fmt_rest) = parse str_ind end_ind in
          if get_ign () then
    ...
    (* this last test must not be disabled in legacy mode, as ignoring it would
       typically result in a different typing than what the legacy parser used *)
   ...
   fun is_open_tag str_ind end_ind ->
    try
      ...
      | '<' ->
          ...
          let formatting =
            if is_open_tag then Open_tag sub_format
            else (check_open_box sub_fmt ; Open_box sub_format) in
    ...
    let next_ind, formatting_lit =
      try
        ...
        | '0' .. '9' | '-' -> (
            ...
            | '>' ->
                let s =
                  String.sub str (str_ind - 2) (str_ind_3 - str_ind + 3) in
            ...
            | '0' .. '9' | '-' ->
                ...
                let s =
                  String.sub str (str_ind - 2) (str_ind_5 - str_ind + 3) in
                (str_ind_5 + 1, Break (s, width, offset))
            | _ -> raise Not_found )
        | _ -> raise Not_found
      with Not_found | Failure _ -> (str_ind, Break ("@;", 1, 0)) in
    let (Fmt_EBB fmt_rest) = parse next_ind end_ind in
  ...
  and parse_char_set str_ind end_ind =
    ...
    and parse_char_set_after_minus str_ind end_ind c =
      ...
      | c' ->
          add_range c c' ;
          parse_char_set_content (str_ind + 1) end_ind in
    let str_ind, reverse =
      if str_ind = end_ind then unexpected_end_of_format end_ind ;
      match str.[str_ind] with
      | '^' -> (str_ind + 1, true)
    ...
    | '%' -> (
        ...
        if str.[str_ind + 1] = c then (* End of format found *) str_ind
          ...
          | '_' -> (
              ...
              | '{' ->
                  let sub_end =
                    search_subformat_end (str_ind + 3) end_ind '}' in
  ...
  and compute_float_conv pct_ind str_ind plus space symb =
    ...
    | false, false, 'e' -> Float_e
    | false, true, 'f' -> Float_sf
    | false, true, 'e' -> Float_se
    | true, false, 'f' -> Float_pf
    | true, false, 'e' -> Float_pe
    | false, false, 'E' -> Float_E
   ...
   fun pct_ind str_ind symb option ->
    ...
    failwith_message
      "invalid format %S: at character number %d, %s is incompatible with \
       '%c' in sub-format %S"
      str pct_ind option symb subfmt in
