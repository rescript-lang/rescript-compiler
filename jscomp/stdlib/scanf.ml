...
(* alias to avoid warning for ambiguity between Pervasives.format6 and
   CamlinternalFormatBasics.format6

   (the former is in fact an alias for the latter, but the ambiguity warning
   doesn't care) *)
...
module type SCANNING = sig
  ...
  (* The scanning buffer reading from [Pervasives.stdin]. [stdib] is equivalent
     to [Scanning.from_channel Pervasives.stdin]. *)
  ...
  (* [Scanning.next_char ib] advance the scanning buffer for one character. If
     no more character can be read, sets a end of file condition and returns
     '\000'. *)
  ...
  (* [Scanning.peek_char ib] returns the current char available in the buffer
     or reads one if necessary (when the current character is already scanned).
     If no character can be read, sets an end of file condition and returns
     '\000'. *)
  ...
  (* [Scanning.store_char lim ib c] adds [c] to the token buffer of the
     scanning buffer [ib]. It also advances the scanning buffer for one
     character and returns [lim - 1], indicating the new limit for the length
  ...
  (* [Scanning.ignore_char ib lim] ignores the current character and decrements
     the limit. *)

  val token : scanbuf -> string

  (* [Scanning.token ib] returns the string stored into the token buffer of the
     scanning buffer: it returns the token matched by the format. *)

  val reset_token : scanbuf -> unit

  (* [Scanning.reset_token ib] resets the token buffer of the given scanning
     buffer. *)

  val char_count : scanbuf -> int

  (* [Scanning.char_count ib] returns the number of characters read so far from
     the given buffer. *)

  val line_count : scanbuf -> int

  (* [Scanning.line_count ib] returns the number of new line characters read so
     far from the given buffer. *)

  val token_count : scanbuf -> int

  (* [Scanning.token_count ib] returns the number of tokens read so far from
     [ib]. *)

  val eof : scanbuf -> bool

  (* [Scanning.eof ib] returns the end of input condition of the given buffer. *)

  val end_of_input : scanbuf -> bool

  (* [Scanning.end_of_input ib] tests the end of input condition of the given
     buffer (if no char has ever been read, an attempt to read one is
     performed). *)
  ...
  (* [Scanning.name_of_input ib] returns the name of the character source for
     input buffer [ib]. *)
...
module Scanning : SCANNING = struct
  ...
  (* Reads a new character from input buffer. Next_char never fails, even in
     case of end of input: it then simply sets the end of file condition. *)
  let next_char ib =
    try
  ...
  let peek_char ib =
    if ib.ic_current_char_is_valid then ib.ic_current_char else next_char ib

  (* Returns a valid current char for the input buffer. In particular no
     irrelevant null character (as set by [next_char] in case of end of input)
     is returned, since [End_of_file] is raised when [next_char] sets the end
     of file condition while trying to read a new character. *)
  ...
  let char_count ib =
    if ib.ic_current_char_is_valid then ib.ic_char_count - 1
  ...
  let skip_char width ib = invalidate_current_char ib ; width
  ...
  let from_string s =
    ...
    let next () =
      if !i >= len then raise End_of_file
      else
        let c = s.[!i] in
        incr i ; c in
  ...
  (* Position of the problem:
     ...
     used to return the token read. It is thus mandatory to be able to store an
     unused lookahead character somewhere to get it as the first character of
     the next scan.
     ...
     successive calls to scanners indeed read from the same input buffer. In
     effect, if a scanner [scan1] is reading from [ib1] and stores an unused
     lookahead character [c1] into its input buffer [ib1], then another scanner
     [scan2] not reading from the same buffer [ib1] will miss the character
     [c1], seemingly vanished in the air from the point of view of [scan2].
  ...
  (* The scanner does not close the input channel at end of input: it just
     raises [End_of_file]. *)
  ...
  let from_ic scan_close_ic iname ic =
    ...
    let next () =
      if !i < !lim then (
        ...
        incr i ; c )
      else if !eof then raise End_of_file
      else (
  ...
  (* The scanning buffer reading from [Pervasives.stdin]. One could try to
     define [stdib] as a scanning buffer reading a character at a time (no
     bufferization at all), but unfortunately the top-level interaction would
     be wrong. This is due to some kind of 'race condition' when reading from
     [Pervasives.stdin], since the interactive compiler and [Scanf.scanf] will
     simultaneously read the material they need from [Pervasives.stdin]; then,
     confusion will result from what should be read by the top-level and what
     should be read by [Scanf.scanf]. This is even more complicated by the one
     character lookahead that [Scanf.scanf] is sometimes obliged to maintain:
     the lookahead character will be available for the next [Scanf.scanf]
     entry, seemingly coming from nowhere. Also no [End_of_file] is raised when
     reading from stdin: if not enough characters have been read, we simply ask
     to read more. *)
  let stdin =
    from_ic scan_raise_at_end
      (From_file ("-", Pervasives.stdin))
      Pervasives.stdin
  ...
  let from_channel ic = from_ic_raise_at_end (From_channel ic) ic
  ...
  (* Obsolete: a memo [from_channel] version to build a [Scanning.in_channel]
     scanning buffer out of a [Pervasives.in_channel]. This function was used
     to try to preserve the scanning semantics for the (now obsolete) function
     [fscanf]. Given that all scanner must read from a [Scanning.in_channel]
     scanning buffer, [fscanf] must read from one! More precisely, given [ic],
     all successive calls [fscanf ic] must read from the same scanning buffer.
     This obliged this library to allocated scanning buffers that were not
     properly garbage collectable, hence leading to memory leaks. If you need
     to read from a [Pervasives.in_channel] input channel [ic], simply define a
     [Scanning.in_channel] formatted input channel as in [let ib =
     Scanning.from_channel ic], then use [Scanf.bscanf ib] as usual. *)
  let memo_from_ic =
    let memo = ref [] in
    fun scan_close_ic ic ->
      try List.assq ic !memo
      with Not_found ->
...
let bad_end_of_input message =
  bad_input
    (Printf.sprintf
       "scanning of %s failed: premature end of file occurred before end of \
        token"
       message)

let bad_float () = bad_input "no dot or exponent part found in float token"
let bad_hex_float () = bad_input "not a valid float in hexadecimal notation"

let character_mismatch_err c ci =
  Printf.sprintf "looking for %C, found %C" c ci

let character_mismatch c ci = bad_input (character_mismatch_err c ci)
...
(* Checking that [c] is indeed in the input, then skips it. In this case, the
   character [c] has been explicitly specified in the format as being mandatory
   in the input; hence we should fail with [End_of_file] in case of
   end_of_input. (Remember that [Scan_failure] is raised only when (we can
   prove by evidence) that the input does not match the format string given. We
   must thus differentiate [End_of_file] as an error due to lack of input, and
   [Scan_failure] which is due to provably wrong input. I am not sure this is
   worth the burden: it is complex and somehow subliminal; should be clearer to
   fail with Scan_failure "Not enough input to complete scanning"!)

   That's why, waiting for a better solution, we use checked_peek_char here. We
   are also careful to treat "\r\n" in the input as an end of line marker: it
...
and check_this_char ib c =
  ...
  if ci = c then Scanning.invalidate_current_char ib
...
(* Extract an integer literal token. Since the functions
   Pervasives.*int*_of_string do not accept a leading +, we skip it if
   necessary. *)
...
(* All the functions that convert a string to a number raise the exception
   Failure when the conversion is not possible. This exception is then trapped
   in [kscanf]. *)
let token_int conv ib = int_of_string (token_int_literal conv ib)
let token_float ib = float_of_string (Scanning.token ib)

(* To scan native ints, int32 and int64 integers. We cannot access to
   conversions to/from strings for those types, Nativeint.of_string,
   Int32.of_string, and Int64.of_string, since those modules are not available
   to [Scanf]. However, we can bind and use the corresponding primitives that
...
(* Digits scanning functions suppose that one character has been checked and is
   available, since they return at end of file with the currently found token
   selected.
...
let scan_decimal_digit_plus width ib =
  if width = 0 then bad_token_length "decimal digits"
  else
    ...
    | '0' .. '9' ->
        let width = Scanning.store_char width ib c in
        scan_decimal_digit_star width ib
    | c -> bad_input (Printf.sprintf "character %C is not a decimal digit" c)

(* To scan numbers from other bases, we use a predicate argument to scan
   digits. *)
...
let scan_digit_plus basis digitp width ib =
  (* Ensure we have got enough width left, and read at least one digit. *)
  if width = 0 then bad_token_length "digits"
  else
    let c = Scanning.checked_peek_char ib in
    if digitp c then
      let width = Scanning.store_char width ib c in
...
(* Scan an unsigned integer that could be given in any (common) basis. If
   digits are prefixed by one of 0x, 0X, 0o, or 0b, the number is assumed to be
   written respectively in hexadecimal, hexadecimal, octal, or binary. *)
...
(* Scan the integer part of a floating point number, (not using the OCaml
   lexical convention since the integer part can be empty): an optional sign,
   followed by a possibly empty sequence of decimal digits (e.g. -.1). *)
let scan_integer_part width ib =
  let width = scan_sign width ib in
  scan_decimal_digit_star width ib

(* For the time being we have (as found in scanf.mli): the field width is
   composed of an optional integer literal indicating the maximal width of the
   token to read. Unfortunately, the type-checker let the user write an
   optional precision, since this is valid for printf format strings.

   Thus, the next step for Scanf is to support a full width and precision
   indication, more or less similar to the one for printf, possibly extended to
   the specification of a [max, min] range for the width of the token read for
   strings. Something like the following spec for scanf.mli:

   The optional [width] is an integer indicating the maximal width of the token
   read. For instance, [%6d] reads an integer, having at most 6 characters.

   The optional [precision] is a dot [.] followed by an integer:

   - in the floating point number conversions ([%f], [%e], [%g], [%F], [%E],
   and [%F] conversions, the [precision] indicates the maximum number of digits
   that may follow the decimal point. For instance, [%.4f] reads a [float] with
   at most 4 fractional digits,

   - in the string conversions ([%s], [%S], [%\[ range \]]), and in the integer
   number conversions ([%i], [%d], [%u], [%x], [%o], and their [int32],
   [int64], and [native_int] correspondent), the [precision] indicates the
   required minimum width of the token read,
...
let scan_float width precision ib =
  ...
  if width = 0 then (width, precision)
  else
    let c = Scanning.peek_char ib in
    if Scanning.eof ib then (width, precision)
    else
      match c with
      | '.' ->
          let width = Scanning.store_char width ib c in
      ...
      | _ -> (scan_exponent_part width ib, precision)
...
let scan_hex_float width precision ib =
  ...
  | '0' as c -> (
      ...
      if width = 0 || Scanning.end_of_input ib then width
      else
        let width =
          match Scanning.peek_char ib with
          | '.' | 'p' | 'P' -> width
...
let scan_caml_float_rest width precision ib =
  ...
  | '.' ->
      let width = Scanning.store_char width ib c in
      (* The effective width available for scanning the fractional part is the
         minimum of declared precision and width left. *)
      let precision = min width precision in
      ...
      (* Hence, scanning the fractional part took exactly [precision -
         width_precision] chars. *)
  ...
  | 'e' | 'E' -> scan_exponent_part width ib
  | _ -> bad_float ()
      ...
      | ('x' | 'X') as c -> (
          ...
          if width = 0 || Scanning.end_of_input ib then width
            ...
            | ('p' | 'P') as c ->
                ...
                scan_optionally_signed_decimal_int width ib
            | _ -> width )
      | _ -> scan_caml_float_rest width precision ib )
  | '1' .. '9' as c ->
      let width = Scanning.store_char width ib c in
      if width = 0 || Scanning.end_of_input ib then bad_float () ;
      scan_caml_float_rest width precision ib
  (* Special case of nan and infinity: | 'i' -> | 'n' -> *)
  | _ -> bad_float ()

(* Scan a regular string: stops when encountering a space, if no scanning
   indication has been given; otherwise, stops when encountering the characters
   in the scanning indication [stp]. It also stops at end of file or when the
   maximum number of characters has been read. *)
...
let scan_char width ib =
  (* The case width = 0 could not happen here, since it is tested before
     calling scan_char, in the main scanning function. if width = 0 then
     bad_token_length "a character" else *)
...
(* The integer value corresponding to the facial value of a valid decimal digit
   character. *)
...
let char_for_decimal_code c0 c1 c2 =
  ...
  if c < 0 || c > 255 then
    bad_input
      (Printf.sprintf "bad character decimal encoding \\%c%c%c" c0 c1 c2)
  else char_of_int c

(* The integer value corresponding to the facial value of a valid hexadecimal
   digit character. *)
let hexadecimal_value_of_char c =
  let d = int_of_char c in
  (* Could also be: if d <= int_of_char '9' then d - int_of_char '0' else if d
     <= int_of_char 'F' then 10 + d - int_of_char 'A' else if d <= int_of_char
     'f' then 10 + d - int_of_char 'a' else assert false *)
  if d >= int_of_char 'a' then d - 87
    (* 10 + int_of_char c - int_of_char 'a' *)
  else if d >= int_of_char 'A' then d - 55
    (* 10 + int_of_char c - int_of_char 'A' *)
  else d - int_of_char '0'

let char_for_hexadecimal_code c1 c2 =
  let c = (16 * hexadecimal_value_of_char c1) + hexadecimal_value_of_char c2 in
  if c < 0 || c > 255 then
    bad_input
      (Printf.sprintf "bad character hexadecimal encoding \\%c%c" c1 c2)
  else char_of_int c

(* Called in particular when encountering '\\' as starter of a char. Stops
   before the corresponding '\''. *)
let check_next_char message width ib =
  if width = 0 then bad_token_length message
  else
    let c = Scanning.peek_char ib in
    if Scanning.eof ib then bad_end_of_input message else c
...
let scan_backslash_char width ib =
  ...
  | '0' .. '9' as c ->
      let get_digit () =
        ...
        match c with '0' .. '9' as c -> c | c -> bad_input_escape c in
      let c0 = c in
      let c1 = get_digit () in
      let c2 = get_digit () in
...
let scan_bool ib =
  ...
  let m =
    ...
    | 'f' -> 5
    | c ->
        bad_input (Printf.sprintf "the character %C cannot start a boolean" c)
  in
...
let width_of_pad_opt pad_opt =
  match pad_opt with None -> max_int | Some width -> width

let stopper_of_formatting_lit fmting =
  if fmting = Escaped_percent then ('%', "")
  else
...
(* A call to take_format_readers on a format is evaluated into functions taking
   readers as arguments and aggregate them into an heterogeneous list *)
(* When all readers are taken, finally pass the list of the readers to the
   continuation k. *)
let rec take_format_readers :
    type a c d e f.
    ((d, e) heter_list -> e) -> (a, Scanning.in_channel, c, d, e, f) fmt -> d =
 fun k fmt ->
  match fmt with
  | Reader fmt_rest ->
      fun reader ->
...
and take_fmtty_format_readers :
    type x y a c d e f.
       ((d, e) heter_list -> e)
    -> (a, Scanning.in_channel, c, d, x, y) fmtty
    -> (y, Scanning.in_channel, c, x, e, f) fmt
    -> d =
 fun k fmtty fmt ->
  match fmtty with
  | Reader_ty fmt_rest ->
...
and take_ignored_format_readers :
    type x y a c d e f.
       ((d, e) heter_list -> e)
    -> (a, Scanning.in_channel, c, d, x, y) ignored
    -> (y, Scanning.in_channel, c, x, e, f) fmt
    -> d =
 fun k ign fmt ->
  match ign with
...
(* Scan a stream according to a format and readers obtained by
   take_format_readers, and aggregate scanned values into an heterogeneous
   list. *)
(* Return the heterogeneous list of scanned values. *)
let rec make_scanf :
    type a c d e f.
       Scanning.in_channel
    -> (a, Scanning.in_channel, c, d, e, f) fmt
    -> (d, e) heter_list
    -> (a, f) heter_list =
 fun ib fmt readers ->
  match fmt with
  ...
  | Alpha _ -> invalid_arg "scanf: bad conversion \"%a\""
  | Theta _ -> invalid_arg "scanf: bad conversion \"%t\""
  | Custom _ -> invalid_arg "scanf: bad conversion \"%?\" (custom converter)"
  | Reader fmt_rest -> (
    match readers with
    | Cons (reader, readers_rest) ->
        let x = reader ib in
        Cons (x, make_scanf ib fmt_rest readers_rest)
    | Nil -> invalid_arg "scanf: missing reader" )
  | Flush rest ->
  ...
  | Char_literal (chr, rest) -> check_char ib chr ; make_scanf ib rest readers
  | Format_arg (pad_opt, fmtty, rest) ->
      let _ = scan_caml_string (width_of_pad_opt pad_opt) ib in
      let s = token_string ib in
      let fmt =
        try format_of_string_fmtty s fmtty with Failure msg -> bad_input msg
      in
  ...
  | Format_subst (pad_opt, fmtty, rest) ->
      ...
      let fmt, fmt' =
        try
          ...
          (* TODO: these type-checks below *can* fail because of type ambiguity
             in presence of ignored-readers: "%_r%d" and "%d%_r" are typed in
             the same way.

             # Scanf.sscanf "\"%_r%d\"3" "%(%d%_r%)" ignore (fun fmt n ->
             string_of_format fmt, n) Exception:
      ...
      Cons (Format (fmt, s), make_scanf ib (concat_fmt fmt' rest) readers)
...
and pad_prec_scanf :
    ...
    -> (x, f) heter_list =
 fun ib fmt readers pad prec scan token ->
  match (pad, prec) with
  ...
  | Lit_padding ((Right | Zeros), w), Lit_precision p ->
      ...
      let x = token ib in
      Cons (x, make_scanf ib fmt readers)
  | Lit_padding (Left, _), _ -> invalid_arg "scanf: bad conversion \"%-\""
  | Lit_padding ((Right | Zeros), _), Arg_precision ->
      invalid_arg "scanf: bad conversion \"%*\""
  | Arg_padding _, _ -> invalid_arg "scanf: bad conversion \"%*\""
  | No_padding, Arg_precision -> invalid_arg "scanf: bad conversion \"%*\""
...
let kscanf ib ef (Format (fmt, str)) =
  let rec apply : type a b. a -> (a, b) heter_list -> b =
   fun f args -> match args with Cons (x, r) -> apply (f x) r | Nil -> f in
  let k readers f =
    Scanning.reset_token ib ;
    match
      try Args (make_scanf ib fmt readers) with
...
let bscanf_format :
 ...
 fun ib format f ->
  ...
  let fmt' =
    try format_of_string_format str format with Failure msg -> bad_input msg
  in
  f fmt'
...
let unescaped s = sscanf ("\"" ^ s ^ "\"") "%S%!" (fun x -> x)
