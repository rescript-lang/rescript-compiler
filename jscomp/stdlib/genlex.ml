...
let get_string () =
  let s = Bytes.sub_string !buffer 0 !bufpos in
  buffer := initial_buffer ;
  s
...
let make_lexer keywords =
  ...
  let rec next_token (strm__ : _ Stream.t) =
    ...
    | Some '\'' -> (
        Stream.junk strm__ ;
        let c =
          try char strm__ with Stream.Failure -> raise (Stream.Error "") in
        match Stream.peek strm__ with
  ...
  and string (strm__ : _ Stream.t) =
    ...
    | Some '\\' ->
        Stream.junk strm__ ;
        let c =
          try escape strm__ with Stream.Failure -> raise (Stream.Error "")
        in
        let s = strm__ in
        store c ; string s
    | Some c ->
