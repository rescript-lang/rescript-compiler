...
(** A generic lexical analyzer.
    ...
    lexical conventions of OCaml, but is parameterized by the set of keywords
    of your language.
...
let lexer = make_lexer ["+";"-";"*";"/";"let";"="; "("; ")"]
    ...
    means that one has to preprocess its sources {i e. g.} by using the ["-pp"]
...
(** Construct the lexer function. The first argument is the list of keywords.
    An identifier [s] is returned as [Kwd s] if [s] belongs to this list, and
    as [Ident s] otherwise. A special character [s] is returned as [Kwd s] if
    [s] belongs to this list, and cause a lexical error (exception
    ...
    skipped as well, and can be nested. A {!Stream.Failure} exception is raised
    if end of stream is unexpectedly reached.*)
