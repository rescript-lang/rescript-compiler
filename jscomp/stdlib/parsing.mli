...
(** [symbol_start] and {!Parsing.symbol_end} are to be called in the action
    ...
    matches the left-hand side of the rule: [symbol_start()] returns the offset
    of the first character; [symbol_end()] returns the offset after the last
...
(** Same as {!Parsing.symbol_start} and {!Parsing.symbol_end}, but return the
    offset of the string matching the [n]th item on the right-hand side of the
    rule, where [n] is the integer parameter to [rhs_start] and [rhs_end]. [n]
...
(** Control debugging support for [ocamlyacc]-generated parsers. After
    ...
    by a rule) on standard output. [Parsing.set_trace false] turns this
    debugging trace off. The boolean returned is the previous state of the
    trace flag.
    @since 3.11.0 *)
...
(** The following definitions are used by the generated parsers only. They are
