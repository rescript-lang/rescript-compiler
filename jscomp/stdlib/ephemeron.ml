...
module GenHashTable = struct
  ...
  module MakeSeeded (H : sig
    ...
    let clean h =
      let rec do_bucket = function
        ...
        | Cons (_, c, rest) when not (H.check_key c) ->
            h.size <- h.size - 1 ;
            do_bucket rest
        | Cons (hkey, c, rest) -> Cons (hkey, c, do_bucket rest) in
      let d = h.data in
    ...
    (** resize is the only function to do the actual cleaning of dead keys
        ...
        {ul
         {- not resize infinitely when the actual number of alive keys is
            bounded but keys are continuously added. That would happen if this
            function always resize.}
         {- not call this function after each addition, that would happen if
            this function don't resize even when only one key is dead.}}
        ...
        The second problem remains if the table reaches
    ...
    let remove h key =
      ...
      let rec remove_bucket = function
        ...
        | Cons (hk, c, next) when hkey = hk -> (
          ...
          | EDead ->
              (* The dead key is automatically removed. It is acceptable for
                 this function since it already removes a binding *)
              h.size <- h.size - 1 ;
              remove_bucket next )
    ...
    (** {!find} don't remove dead keys because it would be surprising for the
        user that a read-only function mutates the state (eg. concurrent
    ...
    let rec find_rec key hkey = function
      ...
      | Cons (hk, c, rest) when hkey = hk -> (
        ...
        | EDead -> find_rec key hkey rest )
      | Cons (_, _, rest) -> find_rec key hkey rest
    ...
    let find_all h key =
      ...
      let rec find_in_bucket = function
        ...
        | Cons (hk, c, rest) when hkey = hk -> (
          ...
          | ETrue -> (
            match H.get_data c with
            | None -> find_in_bucket rest
            | Some d -> d :: find_in_bucket rest )
          | EFalse -> find_in_bucket rest
          | EDead -> find_in_bucket rest )
        | Cons (_, _, rest) -> find_in_bucket rest in
    ...
    let replace h key info =
      ...
      let rec replace_bucket = function
        ...
        | Cons (_, _, next) -> replace_bucket next in
      let i = key_index h hkey in
      let l = h.data.(i) in
      try replace_bucket l
      with Not_found ->
        let container = H.create key info in
    ...
    let filter_map_inplace f h =
      let rec do_bucket = function
        ...
        | Cons (hk, c, rest) -> (
          match (H.get_key c, H.get_data c) with
          | None, _ | _, None -> do_bucket rest
          | Some k, Some d -> (
            match f k d with
            | None -> do_bucket rest
            | Some new_d ->
                H.set_key_data c k new_d ;
                Cons (hk, c, do_bucket rest) ) ) in
      let d = h.data in
      for i = 0 to Array.length d - 1 do
        d.(i) <- do_bucket d.(i)
      done
    ...
    let stats_alive h =
      let size = ref 0 in
      let mbl =
        Array.fold_left (fun m b -> max m (bucket_length_alive 0 b)) 0 h.data
      in
      let histo = Array.make (mbl + 1) 0 in
      Array.iter
        (fun b ->
...
let _obj_opt : Obj.t option -> 'a option =
 fun x -> match x with None -> x | Some v -> Some (Obj.obj v)
...
module K1 = struct
  ...
  let get_key_copy (t : ('k, 'd) t) : 'k option =
    obj_opt (ObjEph.get_key_copy t 0)

  let set_key (t : ('k, 'd) t) (k : 'k) : unit =
  ...
  let get_data_copy (t : ('k, 'd) t) : 'd option =
    obj_opt (ObjEph.get_data_copy t)

  let set_data (t : ('k, 'd) t) (d : 'd) : unit =
  ...
  let blit_data (t1 : (_, 'd) t) (t2 : (_, 'd) t) : unit =
    ObjEph.blit_data t1 t2
  ...
  GenHashTable.MakeSeeded (struct
    ...
    let get_key = get_key
    let set_key_data c k d = unset_data c ; set_key c k ; set_data c d
...
module K2 = struct
  ...
  let get_key1 (t : ('k1, 'k2, 'd) t) : 'k1 option =
    obj_opt (ObjEph.get_key t 0)
  ...
  let get_key2 (t : ('k1, 'k2, 'd) t) : 'k2 option =
    obj_opt (ObjEph.get_key t 1)
  ...
  let blit_data (t1 : (_, _, 'd) t) (t2 : (_, _, 'd) t) : unit =
    ObjEph.blit_data t1 t2
  ...
  GenHashTable.MakeSeeded (struct
    ...
    let create (k1, k2) d =
      let c = create () in
      set_data c d ; set_key1 c k1 ; set_key2 c k2 ; c

    let hash seed (k1, k2) = H1.hash seed k1 + (H2.hash seed k2 * 65599)

    let equal c (k1, k2) =
      match (get_key1 c, get_key2 c) with
      | None, _ | _, None -> GenHashTable.EDead
      | Some k1', Some k2' ->
          if H1.equal k1 k1' && H2.equal k2 k2' then GenHashTable.ETrue
...
module Kn = struct
  ...
  let get_key (t : ('k, 'd) t) (n : int) : 'k option =
    obj_opt (ObjEph.get_key t n)
  ...
  let blit_key (t1 : ('k, 'd) t) (o1 : int) (t2 : ('k, 'd) t) (o2 : int)
      (l : int) : unit =
  ...
  GenHashTable.MakeSeeded (struct
      ...
      if len != len' then GenHashTable.EFalse
        ...
        let rec equal_array k c i =
          if i < 0 then GenHashTable.ETrue
            ...
            | None -> GenHashTable.EDead
            | Some ki ->
                if H.equal k.(i) ki then equal_array k c (i - 1)
                else GenHashTable.EFalse in
    ...
    let check_key c =
      let rec check c i = i < 0 || (check_key c i && check c (i - 1)) in
      check c (length c - 1)
  end)

  module Make (H : Hashtbl.HashedType) : S with type key = H.t array = struct
    include MakeSeeded (struct
      type t = H.t
