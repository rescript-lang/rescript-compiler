...
and buffio = {ic: in_channel; buff: bytes; mutable len: int; mutable ind: int}

exception Failure
exception Error of string

let count = function None -> 0 | Some {count} -> count
let data = function None -> Sempty | Some {data} -> data

let fill_buff b =
  b.len <- input b.ic b.buff 0 (Bytes.length b.buff) ;
  b.ind <- 0

let rec get_data : type v. int -> v data -> v data =
 fun count d ->
  match d with
  (* Returns either Sempty or Scons(a, _) even when d is a generator or a
     buffer. In those cases, the item a is seen as extracted from the
     generator/buffer. The count parameter is used for calling
     `Sgen-functions'. *)
  | Sempty | Scons (_, _) -> d
  | Sapp (d1, d2) -> (
...
let rec peek_data : type v. v cell -> v option =
 fun s ->
...
let peek = function None -> None | Some s -> peek_data s

let rec junk_data : type v. v cell -> unit =
 fun s ->
  match s.data with
  | Scons (_, d) ->
  ...
  | Sbuffio b ->
      s.count <- succ s.count ;
      b.ind <- succ b.ind
  | _ -> ( match peek_data s with None -> () | Some _ -> junk_data s )

let junk = function None -> () | Some data -> junk_data data
...
let npeek n = function None -> [] | Some d -> npeek_data n d
let next s = match peek s with Some a -> junk s ; a | None -> raise Failure
let empty s = match peek s with Some _ -> raise Failure | None -> ()
...
let of_string s =
  let count = ref 0 in
  from (fun _ ->
      (* We cannot use the index passed by the [from] function directly because
         it returns the current stream count, with absolutely no guarantee that
         it will start from 0. For example, in the case of [Stream.icons 'c'
         (Stream.from_string "ab")], the first access to the string will be
         made with count [1] already. *)
      let c = !count in
      if c < String.length s then (
        incr count ;
        Some s.[c] )
      else None)
...
let of_bytes s =
  ...
  from (fun _ ->
      let c = !count in
      if c < Bytes.length s then (
        incr count ;
        Some (Bytes.get s c) )
      else None)

let of_channel ic =
  Some {count= 0; data= Sbuffio {ic; buff= Bytes.create 4096; len= 0; ind= 0}}
...
let rec dump : type v. (v -> unit) -> v t -> unit =
 fun f s ->
  print_string "{count = " ;
  print_int (count s) ;
  print_string "; data = " ;
...
and dump_data : type v. (v -> unit) -> v data -> unit =
 fun f -> function
