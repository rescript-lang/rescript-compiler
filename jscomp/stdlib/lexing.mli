...
(** A value of type [position] describes a point in a source file. [pos_fname]
    ...
    the beginning of the line (number of characters between the beginning of
    the lexbuf and the beginning of the line); [pos_cnum] is the offset of the
...
(** The type of lexer buffers. A lexer buffer is the argument passed to the
    ...
    number of characters read since the start of the [lexbuf]. The other fields
    are left unchanged by the lexing engine. In order to keep them accurate,
    they must be initialised before the first use of the lexbuf, and updated by
    the relevant lexer actions (i.e. at each end of line -- see also
    [new_line]). *)
...
(** Create a lexer buffer with the given function as its reading method. When
    ...
    it a byte sequence [s] and a byte count [n]. The function should put [n]
    bytes or fewer in [s], starting at index 0, and return the number of bytes
...
(** The following definitions are used by the generated scanners only. They are
    not intended to be used directly by user programs. *)
