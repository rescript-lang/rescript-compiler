...
(* Reasons that we don't [output_value] the set: 1. for performance , easy
   skipping and calcuate the length 2. cut dependency, otherwise its type is
   {!Ast_extract.String_set.t} *)
let write_ast (type t) ~(fname : string) ~output (kind : t Ml_binary.kind)
    (pt : t) : unit =
