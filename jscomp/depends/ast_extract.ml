...
(* dfs *)
let sort_files_by_dependencies ~(domain : String_set.t)
    (dependency_graph : String_set.t String_map.t) : string Queue.t =
  let next current = String_map.find_exn dependency_graph current in
...
let sort project_ml project_mli (ast_table : _ t String_map.t) =
  let domain =
    String_map.fold ast_table String_set.empty (fun k _ acc ->
        String_set.add k acc) in
  let h =
    String_map.map ast_table (fun {ast_info} ->
        match ast_info with
        ...
        | Ml_mli (_, impl, _, _, intf, _) ->
            String_set.union
              ...
              (read_parse_and_extract Mli (project_mli intf))) in
...
let check_suffix name =
  if
    Ext_path.check_suffix_case name ".ml"
    || Ext_path.check_suffix_case name ".mlt"
  then (`Ml, Ext_path.chop_extension_if_any name)
...
let collect_ast_map ppf files parse_implementation parse_interface =
  Ext_list.fold_left files String_map.empty (fun acc source_file ->
      match check_suffix source_file with
      | `Ml, opref -> (
          let module_name = Ext_modulename.module_name_of_file source_file in
          match String_map.find_exn acc module_name with
          | exception Not_found ->
              String_map.add acc module_name
                { ast_info=
                    Ml
                      (source_file, parse_implementation ppf source_file, opref)
                ; module_name }
          | { ast_info=
...
let collect_from_main ?(extra_dirs = []) ?(excludes = []) ?alias_map
  ...
  let ast_table =
    collect_ast_map ppf files parse_implementation parse_interface in
  ...
  let next module_name : String_set.t =
    let module_set =
      ...
      | {ast_info= Ml_mli (_, impl, _, _, intf, _)} ->
          String_set.union
            (read_parse_and_extract Ml (project_impl impl))
            (read_parse_and_extract Mli (project_intf intf)) in
    match alias_map with
    | None -> module_set
    | Some map ->
        String_set.fold
          (fun x acc ->
            String_set.add (String_hashtbl.find_default map x x) acc)
          module_set String_set.empty in
  let rec visit visiting path current =
    if String_set.mem current visiting then
      Bs_exception.error (Bs_cyclic_depends (current :: path))
    else if
      (not (String_hashtbl.mem visited current))
      && String_map.mem ast_table current
    then (
      String_set.iter
        (visit (String_set.add current visiting) (current :: path))
        (next current) ;
      Queue.push current result ;
      String_hashtbl.add visited current () ) in
  visit String_set.empty [] main_module ;
  (ast_table, result)

let build_queue ppf queue (ast_table : _ t String_map.t) after_parsing_impl
    after_parsing_sig =
  queue
...
let handle_queue ppf queue ast_table decorate_module_only
    decorate_interface_only decorate_module =
  queue
  |> Queue.iter (fun base ->
...
let build_lazy_queue ppf queue (ast_table : _ t String_map.t)
    after_parsing_impl after_parsing_sig =
  queue
  |> Queue.iter (fun modname ->
         match String_map.find_exn ast_table modname with
