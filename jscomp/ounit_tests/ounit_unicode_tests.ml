...
let suites =
  ...
  >::: [ (__LOC__ >:: fun _ -> Ast_utf8_string.transform_test {|x|} =~ {|x|})
       ; ( __LOC__
         >:: fun _ -> Ast_utf8_string.transform_test "a\nb" =~ {|a\nb|} )
       ...
       ; ( __LOC__
         >:: fun _ ->
         match
           Ast_utf8_string.transform_test {|你BuckleScript,好啊\uffff\|}
         with
         | exception Ast_utf8_string.Error (offset, _) ->
       ...
       ; ( __LOC__
         >:: fun _ ->
         "你好$(this)"
         ==~ [(0, 2, String, "你好"); (2, 9, varParen, "this")] ;
         "你好$this)"
         ==~ [ (0, 2, String, "你好"); (2, 7, var, "this")
             ; (7, 8, String, ")") ] ;
         {|\xff\xff你好 $x |}
         ==~ [ (0, 11, String, {|\xff\xff你好 |})
             ; (11, 13, var, "x"); (13, 14, String, " ") ] ;
         {|\xff\xff你好 $x 不吃亏了buckle $y $z = $sum|}
         ==~ [ (0, 11, String, {|\xff\xff你好 |})
             ; (11, 13, var, "x")
             ; (13, 25, String, {| 不吃亏了buckle |})
             ; (25, 27, var, "y"); (27, 28, String, " "); (28, 30, var, "z")
             ; (30, 33, String, " = "); (33, 37, var, "sum") ] )
       ; ( __LOC__
         >:: fun _ ->
         "你好 $(this_is_a_var)  x"
         ==~ [ (0, 3, String, "你好 ")
