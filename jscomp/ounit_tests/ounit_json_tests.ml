...
let rec strip (x : Ext_json_types.t) : Ext_json_noloc.t =
  ...
  | Obj {map} -> obj (String_map.map map strip)

let id_parsing_serializing x =
  let normal_s =
    Ext_json_noloc.to_string @@ strip
    @@ Ext_json_parse.parse_json_from_string x in
  let normal_ss =
    Ext_json_noloc.to_string @@ strip
    @@ Ext_json_parse.parse_json_from_string normal_s in
  if normal_s <> normal_ss then (
    prerr_endline "ERROR" ; prerr_endline normal_s ; prerr_endline normal_ss ) ;
...
let id_parsing_x2 x =
  ...
  if Ext_json_noloc.equal stru normal_ss then true
  else (
    prerr_endline "ERROR" ;
...
let suites =
  ...
  >::: [ (__LOC__ >:: fun _ -> List.iter id_parsing_serializing test_data)
       ...
       ; ( "empty trails"
         >:: fun _ ->
         ( OUnit.assert_raises Parse_error
         @@ fun _ ->
