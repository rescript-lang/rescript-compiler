...
let expr_mapper (self : mapper) (e : Parsetree.expression) =
  ...
  | Pexp_fun (arg_label, _, pat, body)
    ...
    | Some pexp_attributes -> default_expr_mapper self {e with pexp_attributes}
    )
  | _ -> default_expr_mapper self e

let typ_mapper (self : mapper) (typ : Parsetree.core_type) =
  match typ with
  | {ptyp_attributes; ptyp_desc= Ptyp_arrow (label, args, body); ptyp_loc= loc}
    -> (
    match Ext_list.exclude_with_val ptyp_attributes Ast_attributes.is_bs with
    | None -> default_typ_mapper self typ
    | Some ptyp_attributes -> default_typ_mapper self {typ with ptyp_attributes}
    )
  | _ -> default_typ_mapper self typ

let structure_item_mapper (self : mapper) (str : Parsetree.structure_item) =
  match str.pstr_desc with
  | Pstr_type (_rf, (_ :: _ as tdcls)) ->
