...
(** `jsoo_refmt_main` is the JSOO compilation entry point for building
    BuckleScript + Refmt as one bundle. This is usually the file you want to
    build for the full playground experience. *)
...
let implementation ?prefix ~use_super_errors ?(react_ppx_version = V3) impl str
    : Js.Unsafe.obj =
  ...
  try
    let code =
      match prefix with
      | None -> str
      | Some prefix -> prefix ^ str
    in
    let ast = impl (Lexing.from_string code) in
    let ast =
      match react_ppx_version with
      | V2 -> Reactjs_jsx_ppx_v2.rewrite_implementation ast
      | V3 -> Reactjs_jsx_ppx_v3.rewrite_implementation ast
    in
...
let load_module cmi_path cmi_content cmj_name cmj_content =
  ...
  Js_cmj_datasets.data_sets :=
    Map_string.add !Js_cmj_datasets.data_sets cmj_name
      (lazy (Js_cmj_format.from_string cmj_content))
      ...
      obj
        [|
          ...
          ( "shake_compile",
            inject
            @@ Js.wrap_meth_callback (fun _ code ->
                   shake_compile impl ~use_super_errors:false ~prefix
                     (Js.to_string code)) );
          ...
          ( "compile_super_errors_ppx_v2",
            inject
            @@ Js.wrap_meth_callback (fun _ code ->
                   compile impl ~use_super_errors:true ~react_ppx_version:V2
                     (Js.to_string code)) );
          ( "compile_super_errors_ppx_v3",
            inject
            @@ Js.wrap_meth_callback (fun _ code ->
                   compile impl ~use_super_errors:true ~react_ppx_version:V3
                     (Js.to_string code)) );
          ( "shake_compile_super_errors",
            inject
            @@ Js.wrap_meth_callback (fun _ code ->
                   shake_compile impl ~use_super_errors:true ~prefix
                     (Js.to_string code)) );
          ( "version",
            Js.Unsafe.inject
              (Js.string
          ...
          ( "load_module",
            ...
            @@ Js.wrap_meth_callback
                 (fun _ cmi_path cmi_content cmj_name cmj_content ->
                   ...
                   load_module cmi_path cmi_content (Js.to_string cmj_name)
                     cmj_bytestring) );
        |])


let () =
  make_compiler ~name:"ocaml"
    ~prefix:"[@@@bs.config {no_export}]\n#1 \"repl.ml\"\n" Parse.implementation


let () =
  make_compiler ~name:"reason"
    ~prefix:"[@bs.config {no_export: no_export}];\n#1 \"repl.re\";\n"
    reason_parse

(* local variables: *)
(* compile-command: "ocamlbuild -use-ocamlfind -pkg compiler-libs -no-hygiene
   driver.cmo" *)
(* end: *)
