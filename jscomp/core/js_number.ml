...
(* http://www.ecma-international.org/ecma-262/5.1/#sec-7.8.3
   ...
   [-](0...9){0...9|_}[.{0...9|_}][(e|E)][(e|E)[+|-](0...9){0...9|_}] ]} In
   ocaml, the interpretation of floating-point literals that fall outside the
   range of representable floating-point values is undefined. Also, (_) are
   accepted

   see https://github.com/ocaml/ocaml/pull/268 that ocaml will have HEXADECIMAL
   notation support in 4.3
...
let to_string (v : float) =
  if v = infinity then "Infinity"
    ...
    let vint =
      int_of_float v
      (* TODO: check if 32-bits will loose some precision *) in
...
(* call [to_string (float_of_string v)] directly would loose some precision and
   lost some information like '3.0' -> '3' *)
