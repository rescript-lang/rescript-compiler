...
let after_parsing_sig ppf sourcefile outputprefix ast =
  if !Js_config.binary_ast then
    Binary_ast.write_ast Mli ~fname:sourcefile
  ...
  if !Js_config.syntax_only then Warnings.check_fatal ()
    ...
    if not !Clflags.print_types then
      ...
      in
      Typemod.save_signature modulename tsg outputprefix sourcefile initial_env
        sg )
...
let after_parsing_impl ppf sourcefile outputprefix ast =
  if !Js_config.binary_ast then
    Binary_ast.write_ast ~fname:sourcefile Ml
      ~output:(outputprefix ^ Literals.suffix_mlast)
      ast ;
  if !Js_config.syntax_only then Warnings.check_fatal ()
  else (
    if Js_config.get_diagnose () then
    ...
    try
      let typedtree, coercion, finalenv, current_signature =
        ...
        |> print_if ppf Clflags.dump_typedtree (fun fmt (ty, co, _, _) ->
               Printtyped.implementation_with_coercion fmt (ty, co)) in
      ( if !Clflags.print_types || !Js_config.cmi_only then
        Warnings.check_fatal ()
      else
        (typedtree, coercion)
        ...
        try
          Lam_compile_main.lambda_as_module finalenv sourcefile outputprefix
            lambda
        with e ->
          (* Save to a file instead so that it will not scare user *)
          if Js_config.get_diagnose () then (
            let file = "bsc.dump" in
            Ext_pervasives.with_file_as_chan file (fun ch ->
                output_string ch
                @@ Printexc.raw_backtrace_to_string
                     (Printexc.get_raw_backtrace ())) ;
            Ext_log.err __LOC__
              "Compilation fatal error, stacktrace saved into %s when \
               compiling %s"
              file sourcefile ) ;
          raise e ) ;
...
let make_structure_item ~ns cunit : Parsetree.structure_item =
  ...
  Str.module_
    (Mb.mk {txt= cunit; loc}
       (Mod.ident {txt= Lident (Ext_namespace.make ~ns cunit); loc}))

(** decoding [.mlmap] keep in sync {!Bsb_namespace_map_gen.output} *)
let implementation_map ppf sourcefile outputprefix =
  ...
  let ml_ast =
    Ext_list.fold_left list_of_modules [] (fun acc line ->
        ...
        else make_structure_item ~ns line :: acc) in
  Compmisc.init_path false ;
  ml_ast
  |> print_if ppf Clflags.dump_parsetree Printast.implementation
  |> print_if ppf Clflags.dump_source Pprintast.structure
