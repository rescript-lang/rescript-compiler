...
(* "xx/lib/ocaml/js.cmj" Enhancement: This can be delegated to build system *)
      ...
      if Js_package_info.is_runtime_package current_package_info then
        ...
        | Es6_global ->
            (* lib/ocaml/xx.cmj --
               ...
               HACKING: FIXME maybe we can caching relative package path
               calculation or employ package map *)
            (* assert false *)
...
let string_of_module_id (dep_module_id : Lam_module_ident.t)
  ...
  fix_path_for_windows
    ...
    (* This may not be enough,
       ...
       + For cross packages, we may need settle down a single js package + We
       may need es6 path for dead code elimination

       But frankly, very few JS packages have no dependency, so having plugin
       may sound not that bad *)
    | Runtime ->
        ...
        | cmj_path, dep_package_info, case -> (
            ...
            | Package_found dep_pkg, Package_found cur_pkg -> (
                ...
                then
                  ...
                  (* TODO: we assume that both [x] and [path] could only be
                     relative path which is guaranteed by [-bs-package-output] *)
                else
                  match module_system with
                  | NodeJS | Es6 ->
#if BS_NATIVE then
                  ...
                  | Es6_global ->
                      (* lib/ocaml/xx.cmj --
                         ...
                         HACKING: FIXME maybe we can caching relative package
                         path calculation or employ package map *)
                      (* assert false *)
                      Ext_path.rel_normalized_absolute_path
                        ~from:
                          (Js_package_info.get_output_dir current_package_info
