...
(* let equal (x : t) ({name; module_systems}) = x.name = name &&
   Ext_list.for_all2_no_exn x.module_systems module_systems (fun (a0,a1)
   (b0,b1) -> a0 = b0 && a1 = b1) *)
...
let from_name (name : string) =
  if name = runtime_package_name then {name= Pkg_runtime; module_systems= []}
...
let add_npm_package_path (packages_info : t) (s : string) : t =
  if is_empty packages_info then
    Ext_pervasives.bad_argf
      "please set package name first using -bs-package-name "
  else
    let module_system, path =
      ...
      | [module_system; path] ->
          ( ( match module_system_of_string module_system with
            | Some x -> x
            | None ->
                Ext_pervasives.bad_argf "invalid module system %s"
                  module_system )
          , path )
      | [path] -> (NodeJS, path)
      | module_system :: path ->
          ( ( match module_system_of_string module_system with
            ...
            | None ->
                Ext_pervasives.bad_argf "invalid module system %s"
                  module_system )
          , String.concat ":" path )
      | _ -> Ext_pervasives.bad_argf "invalid npm package path: %s" s in
    { packages_info with
      module_systems= {module_system; path} :: packages_info.module_systems }

(* support es6 modules instead TODO: enrich ast to support import export
   http://www.ecma-international.org/ecma-262/6.0/#sec-imports For every
   module, we need [Ident.t] for accessing and [filename] for import, they are
