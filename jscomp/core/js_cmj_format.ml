...
type cmj_value = { arity : arity; persistent_closed_lambda : Lam.t option }
...
let verify_magic_in_beg ic =
  ...
  if buffer <> cmj_magic_number then
    Ext_fmt.failwithf ~loc:__LOC__
      "cmj files have incompatible versions, please rebuilt using the new \
       compiler : %s"
      __LOC__
...
let rec for_sure_not_changed (name : string) (header : string) =
  if Sys.file_exists name then (
    let ic = open_in_bin name in
    let holder = really_input_string ic fixed_length in
    close_in ic;
    holder = header )
  else false


(* This may cause some build system always rebuild maybe should not be turned on
   by default *)
...
let keyComp (a : string) (b, _) = Map_string.compare_key a b
...
let get_result midVal =
  ...
  ( cmj_value.arity,
    if Js_config.get_cross_module_inline () then
      cmj_value.persistent_closed_lambda
    else None )
...
let rec binarySearchAux arr lo hi (key : string) =
  ...
  if c = 0 then get_result midVal
    ...
    if hi = mid then
      ...
      if fst loVal = key then get_result loVal else not_found
    else binarySearchAux arr lo mid key
  else if (* a[lo] =< a[mid] < key <= a[hi] *)
          lo = mid then
    let hiVal = Array.unsafe_get arr hi in
    if fst hiVal = key then get_result hiVal else not_found
...
let binarySearch (sorted : keyed_cmj_values) (key : string) =
  ...
  if len = 0 then not_found
    ...
    if c < 0 then not_found
      ...
      if c2 > 0 then not_found else binarySearchAux sorted 0 (len - 1) key


(* FIXME: better error message when ocamldep get self-cycle *)
