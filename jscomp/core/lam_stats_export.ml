...
let values_of_export (meta : Lam_stats.t) (export_map : Lam.t Ident_map.t) :
    Js_cmj_format.cmj_value String_map.t =
  Ext_list.fold_left meta.exports String_map.empty (fun acc x ->
      let arity : Js_cmj_format.arity =
        ...
        | Some _ | None -> (
          match Ident_map.find_opt export_map x with
          ...
          | Some lambda ->
              ...
              then
                ...
                then
                  ...
                  (* TODO: 1. global need re-assocate when do the beta
                     reduction 2. [lambda_exports] is not precise *)
                  let free_variables =
                    Lam_closure.free_variables Ident_set.empty Ident_map.empty
                      lambda in
                  if
...
(* ATTENTION: all runtime modules, if it is not hard required, it should be
   okay to not reference it *)
let get_dependent_module_effect (meta : Lam_stats.t)
    (maybe_pure : string option) (external_ids : Lam_module_ident.t list) =
  if maybe_pure = None then
    let non_pure_module =
      Ext_list.find_first_not external_ids (fun id ->
          id.kind = Runtime
          || Lam_compile_env.query_and_add_if_not_exist id (Has_env meta.env)
               ~not_found:(fun _ -> false)
               ~found:(fun {pure} -> pure)) in
    Ext_option.map non_pure_module (fun x -> Lam_module_ident.name x)
  else maybe_pure

(* Note that [lambda_exports] is lambda expression to be exported for the js
   backend, we compile to js for the inliner, we try to seriaize it -- relies
   on other optimizations to make this happen {[ exports.Make = function ()
...
(* FIXME: make sure [-o] would not change its case add test for ns/non-ns *)
