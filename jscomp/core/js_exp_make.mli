...
val external_var_dot :
  ?comment:string -> external_name:string -> ?dot:string -> Ident.t -> t
(** [external_var_dot ~external_name ~dot id] Used in FFI *)
...
(** When in ES6 mode, we can use Symbol to guarantee its uniquess, we can not
    tag [js] object, since it can be frozen *)

(* val var_dot : ?comment:string -> Ident.t -> string -> t *)

(* val bind_var_call : ?comment:string -> Ident.t -> string -> t list -> t *)

(* val bind_call : ?comment:string -> J.expression -> string -> J.expression
   list -> t *)
...
val assign_by_int : ?comment:string -> t -> Js_op.jsint -> t -> t
(** [assign_by_int e i v] if the expression [e] is a temporay block which has
    no side effect, write to it does not really make sense, optimize it away *)

val assign_by_exp : ?comment:string -> t -> t -> t -> t
...
(** Note that this is coupled with how we encode block, if we use the
    `Object.defineProperty(..)` since the array already hold the length, this
    should be a nop *)
...
(** we don't expose a general interface, since a general interface is generally
    not safe *)
...
val resolve_and_apply : ?comment:string -> string -> t list -> t
