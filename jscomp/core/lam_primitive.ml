...
type t =
  ...
  | Pnegfloat
  (* | Pabsfloat *)
  (* is {!Pervasives.abs_float} %abs_float*)
  | Paddfloat
  | Psubfloat
  | Pmulfloat
  | Pdivfloat
...
let eq_record_representation (p : record_representation)
  ...
  | Record_unboxed b0 -> (
    match p1 with Record_unboxed b1 -> b0 = b1 | _ -> false )
...
let eq_primitive_approx (lhs : t) (rhs : t) =
  ...
  | Pccall {prim_name= n0; prim_native_name= nn0; _} -> (
    match rhs with
    | Pccall {prim_name= n1; prim_native_name= nn1; _} -> n0 = n1 && nn0 = nn1
    | _ -> false )
  | Pfield (n0, _dbg_info0) -> (
    match rhs with Pfield (n1, _dbg_info1) -> n0 = n1 | _ -> false )
  | Psetfield (i0, _dbg_info0) -> (
    match rhs with Psetfield (i1, _dbg_info1) -> i0 = i1 | _ -> false )
  | Pglobal_exception ident -> (
    match rhs with
    | Pglobal_exception ident2 -> Ident.same ident ident2
    | _ -> false )
  | Pmakeblock (i, _tag_info, mutable_flag) -> (
    match rhs with
    | Pmakeblock (i1, _, mutable_flag1) ->
        i = i1 && mutable_flag = mutable_flag1
    | _ -> false )
  | Pfloatfield (i0, _dbg_info) -> (
    match rhs with Pfloatfield (i1, _) -> i0 = i1 | _ -> false )
  | Psetfloatfield (i0, _dbg_info) -> (
    match rhs with Psetfloatfield (i1, _) -> i0 = i1 | _ -> false )
  | Pduprecord (record_repesentation0, i1) -> (
    match rhs with
    | Pduprecord (record_repesentation1, i2) ->
        eq_record_representation record_repesentation0 record_repesentation1
        && i1 = i2
    | _ -> false )
  | Pjs_call (prim_name, arg_types, ffi) -> (
    match rhs with
    | Pjs_call (prim_name1, arg_types1, ffi1) ->
        prim_name = prim_name1 && arg_types = arg_types1 && ffi = ffi1
    | _ -> false )
  ...
  | Pmakearray array_kind -> (
    match rhs with
    | Pmakearray array_kind1 -> Lam_compat.eq_array_kind array_kind array_kind1
    | _ -> false )
  ...
  | Pbintofint boxed_integer -> (
    match rhs with
    | Pbintofint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Pintofbint boxed_integer -> (
    match rhs with
    | Pintofbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Pnegbint boxed_integer -> (
    match rhs with
    | Pnegbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Paddbint boxed_integer -> (
    match rhs with
    | Paddbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Psubbint boxed_integer -> (
    match rhs with
    | Psubbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Pmulbint boxed_integer -> (
    match rhs with
    | Pmulbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Pdivbint boxed_integer -> (
    match rhs with
    | Pdivbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Pmodbint boxed_integer -> (
    match rhs with
    | Pmodbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Pandbint boxed_integer -> (
    match rhs with
    | Pandbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Porbint boxed_integer -> (
    match rhs with
    | Porbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Pxorbint boxed_integer -> (
    match rhs with
    | Pxorbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Plslbint boxed_integer -> (
    match rhs with
    | Plslbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Plsrbint boxed_integer -> (
    match rhs with
    | Plsrbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Pasrbint boxed_integer -> (
    match rhs with
    | Pasrbint boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Pbbswap boxed_integer -> (
    match rhs with
    | Pbbswap boxed_integer1 ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
    | _ -> false )
  | Pcvtbint (boxed_integer, boxed_integer1) -> (
    match rhs with
    | Pcvtbint (boxed_integer10, boxed_integer11) ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer10
        && Lam_compat.eq_boxed_integer boxed_integer1 boxed_integer11
    | _ -> false )
  | Pbintcomp (boxed_integer, comparison) -> (
    match rhs with
    | Pbintcomp (boxed_integer1, comparison1) ->
        Lam_compat.eq_boxed_integer boxed_integer boxed_integer1
        && Lam_compat.eq_comparison comparison comparison1
    | _ -> false )
  | Pbigarraydim dim -> (
    match rhs with Pbigarraydim dim1 -> dim = dim1 | _ -> false )
  | Pstring_load_16 str -> (
    match rhs with Pstring_load_16 str1 -> str = str1 | _ -> false )
  | Pstring_load_32 b -> (
    match rhs with Pstring_load_32 b1 -> b = b1 | _ -> false )
  | Pstring_load_64 b -> (
    match rhs with Pstring_load_64 b1 -> b = b1 | _ -> false )
  | Pstring_set_16 b -> (
    match rhs with Pstring_set_16 b1 -> b = b1 | _ -> false )
  | Pstring_set_32 b -> (
    match rhs with Pstring_set_32 b1 -> b = b1 | _ -> false )
  | Pstring_set_64 b -> (
    match rhs with Pstring_set_64 b1 -> b = b1 | _ -> false )
  | Pbigstring_load_16 b -> (
    match rhs with Pbigstring_load_16 b1 -> b = b1 | _ -> false )
  | Pbigstring_load_32 b -> (
    match rhs with Pbigstring_load_32 b1 -> b = b1 | _ -> false )
  | Pbigstring_load_64 b -> (
    match rhs with Pbigstring_load_64 b1 -> b = b1 | _ -> false )
  | Pbigstring_set_16 b -> (
    match rhs with Pbigstring_set_16 b1 -> b = b1 | _ -> false )
  | Pbigstring_set_32 b -> (
    match rhs with Pbigstring_set_32 b1 -> b = b1 | _ -> false )
  | Pbigstring_set_64 b -> (
    match rhs with Pbigstring_set_64 b1 -> b = b1 | _ -> false )
  | Pctconst compile_time_constant -> (
    ...
    | _ -> false )
  | Pjs_unsafe_downgrade (s, _loc) -> (
    match rhs with Pjs_unsafe_downgrade (s1, _) -> s = s1 | _ -> false )
  | Pjs_fn_make i -> ( match rhs with Pjs_fn_make i1 -> i = i1 | _ -> false )
  | Pjs_fn_run i -> ( match rhs with Pjs_fn_run i1 -> i = i1 | _ -> false )
  | Pjs_fn_method i -> (
    match rhs with Pjs_fn_method i1 -> i = i1 | _ -> false )
  | Pjs_fn_runmethod i -> (
    match rhs with Pjs_fn_runmethod i1 -> i = i1 | _ -> false )
  | Pbigarrayref _ | Pbigarrayset _ | Praw_js_function _ | Praw_js_code_exp _
   |Praw_js_code_stmt _ ->
