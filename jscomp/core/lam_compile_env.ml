...
type env_value =
  ...
  | Runtime of Js_cmj_load.cmj_load_info
      (** [Runtime (pure, path, cmj_format)] A built in module probably from our
          runtime primitives, so it does not have any [signature] *)
...
(* refer: [Env.find_pers_struct] [ find_in_path_uncap !load_path (name ^
   ".cmi")] *)
...
(** We should not provide "#moduleid" as output since when we print it in the
    end, it will be escaped quite ugly *)
let add_js_module (hint_name : External_ffi_types.module_bind_name)
  ...
  let id =
    Ident.create
      ...
      (* make sure the module name is capitalized TODO: maybe a warning if the
         user hint is not good *)
      | Phint_nothing -> Ext_modulename.js_id_name_of_hint_name module_name )
  in
  let lam_module_ident = Lam_module_ident.of_external id module_name in
  match Lam_module_ident.Hash.find_key_opt cached_tbl lam_module_ident with
  | None ->
      Lam_module_ident.Hash.add cached_tbl lam_module_ident External;
      id
...
let query_external_id_info (module_id : Ident.t) (name : string) : ident_info =
  ...
  let cmj_table =
    ...
    | None ->
        let cmj_load_info =
          Js_cmj_load.find_cmj_exn (module_id.name ^ Literals.suffix_cmj)
        in
  ...
  in
  let arity, closed_lambda = Js_cmj_format.query_by_name cmj_table name in
  { name; arity; closed_lambda (* TODO shall we cache the arity ?*) }
  ...
  | None -> (
      ...
      | Ml ->
          let ({ Js_cmj_load.cmj_table } as cmj_load_info) =
            Js_cmj_load.find_cmj_exn
              (Lam_module_ident.name id ^ Literals.suffix_cmj)
          in
          id +> Ml cmj_load_info;
...
let is_pure_module (oid : Lam_module_ident.t) =
  ...
  | Ml -> (
      ...
      | None -> (
          match
            Js_cmj_load.find_cmj_exn
              (Lam_module_ident.name oid ^ Literals.suffix_cmj)
          with
          | cmj_load_info ->
              oid +> Ml cmj_load_info;
...
let get_required_modules extras
    (hard_dependencies : Lam_module_ident.Hash_set.t) : Lam_module_ident.t list
    =
  ...
  Lam_module_ident.Hash_set.iter extras (fun id ->
      (if not @@ is_pure_module id then add hard_dependencies id : unit));
