...
let make_const ~lo ~hi =
  E.make_block ~comment:"int64" E.zero_int_literal record_info
    ...
    (* If we use unsigned int for lo field, then we can not use [E.int] which
       is assumed to to be signed int. Or we can use [Int64] to encode in the
       ast node? *)
    Immutable
...
let to_int32 args = E.to_int32 @@ get_lo (Ext_list.singleton_exn args)

let of_int32 (args : J.expression list) =
  match args with
  | [{expression_desc= Number (Int {i}); _}] ->
      if i < 0l then make_const ~lo:i ~hi:(-1l) else make_const ~lo:i ~hi:0l
...
(** Note if operands are not pure, we need hold shared value, which is a
    statement [var x = ... ; x], it does not fit current pipe-line fall back to
    a function call *)
let bit_op op runtime_call args =
  ...
  | [l; r] ->
      ...
      if
        Js_analyzer.is_okay_to_duplicate l
        && Js_analyzer.is_okay_to_duplicate r
      then make ~lo:(op (get_lo l) (get_lo r)) ~hi:(op (get_hi l) (get_hi r))
...
(* Safe constant propgation {[ Number.MAX_SAFE_INTEGER: Math.pow(2,53) - 1 ]}
   {[ Number.MIN_SAFE_INTEGER: - (Math.pow(2,53) -1) ]} Note that
   [Number._SAFE_INTEGER] is in ES6, we can hard code this number without
   bringing browser issue. *)
let of_float (args : J.expression list) = int64_call "of_float" args
let compare (args : J.expression list) = int64_call "compare" args
let of_string (args : J.expression list) = int64_call "of_string" args
let get64 = int64_call "get64"
let float_of_bits = int64_call "float_of_bits"
let bits_of_float = int64_call "bits_of_float"
let min args = int64_call "min" args
let max args = int64_call "max" args
