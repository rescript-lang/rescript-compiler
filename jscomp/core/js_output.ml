...
(** This can be merged with {!output_of_block_and_expression} *)
let output_of_expression (continuation : continuation) (exp : J.expression)
  ...
  | EffectCall (ReturnTrue _) -> make [S.return_stmt exp] ~output_finished:True
...
let output_of_block_and_expression (continuation : continuation)
  ...
  | EffectCall (ReturnTrue _) ->
      make
        (Ext_list.append_one block (S.return_stmt exp))
...
let to_break_block (x : t) : J.block * bool =
  ...
  | {output_finished= True; _} ->
      (block, false)
      (* value does not matter when [finished] is true TODO: check if it has
         side efects *)
...
(** TODO: make everything expression make inlining hard, and code not readable?
    1. readability dpends on how we print the expression 2. inlining needs
    generate symbols, which are statements, type mismatch we need capture
    [Exp e]
  ...
  | ( {block= []; value= Some e1; _}
    , ({block= []; value= Some e2; output_finished} as z) ) ->
      if Js_analyzer.no_side_effect_expression e1 then z
        (* It would optimize cases like [module aliases] Bigarray, List *)
