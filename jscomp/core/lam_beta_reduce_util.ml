...
let simple_beta_reduce params body args = 
  ...
  | Lapply { ap_func = 
          (Lvar _ | Lprim {primitive = Pfield _; args = [Lglobal_module _ ]} as f) ; ap_args ;  ap_info  ; ap_status = status;}
    ->  
    let () = 
      List.iter2 (fun p a -> Hash_ident.add param_hash p {lambda = a; used = false }) params args  
    in 
    ...
    begin match aux [] ap_args with 
      | new_args -> 
        let f = 
          ...
          | _ -> f in
        let result = 
          Hash_ident.fold param_hash (Lam.apply  f new_args  ap_info status )
