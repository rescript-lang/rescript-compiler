...
let mk_keep_locs f = ("-keep-locs", Arg.Unit f, " Keep locations in .cmi files")
...
let mk_safe_string f = ("-safe-string", Arg.Unit f, " Make strings immutable")
...
let bs_version_string =
  "BuckleScript " ^ Bs_version.version ^ " (Using OCaml" ^ Config.version
  ^ " )"
...
let print_standard_library () =
  print_string Bs_conditional_initial.standard_library ;
  print_newline () ;
  exit 0
...
let ocaml_options =
  ...
  [ mk_absname _absname; mk_annot _annot; mk_binannot _binannot; mk_c _c
  ; mk_config _config; mk_g_byt _g; mk_i _i; mk_I _I; mk_color _color
  ; (* mk_impl _impl; *)
    (* mk_intf _intf; *) mk_intf_suffix _intf_suffix
  ; mk_keep_docs _keep_docs; mk_keep_locs _keep_locs; mk_labels _labels
  ; mk_no_alias_deps _no_alias_deps
  ; mk_no_app_funct _no_app_funct
  ; mk_noassert _noassert; mk_nolabels _nolabels; mk_nostdlib _nostdlib
  ; mk_o _o; mk_open _open; mk_pp _pp; mk_ppx _ppx; mk_principal _principal
  ...
  ; mk_unsafe _unsafe; mk_v _v; mk_verbose _verbose; mk_version _version
  ; mk_vnum _vnum; mk_w _w; mk_warn_error _warn_error; mk_warn_help _warn_help
  ; mk_where _where; mk_color _color
  ; (* mk__ anonymous; *) mk_nopervasives _nopervasives
  ; mk_dsource _dsource; mk_dparsetree _dparsetree; mk_dtypedtree _dtypedtree
  ; mk_drawlambda _drawlambda; mk_dlambda _dlambda ]
