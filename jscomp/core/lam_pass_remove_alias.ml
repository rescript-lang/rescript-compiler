    ...
    *)      
    | Lapply{ap_func = 
               Lprim {primitive = Pfield (_, Fld_module {name = fld_name}) ;
                      ...
                      _} as l1;
             ap_args = args; ap_loc = loc ; ap_status = status; ap_inlined} ->
      begin
        ...
        | _ -> 
          Lam.apply (simpl l1) (Ext_list.map args simpl) loc status ap_inlined
    ...
    (* Function inlining interact with other optimizations...
        ...
        - code bloat 
    *)      
    | Lapply{ap_func = (Lvar v as fn);  ap_args = args; ap_loc = loc ; ap_status = status ; ap_inlined} ->
      (* Check info for always inlining *)

      (* Ext_log.dwarn __LOC__ "%s/%d" v.name v.stamp;     *)
      let normal () = Lam.apply ( simpl fn) (Ext_list.map args simpl) loc status ap_inlined in
      begin 
    ...
    | Lapply { ap_func = l1; ap_args =  ll;  ap_loc = loc; ap_status = status; ap_inlined} ->
      Lam.apply (simpl  l1) (Ext_list.map ll simpl) loc status ap_inlined
