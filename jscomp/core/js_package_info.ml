...
(* in runtime lib, [es6] and [es6-global] are treated the same way *)
...
(* for a single pass compilation, [output_dir] can be cached *)
...
(* support es6 modules instead
   ...
   http://www.ecma-international.org/ecma-262/6.0/#sec-imports For every module,
   we need [Ident.t] for accessing and [filename] for import, they are not
   necessarily the same.

   Es6 modules is not the same with commonjs, we use commonjs currently (play
   better with node)
