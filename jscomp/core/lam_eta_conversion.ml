...
*) 
let transform_under_supply n ap_info status fn args = 
  ...
  begin match Ext_list.fold_right (fn::args) ([],[])  (fun (lam : Lam.t) (acc, bind) ->
      ...
      | Lconst (Const_int _  
        ...
        | Const_pointer _ | Const_immstring _ | Const_js_true | Const_js_false | Const_js_undefined) 
...
let unsafe_adjust_to_arity loc ~(to_:int) ?(from : int option) (fn : Lam.t) : Lam.t = 
  let ap_info : Lam.ap_info = {ap_loc = loc; ap_inlined = Default_inline } in
  begin match from, fn  with 
      ...
      if from = to_ then 
        ...
        | _ -> 
          ...
          let cont = Lam.function_ 
              ...
              ~params:[]
              ~body:(
                Lam.apply new_fn [Lam.unit ] ap_info App_na 
              ) in 
        ...
        | Lfunction{params;body} -> 
          ...
          *)
          ...
          Lam.function_ ~attr:Lam.default_fn_attr
            ...
            ~body:(Lam.apply body (Ext_list.map extra_args Lam.var) ap_info App_na )
        | _ -> 
          let arity = to_ in 
          ...
          let cont = 
            Lam.function_ 
              ...
              ~body:(
                ...
                Lam.apply (Lam.apply new_fn (Ext_list.map first_args  Lam.var) ap_info App_infer_full ) (Ext_list.map rest_args Lam.var) ap_info App_na ) in 
    ...
    | None, _ ->      
      ...
      if to_ = 0 then 
        ...
        transform_under_supply to_ ap_info App_na fn [] 
  end 
