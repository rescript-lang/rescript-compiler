...
val if_ :
  ...
  -> ?else_:(* when it's not None, we also need make a variable declaration in
               the begininnig, however, we can optmize such case *)
            J.block
  -> J.expression
  -> J.block
  -> t
...
(** [int_switch ~declaration e clauses]
    ...
    The [declaration] is attached to peepwhole such pattern
...
val alias_variable : ?comment:string -> exp:J.expression -> Ident.t -> t
...
(** for ocaml function which returns unit it will be compiled into [return 0]
    in js *)

val continue_stmt : ?comment:string -> ?label:J.label -> unit -> t
