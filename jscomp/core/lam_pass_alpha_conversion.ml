...
let alpha_conversion (meta : Lam_stats.t) (lam : Lam.t) : Lam.t =
  let rec populateApplyInfo (args_arity : int list) (len : int) (fn : Lam.t)
      (args : Lam.t list) loc status : Lam.t =
    match args_arity with
    | 0 :: _ | [] -> Lam.apply (simpl fn) (Ext_list.map args simpl) loc status
    | x :: xs ->
        if x = len then
          ...
          Lam.apply
            (Lam.apply (simpl fn) (Ext_list.map first simpl) loc App_ml_full)
            (Ext_list.map rest simpl) loc status
  (* TODO refien *)
  and simpl (lam : Lam.t) =
    match lam with
    | Lconst _ -> lam
    ...
    | Lapply {fn= l1; args= ll; loc; status} ->
        ...
        let args_arity =
          Lam_arity.extract_arity (Lam_arity_analysis.get_arity meta l1) in
    ...
    | Lsend (u, m, o, ll, v) ->
        Lam.send u (simpl m) (simpl o) (Ext_list.map ll simpl) v in
  simpl lam
