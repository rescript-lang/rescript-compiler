...
(* Note [return] does indicate it is a tail position in most cases however, in
   an exception handler, return may not be in tail position to fix #1701 we
   play a trick that (ReturnTrue None) would never trigger tailcall, however,
   it preserves [return] semantics *)
...
let continuation_is_return (x : continuation) =
  match x with
  | EffectCall (ReturnTrue _) | NeedValue (ReturnTrue _) -> true
  | EffectCall ReturnFalse | NeedValue ReturnFalse | Declare _ | Assign _ ->
      false
...
let add_jmps (m : jmp_table) exit_id code_table =
  let map, handlers =
    Ext_list.fold_left_with_offset code_table (m, [])
      ...
      (fun {label; handler; bindings} (acc, handlers) order_id ->
        ...
        , (order_id, handler) :: handlers )) in
  (map, List.rev handlers)

let find_exn i cxt = Int_map.find_exn cxt.jmp_table i
