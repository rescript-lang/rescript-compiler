...
(** [hit_mask mask lambda] iters through the lambda set the bit of
    corresponding [id] if [id] is hit. As an optimization step if
    [mask_check_all_hit], there is no need to iter such lambda any more *)
let hit_mask (mask : Hash_set_ident_mask.t) (l : Lam.t) : bool =
  ...
  and hit (l : Lam.t) =
    ...
    | Lstaticcatch (e1, (_, vars), e2) -> hit e1 || hit e2
    | Ltrywith (e1, exn, e2) -> hit e1 || hit e2
    | Lfunction {body; params} -> hit body
    | Llet (str, id, arg, body) -> hit arg || hit body
    | Lletrec (decl, body) -> hit body || hit_list_snd decl
    | Lfor (v, e1, e2, dir, e3) -> hit e1 || hit e2 || hit e3
    | Lconst _ -> false
    | Lapply {fn; args; _} -> hit fn || hit_list args
    | Lglobal_module id (* playsafe *) -> false
    ...
    | Lsend (k, met, obj, args, _) -> hit met || hit obj || hit_list args in
  hit l
...
let preprocess_deps (groups : bindings) : _ * Ident.t array * Int_vec.t array =
  ...
  |> Ordered_hash_map_local_ident.iter (fun id lam key_index ->
         ...
         mask
         |> Hash_set_ident_mask.iter_and_unmask (fun ident hit ->
                if hit then
...
(** TODO: even for a singleton recursive function, tell whehter it is recursive
    or not ? *)
...
(* single binding, it does not make sense to do scc, we can eliminate {[ let
   rec f x = x + x ]}, but it happens rarely in real world *)
