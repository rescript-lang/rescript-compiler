...
let primitive ppf (prim : Lam_primitive.t) =
  ...
  | Pglobal_exception id -> fprintf ppf "global exception %a" Ident.print id
  ...
  | Pjs_call (prim_name, _, _) -> fprintf ppf "%s[js]" prim_name
  | Pjs_object_create obj_create -> fprintf ppf "[js.obj]"
...
type left_var = {kind: print_kind; id: Ident.t}
type left = Id of left_var | Nop
      ...
      let serializable_sigs =
        List.filter
          (fun x ->
            ...
            | _ -> false)
          signature in
      ( match Ext_list.nth_opt serializable_sigs pos with
      | Some
          ( Sig_value (i, _)
          | Sig_module (i, _, _)
          | Sig_typext (i, _, _)
...
let lambda use_env env ppf v =
  let rec lam ppf (l : Lam.t) =
    ...
    | Lam.Lglobal_module id -> fprintf ppf "global %a" Ident.print id
    ...
    | Lprim {primitive= Pfield (n, _); args= [Lglobal_module id]; _}
      when use_env ->
        fprintf ppf "%s.%s/%d" id.name (get_string (id, n) env) n
    | Lprim {primitive= Psetfield (n, _); args= [Lglobal_module id; e]; _}
      when use_env ->
        fprintf ppf "@[<2>(%s.%s/%d <- %a)@]" id.name
          (get_string (id, n) env)
          n lam e
    ...
    | Lswitch (larg, sw) ->
        let switch ppf (sw : Lam.switch) =
          ...
          | Some l ->
              if !spc then fprintf ppf "@ " else spc := true ;
              fprintf ppf "@[<hv 1>default:@ %a@]" lam l in
    ...
    | Lstringswitch (arg, cases, default) ->
        let switch ppf cases =
          ...
          | None -> () in
        fprintf ppf "@[<1>(stringswitch %a@ @[<v 0>%a@])@]" lam arg switch
          cases
    | Lstaticraise (i, ls) ->
        let lams ppf largs =
          List.iter (fun l -> fprintf ppf "@ %a" lam l) largs in
...
let rec flatten_seq acc (lam : Lam.t) =
  match lam with
  | Lsequence (l1, l2) -> flatten_seq (flatten_seq acc l1) l2
  | x -> x :: acc
...
let lambda_as_module env ppf (lam : Lam.t) =
  try
    ...
    | (Nop, Lprim {primitive= Pmakeblock (_, _, _); args= toplevels; _})
      :: rest ->
        (* let spc = ref false in *)
        List.iter (fun (left, l) ->
            match left with
            | Id {kind= k; id} ->
                fprintf ppf "@[<2>%a =%s@ %a@]@." Ident.print id (kind k)
                  (env_lambda env) l
            | Nop -> fprintf ppf "@[<2>%a@]@." (env_lambda env) l)
        @@ List.rev rest
