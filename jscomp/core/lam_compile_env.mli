...
(* The other dependencies are captured by querying either when [access] or when
   ...
   When we register such compile time dependency we classified it as Visit (ml),
   Builtin(built in js), External()

   For external, we never remove, we only consider remove dependency for Runtime
   and Visit, so when compile OCaml to Javascript, we only need pay attention to
   for those modules are actually used or not *)

val query_external_id_info : Ident.t -> string -> ident_info
(** [query_external_id_info id pos env found] will raise if not found *)

val is_pure_module : Lam_module_ident.t -> bool

val get_package_path_from_cmj :
  Lam_module_ident.t -> string * Js_packages_info.t * Js_cmj_format.cmj_case
