...
let iter h f =
  let rec do_bucket = function [] -> () | k :: rest -> f k ; do_bucket rest in
  let d = h.data in
...
let fold h init f =
  let rec do_bucket b accu =
    match b with [] -> accu | k :: rest -> do_bucket rest (f k accu) in
  let d = h.data in
...
let elements set = fold set [] (fun k acc -> k :: acc)

let stats h =
  let mbl = Ext_array.fold_left h.data 0 (fun m b -> max m (List.length b)) in
...
let rec small_bucket_mem eq_key key lst =
  ...
  | key1 :: rest -> (
      ...
      | key2 :: rest -> (
          ...
          match rest with
          | [] -> false
          | key3 :: rest -> eq_key key key3 || small_bucket_mem eq_key key rest
          ) )
