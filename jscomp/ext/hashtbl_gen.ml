...
and ('a, 'b) bucketlist = Empty | Cons of 'a * 'b * ('a, 'b) bucketlist
...
let iter h f =
  let rec do_bucket = function
    | Empty -> ()
    | Cons (k, d, rest) -> f k d ; do_bucket rest in
  let d = h.data in
...
let rec small_bucket_mem eq key (lst : _ bucketlist) =
  ...
  | Cons (k1, _, rest1) -> (
      ...
      | Cons (k2, _, rest2) -> (
          ...
          match rest2 with
          | Empty -> false
          | Cons (k3, _, rest3) -> eq key k3 || small_bucket_mem eq key rest3 )
      )
...
let rec small_bucket_opt eq key (lst : _ bucketlist) : _ option =
  ...
  | Empty -> None
  | Cons (k1, d1, rest1) -> (
      if eq key k1 then Some d1
      else
        match rest1 with
        | Empty -> None
        | Cons (k2, d2, rest2) -> (
            if eq key k2 then Some d2
            else
              match rest2 with
              | Empty -> None
              | Cons (k3, d3, rest3) ->
                  if eq key k3 then Some d3 else small_bucket_opt eq key rest3
            ) )
...
let rec small_bucket_key_opt eq key (lst : _ bucketlist) : _ option =
  ...
  | Empty -> None
  | Cons (k1, d1, rest1) -> (
      if eq key k1 then Some k1
      else
        match rest1 with
        | Empty -> None
        | Cons (k2, d2, rest2) -> (
            if eq key k2 then Some k2
            else
              match rest2 with
              | Empty -> None
              | Cons (k3, d3, rest3) ->
                  if eq key k3 then Some k3
                  else small_bucket_key_opt eq key rest3 ) )

let rec small_bucket_default eq key default (lst : _ bucketlist) =
  match lst with
  | Empty -> default
  | Cons (k1, d1, rest1) -> (
      if eq key k1 then d1
      else
        match rest1 with
        | Empty -> default
        | Cons (k2, d2, rest2) -> (
            if eq key k2 then d2
            else
              match rest2 with
              | Empty -> default
              | Cons (k3, d3, rest3) ->
                  if eq key k3 then d3
                  else small_bucket_default eq key default rest3 ) )
