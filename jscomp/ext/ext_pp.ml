...
module L = struct let space = " " let indent_str = "  " end
...
(* If we have [newline] in [s], all indentations will be broken in the future,
   we can detect this in [s] *)
let string t s =
  t.output_string s ;
...
let newline t =
  if not t.last_new_line then (
    t.output_char '\n' ;
    for _i = 0 to t.indent_level - 1 do
