...
(** return an index which is minus when [s] does not end with [beg] *)
...
let check_any_suffix_case_then_chop s suffixes =
  let rec aux suffixes =
    ...
    | x :: xs ->
        ...
        if id >= 0 then Some (String.sub s 0 id) else aux xs in
  aux suffixes

(** In OCaml 4.02.3, {!String.escaped} is locale senstive, this version try to
    make it not locale senstive, this bug is fixed in the compiler trunk *)
let escaped s =
...
(* it is unsafe to expose such API as unsafe since user can provide bad input
   range *)
...
(** TODO: optimize avoid nonterminating when string is empty *)
...
let is_valid_module_file (s : string) =
  ...
  | 'A' .. 'Z' | 'a' .. 'z' ->
      unsafe_for_all_range s ~start:1 ~finish:(len - 1) (fun x ->
          match x with
          | 'A' .. 'Z' | 'a' .. 'z' | '0' .. '9' | '_' | '\'' -> true
          | _ -> false)
  | _ -> false
...
let is_valid_source_name name : check_result =
  match
    check_any_suffix_case_then_chop name [".ml"; ".re"; ".mli"; ".rei"]
  with
  | None -> Suffix_mismatch
  | Some x -> if is_valid_module_file x then Good else Invalid_module_name
...
external compare : string -> string -> int = "caml_string_length_based_compare"
  [@@noalloc]
