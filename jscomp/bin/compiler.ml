...
include
  struct
    ...
    module Js_config :
      ...
      struct
        ...
        let cmd_set_module str =
          ...
          | "amdjs" -> default_env := Browser
          | _ ->
              if Ext_string.starts_with str "goog"
              then
                let len = String.length str in
                (if len = 4
                 then default_env := (Goog (Some ""))
                 else
                   if ((str.[4]) = ':') && (len > 5)
                   then
                     default_env :=
        ...
        let get_goog_package_name () =
          match !default_env with | Goog x -> x | Browser |NodeJS  -> None
          ...
          (match x with
           ...
           | Caml_block (el,mutable_flag,tag,tag_info) ->
               (match ((tag.expression_desc), tag_info) with
                | (Number (Int
                   { i = 0l;_}),(Tuple |Array |Variant _|Record _|NA |Module
                                 _|Constructor ("Some"|"::")))
    ...
    module Lam_compile_env :
      ...
      struct
        ...
        let get_goog_package_name id =
          query_and_add_if_not_exist id No_env ~not_found:(fun _  -> None)
    ...
    module Js_of_lam_exception :
      ...
      struct
               ...
               (Caml_block
                  ([exception_str;
                   E.runtime_call Js_config.builtin_exceptions "get_id" []],
                    mutable_flag, E.obj_int_tag_literal, NA));
             comment = None
           } : J.expression)
        ...
        let caml_set_oo_id args =
          ...
          | _ ->
              E.runtime_call Js_config.builtin_exceptions "caml_set_oo_id"
                args
      end 
    ...
    module Js_of_lam_record :
      ...
      struct
        module E = Js_exp_make
        let empty_record_info = Lambda.Record [||]
               ...
               (List.iter
                  (fun
                     ...
                     (match x.txt with
                      ...
                      | "js.set_index" -> js_set_index := true
                      | "js.get_index" -> js_get_index := true
                      | "js.obj" -> is_obj := true
                      | _ -> ()))) in
        ...
        let translate (({ meta = { env;_};_} as cxt) : Lam_compile_defs.cxt)
          ...
          (match prim with
           ...
           | Plazyforce |Pbittest |Pstring_load_16 _|Pstring_load_32 _
             ...
             |Pbigstring_set_64 _|Pint_as_pointer  ->
               let comment = "Missing primitve" in
                        ...
                        | Parsetree.PTyp _|Parsetree.PPat (_,_)
                          |Parsetree.PStr _ ->
                            Location.raise_errorf ~loc
                              "js.raw can only be applied to a string")
