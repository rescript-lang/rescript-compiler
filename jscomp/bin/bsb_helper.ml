...
let pack pack_byte_or_native ~batch_files ~includes ~cwd =
  ...
  | PackBytecode -> Literals.suffix_cmo, Literals.suffix_cma , Ext_filename.combine ocaml_dir "ocamlc.opt"
  | PackNative   -> Literals.suffix_cmx, Literals.suffix_cmxa, Ext_filename.combine ocaml_dir "ocamlopt.opt"
  end in
  let module_to_filepath = List.fold_left
    (fun m v ->
      String_map.add
