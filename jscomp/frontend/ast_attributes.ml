...
let first_char_special (x : string) =
  match String.unsafe_get x 0 with '#' | '?' | '%' -> true | _ -> false
...
let process_send_pipe (attrs : t) : (Parsetree.core_type * t) option =
  ...
  | [] -> None
  | _ -> (
      if not (Ext_list.exists_fst attrs (fun { txt } -> txt = "bs.send.pipe"))
      then (* fast path *)
        None
      else
        let ty = ref None in
            ...
            (fun acc (({ txt; loc }, payload) as attr) ->
              ...
              | "bs.send.pipe" -> (
                  ...
                  | None ->
                      ...
                      ({ Asttypes.txt = "bs.send"; loc }, Parsetree.PStr [])
                      :: acc
                  | Some _ ->
                      Location.raise_errorf ~loc "Duplicated bs.send.pipe")
              | _ -> attr :: acc)
        in
        match !ty with None -> assert false | Some ty -> Some (ty, attrs))
