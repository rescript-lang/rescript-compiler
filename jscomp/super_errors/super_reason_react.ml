...
let rec drill_through_tlink_and_tsubst t =
  match t.desc with
  | Tlink t | Tsubst t -> drill_through_tlink_and_tsubst t
  | _ -> t
...
let component_spec_weak_type_variables_in_module_type (mty : Types.module_type)
  ...
  | Mty_signature signature_values ->
      ...
      |> List.map (function
           ...
           | _ -> (false, false, false))
      |> List.filter (function false, false, false -> false | _ -> true)
  | _ -> []

(* `trace` is a funny data structure. It's an always even list of tuples. This
   error: this is foo (aliased as array(int)), wanted bar (aliased as
   array(string)) the incompatible part: int vs string gives the following
   `trace` data structure: [ (foo, array(int)), (bar, array(string)), (_, int),
   (_, string) ] *)
(* recursively walk the trace from right to left, calling f and checking if f
   matches part of the trace *)
let check_each_trace_chunk_bottom_up f t =
  ...
  let rec check f = function
    ...
    | (_alias2, type2) :: (_alias1, type1) :: rest ->
        if f (type1, type2) then true else check f rest
    | _ -> false in
  check f t_flipped

let state_escape_scope =
  check_each_trace_chunk_bottom_up (function
    (* https://github.com/BuckleScript/ocaml/blob/ddf5a739cc0978dab5e553443825791ba7b0cef9/typing/printtyp.ml?#L1348 *)
    (* so apparently that's the logic for detecting "the constructor out of
       scope" error *)
...
let is_component_spec_wanted_react_element =
  check_each_trace_chunk_bottom_up (function
    | ( { desc=
            Tconstr
              ( Pdot (Pident {name= "ReasonReact"}, "componentSpec", _)
      ...
      , { desc=
            Tconstr
              (Pdot (Pident {name= "ReasonReact"}, "reactElement", _), _, _) }
      ) ->
        true
    | _ -> false)
