(* This file has nothing to do with misc.ml in ocaml's source. Just thought
   it'd be an appropriate parallel to call it so *)
...
let leading_space_count str =
  let rec _leading_space_count str str_length current_index =
    ...
    else _leading_space_count str str_length (current_index + 1) in
...
(* Range coordinates all 1-indexed, like for editors. Otherwise this code would
   have way too many off-by-one errors *)
let print_file ~is_warning
    ~range:(* start_line_start_char inclusive, end_line_end_char exclusive *)
    ((start_line, start_line_start_char), (end_line, end_line_end_char)) ~lines
    ppf () =
  (* show 2 lines before & after the erroring lines. if there are too many
     lines, trim the middle *)
  ...
  (* sometimes the code's very indented, and we'd end up displaying quite a few
     columsn of leading whitespace; left-trim these. The general spirit is to
     center the erroring spot. In this case, almost literally *)
  (* to achieve this, go through the shown lines and check the minimum number
  ...
  for i = first_shown_line to last_shown_line do
    ...
    if String.length (String.trim current_line) == 0 then ()
      ...
      | None -> columns_to_cut := Some current_line_leading_space_count
      | Some n when n > current_line_leading_space_count ->
          columns_to_cut := Some current_line_leading_space_count
      | Some n -> ()
  done ;
  let columns_to_cut = match !columns_to_cut with None -> 0 | Some n -> n in
  (* coloring *)
  let highlighted_line_number : _ format =
    if is_warning then "@{<info>%s@}%a" else "@{<error>%s@}%a" in
  let print_char_maybe_highlight ~begin_highlight_line ~end_highlight_line ch =
    let highlighted_open_tag : _ format =
      if is_warning then "@{<info>" else "@{<error>" in
    if begin_highlight_line then fprintf ppf highlighted_open_tag ;
    fprintf ppf "%c@," ch ;
    if end_highlight_line then fprintf ppf "@}" in
  let print_separator ppf () =
    ...
    else fprintf ppf " @{<dim>@<1>┆@} " in
  fprintf ppf "@[<v 0>" ;
  (* inclusive *)
  for i = first_shown_line to last_shown_line do
    (* should some lines be ellipsed from the output? If we're showing more
       than 5 lines, then yes *)
    if end_line - start_line >= 5 && i >= start_line + 2 && i <= end_line - 2
    then (
      if i = start_line + 2 then
        (* Insert one line that's just a dimmed "..." *)
        let padded_line_number = pad "." max_line_number_number_of_digits in
        fprintf ppf "@{<dim>%s@}%a@{<dim>...@}@," padded_line_number
          print_separator () )
    else
      let current_line = lines.(i - 1) in
      let padded_line_number =
        pad (string_of_int i) max_line_number_number_of_digits in
      fprintf ppf "@[<h 0>" ;
      fprintf ppf "@[<h 0>" ;
      ...
      for j = 1 to offset_current_line_length do
        ...
        | Is_error_end_line ->
            print_char_maybe_highlight ~begin_highlight_line:(j = 1)
              ...
              current_char
        | Not_error_line ->
            print_char_maybe_highlight ~begin_highlight_line:false
              ~end_highlight_line:false current_char
      done ;
      fprintf ppf "@]" ;
      (* hov *)
      fprintf ppf "@]@," (* h *)
