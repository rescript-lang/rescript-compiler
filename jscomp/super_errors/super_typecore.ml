...
let tagged tag fn ppf arg =
  Format.pp_open_tag ppf tag ; fn ppf arg ; Format.pp_close_tag ppf ()
...
let print_simple_conversion ppf (actual, expected) =
  try
    ...
    Format.pp_print_newline ppf () ;
    fprintf ppf
      "You can convert a @{<info>%s@} to a @{<info>%s@} with @{<info>%s@}."
...
let print_simple_message ppf = function
  | "float", "int" ->
      fprintf ppf
        "@[If this is a literal, you want a number without a trailing dot \
         (e.g. @{<info>20@}).@]"
  | "int", "float" ->
      fprintf ppf
        "@[If this is a literal, you want a number with a trailing dot (e.g. \
         @{<info>20.@}).@]"
  | _ -> ()

let show_extra_help ppf env trace =
  match bottom_aliases trace with
  | Some
      ( {desc= Tconstr (actualPath, actualArgs, _)}
      , {desc= Tconstr (expectedPath, expextedArgs, _)} ) -> (
    match (actualPath, actualArgs, expectedPath, expextedArgs) with
    | Pident {name= actualName}, [], Pident {name= expectedName}, [] ->
        print_simple_conversion ppf (actualName, expectedName) ;
        print_simple_message ppf (actualName, expectedName)
    | _ -> () )
  | _ -> ()

(* given type1 is foo => bar => baz(qux) and type 2 is bar => baz(qux), return
   Some(foo) *)
let rec collect_missing_arguments env type1 type2 =
  match type1 with
  (* why do we use Ctype.matches here? Please see
     https://github.com/BuckleScript/bucklescript/pull/2554 *)
...
let check_bs_arity_mismatch ppf trace =
  let arity t =
    ...
    | Tvariant {row_fields= [(label, _)]} ->
        ...
        if arity_len < label_len && String.sub label 0 arity_len = arity_str
        then
          try
            Some
              (int_of_string
                 (String.sub label arity_len (label_len - arity_len)))
          with _ -> None
        else None
    | _ -> None in
  let check_mismatch t1 t2 =
    match (arity t1, arity t2) with
    | Some n1, Some n2 ->
        fprintf ppf
          "@[@{<info>Found uncurried application [@bs] with arity %d, where \
           arity %d was expected.@}@]"
          n1 n2 ;
        true
    | None, _ | _, None -> false in
  let rec traverse = function
    | (_arity1, type1) :: (_arity2, type2) :: rest ->
        if traverse rest then true else check_mismatch type1 type2
    | _ -> false in
...
let print_expr_type_clash env trace ppf =
  ...
  if
    Super_reason_react.state_escape_scope trace
    && Super_reason_react.trace_both_component_spec trace
  then
    fprintf ppf
      "@[<v>@[@{<info>Is this a ReasonReact reducerComponent or component \
       with retained props?@}@ If so, is the type for state, retained props \
       or action declared _after_@ the component declaration?@ @{<info>Moving \
       these types above the component declaration@} should resolve this!@]@]"
    (* This one above shouldn't catch any false positives, so we can safely not
       display the original type clash error. *)
  else if Super_reason_react.is_component_spec_wanted_react_element trace then
    fprintf ppf
      "@[<v>@[@{<info>Did you want to create a ReasonReact element without \
       using JSX?@}@ If not, disregard this.@ If so, don't forget to wrap \
       this value in `ReasonReact.element` yourself:@ \
       https://reasonml.github.io/reason-react/docs/en/jsx.html#capitalized@]@,@,@[@{<info>Here's \
  ...
  let print_arguments =
    Format.pp_print_list
      ...
      (fun ppf (label, argtype) ->
        ...
        | Labelled label -> fprintf ppf "@[(~%s: %a)@]" label type_expr argtype
        | Optional label -> fprintf ppf "@[(?%s: %a)@]" label type_expr argtype)
  in
  match missing_arguments with
  | Some [singleArgument] ->
      (* btw, you can't say "final arguments". Intermediate labeled arguments
         might be the ones missing *)
      fprintf ppf "@[@{<info>This call is missing an argument@} of type@ %a@]"
        print_arguments [singleArgument]
  | Some arguments ->
      fprintf ppf
        "@[<hv>@{<info>This call is missing arguments@} of type:@ %a@]"
  ...
  | None ->
      ...
      | Some [singleParameter] ->
          fprintf ppf
            "@[This value might need to be @{<info>wrapped in a function@ \
             that@ takes@ an@ extra@ parameter@}@ of@ type@ %a@]@,@,"
            print_arguments [singleParameter] ;
          fprintf ppf "@[@{<info>Here's the original error message@}@]@,"
      | Some arguments ->
          fprintf ppf
            "@[This value seems to @{<info>need to be wrapped in a function \
             that takes extra@ arguments@}@ of@ type:@ @[<hv>%a@]@]@,@,"
            print_arguments arguments ;
          fprintf ppf "@[@{<info>Here's the original error message@}@]@,"
      | None -> () ) ;
      ...
      super_report_unification_error ppf env trace
        (function ppf -> fprintf ppf "This has type:")
        (function ppf -> fprintf ppf "But somewhere wanted:") ;
      show_extra_help ppf env trace

(* taken from
   https://github.com/BuckleScript/ocaml/blob/d4144647d1bf9bc7dc3aadc24c25a7efa3a67915/typing/typecore.ml#L3769 *)
(* modified branches are commented *)
let report_error env ppf = function
  | Typecore.Constructor_arity_mismatch (lid, expected, provided) ->
      (* modified *)
      fprintf ppf
        "@[This variant constructor, %a, expects %i %s; here, we've %sfound \
         %i.@]"
  ...
  | Label_mismatch (lid, trace) ->
      ...
      super_report_unification_error ppf env trace
        (function
          | ppf ->
              fprintf ppf "The record field %a@ belongs to the type" longident
                lid)
  ...
  | Apply_non_function typ -> (
      ...
      | Tarrow (_, _inputType, returnType, _) ->
          let rec countNumberOfArgs count {desc} =
            match desc with
            | Tarrow (_, _inputType, returnType, _) ->
                countNumberOfArgs (count + 1) returnType
            | _ -> count in
          ...
          fprintf ppf "@[<v>@[<2>This function has type@ @{<info>%a@}@]"
            type_expr typ ;
          fprintf ppf
            "@ @[It only accepts %i %s; here, it's called with more.@]@]"
            acceptsCount
            (if acceptsCount == 1 then "argument" else "arguments")
      | Tconstr
          ( Path.Pdot
              ( ( Pdot (Path.Pident {name= "Js"}, "Internal", _)
                | Pident {name= "Js_internal"} )
              , ("fn" | "meth")
              , _ )
          , _
          , _ ) ->
          fprintf ppf
            "@[<v>This is an uncurried BuckleScript function. @{<info>It must \
             be applied with a dot@}.@,@,Like this: @{<info>foo(. a, \
             b)@}@,Not like this: @{<dim>foo(a, b)@}@,@,This guarantees that \
             your function is fully applied. More info \
             here:@,https://bucklescript.github.io/docs/en/function.html#solution-guaranteed-uncurrying@]"
      | _ ->
          fprintf ppf "@[<v>@[<2>This expression has type@ %a@]@ %s@]"
            type_expr typ "It is not a function." )
      ...
      super_report_unification_error ppf env trace
        (function
          | ppf ->
              ...
              fprintf ppf
                "This expression cannot be coerced to type@;<1 2>%a;@ it has \
                 type"
...
let setup () =
  Location.register_error_of_exn (function
    ...
    | Typecore.Error_forward err -> Some err
    | _ -> None)
