...
let print_loc ~normalizedRange ppf loc =
  ...
  if file = "//toplevel//" then
    if highlight_locations ppf [loc] then ()
    else
      fprintf ppf "Characters %i-%i" loc.loc_start.pos_cnum
      ...
      | Some
        ...
        ->
          if start_line = end_line then
            ...
            fprintf ppf " @{<dim>%i:%i-%i:%i@}" start_line
              start_line_start_char end_line end_line_end_char in
    fprintf ppf "@{<filename>%a@}%a" print_filename file dim_loc
      normalizedRange

let print ~message_kind intro ppf loc =
  ( if loc.loc_start.pos_fname = "//toplevel//" && highlight_locations ppf [loc]
  then ()
  else
  ...
  (* line is 1-indexed, column is 0-indexed. We convert all of them to
     1-indexed to avoid confusion *)
  ...
  let normalizedRange =
    if start_char == -1 || end_char == -1 then
      ...
      Some ((start_line, start_char + 1), (end_line, end_char)) in
  ...
  | Some range -> (
    try
      ...
      (* we're putting the line break `@,` here rather than above, because this
         branch might not be reached (aka no inline file content display) so we
         don't wanna end up with two line breaks in the the consequent *)
      fprintf ppf "@,%a"
        ...
        ()
    with
    (* this might happen if the file is e.g. "", "_none_" or any of the fake
       file name placeholders. we've already printed the location above, so
       nothing more to do here. *)
    | Sys_error _ ->
      () )

(* taken from
   https://github.com/BuckleScript/ocaml/blob/d4144647d1bf9bc7dc3aadc24c25a7efa3a67915/parsing/location.ml#L380 *)
(* This is the error report entry point. We'll replace the default reporter
   with this one. *)
  ...
  let highlighted =
    if if_highlight <> "" then
      let rec collect_locs locs {Location.loc; sub; if_highlight; _} =
        List.fold_left collect_locs (loc :: locs) sub in
      let locs = collect_locs [] err in
      Location.highlight_locations ppf locs
    else false in
  if highlighted then Format.pp_print_string ppf if_highlight
  else (
    setup_colors () ;
    (* open a vertical box. Everything in our message is indented 2 spaces *)
    Format.fprintf ppf "@[<v 2>@,%a@,%s@,@]"
      (print ~message_kind:`error "We've found a bug for you!")
      loc msg ;
    List.iter (Format.fprintf ppf "@,@[%a@]" super_error_reporter) sub
    (* no need to flush here; location's report_exception (which uses this
       ultimately) flushes *) )

(* extracted from
   https://github.com/BuckleScript/ocaml/blob/d4144647d1bf9bc7dc3aadc24c25a7efa3a67915/parsing/location.ml#L299 *)
(* This is the warning report entry point. We'll replace the default printer
   with this one *)
let super_warning_printer loc ppf w =
  ...
  | `Active {Warnings.number; message; is_error; sub_locs} ->
      ...
      Format.fprintf ppf "@[<v 2>@,%a@,%s@,@]"
        ...
        loc (Super_warnings.message w)

(* at this point, you can display sub_locs too, from e.g.
   https://github.com/ocaml/ocaml/commit/f6d53cc38f87c67fbf49109f5fb79a0334bab17a
   but we won't bother for now *)

(* taken from
   https://github.com/BuckleScript/ocaml/blob/d4144647d1bf9bc7dc3aadc24c25a7efa3a67915/parsing/location.ml#L354 *)
...
let errorf ?(loc = none) ?(sub = []) ?(if_highlight = "") fmt =
  Location.pp_ksprintf ~before:print_phanton_error_prefix
...
(* This will be called in super_main. This is how you override the default
