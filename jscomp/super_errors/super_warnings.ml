...
(* taken from
   https://github.com/BuckleScript/ocaml/blob/d4144647d1bf9bc7dc3aadc24c25a7efa3a67915/utils/warnings.ml#L251 *)
...
let message (warning : Warnings.t) =
  ...
  | Unerasable_optional_argument ->
      String.concat ""
        [ "This optional parameter in final position will, in practice, not \
           be optional.\n"
        ...
        ; "  Formal rule: an optional argument is considered intentionally \
           omitted when the 1st positional (i.e. neither labeled nor \
           optional) argument defined after it is passed in." ]
  ...
  | Statement_type ->
      "This expression returns a value, but you're not doing anything with \
       it. If this is on purpose, wrap it with `ignore`."
  | Useless_record_with ->
