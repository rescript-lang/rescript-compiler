...
(* taken from
   https://github.com/BuckleScript/ocaml/blob/d4144647d1bf9bc7dc3aadc24c25a7efa3a67915/typing/env.ml#L1842 *)
...
let report_error ppf = function
  ...
  | Inconsistent_import (name, source1, source2) ->
      (* modified *)
      fprintf ppf
        "@[<v>@[@{<info>It's possible that your build is stale.@}@ Try to \
         clean the artifacts and build again?@]@,@,@[@{<info>Here's the \
...
let setup () =
  Location.register_error_of_exn (function
    | Env.Error
        ((Missing_module (loc, _, _) | Illegal_value_name (loc, _)) as err)
