...
let lognot n = n lxor (-1l)
let cmn q a b x s t = 
  let a = a + q + x + t in
  ((a lsl s) lor (a lsr (32 - s))) +  b


let  f a b c d x s t = 
  cmn ((b land c) lor (lognot b land d)) a b x s t


let g a b c d x s t =
  cmn ((b land d) lor (c land (lognot d))) a b x s t

;;
let h a b c d x s t = 
  cmn (b lxor c lxor d) a b x s t
;;

let i a b c d x s t = 
  cmn (c lxor (b lor (lognot d))) a b x s t
...
let md5_string (s : string) (start:int) (len:int) : string = 
  ...
  for i = 0 to  i_end do 
    md5blk.!(i / 4 ) <- 
      md5blk.!(i / 4) lor ( s_tail.![i] lsl ((i mod 4) lsl 3))
  done ;
  let i = i_end + 1 in
  md5blk.!(i / 4 ) <-  md5blk.!(i / 4 ) lor (0x80l lsl ((i mod 4) lsl 3)) ;
  if i > 55 then
    begin 
      cycle state md5blk;
      for i = 0 to 15 do 
  ...
  Caml_string_extern.of_small_int32_array [|
    state.!(0) land 0xff;
    (state.!(0) asr 8) land 0xff;
    (state.!(0) asr 16) land 0xff;
    (state.!(0) asr 24) land 0xff;

    state.!(1) land 0xff;
    (state.!(1) asr 8) land 0xff;
    (state.!(1) asr 16) land 0xff;
    (state.!(1) asr 24) land 0xff;

    state.!(2) land 0xff;
    (state.!(2) asr 8) land 0xff;
    (state.!(2) asr 16) land 0xff;
    (state.!(2) asr 24) land 0xff;

    state.!(3) land 0xff;
    (state.!(3) asr 8) land 0xff;
    (state.!(3) asr 16) land 0xff;
    (state.!(3) asr 24) land 0xff;
