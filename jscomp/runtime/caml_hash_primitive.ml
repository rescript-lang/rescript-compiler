...
let caml_hash_mix_string h s =
  ...
  for i = 0 to block do
    ...
    let w =
      Caml_char.code s.[j]
      lor (Caml_char.code s.[j + 1] lsl 8)
      lor (Caml_char.code s.[j + 2] lsl 16)
      lor (Caml_char.code s.[j + 3] lsl 24) in
  ...
  ( if modulo <> 0 then
    let w =
      if modulo = 3 then
        (Caml_char.code s.[len - 1] lsl 16)
        lor (Caml_char.code s.[len - 2] lsl 8)
        lor Caml_char.code s.[len - 3]
      else if modulo = 2 then
        (Caml_char.code s.[len - 1] lsl 8) lor Caml_char.code s.[len - 2]
      else Caml_char.code s.[len - 1] in
    hash := caml_hash_mix_int !hash (Caml_nativeint_extern.of_int w) ) ;
