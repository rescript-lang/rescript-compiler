...
(* let int32 = Int32_array.make [| x |] in let float32 =
   Float32_array.fromBuffer ( Int32_array.buffer int32) in
...
(* let float32 = Float32_array.make [|x|] in Int32_array.unsafe_get
   (Int32_array.fromBuffer (Float32_array.buffer float32)) 0 *)
...
let caml_log1p_float : float -> float = function
  | x ->
      let y = 1. +. x in
      let z = y -. 1. in
