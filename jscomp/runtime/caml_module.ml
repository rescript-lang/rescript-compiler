...
(** This module replaced camlinternalMod completely. Note we can replace
    {!CamlinternalMod} completely, but it is not replaced due to we believe
    this is an even low level dependency *)
...
let init_mod (loc : string * int * int) (shape : shape) =
  ...
  let rec loop (shape : shape) (struct_ : Caml_obj_extern.t array) idx =
    ...
    | Value v -> struct_.(idx) <- v in
  let res = (Obj.magic [||] : Caml_obj_extern.t array) in
  loop shape res 0 ; res.(0)

(* Note the [shape] passed between [init_mod] and [update_mod] is always the
   same and we assume [module] is encoded as an array *)
let update_mod (shape : shape) (o : Caml_obj_extern.t) (n : Caml_obj_extern.t)
    : unit =
  let module Array = Caml_array_extern in
  let rec aux (shape : shape) o n parent i =
    match shape with
    | Function -> Caml_obj_extern.set_field parent i n
    | Lazy | Class -> Caml_obj.caml_update_dummy o n
    | Module comps ->
        for i = 0 to Array.length comps - 1 do
          aux comps.(i)
            (Caml_obj_extern.field o i)
            (Caml_obj_extern.field n i)
            o i
        done
    | Value _ -> () in
  match shape with
