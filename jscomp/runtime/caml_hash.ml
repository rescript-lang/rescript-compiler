...
type 'a cell = {content: 'a; mutable next: 'a cell_opt}

and 'a cell_opt = 'a cell Caml_undefined_extern.t

and 'a t =
  {mutable length: int; mutable first: 'a cell_opt; mutable last: 'a cell_opt}
[@@bs.deriving abstract]

let create_queue () =
  t ~length:0 ~first:Caml_undefined_extern.empty
...
let push_back (q : 'a t) (v : 'a) =
  let cell =
    ...
    @@ cell ~content:v ~next:Caml_undefined_extern.empty in
...
let caml_hash (count : int) _limit (seed : nativeint) (obj : Caml_obj_extern.t)
    : nativeint =
  ...
  if Js.typeof obj = "number" then (
    ...
    caml_hash_final_mix !hash (* TODO: hash [null] [undefined] as well *) )
  else
    let queue = create_queue () in
    let num = ref count in
    let () = push_back queue obj ; decr num in
    while (not (is_empty_queue queue)) && !num > 0 do
      ...
      if Js.typeof obj = "number" then (
      ...
      else if Js.typeof obj = "symbol" then assert false (* TODO *)
        ...
        | Some size ->
            ...
            if tag = 248 (* Obj.object_tag*) then
              hash :=
                caml_hash_mix_int !hash
                  (Caml_nativeint_extern.of_int (oo_id obj))
            else (
              hash :=
                caml_hash_mix_int !hash (Caml_nativeint_extern.of_int tag) ;
              let block =
                let v = size - 1 in
                if v < !num then v else !num in
              for i = 0 to block do
