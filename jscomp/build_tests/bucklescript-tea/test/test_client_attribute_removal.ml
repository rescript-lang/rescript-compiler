...
type message = Select of string | Delete [@@bs.deriving {accessors}]
...
let lang l is_selected =
  li
    [ onClick (Select l); style "color" "blue"
...
let render_languages selected languages =
  let is_selected selected language =
    match selected with Some l -> language == l | None -> false in
  let rendered =
    List.map (fun l -> lang l (is_selected selected l)) languages in
  ul [] rendered
...
let main =
  let initialState =
    {selected= Some "Erlang"; languages= ["Erlang"; "Ocaml"; "Clojure"]} in
