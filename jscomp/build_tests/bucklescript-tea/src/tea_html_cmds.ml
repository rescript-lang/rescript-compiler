let focus id =
  Tea_cmd.call (fun _enqueue ->
      match Js.Null_undefined.to_opt (Web.Document.getElementById id) with
      ...
      | Some elem ->
          ...
          let cb _ignored =
            let _unhandledID = Web.Window.requestAnimationFrame ecb in
            () in
          (* One to get out of the current render frame*)
          let _unhandledID = Web.Window.requestAnimationFrame cb in
