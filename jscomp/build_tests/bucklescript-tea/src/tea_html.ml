...
(* `href` is actually an attribute, not a property, but need it here for Elm
   compat... *)
let href str = attribute "" "href" str

(* `src` is actually an attribute, not a property, but need it here for Elm
   compat... *)
...
let onInputOpt ?(key = "") msg =
  onCB "input" key (fun ev ->
      ...
      | None -> None
      | Some target -> (
        match Js.Undefined.to_opt target##value with
        | None -> None
...
let onCheckOpt ?(key = "") msg =
  onCB "change" key (fun ev ->
      ...
      | None -> None
      | Some target -> (
        match Js.Undefined.to_opt target##checked with
        | None -> None
        | Some value -> msg value ))
