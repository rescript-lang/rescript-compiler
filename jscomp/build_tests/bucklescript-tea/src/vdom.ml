...
(* type 'msg userkey = | UserkeyString of string | UserkeyMsg of 'msg *)
...
type 'msg eventCache =
  {handler: Web.Node.event_cb; cb: (Web.Node.event -> 'msg option) ref}

type 'msg property =
  | NoProp
  | RawProp of string * string
  (* TODO: This last string needs to be made something more generic, maybe a
     function... *)
...
(* type 'msg taggerCallbacks = { renderToHtmlString : unit -> string ;
...
type 'msg t =
  ...
  (* | ArrayNode of string * string * string * string * 'msg property array *
     'msg t array *)
  (* LazyGen (key, fnGenerator) *)
  | LazyGen of string * (unit -> 'msg t) * 'msg t ref
  (* Tagger (toString, toDom, toVNodes) *)
  (* | Tagger of (unit -> string) * ('msg applicationCallbacks ref ->
     Web.Node.t -> Web.Node.t -> int -> 'msg t list -> Web.Node.t) * (unit ->
     'msg t) *)
  (* Tagger (tagger, vdom) *)
  | Tagger of
...
(* let arraynode namespace tagName key unique props vdoms = ArrayNode
...
(* TODO: Make a vdom 'patcher' that binds into the actual DOM for hot-loading
...
let eventHandler_Mutate callbacks elem (oldName : string) (newName : string)
    oldHandlerType newHandlerType oldCache newCache =
  match !oldCache with
  | None ->
...
let patchVNodesOnElems_PropertiesApply_RemoveAdd callbacks elem idx oldProp
    newProp =
  let () =
...
let patchVNodesOnElems_PropertiesApply_Mutate _callbacks elem _idx oldProp =
  function
  | NoProp as _newProp ->
      failwith
  ...
  | RawProp (k, v) as _newProp ->
      ...
      (* Wow setting properties is slow, unsure how to optimize this further
         though... *)
  ...
  | Style s as _newProp -> (
    match(* let () = Js.log ("Mutating Style", elem, oldProp, _newProp) in *)
         [@ocaml.warning "-4"]
      oldProp
    with
    | Style oldS ->
...
let rec patchVNodesOnElems_PropertiesApply callbacks elem idx oldProperties
    newProperties =
  (* let () = Js.log ("PROPERTY-APPLY", elem, idx, oldProperties,
     newProperties) in *)
  ...
  (* let () = patchVNodesOnElems_PropertiesApply_Add callbacks elem idx newProp
     in patchVNodesOnElems_PropertiesApply callbacks elem (idx+1) [] newRest *)
  | _oldProp :: _oldRest, [] ->
      (* Well this is wrong, the lengths should never differ, recreate node *)
      false
  (* let () = patchVNodesOnElems_PropertiesApply_Remove callbacks elem idx
     oldProp in patchVNodesOnElems_PropertiesApply callbacks elem (idx+1) []
     oldRest *)
  (* NoProp *)
  ...
  | ( (RawProp (oldK, oldV) as oldProp) :: oldRest
    , (RawProp (newK, newV) as newProp) :: newRest ) ->
      (* let () = Js.log ("RawProp Test", elem, idx, oldProp, newProp, oldK =
         newK && oldV = newV, oldRest, newRest) in *)
      let () =
  ...
  | ( (Data (oldK, oldV) as oldProp) :: oldRest
    , (Data (newK, newV) as newProp) :: newRest ) ->
      ...
      patchVNodesOnElems_PropertiesApply callbacks elem (idx + 1) oldRest
        newRest
  (* Event *)
  (* | Event (oldTyp, oldKey, oldCbev) :: oldRest, Event (newTyp, newKey,
     newCbev) :: newRest -> let () = if oldTyp = newTyp && oldKey = newKey then
     () else *)
...
let patchVNodesOnElems_Properties callbacks elem oldProperties newProperties =
  (* Profiling here show `=` to be very slow, but testing reveals it to be
     faster than checking through the properties manually on times when there
     are few to no changes, which is most of the time, so keeping it for now... *)
  (* TODO: Look into if there is a better way to quick test property
     comparisons, especially since it likely returns false when events are
     included regardless of anything else. *)
  (* if oldProperties = newProperties then () else *)
  ...
  | ( ( Node
    ...
    , ( Node
      ...
      if oldUnique <> newUnique || oldTagName <> newTagName then
        (* let () = Js.log ("Node test", "unique swap", elem, elems.(idx),
           newNode) in *)
        patchVNodesOnElems_ReplaceNode callbacks elem elems idx newNode
      else
        (* Same node type, just mutate things *)
        (* let () = Js.log ("Node test", "non-unique mutate", elem,
           elems.(idx), newNode) in *)
            ...
            let () =
              Js.log
                "VDom:  Failed swapping properties because the property list \
                 ...
                 inefficiency until this issue is resolved." in
            patchVNodesOnElems_ReplaceNode callbacks elem elems idx newNode
        in
...
and patchVNodesOnElems callbacks elem elems idx oldVNodes newVNodes =
  (* let () = Js.log ("patchVNodesOnElems", elem, elems, idx, oldVNodes,
     newVNodes) in *)
  ...
  | _oldVnode :: oldRest, [] ->
      ...
      (* Not changing idx so we can delete the rest too *)
  ...
  | ( LazyGen (oldKey, _oldGen, oldCache) :: oldRest
    , LazyGen (newKey, newGen, newCache) :: newRest ) -> (
      if oldKey = newKey then
        ...
        let () = newCache := !oldCache in
        (* Don't forget to pass the cache along... *)
        patchVNodesOnElems callbacks elem elems (idx + 1) oldRest newRest
      else
        ...
        | ( LazyGen (olderKey, _olderGen, _olderCache) :: olderRest
          ...
          when olderKey = newKey && oldKey = newerKey ->
            (* let () = Js.log ("Lazy older newer swap", olderKey, oldKey,
               newKey, newerKey, elem, elems.(idx)) in *)
        ...
        | LazyGen (olderKey, _olderGen, olderCache) :: olderRest, _
          when olderKey = newKey ->
            (* let () = Js.log ("Lazy older match", olderKey, oldKey, newKey,
               elem, elems.(idx)) in *)
            let oldChild = elems.(idx) in
            let _removedChild = Web.Node.removeChild elem oldChild in
        ...
        | _, LazyGen (newerKey, _newerGen, _newerCache) :: _newerRest
          when newerKey = oldKey ->
            (* let () = Js.log ("Lazy newer match", "parse", oldKey, newKey,
               newerKey, elem, elems.(idx)) in *)
            let oldChild = elems.(idx) in
        ...
        | _ ->
            (* let () = Js.log ("Lazy nomatch", oldKey, newKey, elem,
               elems.(idx)) in *)
  ...
  | ( ( Node
    ...
    , ( Node
      ...
      if oldKey = newKey && oldKey <> "" then
        ...
        (* let () = Js.log ("Node test", "match", elem, elems.(idx), newNode)
           in *)
        patchVNodesOnElems callbacks elem elems (idx + 1) oldRest newRest
        ...
        | ( Node
          ...
          when olderNamespace = newNamespace
            ...
            (* let () = Js.log ("Node test", "older newer swap", elem,
               elems.(idx), newNode) in *)
            (* TODO: Test this branch, it is untested thus far *)
            let firstChild = elems.(idx) in
            let secondChild = elems.(idx + 1) in
        ...
        | ( Node
          ...
          when olderNamespace = newNamespace
               && olderTagName = newTagName && olderKey = newKey ->
            (* let () = Js.log ("Node test", "older match", elem, elems.(idx),
               newNode) in *)
            let oldChild = elems.(idx) in
            let _removedChild = Web.Node.removeChild elem oldChild in
            patchVNodesOnElems callbacks elem elems (idx + 1) olderRest newRest
        | ( _
          ...
          when oldNamespace = newerNamespace
               && oldTagName = newerTagName && oldKey = newerKey ->
            (* let () = Js.log ("Node test", "newer match", elem, elems.(idx),
               newNode) in *)
            let oldChild = elems.(idx) in
            let newChild = patchVNodesOnElems_CreateElement callbacks newNode in
            let _attachedChild = Web.Node.insertBefore elem newChild oldChild in
...
let patchVNodesIntoElement callbacks elem oldVNodes newVNodes =
  ...
  (* Planning to return an altered vdom set here instead of using mutation... *)
  newVNodes
...
(* | Node of string option * string option * string * 'msg property list * 'msg
   velem list *)

let wrapCallbacks func callbacks =
  ref {enqueue= (fun msg -> !callbacks.enqueue (func msg))}

let map : ('a -> 'b) -> 'a t -> 'b t =
 fun func vdom ->
  let tagger callbacks =
    ref {enqueue= (fun msg -> !callbacks.enqueue (func msg))} in
  Tagger (Obj.magic tagger, Obj.magic vdom)

(* let map func vdom = let toString () = renderToHtmlString vdom in let toDom
   in Tagger (toString, toDom, toVNodes) *)
