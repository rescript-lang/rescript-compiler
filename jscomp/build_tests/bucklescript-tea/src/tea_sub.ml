...
let map msgMapper sub =
  let open Vdom in
  let func callbacks =
    ref {enqueue= (fun userMsg -> !callbacks.enqueue (msgMapper userMsg))}
  in
  Mapper (func, sub)
 ...
 fun oldCallbacks newCallbacks oldSub newSub ->
  let rec enable : type msg. msg Vdom.applicationCallbacks ref -> msg t -> unit
   ...
   fun callbacks -> function
    ...
    | Registration (_key, enCB, diCB) -> diCB := Some (enCB callbacks) in
  let rec disable :
      type msg. msg Vdom.applicationCallbacks ref -> msg t -> unit =
   fun callbacks -> function
    | NoSub -> ()
    | Batch [] -> ()
