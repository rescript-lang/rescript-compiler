module Decoder = struct
  ...
  (* | Parser : (Web.Json.t -> ('result, error) Tea_result.t) -> ('result,
     error) Tea_result.t t *)
  (* | Value : (Web.Json.t, error) Tea_result.t t | Succeed : 'result ->
     ('result, error) Tea_result.t t | Fail : error -> (_, error) Tea_result.t
     t | Null : 'result -> ('result, error) Tea_result.t t | String : (string,
     error) Tea_result.t t *)
      ...
      (fun value ->
        ...
        | JSONArray a -> (
            let parse v =
              ...
              | Tea_result.Ok r -> r
              | Tea_result.Error e -> raise (ParseFail e) in
    ...
    Decoder
      (fun value ->
        ...
        | JSONObject o -> (
            ...
            with ParseFail e ->
              Tea_result.Error ("Invalid dict parsing: " ^ e) )
