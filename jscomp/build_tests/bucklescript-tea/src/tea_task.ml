...
(* Task : ((('succeed, 'fail) Tea_result.t -> (unit -> unit)) -> (unit ->
   unit)) -> ('succeed, 'fail) t *)
...
let performOpt (toOptionalMessage : 'value -> 'msg)
  ...
  Tea_cmd.call (fun callbacks ->
      ...
      let cb = function
        ...
        | Ok v -> (
          ...
          | None -> ()
          | Some result -> !callbacks.enqueue result ) in
      task cb)

let perform toMessage task = performOpt (fun v -> Some (toMessage v)) task
...
let nativeBinding func = Task func
...
let map2 func task1 task2 =
  task1
  |> andThen (fun v1 -> task2 |> andThen (fun v2 -> succeed (func v1 v2)))
...
let rec sequence = function
  ...
  | task :: remainingTasks ->
      map2
        (fun l r ->
          l :: r
          (* TODO: Replace with `List.cons` when updated to version 4.03 *))
        task (sequence remainingTasks)
...
let testing () =
  ...
  let doTest expected (Task task) =
    let testAssert v =
      ...
      else Js.log ("FAILED:", expected, v) in
    task testAssert in
  let s = succeed 42 in
  ...
  let n0 =
    sequence
      [mapError string_of_int (fail 42); mapError string_of_float (fail 3.14)]
  in
  let () = doTest (Error "42") n0 in
  let n1 =
    sequence
      [mapError string_of_int (succeed 1); mapError string_of_float (fail 3.14)]
  in
  let () = doTest (Error "3.14") n1 in
  let n2 =
    sequence
      [mapError string_of_int (succeed 1); mapError string_of_float (succeed 2)]
  in
