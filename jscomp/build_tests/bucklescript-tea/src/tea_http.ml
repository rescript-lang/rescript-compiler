  ...
  Tea_cmd.call (fun callbacks ->
      ...
      let () =
        ...
        | Some {onprogress; onreadystatechange} ->
            ...
            let () =
              mayCB (setEvent set_onreadystatechange) onreadystatechange in
      ...
      in
      let () =
        setEvent Web.XMLHttpRequest.set_ontimeout (enqResError Timeout) in
...
module Progress = struct
  (* type bytesProgressed = { bytes : int ; bytesExpected : int }

     type ('data, 'parseFailData) t = | NoProgress (* SomeProgress (bytes,
     bytesExpected) *) | SomeProgress of bytesProgressed | FailProgress of
     'parseFailData error | DoneProgress of 'data

     type ('msg, 'parseFailData) trackedRequest = { request : 'msg rawRequest ;
     toProgress : bytesProgressed -> 'msg ; toError : 'parseFailData error ->
     'msg } *)
  ...
  (* Yeah this does not follow the original API, but that original API is...
     not extensible... Instead, we have generic event listener support here so
     no need to constrain the API. Might still want to make a subscription
     variant though... *)
          ...
          if lengthComputable then
            ...
            let decoder =
              map2
                ...
                (field "loaded" int) (field "total" int) in
            match decodeValue decoder ev with
            | Error _e -> ()
            | Ok t -> !callbacks.enqueue (toMessage t)) in
