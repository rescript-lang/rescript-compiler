...
(* module T = struct type t = DirTree.t type label = DirTree.t let id =
   DirTree.id let edges = Hashtbl.create 97 let children t = let l =
   DirTree.children t in List.iter (fun i -> Hashtbl.replace edges (DirTree.id
   t, DirTree.id i) ()) l; l let label x = x let string_of_label x =
   DirTree.string_of_label (DirTree.label x)

   let root = DirTree.from_dir "" Sys.argv.(1) end *)
...
let show_tree canvas t width height =
  let rlimit = 0.98 in
  let xzoom = float width /. 2.0 and yzoom = float height /. 2.0 in
  let origin = ref (-0.5, 0.0) in
  let xy2gtk x y = (float x -. 300., (float height /. 2.) -. float y +. 50.) in
  let gtk2xy gx gy =
    (truncate (gx +. 300.), truncate ((float height /. 2.) +. 50. -. gy)) in
  let xy2c (x, y) =
    let zx = (float x -. xzoom) /. xzoom and zy = (float y -. yzoom) /. yzoom in
    let zn = sqrt ((zx *. zx) +. (zy *. zy)) in
    if zn > rlimit then (rlimit *. zx /. zn, rlimit *. zy /. zn) else (zx, zy)
  in
  let draw_edges () =
    let draw_edge (i, j) () =
      try
        ...
        let l =
          try Hashtbl.find lines (i, j)
          with
          (* if !debug then Format.printf "find edge %s ---- %s@."
             (dbi.db_nom)(dbj.db_nom) else (); *)
          | Not_found -> (
            try Hashtbl.find lines (j, i)
            with
            (* if !debug then Format.printf "find edge %s ---- %s@."
               (dbi.db_nom)(dbj.db_nom) else (); *)
            | Not_found ->
              let l =
                GnoCanvas.line canvas
                  ~props:[`FILL_COLOR "black"; `WIDTH_PIXELS 1; `SMOOTH true]
              in
              Hashtbl.add lines (i, j) l ;
              if !debug then
                Format.printf "draw_edge %s ---- %s@." dbi.db_nom dbj.db_nom
              else () ;
              l ) in
  ...
  let rec draw_label lab (zx, zy) facteur_reduction =
    ...
    with Not_found ->
      ...
      let _ =
        GnoCanvas.text
          ~props:[`X 0.0; `Y 0.0; `TEXT name; `FILL_COLOR "blue"]
          noeud in
  ...
  and drag_label db ev =
    ...
    ( match ev with
    | `ENTER_NOTIFY _ -> item#set [`FILL_COLOR "steelblue"]
    | `LEAVE_NOTIFY ev ->
