...
module DotPrinter = Graphviz.Dot (struct
  include G
...
let save_graph name =
  if Filename.check_suffix name "gml" then (
    gml_output !graph name ;
    graph_name := Some name )
  else if Filename.check_suffix name "dot" then (
    dot_output !graph name ;
    graph_name := Some name )
  else
    let name = name ^ ".dot" in
    dot_output !graph name ;
    graph_name := Some name
...
let is_selected (x : G.V.t) =
  ...
  mode = Selected || mode = Selected_Focused

type mode_select_list = REMOVE_FROM of G.V.t | ADD_FROM of G.V.t | NONE
  ...
  G.iter_vertex
    (fun v ->
      if
        ...
        | NONE -> true
      then vertex_selection := v :: !vertex_selection)
    !graph ;
  let compare s1 s2 =
    String.compare (string_of_label s1) (string_of_label s2) in
...
let update_vertex vertex event =
  ...
  | Normal, Select ->
      vertex_info.vertex_mode <- Selected ;
      incr nb_selected
  | Normal, Focus -> vertex_info.vertex_mode <- Focused
  | Normal, _ -> ()
  | Selected, Focus -> vertex_info.vertex_mode <- Selected_Focused
  | Selected, Unselect ->
      vertex_info.vertex_mode <- Normal ;
      decr nb_selected
  | Selected, _ -> ()
  | Focused, Select ->
      vertex_info.vertex_mode <- Selected_Focused ;
      incr nb_selected
  | Focused, Unfocus -> vertex_info.vertex_mode <- Normal
  | Focused, _ -> ()
  | Selected_Focused, Unselect ->
      vertex_info.vertex_mode <- Focused ;
      decr nb_selected
  ...
  G.iter_succ_e
    (fun edge ->
      ...
      match (edge_info.edge_mode, event) with
      ...
      | Selected, Unselect ->
          if not (is_selected dest_vertex) then edge_info.edge_mode <- Normal
      | Selected, _ -> ()
      | Focused, Select -> edge_info.edge_mode <- Selected_Focused
      | Focused, Unfocus -> edge_info.edge_mode <- Normal
      | Focused, _ -> ()
      | Selected_Focused, Unselect ->
          if not (is_selected dest_vertex) then edge_info.edge_mode <- Focused ;
          decr nb_selected
...
let select_all () =
  ...
  G.iter_edges_e
    (fun e ->
      let e = G.E.label e in
      e.edge_mode <- Selected)
    !graph
