...
let from_tortue (x, y) =
  let xzoom = w /. 2. and yzoom = h /. 2. in
...
let tdraw_edge_gtk tor d etapes line =
  ...
  let rec list_points t liste = function
    | 0 -> (t, liste)
    | n ->
        let t = advance t d in
        list_points t (tlineto_gtk t liste) (n - 1) in
  let l =
    let x, y = from_tortue tor.pos in
    [float x; float y] in
  let t, lpoints = list_points tor l etapes in
  (* debug *)
  ( if !debug_outil_tort then
    let ltext =
      let rec chaine = function
        | [] -> ""
        | e :: l -> string_of_float e ^ " " ^ chaine l in
      chaine lpoints in
