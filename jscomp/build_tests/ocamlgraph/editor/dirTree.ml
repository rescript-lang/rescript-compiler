...
type t = {id: int; name: string; children: t list Lazy.t}
...
let is_dir path = try (lstat path).st_kind = S_DIR with Unix_error _ -> false
let less t1 t2 = t1.name <= t2.name

let rec tree_list_from_path path =
  let opt_handle = try Some (opendir path) with Unix_error _ -> None in
...
and tree_list_from_handle path handle =
  let opt_name = try Some (readdir handle) with End_of_file -> None in
  match opt_name with
  | None -> []
  | Some name ->
      let path' = path ^ "/" ^ name in
      if name <> "." && name <> ".." && is_dir path' then
        {id= newid (); name; children= lazy (tree_list_from_path path')}
        :: tree_list_from_handle path handle
      else tree_list_from_handle path handle
