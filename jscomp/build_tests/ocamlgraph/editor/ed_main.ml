...
module Model = struct
  ...
  let find_children row_v w =
    ...
    let rec find n =
      ...
      match n with
      | 0 -> raise Not_found
      | n -> if G.V.equal child_vertex w then child else find (n - 1) in
    find nb_child

  let remove_edge_1 row_v w = ignore (model#remove (find_children row_v w))
...
let set_window_title () =
  window#set_title
    ...
    | None -> ed_name
    | Some name ->
        ed_name ^ " : " ^ Filename.chop_extension (Filename.basename name) )
  ...
  let circle =
    GnoCanvas.ellipse
      ~props:
        ...
        ; `WIDTH_PIXELS (truncate w2) ]
      circle_group in
  circle_group#lower_to_bottom () ;
  circle#show () ;
  let graph_root = GnoCanvas.group ~x:(-300.0) ~y:(-300.0) circle_group in
  graph_root#raise_to_top () ; set_window_title () ; graph_root
...
(* refresh rate *)
let refresh = ref 0
let do_refresh () = !refresh mod !refresh_rate = 0
...
let draw tortue canvas =
  ...
  | Some root ->
      ...
      if do_refresh () then (canvas_root#canvas)#update_now ()
...
let refresh_display () = Ed_display.draw_graph !root canvas_root
...
let add_node () =
  let window =
    GWindow.window ~title:"Choose vertex label" ~width:300 ~height:50
      ~position:`MOUSE () in
  let vbox = GPack.vbox ~packing:window#add () in
  let entry = GEdit.entry ~max_length:50 ~packing:vbox#add () in
  entry#set_text "Label" ;
  entry#select_region ~start:0 ~stop:entry#text_length ;
  (*two check buttons allowing to add node to selection list and to choose this
    node as root*)
  let hbox = GPack.hbox ~packing:vbox#add () in
  let is_in_selection = ref false in
  let in_selection =
    GButton.check_button ~label:"Add to selection" ~active:!is_in_selection
      ~packing:hbox#add () in
  ignore
    ((in_selection#connect)#toggled ~callback:(fun () ->
         is_in_selection := in_selection#active)) ;
  let is_as_root = ref (G.nb_vertex !graph = 0) in
  let as_root =
    GButton.check_button ~label:"Choose as root" ~active:!is_as_root
      ~packing:hbox#add () in
  ignore
    ((as_root#connect)#toggled ~callback:(fun () ->
         is_as_root := as_root#active)) ;
  window#show () ;
  (*entry's callback*)
  ignore
...
let remove_edge_no_refresh n1 n2 () =
  if edge n1 n2 then (G.remove_edge !graph n1 n2 ; Model.remove_edge n1 n2)

(* add successor node to selected node *)
let add_successor node () =
  let window =
    GWindow.window ~title:"Choose label name" ~width:300 ~height:50
      ~position:`MOUSE () in
  let vbox = GPack.vbox ~packing:window#add () in
  let entry = GEdit.entry ~max_length:50 ~packing:vbox#add () in
  entry#set_text "Label" ;
...
let remove_vertex vertex () =
  ...
  ( match !root with
  | None -> ()
  | Some root_v -> if G.V.equal root_v vertex then root := choose_root () ) ;
  refresh_draw ()
...
let sub_edge_to modif_type vertex list =
  ...
  let add_menu_edge vertex v2 =
    if not (G.V.equal v2 vertex) then
      match modif_type with
      | Add ->
          ignore
            (!sub_menu#add_item (string_of_label v2)
               ~callback:(add_edge v2 vertex))
      | Remove ->
          ignore
            (!sub_menu#add_item (string_of_label v2)
               ~callback:(remove_edge v2 vertex)) in
  let rec make_sub_menu vertex list nb =
    ...
    | v :: list -> (
      ...
      | 0 ->
          ...
          ignore
            (menu#add_item
               (String.sub string 0 (min (String.length string) 3) ^ "...")
      ...
      | _ ->
          add_menu_edge vertex v ;
          make_sub_menu vertex list (nb + 1) ) in
  if ll > 10 then (
    make_sub_menu vertex list 0 ;
    menu )
  else
    let rec make_sub_bis list =
      match list with
      | [] -> ()
      | v :: list -> add_menu_edge vertex v ; make_sub_bis list in
    make_sub_bis list ; !sub_menu
...
let all_edges (edge_menu : #GMenu.menu GMenu.factory) vertex list =
  ...
  let add_all_edge vertex list () =
    List.iter
      (fun v ->
        if not (G.V.equal v vertex) then add_edge_no_refresh v vertex ())
      list ;
    refresh := 0 ;
    let tor = make_turtle !origine 0.0 in
    draw tor canvas_root in
  ignore
    (edge_menu#add_item "Add all edges" ~callback:(add_all_edge vertex list))
...
let contextual_menu vertex ev =
  ...
  (* successor *)
  ignore
    (vertex_menu#add_item "Add successor" ~callback:(add_successor vertex)) ;
  ignore (vertex_menu#add_separator ()) ;
  (* remove vertex *)
  ignore
    (vertex_menu#add_item "Remove vertex" ~callback:(remove_vertex vertex)) ;
  ...
  (let add_list = selected_list (ADD_FROM vertex) in
   ...
   (* add menu *)
   if (isel && al = 2) || ((not isel) && al = 1) then (
     ignore
       (edge_menu#add_item "Add edge with"
          ~submenu:(edge_to Add vertex add_list)#menu) ;
     menu_bool := true )
   else if (isel && al > 2) || ((not isel) && al > 1) then (
     ignore
       (edge_menu#add_item "Add edge with"
          ~submenu:(edge_to Add vertex add_list)#menu) ;
     ...
     menu_bool := true ) ;
   (* remove menu *)
   if (isel && rl >= 2) || ((not isel) && rl >= 1) then (
     if !menu_bool then ignore (edge_menu#add_separator ()) ;
     ignore
       (edge_menu#add_item "Remove edge with"
          ~submenu:(edge_to Remove vertex rem_list)#menu) ;
     menu_bool := true ) ;
...
let vertex_event vertex item ellispe ev =
  ...
  ( match ev with
  | `ENTER_NOTIFY _ ->
      item#grab_focus () ; update_vertex vertex Focus ; refresh_display ()
  | `LEAVE_NOTIFY ev ->
      if not (Gdk.Convert.test_modifier `BUTTON1 (GdkEvent.Crossing.state ev))
      then (
        update_vertex vertex Unfocus ;
        refresh_display () )
  | `BUTTON_RELEASE ev -> (ellispe#parent)#ungrab (GdkEvent.Button.time ev)
  | `MOTION_NOTIFY ev ->
      incr refresh ;
      ...
      if Gdk.Convert.test_modifier `BUTTON1 state then (
        ...
        if do_refresh () then
          ...
          if hspace_dist_sqr turtle <= rlimit_sqr then draw turtle canvas_root
          else (
            origine := old_origin ;
            let turtle = {turtle with pos= old_origin} in
            draw turtle canvas_root ) )
  | `BUTTON_PRESS ev ->
      if GdkEvent.Button.button ev = 3 then contextual_menu vertex ev
  | `TWO_BUTTON_PRESS ev ->
      if GdkEvent.Button.button ev = 1 then (
        if Gdk.Convert.test_modifier `CONTROL (GdkEvent.Button.state ev) then
          if !nb_selected = 0 then (select_all () ; update_vertex vertex Focus)
          else (unselect_all () ; update_vertex vertex Focus)
        else if is_selected vertex then update_vertex vertex Unselect
...
let ask_for_file (mode : [< `OPEN | `SAVE]) =
  let default_file d = function None -> d | Some v -> v in
  let all_files () =
  ...
  let dialog =
    ...
    | `OPEN ->
        let dialog =
          GWindow.file_chooser_dialog ~action:`OPEN ~title:"Open graph file"
            ~parent:window () in
    ...
    | `SAVE ->
        let dialog =
          GWindow.file_chooser_dialog ~action:`SAVE ~title:"Save graph as..."
            ~parent:window () in
...
let new_graph () =
  let alert_window =
    GWindow.message_dialog
      ~message:
        ( "Are you sure you want to start" ^ " a new graph and discard all"
        ^ " unsaved changes to :\n\n" ^ "<tt>\t"
        ^ (match !graph_name with None -> "unamed" | Some name -> name)
        ^ "</tt>" )
      ~use_markup:true ~title:"New graph ?" ~message_type:`QUESTION
      ~buttons:GWindow.Buttons.yes_no ~parent:window ~resizable:false
...
let open_graph () =
  ...
  if file <> "<none>" then (
    ...
    reset_table_and_canvas () ;
    let turtle = make_turtle_origine () in
    draw turtle canvas_root ; set_canvas_event () )

(* menu action save graph as *)
let save_graph_as () =
  let file = ask_for_file `SAVE in
  if file <> "<none>" then (save_graph file ; set_window_title ())

(* menu action save graph *)
let save_graph () =
  match !graph_name with
  | None -> ()
  | Some name -> save_graph name ; set_window_title ()
...
let quit () =
  let alert_window =
    GWindow.message_dialog
      ~message:
        ( "Are you sure you want to quit" ^ "  and discard all"
        ^ " unsaved changes to :\n\n" ^ "<tt>\t"
        ^ (match !graph_name with None -> "unamed" | Some name -> name)
        ^ "</tt>" )
      ~use_markup:true ~title:"Quit ?" ~message_type:`QUESTION
      ~buttons:GWindow.Buttons.yes_no ~parent:window ~resizable:false
      ~position:`CENTER_ON_PARENT () in
  match alert_window#run () with
  | `YES -> window#destroy ()
...
let about () =
  let dialog =
    GWindow.about_dialog
      ~authors:
        [ "Ocamlgraph :"; "   Sylvain Conchon"; "   Jean-Christophe Filliatre"
        ; "   Julien Signoles"; ""; ed_name ^ " :"; "   Vadon Benjamin" ]
      ...
      ~name:ed_name ~version:"0.99" ~website:"http://ocamlgraph.lri.fr/"
      ~parent:window ~title:"About" ~resizable:false
      ~position:`CENTER_ON_PARENT () in
  try ignore (dialog#run ()) with Not_found -> dialog#destroy ()

let handbook_text (view : GText.view) =
  let buffer = view#buffer in
  (* text's tags *)
  ignore
    (buffer#create_tag ~name:"annotation"
       [`LEFT_MARGIN 10; `RIGHT_MARGIN 10; `SIZE (7 * Pango.scale)]) ;
  ignore (buffer#create_tag ~name:"center" [`JUSTIFICATION `CENTER]) ;
  ignore
    (buffer#create_tag ~name:"heading"
       [`UNDERLINE `SINGLE; `WEIGHT `BOLD; `SIZE (14 * Pango.scale)]) ;
  ignore
    (buffer#create_tag ~name:"italic"
       [`LEFT_MARGIN 10; `RIGHT_MARGIN 10; `STYLE `ITALIC]) ;
  ignore (buffer#create_tag ~name:"item" [`LEFT_MARGIN 20; `RIGHT_MARGIN 10]) ;
  ignore
    (buffer#create_tag ~name:"subsection" [`LEFT_MARGIN 10; `RIGHT_MARGIN 10]) ;
  ignore
    (buffer#create_tag ~name:"title"
       [`WEIGHT `BOLD; `SIZE (17 * Pango.scale); `JUSTIFICATION `CENTER]) ;
  ignore
    (buffer#create_tag ~name:"word_wrap" [`WRAP_MODE `WORD; `EDITABLE false]) ;
  ...
  buffer#insert ~iter ~tag_names:["subsection"]
    ...
    ^ "This application has only basic functionalities on graphs, so if you \
       want a new functionality, send it too.\n" ) ;
  buffer#insert ~iter ~tag_names:["subsection"]
    ( ed_name
    ^ " represents a graph in hyperbolic geometry, and specifically in \
       Poincaré's disk representation.\n\n" ^ ed_name
  ...
  buffer#insert ~iter ~tag_names:["item"]
    "- and an associated contextual menu\n\n" ;
  buffer#insert ~iter ~tag_names:["heading"] "Menu bar\n" ;
  buffer#insert ~iter ~tag_names:["subsection"]
    "\t It provides standard functionalities. You can create a new graph, \
     open and save graphs from/to the Gml and Dot formats.\n" ;
  ...
  buffer#insert ~iter ~tag_names:["subsection"]
    "\t You can change the root the of graph diagram by clicking on a vertex \
     name. If you expand one, you can see its descendants.\n\n" ;
  buffer#insert ~iter ~tag_names:["heading"] "Poincaré's disk\n" ;
  buffer#insert ~iter ~tag_names:["subsection"]
    "\t The graph is displayed in a disk. You can drag vertex to move the \
     whole graph around." ;
  buffer#insert ~iter ~tag_names:["annotation"] "[1]\n" ;
  buffer#insert ~iter ~tag_names:["subsection"]
    ( "By double-clicking on a node, you add/remove it to the current selection.\n\
       With a <Ctrl>+double-click you select all nodes, or unselect all (if \
       one or more node is already selected)" ^ "\n\n" ) ;
  buffer#insert ~iter ~tag_names:["heading"] "Contextual menu\n" ;
  buffer#insert ~iter ~tag_names:["subsection"]
    ( "\t This is the main way (and the only for the moment) to edit a graph. \
       There are two different menus, but the distinction is transparent for \
       the user.\n"
    ^ "The first is only composed by an 'adding node' menu, and appears when \
       you click anywhere in the background of the disk.\n"
    ^ "The second menu appears when you click on a vertex."
    ^ " You can change root of the graph diagram, add a child node or remove \
       the focused vertex."
    ^ " The remainder of the menu depends on which nodes are selected. You \
       can add or remove an edge with one of them selected, or with all."
    ^ "\n\n" ) ;
  buffer#insert ~iter ~tag_names:["annotation"] "[1] :" ;
  buffer#insert ~iter ~tag_names:["subsection"]
    " a bug still remains, you can't drag root to much on the right-side, but \
     on the left-side it is infinite" ;
  let start, stop = buffer#bounds in
...
let handbook () =
  let dialog = GWindow.dialog ~width:450 ~height:450 ~title:"Handbook" () in
  let view = GText.view () in
  let sw = GBin.scrolled_window ~packing:(dialog#vbox)#add () in
  sw#add view#coerce ;
  handbook_text view ;
  dialog#add_button_stock `CLOSE `CLOSE ;
  match dialog#run () with `CLOSE | `DELETE_EVENT -> dialog#destroy ()

(* menu bar, based on ui_manager *)
let ui_info =
  "<ui><menubar name='MenuBar'><menu action='FileMenu'><menuitem action='New \
   graph'/><menuitem action='Open graph'/><menuitem action='Save \
   graph'/><menuitem action='Save graph as...'/><separator/><menuitem \
   action='Quit'/></menu><menu action='HelpMenu'><menuitem \
   action='About'/><menuitem action='Handbook'/></menu></menubar></ui>"
...
let setup_ui window =
  ...
  GAction.add_actions actions
    ...
    ; add_action "Save graph as..." ~stock:`SAVE_AS ~accel:"<Control><Shift>S"
        ~tooltip:"Save current graph to specified file" ~callback:activ_action
    ; add_action "Quit" ~stock:`QUIT ~tooltip:"Quit" ~callback:activ_action
    ; add_action "About" ~label:"_About" ~tooltip:"Who build this"
        ~callback:activ_action
    ; add_action "Handbook" ~label:"_Handbook" ~accel:"<Control>H"
        ~tooltip:"How to.." ~callback:activ_action ] ;
