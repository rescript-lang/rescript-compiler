...
(* light mode (* differents definitions *) let color_circle = "grey99"

   let color_intern_edge = "grey69" let color_successor_edge = "black"
   (*"grey38"*) let color_vertex = "grey75"

   let color_selected_intern_edge = "#9f1a1a" (* "#74885e"*) let
   color_selected_successor_edge = "#9f1a1a" let color_selected_vertex =
   "#9f1a1a"

   let color_focused_intern_edge = "#4d51a9" let color_focused_successor_edge =
   "#4d51a9" let color_focused_vertex = "#4d51a9"

   let color_selected_focused_intern_edge = "LightCoral" let
   color_selected_focused_vertex ="LightCoral" let
   color_selected_focused_successor_edge = "LightCoral"

   let color_text = "black"

   let width_successor_edge = 2 let width_intern_edge = 2 let point_size_text =
   12.*)

(* dark mode (* differents definitions *) let color_circle = "grey99"

   let color_intern_edge = "black" let color_successor_edge ="black" let
   color_vertex = "grey58"

   let color_selected_intern_edge = "OrangeRed4" let
   color_selected_successor_edge = "OrangeRed4" let color_selected_vertex =
   "OrangeRed4"

   let color_focused_intern_edge = "blue" let color_focused_successor_edge =
   "blue" let color_focused_vertex = "blue"

   let color_selected_focused_intern_edge = "dark magenta" let
   color_selected_focused_vertex ="dark magenta" let
   color_selected_focused_successor_edge = "dark magenta"
...
(* two tables for two types of edge : successor_edges = edges with successor of
   root intern_edges = edges between successors of root *)
...
let to_turtle (x, y) =
  let ((zx, zy) as r) =
    ((float x *. (2. /. w)) -. 1., 1. -. (float y *. (2. /. h))) in
  let zn = sqrt ((zx *. zx) +. (zy *. zy)) in
  if zn > rlimit then (rlimit *. zx /. zn, rlimit *. zy /. zn) else r
...
(* origine is a reference to the start point drawing for the graph in GTK
   coordinates *)
let origine = ref start_point

(* Current point in Hyperbolic view with an initialization in center of
   hyperbolic circle *)
...
let make_turtle_origine () =
  let x, y =
    let x, y = !origine in
    (truncate x, truncate y) in
  moveto_gtk x y ; make_turtle !origine 0.0

(* Append turtle coordinates to line, set current point and return the "new"
   line *)
...
(* Set line points for a distance with a number of steps, set current point to
   last line's point, by side-effect of tlineto_gtk, and return the final
   turtle *)
let set_successor_edge turtle distance steps line =
  ...
  let rec list_points turtle liste = function
    | 0 -> (turtle, liste)
    | n ->
        let turt = advance turtle d in
        list_points turt (tlineto_gtk turt liste) (n - 1) in
  let start =
    let x, y = from_turtle turtle.pos in
    [float x; float y] in
...
let add_node canvas v =
  ...
  let ellipse =
    GnoCanvas.ellipse
      ~props:[`FILL_COLOR color_vertex; `OUTLINE_COLOR "black"; `WIDTH_PIXELS 0]
      node_group in
  let texte =
    GnoCanvas.text
      ~props:[`X 0.0; `Y 0.0; `TEXT s; `FILL_COLOR color_text]
      node_group in
  node_group#hide () ;
...
(* draws but don't show intern edges, and return a couple bpath (gtk_object),
   and line (gtw_widget)*)
    ...
    with Not_found ->
      ...
      let line =
        GnoCanvas.bpath canvas
          ~props:[`BPATH bpath; `WIDTH_PIXELS width_intern_edge] in
      line#lower_to_bottom () ;
      H2.add intern_edges vw (bpath, line) ;
      let v, w = vw in
      if is_selected w || is_selected v then edge.edge_mode <- Selected ;
      (bpath, line) in
...
let draw_successor_edge vw edge canvas =
  let line =
    try H2.find successor_edges vw
    with Not_found ->
      let line =
        GnoCanvas.line canvas
          ~props:
            [ `FILL_COLOR color_successor_edge
            ; `WIDTH_PIXELS width_successor_edge; `SMOOTH true ] in
      line#lower_to_bottom () ;
...
let motion_turtle item ev =
  ...
  let z1 =
    to_turtle
      ( truncate ((bounds.(0) +. bounds.(2)) /. 2.)
      , truncate ((bounds.(1) +. bounds.(3)) /. 2.) ) in
  let z2 =
    to_turtle (truncate (GdkEvent.Motion.x ev), truncate (GdkEvent.Motion.y ev))
  in
...
let hide_intern_edge vw =
  try
    let _, line = H2.find intern_edges vw in
    line#hide ()
  with Not_found -> ()

let hide_succesor_edge vw =
  try
    let line = H2.find successor_edges vw in
    line#hide ()
  with Not_found -> ()
...
let draw_graph _root canvas =
  ...
  G.iter_edges_e
    (fun e ->
      ...
      let w = G.E.dst e in
      let vw = (v, w) in
      if edge.visited then (
        (* successor edge *)
        let line = draw_successor_edge vw edge canvas in
        ( match edge.edge_mode with
        | Normal -> color_change_successor_edge line color_successor_edge
        | Selected ->
            color_change_successor_edge line color_selected_successor_edge
        | Focused ->
            color_change_successor_edge line color_focused_successor_edge
        | Selected_Focused ->
            color_change_successor_edge line
              color_selected_focused_successor_edge ) ;
        line#show () ; hide_intern_edge vw )
      else (
        (* intern edges *)
        ...
        if labv.visible = Visible && labw.visible = Visible && v != w then (
          let _, line = draw_intern_edge vw edge turv turw canvas in
          ( match edge.edge_mode with
          | Normal -> color_change_intern_edge line color_intern_edge
          | Selected ->
              color_change_intern_edge line color_selected_intern_edge
          | Focused -> color_change_intern_edge line color_focused_intern_edge
          | Selected_Focused ->
              color_change_intern_edge line color_selected_focused_intern_edge
          ) ;
          line#show () )
        else hide_intern_edge vw ))
    !graph

let reset_display canvas = init_nodes canvas
