...
let expi t = (cos t, sin t)

(*** Hyperbolic functions ***)

let th t =
  let ept = exp t and emt = exp (-.t) in
  (ept -. emt) /. (ept +. emt)

let ath x = 0.5 *. log ((1.0 +. x) /. (1.0 -. x))
let pi = 3.14159265358979323846
let pi_over_2 = pi /. 2.0
let pi_over_4 = pi /. 4.0
let one = (1.0, 0.0)
let translate a z = (a +& z) /& (one +& (~&a *& z))
...
let delta a u t =
  let atht = th t *.& a and utht = th t *.& u in
...
type turtle = {pos: coord; (* with |pos| < 1 *)
                           dir: coord (* with |dir| = 1 *)}

let make_turtle pos angle = {pos; dir= expi angle}

let advance turt step =
  {pos= gamma turt.pos turt.dir step; dir= delta turt.pos turt.dir step}

let turn turtle u = {pos= turtle.pos; dir= turtle.dir *& u}
let turn_left turtle angle = turn turtle (expi angle)
let turn_right turtle angle = turn turtle (expi (-.angle))

(*** Tree-independent functions ***)

let shrink_factor (x, y) = 1.0 -. ((x *. x) +. (y *. y))
...
let hspace_dist_sqr turtle =
  ...
  if (ax *. dx) +. (ay *. dy) < 0.0 then 0.0
    ...
    if beta = 0.0 then alpha
      ...
      let sol =
        if beta > 0.0 then -.gamma +. sqrt delta else -.gamma -. sqrt delta
      in
  ...
  let draw_linear_edges drv tree turtle =
    ...
    let rec do_tree tur t =
      if hspace_dist_sqr tur <= rlimit_sqr then
        ...
        if num > 0 then
          let step = step_from (2 * max 3 num) and angle = pi /. float num in
  ...
  let draw_curved_edges drv tree turtle =
    ...
    and do_list tur step u = function
      ...
      | t :: rest ->
          ...
          let dx = a3x -. a0x and dy = a3y -. a0y in
