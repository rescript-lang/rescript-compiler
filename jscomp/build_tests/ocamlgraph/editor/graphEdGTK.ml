...
module Gr = struct
  ...
  let parse_file f =
    if Filename.check_suffix f ".gml" then GmlParser.parse f
    else DotParser.parse f
...
let trace f x =
  try f x
  with e ->
    eprintf "TRACE: %s@." (Printexc.to_string e) ;
    raise e
...
(* [step_from n] computes the best `distance' for solving the dictator's
   problem in the complex hyperbolic plane for [n] dictators. In a half-plane,
   we have to use the distance given by [step_from (2*n)] or, better, the
   distance given by [step_from (2*max(3 n))]. *)
let step_from n = ath (tan (pi_over_4 -. (pi /. float (2 * n))))

(* [hspace_dist_sqr turtle] computes the square of the distance between the
   origin and the half-space in front of [turtle]. *)
let hspace_dist_sqr turtle =
  let ax, ay = turtle.pos and dx, dy = turtle.dir in
  (* if ax*.dx +. ay*.dy < 0.0 then 0.0 else*)
  let ux = dy and uy = -.dx in
  let alpha = (ax *. ax) +. (ay *. ay)
  and beta = 2.0 *. ((ax *. ux) +. (ay *. uy)) in
  if beta = 0.0 then alpha
  else
    let gamma = (1.0 +. alpha) /. beta in
    let delta = (gamma *. gamma) -. 1.0 in
    let sol =
      if beta > 0.0 then -.gamma +. sqrt delta else -.gamma -. sqrt delta in
...
let sw =
  GBin.scrolled_window ~shadow_type:`ETCHED_IN ~hpolicy:`NEVER
      ...
      let ellipse =
        GnoCanvas.ellipse
          ~props:
            ...
            ; `X2 (float_of_int (w / 2))
            ; `Y2 (float_of_int (h / 2))
            ; `FILL_COLOR "grey"; `OUTLINE_COLOR "black"; `WIDTH_PIXELS 0 ]
          noeud in
      let texte =
        GnoCanvas.text
          ~props:[`X 0.0; `Y 0.0; `TEXT s; `FILL_COLOR "blue"]
          noeud in
      let w2 = texte#text_width in
      if w2 > float_of_int w then
...
let tdraw_edge_gtk vw t distance etapes canvas =
  let line =
    ...
    with Not_found ->
      let color = "black" in
      let l =
        GnoCanvas.line canvas
          ~props:[`FILL_COLOR color; `WIDTH_PIXELS 1; `SMOOTH true] in
      H2.add black_edges vw l ; l in
...
let is_selected_node v =
  match !select with None -> false | Some (w, _) -> V.equal v w
...
let rec draw_graph depth noeud tortue canvas =
  ...
  else if noeud <> !root then
    try
      let ellipse = tdraw_string_gtk noeud tortue canvas in
      (ellipse#parent)#hide () ;
      iter_succ
        (fun w ->
          try ignore (H.find pos w)
          with Not_found -> (
            try
...
and drag_label noeud item ev =
  ( match ev with
  | `ENTER_NOTIFY _ ->
      if not (is_selected_node noeud) then (
  ...
  | `BUTTON_RELEASE ev -> (item#parent)#ungrab (GdkEvent.Button.time ev)
  | `MOTION_NOTIFY ev ->
      incr step ;
      ...
      if Gdk.Convert.test_modifier `BUTTON1 state && !step mod 10 = 0 then (
        ...
        let z1 =
          to_tortue
            ( truncate ((ibounds.(0) +. ibounds.(2)) /. 2.)
        ...
        if hspace_dist_sqr tor <= rlimit_sqr then (
          draw tor canvas_root ;
          if !step mod 15 = 0 then (canvas_root#canvas)#update_now () )
        else origine := tmp )
  | `TWO_BUTTON_PRESS ev ->
      if GdkEvent.Button.button ev = 1 then selectionner_noeud noeud item
  | `BUTTON_PRESS ev ->
      if GdkEvent.Button.button ev = 1 then deselectionner_noeud noeud item ;
      if GdkEvent.Button.button ev = 3 then (
        let loc_menu = GMenu.menu () in
        let factory = new GMenu.factory loc_menu in
        ignore
          (factory#add_item "  Ajouter un successeur"
             ~callback:(ajout_successeur noeud)) ;
        ( match !select with
        | None -> ()
        | Some (n, _) ->
            if not (V.equal n noeud) then
              ignore
                (factory#add_item "  Ajouter une arrête"
                   ~callback:(ajout_arrete n noeud)) ) ;
        loc_menu#popup ~button:3 ~time:(GdkEvent.Button.time ev) )
  | _ -> () ) ;
  true

and ajout_successeur noeud () =
  let window =
    GWindow.window ~title:"Choix du nom du label" ~width:300 ~height:50 ()
  in
  ...
  let _ =
    (entry#connect)#activate ~callback:(fun () ->
        ...
        let tor = make_turtle !origine 0.0 in
        draw tor canvas_root) in
  ()

and ajout_arrete n1 n2 () =
  if not (edge n1 n2) then (
...
and deselectionner_noeud noeud item =
  ( if is_selected_node noeud then (
    item#set [`FILL_COLOR "steelblue"] ;
    color_change_intern_edge "blue" noeud ;
    color_change_direct_edge "blue" noeud )
  else
    ...
    | Some (n, i) ->
        ...
        color_change_intern_edge "SlateGrey" n ;
        color_change_direct_edge "black" n
    (* color_change_intern_edge "red" n ; color_change_direct_edge "red" n *)
  ) ;
  select := None
  ...
  iter_edges
    (fun v w ->
      ...
      with Not_found -> (
        ...
        (* /debug *)
        try
          let _, l = H2.find grey_edges (w, v) in
          l#hide () ;
          (* debug *)
          if !debug_graphEdGTK then
            Format.eprintf "J'ai effacé un grey edge@."
          (* /debug *)
        with Not_found -> (
          () ;
          try
            let _, l = H2.find grey_edges (v, w) in
            l#hide () ;
            (* debug *)
            if !debug_graphEdGTK then
              Format.eprintf "J'ai effacé un grey edge@."
...
let canvas_event ev =
  ...
  | `BUTTON_PRESS ev ->
      ( if GdkEvent.Button.button ev = 1 then
        match !select with
        | None -> ()
        | Some (noeud, item) -> deselectionner_noeud noeud item ) ;
      if GdkEvent.Button.button ev = 3 (* && (is_empty !graph)*) then (
        let loc_menu = GMenu.menu () in
        let factory = new GMenu.factory loc_menu in
...
let node_selection ~(model : GTree.tree_store) path =
  ...
  let tortue =
    ...
    moveto_gtk x y ; make_turtle !origine 0.0 in
...
let add_columns ~(view : GTree.view) ~model =
  ...
  (((view#selection)#connect)#after)#changed ~callback:(fun () ->
      List.iter
...
let open_graph () =
  let default d = function None -> d | Some v -> v in
  let all_files () =
    let f = GFile.filter ~name:"All" () in
    f#add_pattern "*" ; f in
  let graph_filter () =
    GFile.filter ~name:"Fichier de graphes" ~patterns:["*.dot"; "*.gml"] ()
  in
  let ask_for_file parent =
    let dialog =
      GWindow.file_chooser_dialog ~action:`OPEN ~title:"Ouvrir un fichier"
        ~parent () in
...
let print msg () = print_endline msg ; flush stdout
