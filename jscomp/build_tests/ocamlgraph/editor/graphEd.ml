...
let draw_label v = draw_string (string_of_int (V.label v))
...
let rec draw_graph noeud tortue =
  if hspace_dist_sqr tortue <= rlimit_sqr then (
    ...
    if n > 0 then (
      let pas = step_from (max 3 n) and angle = 2. *. pi /. float n in
      ...
      List.iter
        (fun (w, tw) ->
          ...
          let n = List.length l in
          if n > 0 then
            let pas = step_from (max 3 n) and angle = pi /. float n in
          ...
          List.iter
            (fun w ->
              try
                ...
                if abs (lw - lv) <> 1 then (tmoveto tv ; tlineto tw)
              with Not_found -> ())
            (succ graph v))
        pos ;
      set_color black ) )
...
and draw_edges t pas angle = function
  ...
  | v :: l ->
      ...
      if hspace_dist_sqr t <= rlimit_sqr then (
        draw_label v ;
        H.add pos v (1, t) ) ;
      let t = turn_left t angle in
      let list = (v, tv) :: draw_edges t pas angle l in
      draw_graph v tv ; list

let draw origine tortue = H.clear pos ; draw_graph root tortue
let () = open_graph (sprintf " %dx%d" (truncate w) (truncate h))

let tortue =
  let x, y = from_tortue !origine in
  moveto x y ; make_turtle !origine 0.0
...
let rec boucle tortue =
  ...
  if st.button then (
    ...
    | None ->
        ...
        boucle tortue
    | Some (x, y) ->
        let z1 = to_tortue (x, y) and z2 = to_tortue (st.mouse_x, st.mouse_y) in
        clear_graph () ;
    ...
    match !old_xy with
    | None -> draw origine tortue ; boucle tortue
    | Some (x, y) ->
        clear_graph () ;
        draw origine tortue ;
        old_xy := None ;
