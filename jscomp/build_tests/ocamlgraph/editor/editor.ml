...
let main () =
  ...
  if Sys.argv.(1) = "--help" then (
    ...
    printf "@." ;
    exit 0 ) ;
  (* if Array.length Sys.argv > 2 then eprintf "%s: ignoring trailing
     arguments@." Sys.argv.(0) ; *)

  (* la Fenetre principale *)
  let window = GWindow.window ~border_width:10 ~title:"Edit'OcamlGraph" () in
  ...
  let menu_bar = GMenu.menu_bar ~packing:v_box#pack () in
  ...
  and menu = create_menu "File" menu_bar in
