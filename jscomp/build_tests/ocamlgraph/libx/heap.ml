...
module Imperative (X : Ordered) = struct
  ...
  (* When [create n] is called, we cannot allocate the array, since there is no
     known value of type [X.t]; we'll wait for the first addition to do it, and
     we remember this situation with a negative size. *)
  ...
  let remove h =
    ...
    let rec movedown i =
      ...
      if j < n then
        let j =
          let j' = j + 1 in
          if j' < n && X.compare d.(j') d.(j) > 0 then j' else j in
