...
module Node = struct
  ...
  let id n = fst n
  let coord n = match snd n with None -> None | Some (_, c, _) -> Some c
  let item n = match snd n with None -> None | Some (_, _, r) -> Some r
end
...
let get_info attr_list_list =
  let get (shp, p, w, h) (attr, val_opt) =
    match (attr, val_opt) with
...
(** Translate the information given by dot * into the coordinate of a rectangle
    in the png image. * see
    http://www.graphviz.org/mywiki/FaqCoordTransformation * to understand the
    [pad] and [factor] variables. *
...
let compute_coord pos w h =
  ...
  try
    ...
    let x2 = x2 *. factor in
    let y2 = y2 *. factor in
    (* Format.printf "compute_coord -> x1=%f y1=%f x2=%f y2=%f@." x1 y1 x2 y2; *)
    Some ((x1, y1), (x2, y2))
  with e ->
...
module DotParser (C : sig
  val mk_node : t_shape -> t_coord -> t_gtk_obj
end) =
  Dot.Parse
    (B)
    (struct
        ...
        let info =
          ...
          | shp, Some pos, Some w, Some h -> (
              let shp =
                ...
                | None -> Sellipse
                (* default shape *) in
              match compute_coord pos w h with
              | None -> None
              | Some coord ->
                  let n_obj = C.mk_node shp coord in
                  Some (shp, coord, n_obj) )
          | _ ->
              Format.printf "info KO for %s@." name ;
              None in
        (name, info)
...
let png_graph_image dot_cmd dot_file png_file =
  ...
  match Sys.command cmd with 0 -> png_file | _ -> raise (DotError cmd)

(** Call 'dot' on the [dot_file] to get a file with position information, * and
    also to have a [png] image of the graph. * Then parse the annotated file to
    get the graph with the nodes coordinates. *
...
let build_graph dot_cmd dot_file annot_dot_file mk_node_item =
  ...
  | 0 ->
      let module Parser = DotParser (struct let mk_node = mk_node_item end) in
...
module M (Cb : SigCb) = struct
  let mk_node_item canvas shp ((x1, y1), (x2, y2)) =
    ...
    let n_obj =
      ...
      | Sellipse -> GnoCanvas.ellipse canvas#root ~props in
    n_obj
  ...
  let graph_event env ev =
    ...
    | `BUTTON_PRESS ev -> (
      ...
      (* | 1 when Gdk.Convert.test_modifier `SHIFT state -> | 1 when
         Gdk.Convert.test_modifier `CONTROL state -> let (x, y) = canvas#w2c_d
         (GdkEvent.Button.x ev) (GdkEvent.Button.y ev) in *)
  ...
  let add_node_items env graph =
    let do_it n =
      ...
      | None -> ()
      | Some n_rect ->
          ignore ((n_rect#connect)#event ~callback:(node_event env n)) in
    B.G.iter_vertex do_it graph

  let remove_node_items graph =
    let do_it n =
      match get_obj n with None -> () | Some n_rect -> n_rect#destroy () in
    B.G.iter_vertex do_it graph
  ...
  let install_image (canvas : GnoCanvas.canvas) png_file =
    ...
    let _ =
      canvas#set_scroll_region ~x1:0. ~y1:0. ~x2:(float w) ~y2:(float h) in
  ...
  let clear _canvas graph =
    ...
    let pixbuf = get_pixbuf graph in
    pixbuf#destroy () ;
    let graph = get_graph graph in
    remove_node_items graph
end
