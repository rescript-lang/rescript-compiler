...
(** This module can be used to add some selection feature to [ViewGraph]. * see
    [show_help] for more details. *)

(** To manage the lists of neighbours of the selected node. * The type [t] is
    made of a boolean [b] and 3 lists [(l1, l2, l3)]. * [b] tells is [l1] is a
    list of predecessors (vs. successors). * [l3] has the same 'kind' than
    [l1], and [l2] the other. * [l1] cannot be empty, except if the 3 lists are
    empty. * The first node of [l1] is the currently selected neighbour. * When
    we go to the next one, [n] is removed and appended to [l3] tail. * If [l1]
    is then empty, [l2] becomes the first list, and [b] is changed. *)
module CircLists = struct
  ...
  let current_node lists =
    match lists with _, ([], _, _) -> None | b, (n :: _, _, _) -> Some (b, n)
  ...
  let goto_n ((_, (l1, l2, l3)) as lists) n =
    ...
    let rec find nb_iter lists =
      ...
      | Some (_, cur_n) ->
          ...
          else if nb_iter >= 0 then find (nb_iter - 1) (go_next lists)
          else raise Not_found in
    find max lists
end

type t_options =
  {sel_1_color: string; sel_2_color: string; center_node_when_selected: bool}

let default_options =
  {sel_1_color= "red"; sel_2_color= "green"; center_node_when_selected= true}
...
let center_selected_node state =
  ...
  | None -> ()
  | Some n -> (
    match ViewGraph_core.get_coord n with
    | None -> ()
    | Some ((x1, y1), (x2, y2)) ->
...
let key_press state ev =
  ...
  | k when k = GdkKeysyms._Up ->
      canvas#scroll_to ~x ~y:(y - 20) ;
      true
  | k when k = GdkKeysyms._Down ->
      canvas#scroll_to ~x ~y:(y + 20) ;
      true
  | k when k = GdkKeysyms._Left ->
      canvas#scroll_to ~x:(x - 10) ~y ;
      true
...
let init options (canvas : GnoCanvas.canvas) pack_txt =
  ...
  let w = {canvas; sel_1_txt; sel_edge_txt; sel_2_txt} in
  let state =
    { w
    ; opt= options
    ; gui_sel= [|None; None|]
    ...
    ; neighbours= None } in
  let _ =
    (((canvas#event)#connect)#after)#key_press ~callback:(key_press state)
  in
  state
...
let move_selection state n_opt1 n_opt2 =
  let set_props o col bitmap =
    match o with
    | None -> ()
    | Some o -> o#set [`FILL_COLOR col; `FILL_STIPPLE bitmap] in
  let n_obj n_opt =
    match n_opt with None -> None | Some n -> ViewGraph_core.get_obj n in
  let obj1 = n_obj n_opt1 in
  let obj2 = n_obj n_opt2 in
  let reset_old i = set_props state.gui_sel.(i) "black" (no_bitmap ()) in
  reset_old 0 ;
  reset_old 1 ;
  state.gui_sel.(0) <- obj1 ;
  state.gui_sel.(1) <- obj2 ;
...
let show_selection state =
        ...
        let n_opt, txt2, txt3 =
          ...
          | Some info -> (
            ...
            | Some (pred_first, n) ->
                ...
                (Some n, txt2, ViewGraph_core.get_id n) ) in
        (n_opt, txt1, txt2, txt3) in
  ...
  if state.opt.center_node_when_selected then center_selected_node state
  ...
  | Some graph -> (
    ...
    | Some n ->
        let new_info =
          ...
          | Some info -> CircLists.go_next info in
...
module SelectCb (UserCb : ViewGraph_core.SigCb) = struct
  ...
  let enter_node (u_env, _state) n =
    ( match ViewGraph_core.get_obj n with
    | None -> assert false
...
let show_help () =
  GToolbox.message_box ~title:"Help"
    "Selection :\n\n\
    \  - use the mouse button-1 to select a node : it should turn red and \
     (optionally) be centered.\n\
    \  - when a node is selected, button-2 selects one of its neighbour : it \
     should turn green.\n\
    ...
    \  Moving arround :\n\n\
    \  - the scrollbars, as well as the arrows, can be used to move in the \
     window.\n\
    \  - the Page-Up and Page-Down keys can be used to zoom (ugly zoom at the \
     moment !).\n\
