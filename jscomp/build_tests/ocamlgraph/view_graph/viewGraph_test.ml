...
(** This is only a test file to show how to use * [ViewGraph] and
    [ViewGraph_select]. * Just compile and test... (click on the Help button to
    know how to use it). *)
...
module CbTest = struct
  ...
  let button_one_press_on_node _env n =
    Format.printf "[CbTest] button_one_press_on_node %s@." (ViewGraph.get_id n)

  let button_two_press_on_node _env n =
    Format.printf "[CbTest] button_two_press_on_node %s@." (ViewGraph.get_id n)
...
let menu_desc =
  "<ui><menubar name='MenuBar'><menu action='FileMenu'><menuitem \
   action='Open'/><separator/><menuitem action='Quit'/></menu><menu \
   action='HelpMenu'><menuitem action='Help'/></menu></menubar></ui>"
...
(* let create_menu2 packing open_cb = let file_item = GMenu.menu_item
   ~label:"File" ~packing () in let file_menu = GMenu.menu () in let item =
   GMenu.menu_item ~label:"Open" ~packing:file_menu#append () in let _ =
   item#connect#activate ~callback:open_cb in let item = GMenu.menu_item
   ~label:"Quit" ~packing:file_menu#append () in let _ = item#connect#activate
   ~callback:GMain.Main.quit in let _ = file_item#set_submenu file_menu in let
   help_item = GMenu.menu_item ~label:"Help" ~right_justified:true ~packing ()
   in let help_menu = GMenu.menu () in let item = GMenu.menu_item ~label:"Help
   about ViewGraph" ~packing:help_menu#append () in let _ =
   item#connect#activate ~callback:ViewGraph_select.show_help in let _ =
   help_item#set_submenu help_menu in () *)

let create_gui () =
  let window =
    GWindow.window ~title:"ViewGraph" ~allow_shrink:true ~allow_grow:true ()
  in
  ...
  let actions =
    match ui_m#get_action_groups with [a] -> a | _ -> assert false in
...
let main () =
  ...
  if Array.length Sys.argv = 2 then open_file select_init_env Sys.argv.(1) ;
  GMain.Main.main ()
