...
(** Graph coloring with marking. Only applies to imperative graphs with marks. *)
module Mark (G : GM) = struct
  ...
  let coloring g k =
    ...
    ( if !nb_to_color > 0 then
      let rec iterate iter =
        let v = Bfs.get iter in
        let m = G.Mark.get v in
        if m > 0 then iterate (Bfs.step iter)
        else (
          for i = 1 to k do
            try
              try_color v i ;
