...
module Make (G : G) (W : Sig.WEIGHT with type edge = G.E.t) = struct
  open G.E
  ...
  module Elt = struct
    ...
    (* weights are compared first, and minimal weights come first in the queue *)
        ...
        G.iter_succ_e
          (fun e ->
            ...
            if not (H.mem visited v) then
              ...
              let improvement =
                try W.compare wuv (fst (H.find key v)) < 0
  ...
  let spanningtree g =
    ...
    with Exit -> (
      match !r with None -> assert false | Some r -> spanningtree_from g r )
end
