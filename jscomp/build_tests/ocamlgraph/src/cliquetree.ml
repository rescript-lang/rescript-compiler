...
module CliqueTree (Gr : Sig.G) = struct
  ...
  module rec CliqueV : sig
    ...
    let create o =
      {mark= 0; orig= o; m= CVS.empty; last= None; number= 0; clique= -1}
    ...
    let last x =
      match x.last with Some v -> v | None -> failwith "last not set"
  ...
  (* The final clique tree vertex type: - set of original vertexes ordered by
     mark. - clique number. *)
  module CliqueTreeV =
    Util.DataV (struct
        type t = CliqueV.t list * CVS.t
      end)
      (struct
        type t = int
        ...
        List.fold_left
          (fun w v ->
            List.fold_left
              (fun w v' ->
                if v <> v' then
                  if (not (Gr.mem_edge g v v')) && Gr.mem_edge tri v v' then
                    succ w
                  else w
                else w)
              w vertices)
  ...
  let mcs_clique g =
    ...
    for i = n downto 1 do
      (* Find greatest unnumbered vertex if CVS.is_empty !unnumbered then
         Printf.printf "No more unnumbered vertices\n%!" else Printf.printf "%i
         unnumbered vertices remaining\n%!" (CVS.cardinal !unnumbered); *)
      let x, mark =
      ...
      ( if mark <= !pmark then (
        ...
        (* Use reverse map of cliques to find what clique we're connected to. m
           x is the width of the ties *)
    ...
    let cliques =
      Array.mapi
        ...
        (Array.sub cliques 0 (succ !j)) in
    let tree = Array.fold_left CliqueTree.add_vertex CliqueTree.empty cliques in
    let tree, _ =
      List.fold_left
        (fun (g, n) (i, verts, j) ->
        ...
        ignore
          (List.fold_left
             (fun vm v' ->
               ...
               if CVS.equal vm' vm then CVS.add v' vm' else raise NotClique)
             (CVS.add x (m x))
             tl) ;
        true
  ...
  let is_chordal g =
    ...
    let rec aux c =
      ...
      first && next && List.for_all aux csons in
    aux root
