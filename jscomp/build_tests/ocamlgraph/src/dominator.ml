...
(* Copyright © 2009 Carnegie-Mellon University, David Brumley, and Ivan Jager.
   From the BAP library; see http://bap.ece.cmu.edu Modified by OCamlGraph's
   authors. *)

(* stuff to read: http://www.hipersoft.rice.edu/grads/publications/dom14.pdf
   Modern Compiler Implementation in ML, by Appel Introduction to Algorithms,
   Cormen et al *)
...
module type S = sig
  ...
  val dominators_to_dom_tree :
    t -> ?pred:(t -> vertex -> vertex list) -> (vertex -> S.t) -> vertex -> S.t
...
module Make (G : G) = struct
  ...
  let compute_idom cfg s0 =
    (* based on the Tiger book, section 19.2. This uses path compression, but
       doesn't yet do balanced path compression, so the runtime is O(N log(N))
       rather than O(N inverseackerman(N)) *)
      ...
      let loop () =
        while not (Stack.is_empty stack) do
          ...
          if not (H.mem dfnum_h n) then (
            ...
            H.add dfnum_h n enn ;
            vertex.(enn) <- n ;
            (match p with Some p -> H.add parent n p | None -> ()) ;
            nn := enn + 1 ;
            G.iter_succ
              (fun m ->
    ...
    let rec ancestor_with_lowest_semi v =
      try
        ...
        if dfnum (semi b) < dfnum (semi best_v) then (H.replace best v b ; b)
        else best_v
      with Not_found -> H.find best v in
    let link p n = H.replace ancestor n p ; H.replace best n n in
    let semidominator n =
        ...
        (fun s v ->
          try
            ...
            let s' =
              ...
              else semi (ancestor_with_lowest_semi v) in
    ...
    while decr nn ; !nn > 0 do
      ...
      H.add bucket s n ;
      link p n ;
      (* now that the path from p to v is in the forest, calculate the
         dominator of v based on the first clause of the Dominator Theorem,
         otherwise defer until y's dominator is known *)
      List.iter
        (fun v ->
          let y = ancestor_with_lowest_semi v in
          if G.V.equal (semi y) (semi v) then H.add idom v p
          else H.add samedom v y ;
          H.remove bucket p
          (*could use H.remove_all if we used extlib*))
        (H.find_all bucket p)
    done ;
    (* now all the defered calculations can be done *)
    for i = 1 to lastn do
      let n = vertex.(i) in
      try H.add idom n (H.find idom (H.find samedom n)) with Not_found -> ()
    done ;
    H.find idom

  (** Given a function from a node to it's dominators, returns a function
      [dom : V.t -> V.t -> bool] s.t. [dom x y] returns true when [x] dominates
      [y] *)
  let dominators_to_dom dominators x y = S.mem x (dominators y)

  (** Given a function from a node to it's dominators, returns a function
      [sdom : V.t -> V.t -> bool] s.t. [sdom x y] returns true when [x]
      strictly dominates [y] *)
  let dominators_to_sdom dominators x y =
    (not (G.V.equal x y)) && dominators_to_dom dominators x y

  let dom_to_sdom dom x y = (not (G.V.equal x y)) && dom x y
  ...
  (** Given a function from a node to it's dominators, returns a function
      [idoms : G.V.t -> G.V.t -> bool] s.t. [idoms x y] returns true when [x]
  ...
  (** Computes a dominator tree (function from x to a list of nodes immediately
      dominated by x) for the given CFG and dominator function. Note: The
      dominator tree is also called [IDom] by Muchnick. Note: If you are
      computing a post-dominator tree, then the optional argument pred should
      be G.succ. *)
    ...
    let () =
      G.iter_vertex
        (fun y ->
          ...
          | [x] ->
              if
                (* a node that is not reachable from start has no idom *)
                S.is_empty (dominators x)
              then ()
              else H.add tree x y
          | _ ->
              S.iter (fun x -> if idoms x y then H.add tree x y) (dominators y))
        cfg in
  ...
  let idom_to_dom_tree cfg idom =
    ...
    let () =
      G.iter_vertex
        (fun v ->
          try H.add tree (idom v) v with Not_found -> ()
          (* s0 doesn't have an idom *))
        cfg in
    H.find_all tree

  let idom_to_idoms (idom : idom) x y =
    try G.V.equal x (idom y) with Not_found -> false

  (* s0 doesn't have an idom *)

  (** Computes the dominance frontier. As specified in section 19.1 of Modern
  ...
  let compute_dom_frontier cfg (dom_tree : dom_tree) (idom : idom) =
    ...
    let df_local n =
      ...
      List.filter (fun y -> not (idoms n y)) (G.succ cfg n) in
    ...
    and add_df_ups s n k = function
      ...
      | c :: chl ->
          df c (fun dfc ->
              add_df_ups
                (List.fold_left
                   (* the appel errata uses sdom, but Muchnick uses idoms,
                      which should be a bit faster and is the same *)
...
module Make_graph (G : I) = struct
  ...
  let compute_dom_graph cfg dom_tree =
    ...
    G.iter_vertex
      (fun p ->
        try List.iter (G.add_edge g p) (dom_tree p) with Not_found -> ())
      cfg ;
    g

  (** Computes all dominance functions. This function computes some things
      eagerly and some lazily, so don't worry about it doing extra work to
      compute functions you don't need, but also don't call it if you aren't
      going to use anything it returns.
      @return
      a record containing all dominance functions for the given graph and entry
      node. *)
  let compute_all cfg s0 =
    ...
    let dom_frontier =
      lazy (compute_dom_frontier cfg (Lazy.force dom_tree) idom) in
    { idom
    ; idoms
