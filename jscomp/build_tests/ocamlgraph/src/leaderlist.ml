...
module Make (G : G) = struct
  ...
  let leader_lists g root =
    ...
    let partition_vertices f g =
      G.fold_vertex
        (fun n (s1, s2) -> if f n then (S.add n s1, s2) else (s1, S.add n s2))
        g (S.empty, S.empty) in
    ...
    let is_leader n =
      if n = root then true (* the root node is always a leader *)
        ...
        | [] -> true
        (* this would be dead code -- it has no predecessor so make it a leader
           anyway *)
        | [x] -> (
          match G.succ g x with
          | [] ->
              assert false (* -> inconsistency in the graph implementation *)
          | [_] -> false (* this is a straight, continuous control flow *)
          | _ -> true (* predecessor has multiple successors *) )
        | _ -> true
      (* more than one predecessor *) in
    ...
    let basic_block x =
      let rec basic_block x bb =
        ...
        | y :: _ -> (
          match S.mem y entourage with
          | true ->
              (* successor is not a leader, continue collecting *)
              basic_block y (x :: bb)
          | false -> x :: bb (* successor is a leader -- end of basic block *)
          ) in
      (* blocks a are built in reverse order for performance reasons -- correct
         that *)
      List.rev (basic_block x []) in
    let basic_block_list = S.fold (fun x ss -> basic_block x :: ss) leader [] in
    List.rev basic_block_list

  (* this will bring the head of the lists in ascending order -- as given by
     Set.S *)
end
