...
struct
  ...
  module Vertex_Set : Set.S with type t = N.Vertex_Set.t and type elt = G.V.t =
    N.Vertex_Set
          ...
          let bigs, tl =
            Vertex_Set.fold
              (fun v _ ->
                ...
                in
                List.fold_left add_neighbourhoods (bigs, tl)
                  (cc (l @ neighbourhood v)))
              s (bigs, tl)
          in
  ...
  let set_of_allminsep g =
    List.fold_left
      (fun bigs s -> VSetset.add s bigs)
      VSetset.empty (allminsep g)
...
struct
  ...
  include Make (struct
    ...
    let cc =
      let module CC = Components.Make (struct
        include G

        let iter_vertex f = iter_vertex (fun v -> if Mark.get v = 0 then f v)
      end) in
      fun g l ->
        G.Mark.clear g ;
