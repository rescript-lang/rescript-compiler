...
module B (B : Builder.S) = struct
  ...
  let identify_extremities g vl =
    let f e accu =
      ...
      if sx || sy then B.G.E.(create x (label e) y) :: accu else accu in
  ...
  (* – former buggy version – the case where v is neither the source nor the
     ...
     let merge_vertex g vl = match vl with | [] -> g | _ :: vl' -> let
     to_be_added = identify_extremities g vl in let g = List.fold_left
     B.remove_vertex g vl' in List.fold_left B.add_edge_e g to_be_added *)
  ...
  let merge_vertex g vl =
    ...
    | v :: vl' ->
        ...
        if to_be_added = [] then B.add_vertex g v
        else List.fold_left B.add_edge_e g to_be_added

  let merge_edges_e ?src ?dst g el =
    match el with
    | e :: el' ->
        ...
        if el' <> [] then
          ...
          let remove accu e =
            try B.remove_edge_e accu e with Invalid_argument _ -> g in
          let g = List.fold_left remove g el in
          if List.exists (fun v -> mem v destinations) sources then
            let v =
              match src with
              | None -> (
                match dst with None -> List.hd sources | Some w -> w )
              | Some v -> v in
            ...
            let w =
              match src with None -> List.hd destinations | Some w -> w in
  ...
  let merge_edges_with_label ?src ?dst ?label g l =
    let update_label e =
      match label with None -> e | Some l -> B.G.E.(create (src e) l (dst e))
    in
    let collect_edge e accu =
      if B.G.E.label e = l then update_label e :: accu else accu in
    ...
    try
      let default_vertex =
        ...
        ( try
            B.G.iter_vertex
              (fun v ->
                a_vertex_of_g := Some v ;
                raise Exit)
              g
          with Exit -> () ) ;
        match !a_vertex_of_g with Some v -> v | None -> raise Exit
        (*hence g is empty*) in
      fun l1 l2 ->
  ...
  let merge_isolabelled_edges g =
    ...
    let module M =
    (* TODO: using [compare] here is really suspicious ... DONE – yet not so
       clean *)
    Map.Make (struct
      type t = B.G.E.label

      let compare = compare_label g
    end) in
    let accumulating e accu =
      ...
      try
        ...
        M.add l (s, d) accu
      with Not_found ->
        M.add l B.G.E.(S.singleton (src e), S.singleton (dst e)) accu in
  ...
  let merge_ends ?(strict = false) ?specified_vertex g =
    ...
    let to_be_merged =
      match specified_vertex with Some v -> v :: ends | None -> ends in
  ...
  let merge_starts ?(strict = false) ?specified_vertex g =
    ...
    let to_be_merged =
      match specified_vertex with Some v -> v :: starts | None -> starts in
    merge_vertex g to_be_merged
  ...
  let merge_scc ?(loop_killer = false) ?specified_vertex g =
    ...
    let alter accu to_be_identified =
      ...
      if loop_killer then B.remove_edge accu v v else accu in
