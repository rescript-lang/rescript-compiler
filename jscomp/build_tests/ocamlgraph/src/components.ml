...
module Make (G : G) = struct
  ...
  let scc g =
    ...
    let rec pop x = function
      | (y, w) :: l when y > x -> H.add hashcomp w !numcomp ; pop x l
      | l -> l in
    let cont = ref [] in
    let visit v =
      if not (H.mem root v) then (
        ...
        G.iter_succ (fun w -> cont := Visit (v, w) :: Test (v, w) :: !cont) g v
        ) in
    let rec finish () =
      match !cont with
      | [] -> ()
      | action :: tail ->
          ...
          finish () in
    let visit_and_finish v = visit v ; finish () in
...
module Undirected (G : U) = struct
  ...
  let components g =
    ...
    let visit v =
      ...
      if not (H.mem comp v) then (H.add comp v !count ; incr count) in
