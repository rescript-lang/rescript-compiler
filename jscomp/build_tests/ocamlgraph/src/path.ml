...
module Dijkstra (G : G) (W : Sig.WEIGHT with type edge = G.E.t) = struct
  open G.E
  ...
  let shortest_path g v1 v2 =
    ...
    let rec loop () =
      ...
      if G.V.compare v v2 = 0 then (List.rev p, w)
      else (
        if not (H.mem visited v) then (
          ...
          G.iter_succ_e
            (fun e ->
              let ev = dst e in
              if not (H.mem visited ev) then
                let dev = W.add w (W.weight e) in
                let improvement =
                  try W.compare dev (H.find dist ev) < 0
...
module BellmanFord (G : G) (W : Sig.WEIGHT with type edge = G.E.t) = struct
  open G.E
  module H = Hashtbl.Make (G.V)
  ...
  let all_shortest_paths g vs =
    ...
    let build_cycle_from x0 =
      let rec traverse_parent x ret =
        ...
        let s = src e in
        if G.V.equal s x0 then e :: ret else traverse_parent s (e :: ret) in
      traverse_parent x0 [] in
    let find_cycle x0 =
      let visited = H.create 97 in
      let rec visit x =
        if H.mem visited x then build_cycle_from x
        else (
          H.add visited x () ;
          let e = H.find admissible x in
          visit (src e) ) in
          ...
          (fun e x ->
            ...
            try
              ...
              let improvement =
                ...
                with Not_found -> true in
              if improvement then (
                H.replace dist ev2 dev2 ; H.replace admissible ev2 e ; Some ev2
                )
              else x
            with Not_found -> x)
          g None in
  ...
  let find_negative_cycle_from g vs =
    try
      let _ = all_shortest_paths g vs in
      raise Not_found
    with NegativeCycle l -> l

  module Comp = Components.Make (G)

  (* This is rather inefficient implementation. Indeed, for each strongly
     connected component, we run a full Bellman-Ford algorithm using one of its
     vertex as source, taking all edges into consideration. Instead, we could
     limit ourselves to the edges of the component. *)
  let find_negative_cycle g =
    let rec iter = function
      | [] -> raise Not_found
      | (x :: _) :: cl -> (
        try find_negative_cycle_from g x with Not_found -> iter cl )
      | [] :: _ -> assert false
      (* a component is not empty *) in
    iter (Comp.scc_list g)
end

module Johnson (G : G) (W : WJ with type edge = G.E.t) = struct
  ...
  module G' = struct
    ...
    module V = struct
      ...
      let hash v = match v with Old v -> G.V.hash v | New -> 42
    ...
    module E = struct
      ...
      let src e = match e with NewE _ -> V.New | OldE e -> V.Old (G.E.src e)
      let dst e = match e with NewE v -> v | OldE e -> V.Old (G.E.dst e)

      let label e =
        match e with NewE _ -> assert false | OldE e -> G.E.label e
    ...
    let iter_vertex f g =
      f V.New ;
      G.iter_vertex (fun v -> f (V.Old v)) g
    ...
    let iter_succ_e f g v =
      match v with
      | V.New -> G.iter_vertex (fun u -> f (E.NewE (V.Old u))) g
      | V.Old v -> G.iter_succ_e (fun e -> f (E.OldE e)) g v

    let fold_edges_e f g acc =
      let acc' = G.fold_vertex (fun x _ -> f (E.NewE (V.Old x)) acc) g acc in
      G.fold_edges_e
        (fun edg ->
  ...
  module W' = struct
    ...
    let zero = W.zero
    let weight e = match e with NewE _ -> zero | OldE e -> W.weight e
