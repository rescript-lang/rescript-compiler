...
module Make (B : Builder.INT) = struct
  ...
  let random_few_edges add_edge ~loops ~v ~e =
    ...
    let rec random_edge g =
      ...
      if (i = j && not loops) || G.mem_edge g a.(i) a.(j) then random_edge g
      else add_edge g a.(i) a.(j) in
           ...
           = add max
               (add
                  ...
                  (of_int
                     ...
                     | false, true ->
                         (i * (i - 1) / 2) + if j > i then i else j
                     | false, false ->
                         (i * (i + 1) / 2) + if j > i then i + 1 else j ))) ) ;
      if nb = 0 then g
      else
        let add_edges =
  ...
  (** G(n,p) graphs See https://en.wikipedia.org/wiki/Random_graph *)
...
module Planar = struct
  ...
  struct
    ...
    let graph ?(loops = false) ~xrange:(xmin, xmax) ~yrange:(ymin, ymax) ~prob
        v =
      ...
      let random_point () =
        ...
        , ymin + Random.int (1 + ymax - ymin) ) in
      ...
      let g =
        if loops then
          Array.fold_left
            (fun g v ->
              if Random.float 1.0 < prob then g
              else
                let e = E.create v 0 v in
                B.add_edge_e g e)
            g vertices
        else g in
...
(* Local Variables: compile-command: "make -C .. src/rand.cmo" End: *)
