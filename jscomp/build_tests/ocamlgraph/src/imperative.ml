...
(* Vertex for abstract imperative graphs: comparing to vertex for abstract
   **persistent** graphs, marks are added. *)
module AbstractVertex (V : sig
  type t
end) =
struct
  type label = V.t
    ...
    let remove_vertex g v =
      if HM.mem v g.edges then (
        let remove s =
          S.fold
            (fun ((v2, _) as e) s ->
              if not (V.equal v v2) then S.add e s else s)
            s S.empty in
        let e = g.edges in
        ignore (HM.remove v e) ;
...
module Matrix = struct
  ...
  module Digraph = struct
    ...
    let fold_vertex f g a =
      ...
      let rec fold i a = if i = n then a else fold (i + 1) (f i a) in
      fold 0 a
    ...
    let iter_succ f g i =
      ...
      for j = 0 to nb_vertex g - 1 do
        if Bitv.unsafe_get si j then f j
      done

    (* optimization w.r.t. [Bitv.iteri (fun j b -> if b then f j) g.(i)] *)

    let iter_pred f g i =
      for j = 0 to nb_vertex g - 1 do
        if Bitv.unsafe_get g.(j) i then f j
      done
    ...
    let fold_pred f g i a =
      fold_vertex (fun j a -> if Bitv.unsafe_get g.(j) i then f j a else a) g a
    ...
    let iter_succ_e f g i =
      let si = g.(i) in
      for j = 0 to nb_vertex g - 1 do
        if Bitv.unsafe_get si j then f (i, j)
      done
