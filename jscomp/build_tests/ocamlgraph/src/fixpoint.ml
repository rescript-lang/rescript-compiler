...
(* maximum fixpoint point calculation with the work list algorithm; to
   implement a concrete analysis, implement a module that satisfies the Rules
   signature. Such a module in the Analysis functor gives a complete
   analysis/optimization module that works on a CFG. *)
  ...
  let analyze initial g =
      ...
      let add =
        ...
        | Backward ->
            fun n ->
              ...
              List.map (fun edge -> (A.analyze edge, G.E.dst edge)) succs in
    ...
    let rec worklist (data : A.data M.t) (wl : N.t) =
      ...
      let meet ~default = function
        | [] -> default
        | [x] -> x
        | x :: xs -> List.fold_left (fun a b -> A.join a b) x xs in
      (* analyze one node, creating a new data-set and node-worklist as
         necessary *)
      let analyze_node analysis n d wl =
        match analysis d n with None -> (d, wl) | Some d' -> (d', N.add n wl)
      in
      (* get some node from the node-set -- this will eventually trigger an
         exception *)
      ...
      | Some n ->
          ...
          let f, ns =
            match A.direction with
            (* analyze all INCOMING edges of all SUCCESSOR nodes of the node to
               be processed *)
            | Forward ->
                (* process one node: analyze all it's incoming edges and merge
                   the resulting data; if the result is different to the
                   previously stored data for this node, return a new tuple,
                   else None *)
                let new_node_data (data : A.data M.t) node =
                  let edges = M.find node nodemap in
                  let analysis =
                    List.map (fun (f, src) -> f (M.find src data)) edges in
                  ...
                  else Some (M.add node node_data' data) in
                (new_node_data, G.succ g n)
            (* analyze all OUTGOING edges of all PREDECESSOR nodes of the node
               to be processed *)
            | Backward ->
                let new_node_data (data : A.data M.t) node =
                  let edges = M.find node nodemap in
                  let analysis =
                    List.map (fun (f, dst) -> f (M.find dst data)) edges in
                  let node_data = M.find node data in
                  let node_data' = meet ~default:node_data analysis in
                  if A.equal node_data node_data' then None
                  else Some (M.add node node_data' data) in
                (new_node_data, G.pred g n) in
          (* analyze all successor nodes by analyzing all of their predecessor
             edges *)
          let data, wl =
            List.fold_left
              (fun (d, wl) n -> analyze_node f n d wl)
              (data, wl) ns in
