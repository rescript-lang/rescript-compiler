...
module Make (B : Builder.S) = struct
  ...
  let mirror g =
    if G.is_directed then
      let g' = G.fold_vertex (fun v g' -> B.add_vertex g' v) g (B.empty ()) in
      G.fold_edges_e
        (fun e g' ->
          let v1 = G.E.src e in
          let v2 = G.E.dst e in
  ...
  let complement g =
    G.fold_vertex
      (fun v g' ->
        G.fold_vertex
          (fun w g' -> if G.mem_edge g v w then g' else B.add_edge g' v w)
          g g')
      g (B.empty ())
    ...
    G.fold_vertex
      (fun v g ->
        try
          ...
          G.fold_succ_e
            (fun e g ->
              if List.exists (fun e' -> G.E.compare e e' = 0) succ then
                B.add_edge_e g e
              else g)
            g1 v (B.add_vertex g v)
        with Invalid_argument _ -> (* [v] not in [g2] *)
                                   g)
      g1 (B.empty ())
...
struct
  ...
  let set_from_vertices g l =
    ...
    let add x s = if Vertex_Set.mem x env_init then s else Vertex_Set.add x s in
