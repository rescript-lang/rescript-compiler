...
module Imperative (G : Sig.IM) (W : Sig.WEIGHT with type edge = G.E.t) = struct
  module S = Set.Make (G.V)
  module M = Map.Make (G.V)
  module V = G.V
  module E = G.E

  (* [G.t] represents graph itself. [unit M.t] maintains a list of source
     vertices to keep track of distances for all vertices. [(G.E.t option *
     W.t) M.t M.t] holds mappings for all vertices, each of which contains its
     shortest-path tree ancestor (parent) and a distances from source vertices. *)
        ...
        M.iter
          (fun k (origin, dist) ->
            print_string
              ...
              ^ ( match origin with
                | None -> "---"
                | Some e -> sov (G.E.src e) ^ ">" ^ sov (G.E.dst e) )
              ^ ":"
              ^ string_of_int (Obj.magic dist)
              ^ ") " ))
          v)
      !dist ;
    print_string "\n"

  (* If an edge is going to be added to the graph, which will cause a negative
     cycle, raises [Negative_cycle] with edges that can form such the cycle. *)
  exception Negative_cycle of G.E.t list

  let create ?size () =
    let g =
      match size with Some size -> G.create ~size () | None -> G.create ()
    in
    (g, ref S.empty, ref M.empty)

  let copy (g, src, dist) = (G.copy g, ref !src, ref !dist)
  ...
  let add_vertex (g, src, dist) v =
    (* Before adding vertex to the graph, make sure that the vertex is not in
       the graph. If already in the graph, just do nothing and return as is. *)
    if not (G.mem_vertex g v) then (
      (* Add a vertex to the original one *)
      G.add_vertex g v ;
      (* The new vertex will immediately be added to the source list *)
      src := S.add v !src ;
      (* The new edge should contain a distance mapping with only from myself
         with distance zero. *)
      ...
      let dist =
        G.fold_succ_e
          (fun e dist ->
            ...
            (* Compare distances from given source vertices. If relax happens,
               record it to the new list. *)
            let v2dist, nextSrc =
              S.fold
                (fun x (v2dist, nextSrc) ->
            ...
            if S.is_empty nextSrc then dist
                ...
                S.fold
                  (fun s x ->
                    let rec build_cycle x ret =
                      ...
                      | _ -> None in
                    match x with None -> build_cycle v2 [] | Some _ -> x)
                  nextSrc None in
              let cycle =
                match cycle with Some x -> x | None -> assert false in
              dump_cycle cycle ;
              raise (Negative_cycle cycle) )
            else (
              (* TODO: Some room for improvement. If queue has (v2, s) already,
                 technically we can merge nextSrc into s, so that the number of
  ...
  let add_edge_internal (g, src, dist) v1 v2 =
    ...
    (* To reduce the amount of codes, we just start propagation from v1. Of
       course, this can be optimized by starting from v2. But it may duplicate
       the same code in multiple places in the file. In addition, such an
       optimization only cost for small amount, which precisely is the
       operations to relax edges from v1, other than which have been existed
       before this [add_edge_e] call. *)
    ...
    if m_cardinal dv1 = 1 && M.mem v2 dv1 then (
      (* Now we definitely introduced a loop (and possibly non-negative)! Let
         me see if this would be negative or not... *)
      Queue.add (v1, S.add v2 S.empty) q ;
      propagate (g, src, dist) q v1 )
    else
      (* Or even if we fall back to else-clause here, the edge addition may
         have introduced a cycle. Anyway, we need to check if one is newly
         created or not at [propagate] *)
      let src, dist, dv1 =
        if not (S.mem v2 src) then
          ...
          (* We can exclude v2 from the list of source because one can reach v2
             from some other vertex. *)
          let src = S.remove v2 src in
          (* Note that following line can be skipped only if the user don't
             remove vertex. Otherwise, such operation like [add_edge g v1 v2] >
             [remove_vertex g v2] > [add_vertex g v2] can result in unexpected
             behavior. *)
          let dist = M.map (M.remove v2) dist in
          (* We need to re-obtain the distance mappings at v1, since it can be
             changed by the line above. *)
          let dv1 = M.find v1 dist in
          (src, dist, dv1) in
  ...
  let add_edge_e (g, src, dist) e =
    (* Before adding edge to the graph, make sure that the edge is not in the
       graph. If already in the graph, just do nothing and return as is. *)
    if not (G.mem_edge_e g e) then (
      ...
      ( try
          (* Because we can restore the graph by calling [G.remove_edge_e] even
             in case of failure, we first add it by [G.add_edge_e]. *)
          G.add_edge_e g e ;
          let _, src', dist' = add_edge_internal (g, !src, !dist) v1 v2 in
          src := src' ;
          dist := dist'
        with exp ->
          (* In case of excecption, restore the graph by removing the edge, and
             rethrow the exception. *)
          G.remove_edge_e g e ; raise exp ) ;
      dump (src, dist) )
  ...
  let add_edge (g, src, dist) v1 v2 =
    ...
    if not (G.mem_edge g v1 v2) then (
      List.iter (add_vertex (g, src, dist)) [v1; v2] ;
      ( try
          (* Because we cannot know the default value for edge length, we first
             try to add one by [G.add_edge]. If there occurs an exception,
             restore the graph by [G.remove_edge] since there were no other
        ...
        with exp ->
          (* In case of excecption, restore the graph by removing the edge, and
             rethrow the exception. *)
          G.remove_edge g v1 v2 ; raise exp ) ;
      dump (src, dist) )

  let remove_edge_internal (g, src) v2 =
    (* Actually, we need to rebuild the distance table, rather than traverse
       precedants to remove the edge. *)
    let q = Queue.create () in
    print_string "dump: " ;
    dump_set src ;
    let dist =
      S.fold
        (fun x dist ->
          ...
          M.add x (M.add x (None, W.zero) M.empty) dist)
        src M.empty in
    let g, src, dist = propagate (g, src, dist) q (S.choose src) in
    if M.mem v2 dist then (g, src, dist)
    else (
  ...
  let map_vertex f (g, src, dist) =
    let map_map update m =
      M.fold (fun v m acc -> M.add (f v) (update m) acc) m M.empty in
    let g, src, dist =
      ( G.map_vertex f g
...
module Persistent (G : Sig.P) (W : Sig.WEIGHT with type edge = G.E.t) = struct
  module S = Set.Make (G.V)
  module M = Map.Make (G.V)
  module E = G.E
  module V = G.V
  ...
  let add_vertex (g, src, dist) v =
    (* Before adding vertex to the graph, make sure that the vertex is not in
       the graph. If already in the graph, just do nothing and return as is. *)
    if G.mem_vertex g v then (g, src, dist)
    else
      (* Add a vertex to the original one *)
      ( G.add_vertex g v
      , (* The new vertex will immediately be added to the source list *)
            ...
            if S.is_empty nextSrc then dist
              ...
              let rec build_cycle x ret =
                ...
                | Some e, _ ->
                    let y = G.E.src e in
                    let cycle = e :: ret in
                    if G.V.equal start y then cycle else build_cycle y cycle
                | _ -> assert false in
  ...
  let add_edge_internal (g, src, dist) v1 v2 =
    ...
    (* We need to check whether v2 should be kept in the source list or not.
       ...
       still maintained. Otherwise, simply ignore the distance from v2 *)
    if m_cardinal dv1 = 1 && M.mem v2 dv1 then (
      (* Now we definitely introduced a loop (but possibly non-negative)! Let
         me see if this would be negative or not... *)
      Queue.add (v1, S.add v2 S.empty) q ;
      propagate (g, src, dist) q v1 )
    else
      ...
      let src, dist, dv1 =
        if not (S.mem v2 src) then
          ...
          , (* Note that following line can be skipped only if the user don't
               remove vertex. Otherwise, such operation like [add_edge g v1 v2]
               > [remove_vertex g v2] > [add_vertex g v2] can result in
               unexpected behaviour. *)
            M.map (M.remove v2) dist
          , (* We need to re-obtain the distance mappings at v1, since it can
               be changed by the line above. *)
            M.find v1 dist ) in
      (* Now let's start propagation. *)
      Queue.add (v1, set_of_map dv1) q ;
      propagate (g, src, dist) q v1
  ...
  let add_edge_e (g, src, dist) e =
    (* Before adding edge to the graph, make sure that the edge is not in the
       graph. If already in the graph, just do nothing and return as is. *)
    if G.mem_edge_e g e then (g, src, dist)
    else
      (* Vertices involved *)
      let v1 = G.E.src e in
      let v2 = G.E.dst e in
      let g, src, dist = List.fold_left add_vertex (g, src, dist) [v1; v2] in
      let g = G.add_edge_e g e in
      add_edge_internal (g, src, dist) v1 v2

  let add_edge (g, src, dist) v1 v2 =
    (* Same as [add_edge_e] *)
    if G.mem_edge g v1 v2 then (g, src, dist)
    else
      let g, src, dist = List.fold_left add_vertex (g, src, dist) [v1; v2] in
  ...
  let remove_edge_e (g, src, dist) e =
    (* Same as [add_edge_e] *)
    if not (G.mem_edge_e g e) then (g, src, dist)
    else
  ...
  let remove_edge (g, src, dist) v1 v2 =
    (* Same as [add_edge] *)
    if not (G.mem_edge g v1 v2) then (g, src, dist)
    else
  ...
  let remove_vertex t v =
    ...
    (* Note that we are iterating on [g] that is being modified during
       iteration. We can do such an above iteration since G is here permanent.
       Do not try this for imperative graph. *)
    let g, src, dist = t in
    (* Now we can feel free to delete [v]. *)
    (G.remove_vertex g v, S.remove v src, M.map (M.remove v) dist)
  ...
  let map_vertex f (g, src, dist) =
    ...
    , let update = function
        | (None, _) as v -> v
        | Some e, w ->
            (Some (E.create (f (E.src e)) (E.label e) (f (E.dst e))), w) in
