  ...
  let fold f g acc =
    ...
    let add_vertex x =
      ...
      let add_edge y =
        ...
        if ix <> iy then (
          ...
          degree.(iy) <- degree.(iy) + 1 ) in
      G.iter_succ add_edge g x in
    ...
    let rec walk acc =
      if Queue.is_empty todo then acc
        ...
        walk acc in
    for i = 0 to n - 1 do
      if degree.(i) = 0 then Queue.push i todo
    done ;
    walk acc
...
struct
  ...
  (* in case of multiple cycles, choose one vertex in a cycle which does not
     depend of any other. *)
  let find_top_cycle checker vl =
    (* choose [v] if each other vertex [v'] is in the same cycle (a path from v
       to v') or is in a separate component (no path from v' to v). So, if
       there is a path from v' to without any path from v to v', discard v. *)
    let on_top_cycle v =
      List.for_all
        (fun v' ->
  ...
  let fold f g acc =
    ...
    let push x = H.remove degree x ; Q.push x todo in
    let rec walk acc =
