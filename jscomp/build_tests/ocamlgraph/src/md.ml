  ...
  let md g =
    ...
    while (not (CT.is_chordal !gtri)) && !i < n do
      let v =
        ...
        match x with
        | Some (v, _) -> v
        | None -> failwith "Expecting some vertex" in
        ...
        List.fold_left
          (fun (g, tri) v ->
            let tri' =
              List.fold_left
                (fun tri v' ->
                  if v <> v' && not (G.mem_edge g v v') then (v, v') :: tri
                  else tri)
                tri ng in
            let g' =
              List.fold_left
                (fun g v' -> if v <> v' then G.add_edge g v v' else g)
                g ng in
            (g', tri'))
          (!gref, []) ng in
      ord := v :: !ord ;
      gtri := List.fold_left (fun g (x, y) -> G.add_edge g x y) !gtri tri' ;
    ...
    while (not (CT.is_chordal gtri)) && !i < n do
      ...
      let tri' =
        List.fold_left
          ...
          [] ng in
      ord := v :: !ord ;
      List.iter (fun (x, y) -> G.add_edge gtri x y) tri' ;
