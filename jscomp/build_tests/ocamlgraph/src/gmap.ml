...
module Vertex (G_Src : V_SRC) (G_Dst : V_DST) = struct
  ...
  let convert_vertex f x =
    try H.find vertices x
    with Not_found ->
      let x' = f x in
      H.add vertices x x' ; x'
  ...
  let filter_map f g =
    G_Src.fold_vertex
      (fun x g -> match f x with Some e -> G_Dst.add_vertex g e | None -> g)
      g (G_Dst.empty ())
end
...
module Edge (G_Src : E_SRC) (G_Dst : E_DST) = struct
  ...
  let filter_map f g =
    G_Src.fold_edges_e
      (fun x g -> match f x with Some e -> G_Dst.add_edge_e g e | None -> g)
      g (G_Dst.empty ())
end

(* Vertex (struct include G_Src module V = E let fold_vertex = fold_edges_e
   end) (struct include G_Dst type vertex = edge let add_vertex = add_edge_e
   end) *)
