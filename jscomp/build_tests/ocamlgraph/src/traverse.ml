...
module Dfs (G : G) = struct
  ...
  let iter ?(pre = fun _ -> ()) ?(post = fun _ -> ()) g =
    ...
    let rec visit v =
      if not (H.mem h v) then (
        H.add h v () ; pre v ; G.iter_succ visit g v ; post v ) in
  ...
  let has_cycle_undirected g =
    ...
    let is_father u v =
      ...
      try G.V.equal (H.find father v) u with Not_found -> false in
    let rec visit v =
      H.add h v true ;
      G.iter_succ
        (fun w ->
        ...
        g v ;
      H.remove father v ;
      H.replace h v false in
    try
      G.iter_vertex (fun v -> if not (H.mem h v) then visit v) g ;
      false
    with Exit -> true
  ...
  module Tail = struct
    let has_cycle g =
      ...
      try
        G.iter_vertex
          (fun v -> if not (H.mem h v) then (Stack.push v stack ; loop ()))
          g ;
        false
      with Exit -> true
      ...
      let loop () =
        while not (Stack.is_empty stack) do
          ...
          if not (H.mem h v) then (
            ...
            f v ;
            G.iter_succ
              (fun w -> if not (H.mem h w) then Stack.push w stack)
              g v )
        done in
      G.iter_vertex
        (fun v -> if not (H.mem h v) then (Stack.push v stack ; loop ()))
  ...
  let get (_, st, _) = match st with [] -> raise Exit | v :: _ -> v
...
module Bfs (G : G) = struct
  ...
  let fold f i (g : G.t) =
    ...
    let push v = if not (H.mem h v) then (H.add h v () ; Queue.add v q) in
    let rec loop s =
      if not (Queue.is_empty q) then (
        let v = Queue.pop q in
        let ns = f v s in
        (* Sticking to OCamlGraph's fold conv *)
        G.iter_succ push g v ; loop ns )
      else s in
  ...
  let get (s, q, _) =
    if Q.is_empty q then if S.is_empty s then raise Exit else S.choose s
    else Q.peek q

  let step (s, q, g) =
    let push v ((s, q) as acc) =
      if S.mem v s then (S.remove v s, Q.push v q) else acc in
    let v, s', q' =
...
module Mark (G : GM) = struct
  let dfs g =
    ...
    let n = ref 0 in
    let rec visit v =
      if G.Mark.get v = 0 then (
        incr n ; G.Mark.set v !n ; G.iter_succ visit g v ) in
    G.iter_vertex visit g

  (* invariant: [h v = 0] means not visited at all; [h v = 1] means already
     visited in the current component; [h v = 2] means already visited in
     another tree *)
  let has_cycle g =
    G.Mark.clear g ;
    let rec visit v =
      ...
      G.Mark.set v 2 in
    try
      G.iter_vertex (fun v -> if G.Mark.get v = 0 then visit v) g ;
      false
    with Exit -> true
end
