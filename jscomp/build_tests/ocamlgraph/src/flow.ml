...
struct
  ...
  let flux_et_reflux g x =
    let s =
      G.fold_succ_e
        (fun e s ->
          if
            F.compare (capacite_restante g e) (F.min_capacity (G.E.label e))
            > 0
          then e :: s
          else s)
        g x [] in
    G.fold_pred_e
      (fun e s ->
        if
          F.compare
            (E.find flot (G.E.src e, G.E.dst e))
            (F.min_capacity (G.E.label e))
          > 0
        then G.E.create (G.E.dst e) (G.E.label e) (G.E.src e) :: s
        else s)
      g x s
  ...
  let elever g p x =
    ...
    && (not (G.V.equal x p))
    && List.for_all
         (fun e -> V.find hauteur (G.E.src e) <= V.find hauteur (G.E.dst e))
         u
    &&
    let min =
  ...
  let maxflow g s p =
    let push_and_pull l x =
      ...
      || G.fold_pred_e (fun e r -> pousser g e l || r) g x false in
    ...
    let flot_max =
      G.fold_pred_e
        (fun e f -> F.add (E.find flot (G.E.src e, p)) f)
        g p F.zero in
    let flot_init =
    ...
    let f e =
      let x, y = (G.E.src e, G.E.dst e) in
      try E.find flot (x, y) with Not_found -> F.flow (G.E.label e) in
...
struct
  ...
  module Result = struct
    ...
    let change op r e f =
      try H.replace r e (op (find r e) f) with Not_found -> assert false
  ...
  let grow_flow r s t a =
    let rec loop u b =
      if G.V.equal s u then (
        match b with
        | F.Infinity ->
            (* source = destination *)
            assert (G.V.equal s t) ;
            a
        | F.Flow f -> set_flow r s t f ; F.add a f )
      else
  ...
  let maxflow g s t =
    ...
    let succ s =
      G.iter_succ_e
        (fun e ->
          ...
          let t = G.E.dst e in
          if not (Mark.mem t || is_full r e) then Mark.set t (Some e) Mark.Plus)
        g s in
    let pred s =
      G.iter_pred_e
        (fun e ->
    ...
    let internal_loop a =
      try
        ...
        assert false
      with Queue.Empty -> if Mark.mem t then grow_flow r s t a else a in
    let rec external_loop a =
      Mark.clear () ;
      Mark.set s None Mark.Plus ;
      let a' = internal_loop a in
      if a = a' then a else external_loop a' in
