...
(** Code follows Don Knuth's algorithm from ``Axioms and hulls'' (LNCS 606,
    Springer-Verlag, 1992), pp. 73-77.

    Some code and comments are taken from the Stanford Graph Base, file
    [gb_plane]. *)
...
module Make (S : CCC) = struct
  ...
  (* Each edge of the current triangulation is represented by two arcs pointing
     in opposite directions; the two arcs are called mates. Each arc
     conceptually has a triangle on its left and a mate on its right. *)
  ...
  and node = Branch of int * int * node ref * node ref | Terminal of arc

  type triangulation =
    {points: S.point array; arcs: arc array; last_used_arc: int}

  let rec dummy_arc =
    {vert= Infinity; next= dummy_arc; inst= ref (Terminal dummy_arc); mate= -1}
  ...
  let triangulate points =
    ...
    let in_circle p q r s =
      S.in_circle points.(p) points.(q) points.(r) points.(s) in
    let n = Array.length points in
    if n < 2 then invalid_arg "triangulate" ;
    let arcs = Array.init ((6 * n) - 6) (make_arc n) in
    let mate i = (6 * n) - 7 - i in
    (*i DEBUG let rec dump d l = eprintf "%s" (String.make (2*d) ' '); match !l
      with | Terminal a -> eprintf "T %d\n" (mate a.mate) | Branch (u, v, l, r)
      -> eprintf "N %d %d\n" u v; dump (d+1) l; dump (d+1) r in i*)

    (* initialization: create a trivial triangulation for the first 2 vertices *)
    let u = 0 in
    let v = 1 in
    let a1 = arcs.(0) in
    let a2 = arcs.(1) in
    ...
    for p = 2 to n - 1 do
      ...
      let rec step_T1 l p =
        match !l with
        | Terminal al -> (l, al)
        | Branch (pl, ql, al, bl) -> step_T1 (if ccw pl ql p then al else bl) p
      in
      ...
      let rec loop c =
        ...
        let t'' = e.vert in
        if t'' <> Infinity && in_circle (finite t'') t' t p then (
          let t'' = finite t'' in
          ...
          flip c d e t'' p n n' ;
          loop e )
        else if t' <> r then loop arcs.(c.next.mate).next
        else ()
        (* break *) in
      loop c
    done ;
    {points; arcs; last_used_arc= !j}
...
module FloatPoints = struct
  ...
  (*i DEBUG let ccw (xu,yu) (xv,yv) (xw,yw) = eprintf
    "ccw((%.0f,%.0f),(%.0f,%.0f),(%.0f,%.0f)) -> " xu yu xv yv xw yw; let r =
    ccw (xu,yu) (xv,yv) (xw,yw) in eprintf "%b\n" r; flush stderr; r i*)
  ...
  (*i DEBUG let in_circle (xt,yt) (xu,yu) (xv,yv) (xw,yw) = eprintf
    "in_circle((%.0f,%.0f),(%.0f,%.0f),(%.0f,%.0f),(%.0f,%.0f)) -> " xt yt xu
    yu xv yv xw yw; let r = in_circle (xt,yt) (xu,yu) (xv,yv) (xw,yw) in
    eprintf "%b\n" r; flush stderr; r i*)
end

module Float = Make (FloatPoints)

(** Points with integer coordinates. We approximate using module [FloatPoints]
    but this could be made exact following Knuth's code in Axioms and Hulls *)
