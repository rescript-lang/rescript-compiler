...
module type S = sig
  ...
  module Abstract (V : sig
    type t
  end) : Sig.P with type V.label = V.t and type E.label = unit
  ...
  (E : ORDERED_TYPE_DFT) : Sig.P with type V.label = V.t and type E.label = E.t
end
...
module Digraph = struct
  ...
  struct
    ...
    let add_vertex g v =
      ...
      else {edges= G.unsafe_add_vertex g.edges v; size= Pervasives.succ g.size}
  ...
  struct
    ...
    let remove_vertex g v =
      if HM.mem v g.edges then
        ...
        { edges= HM.fold (fun k s g -> HM.add k (remove v s) g) edges HM.empty
