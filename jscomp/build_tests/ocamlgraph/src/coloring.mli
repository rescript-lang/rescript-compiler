...
(** Provide a function for [k]-coloring a graph with integer marks. The
    provided function is more efficient that the one provided by functor
    {!Make} above. *)
module Mark (G : GM) : sig
  exception NoColoring

  val coloring : G.t -> int -> unit
  (** [coloring g k] colors the nodes of graph [g] using k colors, assigning
      the marks integer values between 1 and k. raises [NoColoring] when there
      is no possible coloring.

      The graph marks may be partially set before starting; the meaning of
      initial values is as
      follows:
