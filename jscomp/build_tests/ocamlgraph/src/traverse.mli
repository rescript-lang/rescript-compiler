...
module Dfs (G : G) : sig
  ...
  (** [iter pre post g] visits all nodes of [g] in depth-first search, applying
      [pre] to each visited node before its successors, and [post] after them.
      Each node is visited exactly once. Not tail-recursive. *)

  val prefix : (G.V.t -> unit) -> G.t -> unit
  (** applies only a prefix function; note that this function is more efficient
      than [iter] and is tail-recursive. *)
  ...
  (** {2 Step-by-step iterator}
      ...
      This is a variant of the iterators above where you can move on step by
      step. The abstract type [iterator] represents the current state of the
      iteration. The [step] function returns the next state. In each state,
      function [get] returns the currently visited vertex. On the final state
      both [get] and [step] raises exception [Exit].
...
module Bfs (G : G) : sig
  ...
  (** {2 Step-by-step iterator} See module [Dfs] *)
...
module Mark (G : GM) : sig
  ...
  val has_cycle : G.t -> bool
  (** [has_cycle g] checks for a cycle in [g]. Modifies the marks. Linear time,
      constant space. *)
end
