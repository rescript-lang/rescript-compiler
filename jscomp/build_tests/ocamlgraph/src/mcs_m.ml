...
module MaximalCardinalitySearch = struct
  ...
  module P (Gr : Sig.P) = struct
    ...
    let check_path g u v =
      ...
      let rec aux x : bool =
        if H.mem h x then false
        else if x = v then true
        else if NewV.weight x < maxw || x = u then (
          H.add h x () ;
          G.fold_succ
            (fun x found -> if not found then aux x else found)
            g x false )
        else (H.add h x () ; false) in
      aux u

    module Copy = Gmap.Vertex (Gr) (struct include G include Builder.P (G) end)

    let fold f d =
      let rec aux = function true, a -> aux (f a) | false, a -> a in
      aux d
            ...
            if i = 0 then (false, x)
              ...
              let v =
                G.fold_vertex
                  ...
                  g'
                  (ref 0, snd (Choose.choose_vertex g')) in
              let s =
                G.fold_vertex
                  (fun x s ->
                    if x = v then s
                    else if check_path g' x v then VerticesSet.add x s
                    else s)
                  g' VerticesSet.empty in
              let f' =
                VerticesSet.fold
                  (fun x f ->
                    NewV.set_weight x (succ (NewV.weight x)) ;
                    if not (G.mem_edge g' x v) then EdgeSet.add (x, v) f else f)
                  s f in
              let g' = G.remove_vertex g' v in
              let a' = (i, NewV.label v) :: a in
              (true, (i - 1, g', a', f')))
          (true, (Gr.nb_vertex g, g', [], EdgeSet.empty)) in
      ( List.rev ord
      , EdgeSet.fold
  ...
  module I (Gr : Sig.I) = struct
    ...
    let mcsm g =
      ...
      for i = Gr.nb_vertex g downto 1 do
        let v =
          G.fold_vertex
            (fun x max -> if NewV.weight x > NewV.weight max then x else max)
            g'
            (ref 0, snd (Choose.choose_vertex g')) in
        let s =
          G.fold_vertex
      ...
      ( List.rev !a
      , EdgeSet.fold (fun (x, y) e -> (NewV.label x, NewV.label y) :: e) !f []
      )
