...
type arrow_style = [`None | `Normal | `Inv | `Dot | `Odot | `Invdot | `Invodot]
...
module CommonAttributes : sig
  type graph =
    ...
    | `Label of string  (** Caption for graph drawing. *)
    | `HtmlLabel of string
  ...
  type vertex =
    ...
    *)
    ...
    | `Width of float  (** Sets the minimum width.  Default value is [0.75]. *)
    ]
  (** Attributes of vertices. *)
...
module DotAttributes : sig
  ...
  type subgraph =
    {sg_name: string; sg_attributes: vertex list; sg_parent: string option}
...
module NeatoAttributes : sig
  type graph =
    ...
    | `Overlap of bool  (** Default value is [true]. *)
    | `Spline of bool
  ...
  type edge =
    ...
    | `Len of float  (** Preferred length of edge.  Default value is [1.0]. *)
    | `Weight of float
