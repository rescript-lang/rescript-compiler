...
let with_commands ?packing mk_view model =
  ...
  let view =
    ...
    mk_view ~packing model in
  (* Buttons box *)
  let buttons_box =
    GPack.button_box `HORIZONTAL ~border_width:3 ~child_height:10
      ~child_width:85 ~spacing:10 ~layout:`START
  ...
  let add_button label mode =
    ...
    ignore ((b#connect)#clicked ~callback:(fun _ -> view#switch mode)) in
  add_button "Global" Global ;
  add_button "Tree" Tree ;
  add_button "Both" Both ;
  (* Depth of exploration controls *)
  let depth_hbox =
    GPack.hbox ~packing:(main_table#attach ~expand:`X ~top:0 ~left:1) () in
  ...
  let change_depth_forward adj content () =
    content#set_depth_forward (int_of_float adj#value) in
  let change_depth_backward adj content () =
    content#set_depth_backward (int_of_float adj#value) in
  ignore
  $ (depth_forward_adj#connect)#value_changed
      ~callback:(change_depth_forward depth_forward_adj view) ;
  ignore
  $ (depth_backward_adj#connect)#value_changed
  ...
  let depth_forward_spin =
    GEdit.spin_button ~value:2. ~adjustment:depth_forward_adj () in
  let depth_backward_spin =
    GEdit.spin_button ~value:2. ~adjustment:depth_backward_adj () in
...
module Build (G : Sig.G) (T : DGraphTreeModel.S with type Tree.V.label = G.V.t) =
struct
  type graph = G.t
  type vertex = G.V.t
  type edge = G.E.t
  ...
  class view_container ?packing ?(status = Global) ?(default_callbacks = true)
    ~mk_global_view ~mk_tree_view default_tree_root =
    (* widgets *)
    ...
    let connect_global_callback view node =
      ...
      let callback ev =
        let apply f =
          match view#tree_view with
          | None -> false
          | Some tview ->
              let tree = T.tree () in
              let vlist = T.TreeManipulation.get_tree_vertices item tree in
              List.iter
                (fun v -> f (tview : tree_view) (tview#get_node v))
                vlist ;
              false in
        match ev with
        | `BUTTON_PRESS _ ->
    ...
    object (self)
      ...
      val mutable tree_change = false

      (* must recompute the tree? *)
      ...
      method private memo_tree_view () =
        if tree_change then (
          ...
          | Some r ->
              let view =
                mk_tree_view ~depth_backward ~depth_forward paned_box#as_widget
                  r in
              tree_view <- Some view ;
      ...
      method private memo_global_view () =
        match global_view with
        | None ->
            ...
            let scroll =
              GBin.scrolled_window ~hpolicy:`AUTOMATIC ~vpolicy:`AUTOMATIC
                ~packing:global_frame#add () in
            scroll#add view#coerce
        | Some _ -> ()

      method adapt_zoom () =
        let az = function None -> () | Some w -> w#adapt_zoom () in
        az tree_view ; az global_view
...
module Make (G : Graphviz.GraphWithDotAttrs) = struct
  ...
  let from_graph ?packing ?status ?(default_callbacks = true)
      ?(mk_global_view = fun model -> GView.view ~aa:true model)
      ?(mk_tree_view = fun model -> TView.view ~aa:true model) ?root g =
    let status =
      match status with
      | None ->
      ...
      | Some s -> s in
    new view_container
      ?packing ~default_callbacks ~status
      ~mk_global_view:(fun () -> mk_global_view (GlobalModel.from_graph g))
      ~mk_tree_view:(fun ~depth_backward ~depth_forward w v ->
        let model =
          FullTreeModel.from_graph ~depth_forward ~depth_backward w g v in
        mk_tree_view model)
      root

  let from_graph_with_commands ?packing ?status ?(default_callbacks = true)
      ?mk_global_view ?mk_tree_view ?root g =
    with_commands ?packing
      (fun ~packing g ->
        from_graph ~packing ?status ~default_callbacks ?mk_global_view
          ?mk_tree_view ?root g)
      g
end
...
module Dot = struct
  ...
  let from_dot ?packing ?status ?(default_callbacks = true)
      ?(mk_global_view = fun model -> GView.view ~aa:true model)
      ?(mk_tree_view = fun model -> TView.view ~aa:true model) dot_file =
    let gmodel =
      if Filename.check_suffix dot_file "xdot" then
        DGraphModel.read_xdot dot_file
      else DGraphModel.read_dot dot_file in
    let one_vertex =
      try
        gmodel#iter_vertex (fun v -> raise (Found v)) ;
        None
      with Found v -> Some v in
    let status =
      match status with
      | None ->
          let nb f =
            ...
            !cpt in
          if
            nb (fun g -> g#iter_vertex) < 500
            && nb (fun g -> g#iter_edges_e) < 2500
          then Global
          else Tree
      | Some s -> s in
    new view_container
      ?packing ~status ~default_callbacks
      ~mk_global_view:(fun () -> mk_global_view gmodel)
      ~mk_tree_view:(fun ~depth_backward ~depth_forward _ v ->
        mk_tree_view
          (DGraphTreeModel.SubTreeDotModelMake.from_model ~depth_forward
             ~depth_backward gmodel v))
      one_vertex

  let from_dot_with_commands ?packing ?status ?(default_callbacks = true)
      ?mk_global_view ?mk_tree_view dot_file =
    with_commands ?packing
      (fun ~packing d ->
        from_dot ~packing ?status ~default_callbacks ?mk_global_view
          ?mk_tree_view d)
      dot_file
end
