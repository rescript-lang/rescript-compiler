...
(* This file is responsible for converting the coordinates from dot coordinates
   to GnomeCanvas world coordinates. The matrix transformation to apply is: (1
   0) (0 -1) Care must be taken to exchange max and min values on the y axis.
   Outside this module all coordinates are assumed to be in canvas world
   coordinates. *)
...
let mk_node_layout ~name ~pos ~bbox ~draw ~ldraw =
  {n_name= name; n_pos= pos; n_bbox= bbox; n_draw= draw; n_ldraw= ldraw}

let mk_cluster_layout ~pos ~bbox ~draw ~ldraw =
  {c_pos= pos; c_bbox= bbox; c_draw= draw; c_ldraw= ldraw}
...
(** Finds the attributes [pos], [width] and [height] of a node in the attribute
    list *)
let read_common_layout mk_layout attr_list =
  ...
  let fold ((p, w, h, draw, ldraw) as attrs) = function
    ...
    | _ -> attrs in
  let fold_attr acc attr_list = List.fold_left fold acc attr_list in
  let attrs = List.fold_left fold_attr (None, None, None, [], []) attr_list in
  (* Check if we have position, width and height *)
  match attrs with
  | Some pos, Some w, Some h, draw, ldraw ->
      let pos = read_pos pos in
      let coord = bounding_box pos (float_of_string w) (-.float_of_string h) in
...
(** Reads the spline control points of a curve in an xdot file example : "c 5
    -black B 4 65 296 65 288 65 279 65 270 " *)

(* The edge drawing operations are in the following attributes : _hdraw_ Head
   arrowhead _tdraw_ Tail arrowhead _hldraw_ Head label _tldraw_ Tail label *)
...
let read_bounding_box str =
  let x1, y1, x2, y2 =
    Scanf.sscanf str "%f,%f,%f,%f" (fun a b c d -> (a, b, c, d)) in
...
module Make (G : Graph.Graphviz.GraphWithDotAttrs) = struct
  ...
  let parse_graph_attr id conv stmts =
    let read_attr = function
      | Ident ident, Some (String attr) when ident = id -> raise (Found attr)
      | _ -> () in
  ...
  let parse_layouts g stmts =
    ...
    G.iter_edges_e
      (fun e ->
        let comment =
          match get_edge_comment e with Some c -> strip_quotes c | None -> ""
        in
    ...
    let find_edge v v' comment =
      ...
      with Not_found ->
        (* Printf.printf "Did not find edge from %s to %s with comment %s\n"
           (G.vertex_name v) (G.vertex_name v') (match comment with Some c -> c
           | None -> "none");*)
        raise Not_found in
