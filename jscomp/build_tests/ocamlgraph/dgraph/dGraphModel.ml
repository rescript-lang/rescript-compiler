...
module Make (G : Graphviz.GraphWithDotAttrs) = struct
  ...
  class model layout g : [G.vertex, G.edge, cluster] abstract_model =
    object
      ...
      method get_edge_layout e =
        try X.HE.find layout.X.edge_layouts e with Not_found -> assert false
  ...
  let from_graph ?(cmd = "dot") ?(tmp_name = "dgraph") g =
    ...
    let layout =
      ...
      with X.DotError err -> raise (DotError err) in
    let model = new model layout g in
    Sys.remove dot_file ; model
end

(* BUILDING A MODEL WITH A DOT FILE *)

(* Here we build a model from a graph where vertices and edges are labeled with
   xdot layouts *)

module Vertex = struct type t = XDot.node_layout end

module Edge = struct
  type t = XDot.edge_layout

  let default =
    XDot.mk_edge_layout ~draw:[] ~ldraw:[] ~hdraw:[] ~tdraw:[] ~hldraw:[]
      ~tldraw:[]
...
module DotModel = struct
  ...
  class model g clusters_hash bounding_box :
    ...
    object
      ...
      method get_cluster_layout c =
        let attrs =
          try Hashtbl.find clusters_hash c with Not_found -> assert false in
...
let read_dot ?(cmd = "dot") dot_file =
  let basename =
    try Filename.chop_extension dot_file with Invalid_argument _ -> dot_file
  in
...
(* Does not run graphviz. Parses a graph from an xdot file and instantiates the
   model. *)
