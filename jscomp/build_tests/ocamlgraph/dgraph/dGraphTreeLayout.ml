...
let set_if_none field value =
  match field with None -> Some value | Some a -> Some a
...
module Build
    (G : Graphviz.GraphWithDotAttrs) (TreeManipulation : sig
      val is_ghost_node : G.V.t -> bool
    end) =
struct
  ...
  let get_position v geometry_info =
    try HV.find geometry_info.position v with Not_found -> assert false

  let get_dimensions v geometry_info =
    try HV.find geometry_info.dimensions v with Not_found -> assert false

  let set_offset geometry_info =
    geometry_info.y_offset <- 150 ;
    geometry_info.x_offset <-
      HV.fold (fun _ (w, _) maxw -> max w maxw) geometry_info.dimensions 0.
    ...
    let fill_stack tree root =
      ...
      let rec flush_queue queue =
        if not (Queue.is_empty queue) then (
          ...
          flush_queue queue ) in
      Queue.add (root, 0) stack_queue ;
      flush_queue stack_queue in
        ...
        ( if depth = max_depth then (
          ...
          let sum, cpt =
            fold_fun
              (fun v (sum, cpt) ->
                let x, _ =
                  try HV.find table v with Not_found -> assert false in
                (sum +. x, cpt +. 1.))
              tree elem (0., 0.) in
          assert (cpt <> 0.) ;
          HV.add table elem (sum /. cpt, depth) ) ;
        flush_stack stack ) in
    flush_stack stack
  ...
  let bind_tree_tables forward_table backward_table root geometry_info =
    ...
    let max_fwd, max_dim_fwd =
      HV.fold
        ...
        forward_table (0, 0.) in
    HV.iter
      (fun v (x, y) ->
        (*[BM]: why 0.5 and 1.5 ??? *)
        HV.add geometry_info.position v
          ( x
          , ((float (max_fwd + y) -. 0.5) *. float geometry_info.y_offset)
            -. (1.5 *. max_dim_fwd) ))
      backward_table ;
    HV.iter
      (fun v (x, y) ->
  ...
  (** @return
      an array of positions to draw an edge from positions and dimensions of
      vertices *)
  ...
  (** @return
      an array to draw an arrow from start and end positions of the edge *)
...
struct
  ...
  type vattributes =
    ...
    ; mutable shape:
        ...
        option
    ; mutable style:
        [`Rounded | `Filled | `Solid | `Dashed | `Dotted | `Bold | `Invis] list
  ...
  let attributes_list_to_vattributes vattrs = List.iter (set_vattribute vattrs)
  ...
  let fill_vattributes tree vattributes =
    let vertex_to_vattrs v =
      ...
      let dgraph_layout_default =
        [ `Color 0xFFFFFF; `Fontcolor 0x000000; `Fontname "Sans"; `Fontsize 12
        ; `Height 0.
        ; `Label (Tree.vertex_name v)
        ; `Orientation 0.; `Peripheries 1; `Regular false
        ; `Shape `Ellipse
        ; `Width 0.; `Fillcolor 0xFFFFFF ] in
  ...
  let calc_dimensions family ptsize ?(weight = `NORMAL) ?(style = `NORMAL) s
      context_obj =
    let width_margin = 20. in
    let height_margin = 0. in
    let font_description = Pango.Font.from_string "" in
    Pango.Font.modify font_description ~family ~weight ~style
  ...
  let fill_dimensions context tree vattributes geometry_info =
    let add_vertex_dimensions v =
      ...
      let truewidth, trueheight =
        calc_dimensions (the vattrs.fontname) (the vattrs.fontsize)
          (the vattrs.label) context in
      let width = max minwidth truewidth in
      let height = max minheight trueheight in
      HV.replace geometry_info.dimensions v (width, height) in
  ...
  let vattrs_to_draw_operations v vattributes geometry_info =
    ...
    ( XDotDraw.Pen_color (string_color32 (the vattrs.color))
      ...
      else shape_to_operations v vattrs geometry_info (the vattrs.shape) )
    , (* Vertex label drawing *)
      [ XDotDraw.Pen_color (string_color (the vattrs.fontcolor))
      ; XDotDraw.Font (float (the vattrs.fontsize), the vattrs.fontname)
      ; (let x, y = get_position v geometry_info in
         ...
         XDotDraw.Text
           ((x, y +. (h /. 4.)), XDotDraw.Center, width, the vattrs.label)) ]
    )
  ...
  let get_clusters tree =
    ...
    Tree.iter_vertex
      (fun v ->
        match Tree.get_subgraph v with
        | None -> ()
    ...
    let rec find_corners l corners_array =
      ...
      | v :: tl ->
          ...
          then
            ...
            find_corners tl
              [| (newminx, newminy); (newminx, newmaxy); (newmaxx, newmaxy)
               ; (newmaxx, newminy) |] in
    match l with
    | [] ->
        let z = (0., 0.) in
  ...
  let cluster_to_cluster_layout _tree c clusters geometry_info =
    ...
    let vertices =
      try Hashtbl.find_all clusters c with Not_found -> assert false in
    ...
    let add_padding corners_array =
      ...
      [| (x1_padded, y1_padded); (x1_padded, y2_padded); (x2_padded, y2_padded)
       ; (x2_padded, y1_padded) |] in
    let rec _cut_corners_array corners_array =
      ignore (assert false) ;
      (* [JS 2010/09/09] does not work: exponential time seems to be required! *)
      ...
      else [XDotDraw.Unfilled_polygon (add_padding corners_array)] in
    let x1, y1 = corners_array.(0) in
    let x2, y2 = corners_array.(3) in
    { XDot.c_pos= ((x1 +. x2) /. 2., (y1 +. y2) /. 2.)
    ; XDot.c_bbox= ((x1, y1), (x2, y2))
    ; XDot.c_draw=
        [ XDotDraw.Pen_color (string_color (get_cluster_color c.sg_attributes))
  ...
  let eattrs_to_operation tree e geometry_info =
    let eattrs =
      ...
      ; style= [] } in
    let dgraph_layout_default =
      [ `Color 0xFF0000; `Decorate false
      ; `Dir `Forward
      ; `Fontcolor 0x00000; `Fontname "Sans"; `Fontsize 12; `Label ""
      ; `Labelfontcolor 0x000000; `Labelfontname "Sans"; `Labelfontsize 12
      ; `Style `Solid ] in
    attributes_list_to_eattributes eattrs
      ( Tree.default_edge_attributes tree
      @ Tree.edge_attributes e @ dgraph_layout_default ) ;
    let posarray =
      edge_to_posarray (Tree.E.src e) (Tree.E.dst e) geometry_info in
    ...
    , (* Label drawing *)
      ...
      ; XDotDraw.Font (float_of_int (the eattrs.fontsize), the eattrs.fontname)
  ...
  let from_tree context tree root =
    ...
    let cluster_layouts =
      ...
      (* build_cluster_layouts tree geometry_info*) in
    { vertex_layouts
    ; edge_layouts
    ; cluster_layouts
    ; bbox=
...
struct
  ...
  let fill_dimensions model tree geometry_info =
    ...
    let rec get_size = function
      ...
      | ( XDotDraw.Style _ | XDotDraw.Font _ | XDotDraw.Pen_color _
        | XDotDraw.Fill_color _ | XDotDraw.Filled_bspline _
        | XDotDraw.Bspline _ | XDotDraw.Polyline _ | XDotDraw.Text _ )
        :: tl ->
          get_size tl in
    Tree.iter_vertex
      (fun v ->
  ...
  let fill_position tree root geometry_info =
    ...
    fill_tree_positions tree root Tree.iter_succ Tree.fold_succ forward_table
      geometry_info ;
    (*HV.iter (fun k (off,depth) -> Format.printf "off:%f depth:%d@." off
      depth) forward_table;*)
    fill_tree_positions tree root Tree.iter_pred Tree.fold_pred backward_table
      geometry_info ;
    (* HV.iter (fun k (off,depth) -> Format.printf "BACKoff:%f depth:%d@." off
       depth) backward_table; Format.printf "DONE@.";*)
    bind_tree_tables forward_table backward_table root geometry_info
  ...
  let parse_n_draw_operations operations ((abs, ord) as pos) =
    let polygon pts =
      ...
      let oldabssum, oldordsum =
        Array.fold_left
          (fun (xsum, ysum) (x, y) -> (xsum +. x, ysum +. y))
          (0., 0.) pts in
    ...
    let do_one = function
      ...
      | XDotDraw.Unfilled_polygon pts ->
          XDotDraw.Unfilled_polygon (polygon pts)
      | op -> op in
    List.map do_one operations

  let parse_n_ldraw_operations (initial_node_pos_x, initial_node_pos_y)
      (node_pos_x, node_pos_y) operations =
    List.map
      (function
        | XDotDraw.Text ((pos_x, pos_y), align, w, s) ->
            let translate_x, translate_y =
              ( node_pos_x -. initial_node_pos_x
              , node_pos_y -. initial_node_pos_y ) in
            let ((_, _) as pos) =
              (* same affine move as the attached node has had*)
              (pos_x +. translate_x, pos_y +. translate_y) in
  ...
  let parse_vertex_layout _tree v orig_layout geometry_info =
    ...
    ; n_ldraw=
        parse_n_ldraw_operations orig_layout.XDot.n_pos pos
    ...
    ; bbox=
        (let (((_, _), (_, _)) as bb) =
           HV.fold
             ...
             geometry_info.position (root_pos, root_pos) in
         (* Format.printf "BB=%f %f %f %f@." x1 y1 x2 y2;*)
         bb) }
end
