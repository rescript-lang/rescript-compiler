...
module Make (V : Sig.HASHABLE) (E : Sig.HASHABLE) (C : Sig.HASHABLE) = struct
  ...
  (* Widget derived from Gnome Canvas. Supports zooming and scrolling *)
  class view ?delay_node ?delay_edge ?delay_cluster obj
    ...
    object (self)
      ...
      method private add_vertex vertex =
        try
          ...
          let item =
            view_node ~delay:(delay delay_node vertex)
              ~view:(self :> common_view)
              ~vertex ~layout () in
      ...
      method private add_edge edge =
        try
          ...
          let item =
            view_edge ~delay:(delay delay_edge edge)
              ~view:(self :> common_view)
              ~edge ~layout () in
      ...
      method center_node (node : V.t view_item) = node#center ()
      ...
      method highlight ?color node =
        ...
        h node ;
        self#iter_associated_vertex
          (fun v -> h v ; self#iter_succ_e h v ; self#iter_pred_e h v)
          node
      ...
      method connect_highlighting_event () =
        let connect node =
          let callback = function
            ...
            | _ -> false in
          node#connect_event ~callback in
  ...
  let view ?(aa = false) ?delay_node ?delay_edge ?delay_cluster ?border_width
      ?width ?height ?packing ?show
      (model : (vertex, edge, cluster) DGraphModel.abstract_model) =
    let canvas =
      GnoCanvas.canvas ~aa ?border_width ?width ?height ?show ?packing () in
    (* Grab focus to process keyboard input *)
    ignore
    $ ((canvas#event)#connect)#enter_notify ~callback:(fun _ ->
