...
let rgba_to_int32 col =
  (* Graphivz sometimes insert named colors by itself, we need to catch those.
     The list might be incomplete though; if it is, we might need to translate
  ...
  | _ -> (
    try
      ...
      | 7 ->
          Graph.Graphviz.color_to_color_with_transparency
...
let graph_text ?x ?y ?text ?font ?anchor ~size_points ?(props = []) p =
  let add_prop props f x =
    match x with None -> props | Some x -> f x :: props in
...
(* Shape properties. Used when initializing items *)
...
let flatten_points pts =
  let convert i =
    let x, y = pts.(i / 2) in
    if i mod 2 = 0 then x else y in
  Array.init (2 * Array.length pts) convert

(* SHAPE CONSTRUCTORS *)

(* Ellipse, polygon and bpath canvas items do not share the same type in
   lablgtk2 They are kept in separate type constructors along with their
   initial properties. *)
type shape_t =
...
let complete_props props =
  let rec what_props ((fill, width, outline, dash) as acc) = function
    ...
    | `DASH _ :: l -> what_props (fill, width, outline, true) l in
  let fill, width, outline, dash =
    what_props (false, false, false, false) props in
  ...
  let props =
    if outline then props else `OUTLINE_COLOR_RGBA black_color :: props in
  if dash then props else `DASH (0., [||]) :: props

(* Shape class (either a rect, an ellipse, a polygon or a path). Uses a
   properties queue to undo changes *)
class shape ~fill shape init_props =
  object (self)
    inherit GnoCanvas.base_item (shape_item shape)

    method private set_props props =
      match shape with
    ...
    method highlight ?(color = (red_color, green_color)) () =
      ...
      let rec hi_props = function
        ...
        | `OUTLINE_COLOR_RGBA c :: l ->
            ...
            hi_props l
        | p :: l -> p :: hi_props l in
      (* as inserted in head, `WIDTH_UNITS 3. will not apply if there is
         already a specified weight *)
    ...
    method dehighlight () =
      (* as inserted in head, `WIDTH_UNITS 1. will not apply if there is
         already a specified weight *)
      let props =
        if fill then `FILL_COLOR_RGBA black_color :: props else props in
      self#set_props
        (`WIDTH_UNITS 1. :: `OUTLINE_COLOR_RGBA black_color :: props)
...
let pathdef pts =
  if List.length pts mod 3 = 1 then (
    ...
    curveto (List.tl pts) ;
    pathdef )
  else (* failwith "Cannot build pathdef" *)
    assert false

let bspline ~fill draw_st group pts =
  let path = pathdef (Array.fold_right (fun p acc -> p :: acc) pts []) in
  let props = get_props draw_st in
...
let text draw_st group (x, y) _align anchor label =
  ...
  in
  let size_points, height = XDotDraw.string_scale_size font size_points label in
  (* y-height/4 because the base line of the text is 1/4th from the bottom *)
  graph_text group ~x
    ~y:(y -. (height /. 4.))
    ~text:label ~props ~anchor ~font ~size_points
...
(* DGraph item Node and edge derive from this class Contains shapes and texts
   Can be : highlighted and/or selected

   ~pos : center of the container ~ops_list : list of list of operations
...
class ['a] view_item ~fill ~delay ~(view : common_view) ~pos ~ops_list
  ...
  object (self)
    ...
    (* Reads a list of list of operations Updates the shapes and texts lists *)
...
let view_edge ~delay ~view ~edge ~layout () =
  let pos = (0., 0.) in
  let ops_list =
    [ layout.e_draw; layout.e_ldraw; layout.e_tldraw; layout.e_hldraw
...
let view_cluster ~delay ~view ~cluster ~layout () =
  ...
  let view =
    new view_item ~fill:false ~delay ~pos ~ops_list ~view ~item:cluster in
  view#lower_to_bottom () ; view
