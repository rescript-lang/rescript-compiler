...
let default_draw_state () =
  {fill_color= "#FFFFFF"; pen_color= "#000000"; font= (0., ""); style= []}
...
(** Splits a string with a separator returns a list of strings *)
let split c s =
  let rec split_from n =
    try
      let p = String.index_from s n c in
      String.sub s n (p - n) :: split_from (p + 1)
    with Not_found -> [suffix s n] in
...
let string_scale_size font size s =
  ...
  (size *. width /. linear_width, float height)
...
let skip_spaces state =
  let rec loop () =
    if not (over state) then
      if is_space (char state) then (incr state ; loop ()) in
  loop ()
...
let get_word state =
  ...
  let rec get' () =
    if over state then
      if start = String.length state.str then None
      else Some (String.sub state.str start (state.cur - start))
    else if not (is_space (char state)) then (incr state ; get' ())
    else Some (String.sub state.str start (state.cur - start)) in
  get' ()
...
let get_op_id state =
  ...
  match tok with
  | None -> raise NoOperationId
  | Some tok' -> if is_token tok' then tok' else raise NoOperationId

let get_int state =
  match get_word state with
  | Some w -> (
    try (*let w' = filter_int w in*)
        int_of_string w
    with Failure "int_of_string" -> raise (ParseError "Cannot parse int") )
...
let get_float state =
  ...
  | Some w -> (
    ...
    with Failure "float_of_string" -> raise (ParseError "Cannot parse float") )
...
let parse_bytes st =
  ...
  let n = get_int st in
  skip_spaces st ;
  if char st <> '-' then raise (ParseError "Cannot parse bytes")
  else (incr st ; get_n n st)
...
let parse_operation state =
  ...
  try add_operation (operation ()) state with NoOperationId -> ()

let parse_with_state state =
  let rec loop () =
    parse_operation state ;
    if over state then state.operations else loop () in
  try List.rev (loop ()) with NoOperationId -> List.rev state.operations

(* Long drawing operation strings sometimes contain useless backslashes We get
   rid of them to avoid problems with the parser *)
let remove_backslashes s =
  let buf = Buffer.create 30 in
  let rec loop i =
    if i = String.length s then ()
    else if s.[i] = '\\' && i < String.length s - 1 && s.[i + 1] = '\n' then
      loop (i + 2)
    else (
...
let draw_with (f : draw_state -> operation -> unit) operations =
  ...
  let draw_op = function
    ...
    | Fill_color c as op -> set_fill_color st c ; f st op
    | Pen_color c as op -> set_pen_color st c ; f st op
    | Font (sty, font) as op ->
        set_font st (sty, font) ;
        f st op
    | Style stys as op -> set_style st stys ; f st op
    (* No state effects on the other operations *)
    | op -> f st op in
  List.iter draw_op operations

(* let d1 = parse "c 5 -white C 5 -white P 4 0 0 0 409 228 409 228 0 " *)
(* let d2 = parse "S 6 -filled c 9 -lightgrey C 9 -lightgrey P 4 8 72 8 365 101
   365 101 72 " *)
...
(* let d6 = parse "S 5 -solid S 15 -setlinewidth(1) c 5 -black C 5 -black P 3
   69 270 65 260 62 270 " *)
(* let d7 = parse "S 6 -filled c 7 -salmon2 C 7 -salmon2 P 9 865 1177 877 1193
   841 1200 760 1192 695 1178 700 1167 756 1161 810 1160 841 1165 " *)
(* let d8 = parse "F 14.000000 17 -Helvetica-Outline c 5 -black T 529 1005 0 65
   9 -Mini Unix " *)
(* let d9 = parse "S 6 -filled c 11 -greenyellow C 11 -greenyellow P 10 1254
   819 1263 834 1247 843 1197 841 1137 830 1110 817 1131 808 1177 805 121\ *)
   (* 6 804 1238 809 " *)
(* let d10 = parse "S 6 -filled c 11 -greenyellow C 11 -greenyellow P 10 255
   282 264 297 248 306 198 304 138 293 111 280 132 271 178 268 217 267 239\\\n
   272 " *)
