...
(* Algo II.
   ...
   we use marks to color; bits are used as follows: 0: set if node is discarded
   at step 1 1-4: available colors 5-7: the color (0 = not colored, else color
   in 1..4 *)
...
let dump () =
  let dump_mark v =
    ...
    printf "]" in
  iter_vertex dump_mark g0 ; printf "\n" ; flush stdout
...
let coloring_b () =
  ...
  let try_color v i =
    ...
    let m = Mark.get v in
    assert (m lsr 5 = 0) ;
    if (m lsr i) land 1 = 0 then raise NoColor ;
    (* color [i] not available *)
    let remove_color w =
      (* make color [i] unavailable for [w] *)
      let m = Mark.get w in
      if m lsr 5 > 0 then assert (m lsr 5 <> i) (* [w] already colored *)
      else
        let m' = m land mask_color.(i) in
        if m' = 0 then raise NoColor ;
        (* no more color available for [w] *)
        Mark.set w m' in
    iter_succ remove_color g0 v ;
    Mark.set v (m lor (i lsl 5)) in
  let uncolor v =
    let m = Mark.get v in
    let c = m lsr 5 in
    assert (0 <= c && c <= 4) ;
...
(* Local Variables: compile-command: "make -C .. bin/color.opt" End: *)
