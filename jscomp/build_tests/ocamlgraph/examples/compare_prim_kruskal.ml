...
let arg_spec =
  [ ("-v", Arg.Int (fun i -> v_ := i), " <int>  number of vertices")
  ; ("-e", Arg.Int (fun i -> e_ := i), " <int>  number of edges")
...
module IntInt = struct type t = int * int end
...
module Time = struct
  ...
  (* runs f 5 times, removes minimum and maximum timings, and returns the mean
     of the remaining three timings *)
  let time5 f x =
...
(* Local Variables: compile-command: "make -C .. bin/compare_prim_kruskal.opt"
   End: *)
