...
let arg_spec =
  ...
  ; ( "--transitive-closure"
    , Arg.Unit (fun () -> algo := Some TransitiveClosure)
    , "  display transitive closure in blue" )
  ; ( "--transitive-reduction"
    , Arg.Unit (fun () -> algo := Some TransitiveReduction)
    , "  display useless edges in blue" )
  ; ("--prim", Arg.Unit (fun () -> algo := Some Prim), "  Prim's algorithm")
  ; ( "--kruskal"
    , Arg.Unit (fun () -> algo := Some Kruskal)
    , "  Kruskal's algorithm" )
  ; ( "--dijkstra"
    , Arg.Unit (fun () -> algo := Some Dijkstra)
    , "  Dijkstra's algorithm" )
  ; ( "--dfs"
    , Arg.Unit (fun () -> algo := Some Dfs)
    , "  Depth-First Search's algorithm" )
  ; ( "--bfs"
    , Arg.Unit (fun () -> algo := Some Bfs)
...
module Draw = struct
  ...
  let pause () =
    ...
    if st.key = 'q' then (close_graph () ; exit 0)

  let draw_iteration ?(interactive = false) f g =
    f
      (fun v ->
        color_vertex v Graphics.red ;
...
module Selection = struct
  ...
  let draw_selection () =
    ...
    | Two (v1, v2) ->
        Draw.color_vertex v1 Graphics.blue ;
  ...
  let select g =
    ...
    try
      G.iter_vertex
        (fun v ->
          if distance p (G.V.label v) <= Draw.vertex_radius then (
            select_vertex v ;
            Draw.draw_graph g ;
            draw_selection () ;
            raise Exit ))
  ...
  let select2 g =
    ...
    while !continue do
      ...
      if st.Graphics.keypressed then
        ...
        | 'q' -> raise Exit
        | _ -> ()
      else if st.Graphics.button then select g
    done ;
    match !selection with Two (v1, v2) -> (v1, v2) | _ -> assert false
end
      ...
      G.iter_edges
        (fun v1 v2 ->
          if not (G.mem_edge g v1 v2) then
            Draw.draw_edge ~color:Graphics.blue v1 v2)
        tg
...
(* Local Variables: compile-command: "make -C .. bin/demo.opt" End: *)
