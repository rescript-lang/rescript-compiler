...
module Point = struct
  ...
  let in_circle v1 v2 v3 v4 =
    Delaunay.IntPoints.in_circle (V.label v1) (V.label v2) (V.label v3)
      (V.label v4)
...
let read_graph f =
  ...
  with End_of_file ->
    ...
    let rec min_list cmp = function
      ...
      | [x] -> x
      | x :: l ->
          let m = min_list cmp l in
          if cmp x m then x else m in
    ...
    let calibrate (x, y) =
      ...
      , round (20. +. (560. *. (y -. ymin) /. (ymax -. ymin))) ) in
    let vertices =
      Array.map (fun xy -> V.create (calibrate xy)) (Array.of_list !l) in
    let t = Triangulation.triangulate vertices in
    let g = create () in
    Array.iter (G.add_vertex g) vertices ;
    let add_edge v1 v2 =
      let e = E.create v1 (Point.distance v1 v2) v2 in
      G.add_edge_e g e in
...
let draw_arrow ?(color = black) ?(width = 1) (xu, yu) (xv, yv) =
  ...
  let coords theta =
    ...
    , round (yd +. (ra *. sin (pi +. alpha +. theta))) ) in
...
type selection = No | One of G.V.t | Two of G.V.t * G.V.t
...
let draw_graph () =
  ...
  G.iter_vertex
    ...
    !g ;
  set_color black ;
  G.iter_edges (fun v1 v2 -> draw_arrow (G.V.label v1) (G.V.label v2)) !g ;
  draw_selection ()
...
let select () =
  let select_vertex v =
    ...
    | Two (_, v2) -> selection := Two (v2, v) in
  let p = mouse_pos () in
  try
    G.iter_vertex
      (fun v ->
        if distance p (G.V.label v) <= vertex_radius then (
...
let scc () =
  ...
  G.iter_vertex
    (fun v ->
      try
        let color = Hashtbl.find colors (map_scc v) in
...
let four_colors () =
  ...
  let rec loop () =
    if not (Queue.is_empty q) then (
      ...
      assert (Mark.get v == 0) ;
      try_color v 1 || try_color v 2 || try_color v 3 || try_color v 4
      || (Mark.set v 0 ; Queue.add v q ; false) )
    else true
  and try_color v c =
    ( try
        G.iter_succ (fun w -> if Mark.get w == c then raise Exit) !g v ;
        true
      with Exit -> false )
    && (Mark.set v c ; loop ()) in
  ...
  try
    ...
    while true do
      ...
      if st.keypressed then
        ...
        (* | 'c' -> four_colors () *)
        | _ -> ()
      else if st.button then select ()
    done
  with Exit -> close_graph ()

(* Local Variables: compile-command: "make -C .. bin/demo_planar.opt" End: *)
