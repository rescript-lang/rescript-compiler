...
module LabP (G : Sig.P with type V.label = int * int) = struct
  let nodes = Array.init n (fun i -> Array.init m (fun j -> G.V.create (i, j)))

  let g =
    Array.fold_left
      (Array.fold_left (fun g v -> G.add_vertex g v))
      G.empty nodes
  ...
  let () =
    for i = 0 to n - 1 do
      for j = 0 to m - 1 do
        ...
        let x = sx * (i + 1) in
        let y = sy * (j + 1) in
        if i < n - 1 && not (G.mem_edge g v nodes.(i + 1).(j)) then (
          moveto x y ;
          lineto x (y - sy) ) ;
        if j < m - 1 && not (G.mem_edge g v nodes.(i).(j + 1)) then (
          moveto x y ;
          lineto (x - sx) y )
      done
    done ;
  ...
  (*** module Gr = Graphviz.Neato (struct include G let graph_attributes _ = []
    let default_vertex_attributes _ = [] let vertex_name v = let i,j =
    G.V.label v in string_of_int (i+j*m) let vertex_attributes _ = [] let
