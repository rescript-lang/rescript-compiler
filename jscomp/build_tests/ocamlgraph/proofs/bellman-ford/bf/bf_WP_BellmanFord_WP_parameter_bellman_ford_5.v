...
Axiom paths_subset_2 : forall (m:(map vertex dist)) (p:Z), (paths m p
  edges) -> (paths m (p + 1%Z)%Z (empty :(set1 (vertex* vertex)%type))).

Axiom paths_subset : forall (m:(map vertex dist)) (p:Z) (k:(set1 (vertex*
  vertex)%type)), (paths m p k) -> (paths m p (empty :(set1 (vertex*
  vertex)%type))).
...
Theorem WP_parameter_bellman_ford : (paths (set (const Infinite:(map vertex
  ...
  forall (i:Z), ((1%Z <= i)%Z /\ (i <= ((cardinal vertices) - 1%Z)%Z)%Z) ->
  ((paths m i (empty :(set1 (vertex* vertex)%type))) -> forall (es_co:(set1
  (vertex* vertex)%type)), forall (es:(set1 (vertex* vertex)%type)),
  forall (m1:(map vertex dist)), (((inter es es_co) = edges) /\ (paths m1 i
  es_co)) -> forall (result:bool), ((result = true) <-> (is_empty es)) ->
  ((~ (result = true)) -> ((~ (is_empty es)) -> forall (es1:(set1 (vertex*
  vertex)%type)), forall (result1:vertex) (result2:vertex), let result3 := (
  result1, result2) in (((mem result3 es) /\ (es1 = (remove result3 es))) ->
  (((mem (result1, result2) edges) /\ (paths m1 i es_co)) -> forall (m2:(map
  vertex dist)), ((paths m2 i (add (result1, result2) es_co)) /\
  (match (get m1
  result2) with
  | Infinite => True
  | (Finite x) => match (get m2
      result2) with
      | Infinite => False
      | (Finite y) => (y <= x)%Z
      end
  ...
  result2) with
  | (Finite n) => (exists d:Z, (shortest_path s result2 n d)) -> (m1 = m2)
  | Infinite => True
  end /\ forall (x:vertex), (~ (x = result2)) -> ((get m1 x) = (get m2
  x))))) -> forall (es_co1:(set1 (vertex* vertex)%type)), (es_co1 = (add (
  result1, result2) es_co)) -> ((inter es1 es_co1) = edges))))))).
intuition.
