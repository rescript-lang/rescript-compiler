...
Theorem WP_parameter_bellman_ford : (paths (set (const Infinite:(map vertex
  ...
  ((result = true) -> (((paths m (cardinal vertices) (empty :(set1 (vertex*
  vertex)%type))) /\ forall (v:vertex), (mem v vertices) -> (((no_path s
  v) -> ((get m v) = Infinite)) /\ forall (n:Z) (d:Z), (shortest_path s v n
  d) -> ((get m v) = (Finite n)))) -> forall (v:vertex), (mem v vertices) ->
  match (get m
  v) with
  | Infinite => True
