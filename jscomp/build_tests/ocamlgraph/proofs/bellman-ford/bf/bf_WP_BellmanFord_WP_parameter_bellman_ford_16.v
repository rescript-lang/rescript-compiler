...
Theorem WP_parameter_bellman_ford : (((cardinal vertices) - 1%Z)%Z <  1%Z)%Z ->
  ...
  ((result1 = true) -> ((inv2 (set (const Infinite:(map vertex t)) s
  (Finite 0%Z)) edges) -> forall (v:vertex), (mem v vertices) ->
...
intros es Hes.
intros _ _ _ _ v hv.
assert (h1: (cardinal vertices = 1)%Z) by ae.
generalize (cardinal1 _ vertices h1); intros h2.
assert (v = s) by ae.
