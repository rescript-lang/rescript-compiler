...
Theorem WP_parameter_bellman_ford : (inv1 (set (const Infinite:(map vertex
  ...
  ((((cardinal vertices) - 1%Z)%Z <  1%Z)%Z ->
  ((inv1 (set (const Infinite:(map vertex dist)) s (Finite 0%Z))
  (cardinal vertices) (empty :(set1 (vertex* vertex)%type))) ->
  ((inv2 (set (const Infinite:(map vertex dist)) s (Finite 0%Z))
  (empty :(set1 (vertex* vertex)%type))) -> forall (es_co:(set1 (vertex*
  vertex)%type)), forall (es:(set1 (vertex* vertex)%type)),
  ((inv2 (set (const Infinite:(map vertex dist)) s (Finite 0%Z)) es_co) /\
  ...
  ((let pass := (cardinal vertices) in ((mem result2 vertices) ->
  match (get (set (const Infinite:(map vertex dist)) s (Finite 0%Z))
  result2) with
  | (Finite n) => (exists d:Z, ((0%Z <= d)%Z /\ (d <  pass)%Z) /\ (path s
      result2 n d)) /\ forall (dqt:Z), ((0%Z <= dqt)%Z /\ (dqt <  pass)%Z) ->
      forall (nqt:Z), (path s result2 nqt dqt) -> (n <= nqt)%Z
  | Infinite => forall (d:Z), ((0%Z <= d)%Z /\ (d <  pass)%Z) ->
      forall (n:Z), ~ (path s result2 n d)
  end)) -> (match (get (set (const Infinite:(map vertex dist)) s
  (Finite 0%Z))
  result1) with
  | Infinite => False
  | (Finite x) => match (get (set (const Infinite:(map vertex dist)) s
      ...
      result2) with
      | Infinite => True
      | (Finite y) => ((x + (weight result1 result2))%Z <  y)%Z
      end
  end -> match (get (set (const Infinite:(map vertex dist)) s (Finite 0%Z))
  result2) with
  | Infinite => False
  | (Finite dv) => True
  end)))))))).
