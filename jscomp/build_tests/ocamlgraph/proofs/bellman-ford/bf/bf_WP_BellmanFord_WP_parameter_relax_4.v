...
Definition paths(m:(map vertex dist)) (pass:Z) (via:(set1 (vertex*
  ...
  | (Finite n) => (exists d:Z, ((0%Z <= d)%Z /\ (d <  pass)%Z) /\ (path s v n
      ...
      ((0%Z <= d)%Z /\ (d <  pass)%Z) /\ (path s u (n - (weight u v))%Z d)
...
Theorem WP_parameter_relax : forall (u:vertex), forall (v:vertex),
  ...
  v1) with
  | (Finite n) => (exists d:Z, (((0%Z <  d)%Z \/ (0%Z = d)) /\
      (d <  pass)%Z) /\ (path s v1 n d)) \/ exists u1:vertex, (mem (u1, v1)
      via) /\ exists d:Z, (((0%Z <  d)%Z \/ (0%Z = d)) /\ (d <  pass)%Z) /\
      (path s u1 (n - (weight u1 v1))%Z d)
  | Infinite => (forall (d:Z), (((0%Z <  d)%Z \/ (0%Z = d)) /\
      (d <  pass)%Z) -> forall (n:Z), ~ (path s v1 n d)) /\
      forall (u1:vertex), (mem (u1, v1) via) -> forall (d:Z),
      (((0%Z <  d)%Z \/ (0%Z = d)) /\ (d <  pass)%Z) -> forall (n:Z),
      ~ (path s u1 n d)
  end /\ ((forall (n:Z) (d:Z), ~ (path s v1 n d)) -> ((get m
  v1) = Infinite)))) -> (match (get m
  u) with
  | Infinite => False
  ...
  | (Finite n) => (exists d:Z, (((0%Z <  d)%Z \/ (0%Z = d)) /\
      (d <  pass)%Z) /\ (path s v1 n d)) \/ exists u1:vertex, (mem (u1, v1)
      (add (u, v) via)) /\ exists d:Z, (((0%Z <  d)%Z \/ (0%Z = d)) /\
      (d <  pass)%Z) /\ (path s u1 (n - (weight u1 v1))%Z d)
  | Infinite => True
  end).
intuition.
...
subst.
rewrite Select_eq; intuition.
destruct (get m u); intuition.
apply H2 in H3.
clear H2.
...
right.
destruct H3 as [H3 _].
destruct H3 as [] ; exists u ; rewrite add_def1 ; split ; auto.
destruct H as [n A].
destruct A as [A B].
exists n.
split.
omega.
assert (z + weight u v1 - weight u v1 = z)%Z.
omega.
rewrite H.
auto.
