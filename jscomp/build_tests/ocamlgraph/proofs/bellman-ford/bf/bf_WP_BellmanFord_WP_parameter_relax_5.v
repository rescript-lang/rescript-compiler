...
Theorem WP_parameter_relax : forall (u:vertex), forall (v:vertex),
  ...
  vertex dist)), (((1%Z <  pass)%Z \/ (1%Z = pass)) /\ ((mem (u, v) edges) /\
  ((~ (mem (u, v) via)) /\ forall (v1:vertex), (mem v1 vertices) ->
  match (get m
  v1) with
  | (Finite n) => (exists d:Z, (path s v1 n d)) /\ ((forall (dqt:Z),
      ((0%Z <= dqt)%Z /\ (dqt <  pass)%Z) -> forall (nqt:Z), (path s v1 nqt
      dqt) -> (n <= nqt)%Z) /\ forall (nqt:Z), (path_ends_with s v1 nqt pass
      via) -> (n <= nqt)%Z)
  | Infinite => (forall (d:Z), ((0%Z <= d)%Z /\ (d <  pass)%Z) ->
  ...
  v1) with
  | (Finite n) => forall (nqt:Z), (path_ends_with s v1 nqt pass (add (u, v)
      via)) -> (n <= nqt)%Z
  | Infinite => True
  end).
intuition.
destruct (get m1 v1) as [] _eqn; auto.
unfold path_ends_with.
intros n' (u1, (h1, h2)).
generalize (add_def1 _ (u1, v1) (u,v) via); intuition.
injection H7; clear H7.
intros; subst u1 v1.
destruct (get m u) as [] _eqn; intuition.
assert (mem u vertices).
generalize (edges_def u v); intuition.
generalize (H4 u H6).
rewrite Heqd0.
intros (_,(h,_)).
assert (ineq: (0 <= pass-1 < pass)%Z) by omega.

generalize (h (pass-1)%Z ineq (n' - weight u v)%Z h2); clear h.
assert (z = z0 + weight u v)%Z.
subst m1.
rewrite Select_eq in Heqd; auto.
injection Heqd.
omega.
omega.
