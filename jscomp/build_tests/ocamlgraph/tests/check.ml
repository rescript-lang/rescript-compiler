...
module Generic = struct
  ...
  module Make
      (G : Sig.I with type V.label = int) (V : sig
        val v : int
        val e : int
      end) =
  struct
  ...
  let () =
    ...
    in
    let module A =
      Make (Imperative.Digraph.Concrete (Int)) (struct let v = 3 let e = 4 end)
    in
    let module A =
      Make (Imperative.Graph.Concrete (Int)) (struct let v = 3 let e = 3 end)
    in
    let module A =
      Make (Imperative.Digraph.Abstract (Int)) (struct let v = 3 let e = 4 end)
    in
    let module A =
      Make (Imperative.Graph.Abstract (Int)) (struct let v = 3 let e = 3 end)
    in
    let module A =
      Make
        (Imperative.Digraph.ConcreteBidirectional
           (Int))
           (struct
             let v = 3 let e = 4
           end)
  ...
  module MakeP
      (G : Sig.P with type V.label = int) (V : sig
        val v : int
        val e : int
      end) =
  struct
  ...
  let () =
    ...
    let module A =
      MakeP
        (Persistent.Digraph.Concrete (Int)) (struct let v = 3 let e = 4 end)
    in
    let module A =
      MakeP (Persistent.Graph.Concrete (Int)) (struct let v = 3 let e = 3 end)
    in
    let module A =
      MakeP
        (Persistent.Digraph.Abstract (Int)) (struct let v = 3 let e = 4 end)
    in
    let module A =
      MakeP (Persistent.Graph.Abstract (Int)) (struct let v = 3 let e = 3 end)
    in
    let module A =
      MakeP
        (Persistent.Digraph.ConcreteBidirectional
           (Int))
           (struct
             let v = 3 let e = 4
           end)
  ...
  module Make_pair
      (G : Sig.I with type V.label = int * int) (V : sig
        val v : int
        val e : int
      end) =
  struct
  ...
  let () =
    ...
    let module A =
      Make_pair
        (Imperative.Digraph.Concrete (Pair)) (struct let v = 2 let e = 2 end)
    in
    let module A =
      Make_pair
        (Imperative.Graph.Concrete (Pair)) (struct let v = 2 let e = 1 end)
    in
    let module A =
      Make_pair
        (Imperative.Digraph.ConcreteBidirectional
...
module Dijkstra = struct
  ...
  struct
    ...
    let test_not_found g i j =
      try
        let _ = Dij.shortest_path g i j in
        assert false
      with Not_found -> ()
...
module Neighbourhood = struct
  ...
  let g = create ()
  let add = add_edge g
  let () = add 1 2 ; add 1 3 ; add 1 4 ; add 2 5 ; add 3 5 ; add 4 5 ; add 5 6
...
module type RightSigPack = sig
  ...
  module Rand : sig
    ...
    val labeled :
      (V.t -> V.t -> E.label) -> ?loops:bool -> v:int -> e:int -> unit -> t
  end
...
(* Local Variables: compile-command: "make -C .. check" End: *)
