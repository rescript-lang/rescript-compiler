...
struct
  ...
  let bench1 () =
    ...
    let v = 20000 in
    let e = ref 1 in
    while not (has_cycle v !e) do
      e := 2 * !e
    done

  (* résultats : le temps d'une recherche de cycle négative ne dépend pas de E et est
     d'environ 1s pour 10^6 noeuds *)
...
(* Local Variables: compile-command: "make -C .. bench" End: *)
