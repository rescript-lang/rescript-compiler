...
type t = Node of color * t * elt * t | Empty
type enum = End | More of elt * t * enum
...
let rec mem x = function
  | Empty -> false
  | Node (_, l, y, r) ->
      if x = y then true else if x < y then mem x l else mem x r
...
let unbalanced_left = function
  | Node (Red, Node (Black, a, x, b), y, c) ->
      (balance_left (Node (Red, a, x, b)) y c, false)
  | Node (Black, Node (Black, a, x, b), y, c) ->
      (balance_left (Node (Red, a, x, b)) y c, true)
  | Node (Black, Node (Red, a, x, Node (Black, b, y, c)), z, d) ->
      (Node (Black, a, x, balance_left (Node (Red, b, y, c)) z d), false)
  | _ -> assert false

let unbalanced_right = function
  | Node (Red, a, x, Node (Black, b, y, c)) ->
      (balance_right a x (Node (Red, b, y, c)), false)
  | Node (Black, a, x, Node (Black, b, y, c)) ->
      (balance_right a x (Node (Red, b, y, c)), true)
  | Node (Black, a, x, Node (Red, Node (Black, b, y, c), z, d)) ->
      (Node (Black, balance_right a x (Node (Red, b, y, c)), z, d), false)
  | _ -> assert false
...
let add x s =
  match ins x s with Node (Red, a, y, b) -> Node (Black, a, y, b) | s -> s

let rec remove_min = function
  | Empty | Node (Black, Empty, _, Node (Black, _, _, _)) -> assert false
...
let rec remove_aux x n =
  ...
  | Node (c, l, y, r) ->
      if x = y then
        match r with
        | Empty -> if c = Red then (l, false) else blackify l
        | _ ->
