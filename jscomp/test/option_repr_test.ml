...
type 'a u = 'a option = private None | Some of 'a

let f0 x = match x with _, Some true -> 1 | _, _ -> 2
...
let f2 ?x ?(y : int option) ?(z = 3) () =
  Js.log x ;
  match y with None -> 0 | Some y -> y + z

let f3 x = match x with None -> 0 | Some _ -> 1
let f4 x = match x with None -> 0 | Some x -> x + 1
...
let create ?(random = !randomized) () = if random then 2 else 1
...
type 'a xx = 'a option = None | Some of 'a

let f13 () = N.take length_10_id 8 = (Some [1; 2; 3] : _ option)
...
external log3 :
     req:([`String of string | `Int of int][@bs.unwrap])
  -> ?opt:([`String of string | `Bool of bool][@bs.unwrap])
  -> unit
  -> unit = "console.log"
  [@@bs.val]
...
;;
b __LOC__ @@ all_true [gtx (Some (Some Js.null)) (Some None)]
...
@@ all_true
     ...
     ; ltx None (Some None); ltx None (Some Js.null)
...
b __LOC__
@@ all_true
     [ eqx None None; neqx None (Some Js.null)
