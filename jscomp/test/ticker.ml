    ...
    let rec loop l = function
      ...
      | i -> (
        ...
        | i' ->
            ...
            loop l i'
        | exception Not_found -> String.sub s 0 i :: l ) in
    let len = String.length s in
    match len with 0 -> [] | _ -> loop [] len
...
(** For each market tickers, this function will compute the associated list of
    tickers value to be updated based on the correct graph ordering

    We first rank all the tickers with a depth first search algorithm (lowest
    rank for the deepest nodes).

    We then collect all the tickers which depends on each of the market tickers
    and finally we `sort_uniq` that list by rank to guarantee that a composite
    ticker is update only once and in the correct order. *)
let compute_update_sequences all_tickers =
  ...
  let map =
    List.fold_left
      (fun map ({ticker_name; type_; _} as ticker) ->
        match type_ with
    ...
    (fun ticker ->
      ...
      | {type_= Binary_op {lhs; rhs; op}; ticker_name; _} ->
          let value =
            ...
            | Some x, Some y -> (
              match op with PLUS -> Some (x +. y) | MINUS -> Some (x -. y) )
          in
          ticker.value <- value)
    update_sequence
...
let process_input_line ticker_map all_tickers line =
  ...
  let all_tickers =
    match tokens with
    | ["R"; ticker_name; "S"] ->
        ( {ticker_name; rank= Uninitialized; type_= Market; value= None}
          :: all_tickers
        , ticker_map )
...
let lines =
  [ "R|MSFT|S"; "R|IBM|S"; "R|FB|S"; "R|CP1|+|MSFT|IBM"; "R|CP2|-|FB|IBM"
  ; "R|CP12|+|CP1|CP2"; "Q|MSFT|120."; "Q|IBM|130."; "Q|FB|80." ]

let rec loop lines (all_tickers, ticker_map) =
  match lines with
  | line :: lines ->
      loop lines (process_input_line ticker_map all_tickers line)
