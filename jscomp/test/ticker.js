...
function find_opt(x, _param) {
  while(true) {
    ...
    if (param) {
      var c = Caml_obj.caml_compare(x, param[/* v */1]);
      if (c === 0) {
        return Js_primitive.some(param[/* d */2]);
      } else {
        _param = c < 0 ? param[/* l */0] : param[/* r */3];
        continue ;
      }
...
function update(x, f, m) {
  if (m) {
    ...
    var c = Caml_obj.caml_compare(x, v);
    if (c === 0) {
      var match = Curry._1(f, Js_primitive.some(d));
      if (match !== undefined) {
        var data = Js_primitive.valFromOption(match);
        if (d === data) {
          return m;
        } else {
          return /* Node */[
    ...
    if (match$1 !== undefined) {
      return /* Node */[
              /* l : Empty */0,
              /* v */x,
              /* d */Js_primitive.valFromOption(match$1),
              /* r : Empty */0,
              /* h */1
            ];
    } else {
...
function merge$1(f, s1, s2) {
  ...
  if (s1) {
    ...
    if (s1[/* h */4] >= height(s2)) {
      ...
      return concat_or_join(merge$1(f, s1[/* l */0], match[0]), v1, Curry._3(f, v1, Js_primitive.some(s1[/* d */2]), match[1]), merge$1(f, s1[/* r */3], match[2]));
    } else {
      exit = 1;
    }
  } else if (s2) {
  ...
  if (exit === 1) {
    if (s2) {
      ...
      return concat_or_join(merge$1(f, match$1[0], s2[/* l */0]), v2, Curry._3(f, v2, match$1[1], Js_primitive.some(s2[/* d */2])), merge$1(f, match$1[2], s2[/* r */3]));
    } else {
      throw [
  ...
  if (s1) {
    if (s2) {
      ...
      if (s1[/* h */4] >= s2[/* h */4]) {
        ...
        if (d2$1 !== undefined) {
          return concat_or_join(l, v1, Curry._3(f, v1, d1, Js_primitive.valFromOption(d2$1)), r);
        } else {
        ...
        if (d1$1 !== undefined) {
          return concat_or_join(l$1, v2, Curry._3(f, v2, Js_primitive.valFromOption(d1$1), d2), r$1);
        } else {
