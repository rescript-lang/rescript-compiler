...
function split(delim, s) {
  ...
  var len = s.length;
  return len !== 0 ? loop(/* [] */0, len) : /* [] */0;
}

function string_of_float_option(param) {
  return param ? Pervasives.string_of_float(param[1]) : "nan";
}
...
function print_all_composite(all_tickers) {
  return List.iter(function (param) {
              ...
              return param[4] ? (
                        value ? (console.log(ticker_name), /* () */0) : (console.log(ticker_name), /* () */0)
                      ) : /* () */0;
            }, all_tickers);
}
...
function compute_update_sequences(all_tickers) {
  ...
  return fold(function (k, l, map) {
              var l$1 = List.sort_uniq(function (param, param$1) {
                    var lhs = param[2];
                    var rhs = param$1[2];
                    return typeof lhs === "number" ? Pervasives.failwith("All nodes should be ranked") : (
                              typeof rhs === "number" ? Pervasives.failwith("All nodes should be ranked") : Caml_primitive.caml_int_compare(lhs[1], rhs[1])
                            );
                  }, l);
...
function process_quote(ticker_map, new_ticker, new_value) {
  ...
  return List.iter(function (ticker) {
              ...
              if (match) {
                ...
                return ticker[3] === new_ticker ? (ticker[1] = [
                            /* Some */0,
                            new_value
                          ], /* () */0) : Pervasives.failwith("Only single Market ticker should be udpated upon a new quote");
              }
            }, update_sequence);
}
