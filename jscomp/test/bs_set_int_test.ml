...
let ( =~ ) s i = N.(eq (fromArray i) s)
let ( =* ) a b = N.(eq (fromArray a) (fromArray b))
let ofA = N.fromArray
...
(* inclusive *)
let range i j = Array.init (j - i + 1) (fun k -> k + i)
...
let () =
  b __LOC__
    ( N.(intersect (ofA (revRange 1 100)) (ofA (revRange 50 200)))
    =~ revRange 50 100 )

let () =
  b __LOC__
    (N.(union (ofA (revRange 1 100)) (ofA (revRange 50 200))) =~ revRange 1 200)

let () =
  b __LOC__
    (N.(diff (ofA (revRange 1 100)) (ofA (revRange 50 200))) =~ revRange 1 49)

let () =
  let ss = [|1; 222; 3; 4; 2; 0; 33; -1|] in
  let v = ofA [|1; 222; 3; 4; 2; 0; 33; -1|] in
  let minv, maxv = (N.minUndefined v, N.maxUndefined v) in
  let approx loc (x : int) y = b loc (Js.eqUndefined x y) in
