...
type t0 = A0 of {lbl: int; more: int list} | A1 of {more: int list}
...
type t1 = A0 of {lbl: int; more: int list} | A1
...
type t3 = A0 of {lbl: int; more: int list} | A1
...
let ff (x : t4) =
  match x with A0 u -> u.x <- u.x + 1 | A1 u -> u.z <- u.z + 2
...
let ff1 (x : t1) : t1 =
  match x with A0 u -> A0 {u with lbl= u.lbl + 1} | A1 -> A1
