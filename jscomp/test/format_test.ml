...
let eq3 loc a b c = eq loc a b ; eq loc b c ; eq loc a c
let u () = "xx %s" ^^ "yy"
...
let () = eq __LOC__ (Format.asprintf (u ()) "x") ("xx x" ^ "yy")

let () =
  eq __LOC__ 0x3.fp+1 7.875 ;
  eq __LOC__ (-0x3.fp+1) (-7.875) ;
  (* in standard, it is still infinity, but ideally should give an warning *)
...
let f loc ls = List.iter (fun (a, b) -> eq loc (float_of_string a) b) ls

let () =
  f __LOC__
    [("0x3.fp+1", 0x3.fp+1); (" 0x3.fp2", 0x3.fp2); (" 0x4.fp2", 0x4.fp2)]

let sl f = Printf.sprintf "%h" f
let aux_list loc ls = List.iter (fun (a, b) -> eq loc (sl a) b) ls
...
let () =
  ...
  List.iter (fun (a, b) -> scan_float __LOC__ b a) literals
