...
external current_date_as_string : unit -> string = "" [@@bs.val "Date"]

(* [@@bs.nullary] *)

(* Note here [Date(0)] is the same as [Date()], but this is not always true *)

external parse : string -> number = "" [@@bs.val "Date.parse"]
external now : unit -> number = "" [@@bs.val "Date.now"]
external current : unit -> t = "Date" [@@bs.new]

(* [@@bs.nullary] *)

external of_string : string -> t = "Date" [@@bs.new]

external utc_of_y_m : year:number -> month:number -> unit -> number = ""
  [@@bs.val "Date.UTC"]
...
external utc_of_y_m_d_h :
  year:number -> month:number -> day:number -> hour:number -> unit -> number
  = ""
  [@@bs.val "Date.UTC"]
...
external of_y_m : year:number -> month:number -> unit -> t = "Date" [@@bs.new]
...
external of_y_m_d_h :
  year:number -> month:number -> day:number -> hour:number -> unit -> t
  = "Date"
  [@@bs.new]
