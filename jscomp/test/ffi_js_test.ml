let keys : (Obj.t -> string array[@bs]) =
  [%bs.raw " function (x){return Object.keys(x)}"]
...
external higher_order : int -> (int -> int -> int[@bs]) = "$$higher_order"
  [@@bs.val]
...
type _ kind = Int : int kind | Str : string kind

external config : kind:('a kind[@bs.ignore]) -> hi:int -> low:'a -> _ = ""
  [@@bs.obj]

let int_config = config ~kind:Int ~hi:3 ~low:32
let string_config = config ~kind:Str ~hi:3 ~low:"32"
let () = eq __LOC__ (6, ((higher_order 1) 2 3 [@bs]))
...
external setGADT : t -> ('a kind[@bs.ignore]) -> 'a -> unit = "setGADT"
  [@@bs.set]

external setGADT2 :
  t -> ('a kind[@bs.ignore]) -> ('b kind[@bs.ignore]) -> 'a * 'b -> unit
  = "setGADT2"
  [@@bs.set]
...
external getGADT2 :
  t -> ('a kind[@bs.ignore]) -> ('b kind[@bs.ignore]) -> 'a * 'b = "getGADT2"
  [@@bs.get]

external getGADTI2 :
  t -> ('a kind[@bs.ignore]) -> ('b kind[@bs.ignore]) -> int -> 'a * 'b = ""
  [@@bs.get_index]
...
external setGADTI2 :
  t -> ('a kind[@bs.ignore]) -> ('b kind[@bs.ignore]) -> int -> 'a * 'b -> unit
  = ""
  [@@bs.set_index]
...
let ffff x =
  ...
  (match getGADTI3 x Int Str with cc, dd -> Js.log (cc, dd)) ;
  Js.log @@ getGADT x Int ;
  (match getGADT2 x Int Str with (a : int), (b : string) -> Js.log2 a b) ;
  (match getGADTI2 x Int Str 0 with (a : int), (b : string) -> Js.log2 a b) ;
