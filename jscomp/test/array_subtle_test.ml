...
let () =
  ...
  (* in Js array length can be changing .. *)
...
let f v =
  (match Js.Array2.pop v with Some x -> Js.log "hi" | None -> Js.log "hi2") ;
  Js.log (ignore @@ Js.Array2.pop v)

let fff x = Array.length x >= 0
let fff2 x = if Array.length x >= 10 then Js.log "hi"
let fff3 x = if Array.length x >= 0 then 1 else 2
let fff4 x = if Array.length x > 0 then 1 else 2
