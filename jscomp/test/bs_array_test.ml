...
let () =
  ...
  eq __LOC__ (Js.Vector.map (fun [@bs] x -> x + 1) [|1; 2; 3|]) [|2; 3; 4|] ;
  ...
  eq __LOC__
    ...
    [|1; 3; 5|] ;
  eq __LOC__ (Js.List.toVector [1; 2; 3]) [|1; 2; 3|] ;
  eq __LOC__ (Js.List.toVector [1]) [|1|] ;
  id [] ;
  id [1] ;
  id [1; 2; 3; 4; 5] ;
...
let () =
  ...
  eq __LOC__
    (A.reduceWithIndex [|1; 2; 3; 4|] 0 (fun acc x i -> acc + x + i))
    16 ;
  b __LOC__
...
let () =
  eq __LOC__ (A.makeBy 0 (fun _ -> 1)) [||] ;
  eq __LOC__ (A.makeBy 3 (fun i -> i)) [|0; 1; 2|] ;
...
let id loc x =
  eq __LOC__ (A.reverse x)
    (let u = A.copy x in
     A.reverseInPlace u ; u)
