...
let set_method table label element =
  incr method_count ;
  if Labs.find label table.methods_by_label then put table label element
  else table.hidden_meths <- (label, element) :: table.hidden_meths
...
let widen table =
  let by_name, by_label, saved_hidden_meths, saved_vars, virt_meths, vars =
    List.hd table.previous_states in
  ...
  table.hidden_meths <-
    List.fold_right
      ...
      table.hidden_meths saved_hidden_meths
...
let to_array arr = if arr = Obj.magic 0 then [||] else arr
...
(* module Keys = Map.Make(struct type t = tag array let compare (x:t) y =
   compare x y end) let key_map = ref Keys.empty let get_key tags : item = try
   magic (Keys.find tags !key_map : tag array) with Not_found -> key_map :=
   Keys.add tags tags !key_map; magic tags *)

let create_table public_methods =
  if public_methods == magic 0 then new_table [||]
  else
...
let inherits cla vals virt_meths concr_meths (_, super, _, env) top =
  ...
  let init = if top then super cla env else Obj.repr (super cla) in
  widen cla ;
  Array.concat
    [ [|repr init|]
...
let dummy_class loc =
  let undef _ = raise (Undefined_recursive_module loc) in
...
let create_object_opt obj_0 table =
  if (Obj.magic obj_0 : bool) then obj_0
  else
    (* XXX Appel de [obj_block] *)
    let obj = Obj.new_block Obj.object_tag table.size in
    (* XXX Appel de [caml_modify] *)
...
let run_initializers_opt obj_0 obj table =
  if (Obj.magic obj_0 : bool) then obj
  else
    let inits = table.initializers in
    if inits <> [] then iter_f obj inits ;
    obj
...
let create_object_and_run_initializers obj_0 table =
  if (Obj.magic obj_0 : bool) then obj_0
    ...
    run_initializers obj table ; obj

(* Equivalent primitive below let sendself obj lab = (magic obj : (obj -> t)
   array array).(0).(lab) obj *)
...
type mut_tables = {key: closure; mutable data: tables; mutable next: tables}
...
let rec lookup_keys i keys tables =
  if i < 0 then tables
    ...
    let rec lookup_key tables =
      if tables.key == key then lookup_keys (i - 1) keys tables.data
      else if tables.next <> Empty then lookup_key (mut tables.next)
      else
        let next = Cons (key, Empty, Empty) in
        tables.next <- next ;
        build_path (i - 1) keys (mut next) in
    lookup_key (mut tables)

let lookup_tables root keys =
  let root = mut root in
  if root.data <> Empty then lookup_keys (Array.length keys - 1) keys root.data
  else build_path (Array.length keys - 1) keys root
...
let send_meth m n c =
  ret (fun obj -> sendcache (sendself obj n) m (Array.unsafe_get obj 0) c)
...
let method_impl table i arr =
  ...
  | SetVar ->
      ...
      set_var n
  | AppConst ->
      let f = next () and x = next () in
      app_const f x
  | AppVar ->
      let f = next () and n = next () in
      app_var f n
  | AppEnv ->
      let f = next () and e = next () and n = next () in
      app_env f e n
  | AppMeth ->
      let f = next () and n = next () in
      app_meth f n
  | AppConstConst ->
      let f = next () and x = next () and y = next () in
      app_const_const f x y
  | AppConstVar ->
      let f = next () and x = next () and n = next () in
      app_const_var f x n
  | AppConstEnv ->
  ...
  | AppVarConst ->
      let f = next () and n = next () and x = next () in
      app_var_const f n x
  | AppEnvConst ->
  ...
  | MethAppConst ->
      let n = next () and x = next () in
      meth_app_const n x
  | MethAppVar ->
      let n = next () and m = next () in
      meth_app_var n m
  | MethAppEnv ->
      let n = next () and e = next () and m = next () in
      meth_app_env n e m
  | MethAppMeth ->
      let n = next () and m = next () in
      meth_app_meth n m
  | SendConst ->
      let m = next () and x = next () in
      send_const m x (new_cache table)
  | SendVar ->
      let m = next () and n = next () in
...
let set_methods table methods =
  ...
  while !i < len do
    ...
    set_method table label clo ; incr i
  done
...
let stats () =
  {classes= !table_count; methods= !method_count; inst_vars= !inst_var_count}
