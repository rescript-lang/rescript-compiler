...
module Actors : sig
  ...
  (*type unbblock_typ = | Wood | Earth | Brick | *)
...
module Dom_html = struct
  ...
  (* external createImg: (_ [@bs.as "img"]) -> document -> imageElement =
  ...
  external getElementById : Dom.document -> string -> Dom.element option
    = "getElementById"
  ...
  external addEventListenerImg :
    imageElement -> string -> ('a Dom.event_like -> bool) -> bool -> unit
    = "addEventListener"
    [@@bs.send]
  ...
  external canvasRenderingContext2DToJsObj :
    canvasRenderingContext2D -> < .. > Js.t = "%identity"

  external canvasElementToJsObj : canvasElement -> < .. > Js.t = "%identity"

  external keyboardEventToJsObj : Dom.keyboardEvent -> < .. > Js.t
    = "%identity"
...
module Sprite : sig
  ...
  val make_particle :
    Actors.part_typ -> Dom_html.canvasRenderingContext2D -> sprite
    ...
    | Left -> (
      ...
      | Standing ->
          setup_sprite "mario-small.png" ~bb_off:(3., 1.) ~bb_sz:(11., 15.) 1 0
            (16., 16.) (0., 0.)
      | Jumping ->
          setup_sprite "mario-small.png" ~bb_off:(2., 1.) ~bb_sz:(13., 15.) 2
            10 (16., 16.) (16., 16.)
      | Running ->
  ...
  let make_item = function
    ...
    | Mushroom ->
        setup_sprite "items.png" ~bb_off:(2., 0.) ~bb_sz:(12., 16.) 1 0
          (16., 16.) (0., 0.)
    | Star -> setup_sprite "items.png" 1 0 (16., 16.) (16., 48.)

  (*Sets sprites for blocks: brick, question block, unbreakable block, cloud
    block * panel block, ground block.*)
...
module Particle : sig
  ...
  val make_score :
    int -> float * float -> Dom_html.canvasRenderingContext2D -> particle
...
module Object : sig
  ...
  val normalize_pos :
    xy -> Sprite.sprite_params -> Sprite.sprite_params -> unit

  (* Destroys the object, returning a list of destruction effect objects *)
  val kill : collidable -> Dom_html.canvasRenderingContext2D -> particle list
  ...
  let get_sprite = function
    | Player (_, s, _) | Enemy (_, s, _) | Item (_, s, _) | Block (_, s, _) ->
        s

  let get_obj = function
    | Player (_, _, o) | Enemy (_, _, o) | Item (_, _, o) | Block (_, _, o) ->
        o
    ...
    | CUp ->
        if (not player.jumping) && player.grounded then (
          ...
          player.vel.y <-
            max
              ( player.vel.y
              -. (player_jump +. (abs_float player.vel.x *. 0.25)) )
              player_max_jump )
    | CDown ->
  ...
  let update_player player keys context =
    ...
    then
      ...
      Some
        (pl_typ, Sprite.make (SPlayer (pl_typ, Crouching)) player.dir context)
...
module Draw : sig
  ...
  let clear_canvas canvas =
    ...
    let context =
      Dom_html.canvasRenderingContext2DToJsObj (canvas##getContext "2d") in
  ...
  let hud canvas score coins =
    ...
    ignore
    @@ context##fillText ("Score: " ^ score_string)
...
module Viewport : sig
  ...
  type viewport =
    { pos: Actors.xy
    ; (* Absolute position of viewport relative to map *)
      v_dim: Actors.xy
  ...
  let make (vx, vy) (mx, my) =
    {pos= {x= 0.; y= 0.}; v_dim= {x= vx; y= vy}; m_dim= {x= mx; y= my}}
...
module Director : sig
  ...
  let col_enemy_enemy t1 s1 o1 t2 s2 o2 dir =
    ...
    | GKoopaShell, GKoopaShell
     ...
     |RKoopaShell, GKoopaShell ->
        dec_health o1 ; dec_health o2 ; (None, None)
    ...
    | _, _ -> (
      match dir with
      | West | East -> rev_dir o1 t1 s1 ; rev_dir o2 t2 s2 ; (None, None)
      | _ -> (None, None) )
  ...
  let process_collision (dir : Actors.dir_2d) (c1 : Object.collidable)
    ...
    | Enemy (t1, s1, o1), Block (t2, s2, o2), East
     |Enemy (t1, s1, o1), Block (t2, s2, o2), West -> (
      ...
      | RKoopaShell, Brick | GKoopaShell, Brick ->
          dec_health o2 ; reverse_left_right o1 ; (None, None)
    ...
    | Player (t1, s1, o1), Block (t, s2, o2), North -> (
      ...
      | Brick ->
          if t1 = BigM then (
            collide_block dir o1 ; dec_health o2 ; (None, None) )
    ...
    let rec narrow_helper c cs state acc =
      ...
      | h :: t ->
            ...
            if not (equals c h) then
              ...
              | Some dir ->
                  if (get_obj h).id <> c_obj.id then
                    (*( (if (if is_rkoopa c then begin match c_obj.dir with |
                      Left -> is_block c_obj.dir {x= c_obj.pos.x -. 16.; y=
                      c_obj.pos.y -. 27.} cs | _ -> is_block c_obj.dir {x=
                      c_obj.pos.x +. 16.; y= c_obj.pos.y -. 27.} cs end else
                      false) then rev_dir c_obj RKoopa (Object.get_sprite c)
                      else ());*)
          ...
          let acc =
            ...
            | None, None -> acc in
          narrow_helper c t state acc in
  ...
  let update_collidable state (collid : Object.collidable) all_collids =
    ...
    if (not obj.kill) && viewport_filter then (
      ...
      if obj.vel.x <> 0. || not (is_enemy collid) then
  ...
  let translate_keys () =
    ...
    let ctrls =
      [(k.left, CLeft); (k.right, CRight); (k.up, CUp); (k.down, CDown)] in
  ...
  let update_loop canvas (player, objs) map_dim =
    ...
    let cwidth =
      float_of_int (Dom_html.canvasElementToJsObj canvas)##width /. scale in
    let cheight =
    ...
    let rec update_helper time state player objs parts =
      if state.game_over = true then Draw.game_win state.ctx
        ...
        if (get_obj player).kill = true then Draw.game_loss state.ctx
          ...
          ignore
          @@ Dom_html.requestAnimationFrame (fun (t : float) ->
                 update_helper t state player !collid_objs !particles) ) in
...
module Procedural_generator : sig
  ...
  (*Checks if the given location checkloc is already part of the list of
    locations * in loclist.*)
  ...
  (*Converts list of locations from blocksize to pixelsize by multiplying (x,y)
    by * 16.*)
  ...
  (*Optimizes lst such that there are no two items in the list that have the
    same * coordinates. If there is one, it is removed.*)
  ...
  (*Gets rid of objects with coordinates in the ending frame, within 128 pixels
    of * the start, at the very top, and two blocks from the ground.*)
  ...
  (*Generates a stair formation with block typ being dependent on typ. This
    type * of stair formation requires that the first step be on the ground.*)
  ...
  (*Chooses the form of the blocks to be placed. * When called, leaves a 1
    block gap from canvas size. * 1. If current xblock or yblock is greater
    than canvas width or height * respectively, return an empty list. * 2. If
    current xblock or yblock is within 10 blocks of the left and right sides *
    of the level map, prevent any objects from being initialized. * 3. Else
    call helper methods to created block formations and return obj_coord *
    list. **)
  ...
  let rec generate_block_enemies (block_coord : obj_coord list) :
      obj_coord list =
  ...
  (*Generates the list of brick locations needed to display the ground. * 1/10
    chance that a ground block is skipped each call to create holes.*)
  ...
  (*Converts the obj_coord list called by generate_block_locs to a list of
    objects * with the coordinates given from the obj_coord list. *)
  ...
  let generate_helper (blockw : float) (blockh : float) (cx : float)
      (cy : float) (context : Dom_html.canvasRenderingContext2D) :
      collidable list =
    let block_locs = generate_block_locs blockw blockh 0. 0. [] in
    let converted_block_locs =
      trim_edges (convert_list block_locs) blockw blockh in
    let obj_converted_block_locs =
      convert_to_block_obj converted_block_locs context in
    let ground_blocks = generate_ground blockw blockh 0. [] in
    let obj_converted_ground_blocks =
      convert_to_block_obj ground_blocks context in
    ...
    let undup_enemy_block_locs =
      avoid_overlap enemy_block_locs converted_block_coin_locs in
    let obj_enemy_blocks =
      convert_to_enemy_obj undup_enemy_block_locs context in
  ...
  let generate (w : float) (h : float)
    ...
    let player =
      Object.spawn (SPlayer (SmallM, Standing)) context (100., 224.) in
    (player, collide_list)
...
module Main = struct
  ...
  let load _ =
    ...
    let canvas =
      match Dom_html.getElementById Dom_html.document canvas_id with
      | None ->
          Printf.printf "cant find canvas %s \n" canvas_id ;
          failwith "fail"
      | Some el -> Dom_html.elementToCanvasElement el in
    let context = (Dom_html.canvasElementToJsObj canvas)##getContext "2d" in
    let _ =
      Dom_html.addEventListener Dom_html.document "keydown" Director.keydown
        true in
    let _ =
      Dom_html.addEventListener Dom_html.document "keyup" Director.keyup true
    in
    let () = Pg.init () in
    let _ =
  ...
  let preload _ =
    ...
    List.map
      (fun img_src ->
        ...
        ignore
          (Dom_html.addEventListenerImg img "load"
             (fun ev -> inc_counter () ; true)
             true))
      imgs

  let _ =
    (Dom_html.windowToJsObj Dom_html.window)
    ## onload
    #= (fun _ ->
    ignore (preload ()) ;
    true)
