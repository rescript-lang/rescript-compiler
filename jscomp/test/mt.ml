...
external it : string -> ((unit -> unit)[@bs.uncurry]) -> unit = "it" [@@bs.val]

external it_promise : string -> ((unit -> _ Js.Promise.t)[@bs.uncurry]) -> unit
  = "it"
  [@@bs.val]
...
external ok : bool -> unit = "ok" [@@bs.val] [@@bs.module "assert"]
...
external throws : (unit -> unit) -> unit = "throws"
  [@@bs.val] [@@bs.module "assert"]
(** There is a problem -- it does not return [unit] *)
...
let handleCode spec =
  ...
  | Ok a -> assert_ok a
  | Approx (a, b) ->
      if not (close_enough a b) then assert_equal a b
        ...
        describe name (fun [@bs] () ->
            ...
            |> List.iter (fun (name, code) ->
                   it_promise name (fun _ ->
                       code
                       |> Js.Promise.then_ (fun x -> handleCode x ; val_unit))))
      else Js.log "promise suites" (* TODO*)
  | _ -> ()

(* Note that [require] is a file local value, we need type [require]

   let is_top : unit -> bool = [%bs.raw{| function (_){ console.log('hi'); if
   (typeof require === "undefined"){ return false } else {
   console.log("hey",require.main.filename); return require.main === module; }
   } |}]

   let from_pair_suites_non_top name suites = if not @@ is_top () then
...
let bool_suites ~test_id ~suites loc x =
  incr test_id ;
  suites := (loc ^ " id " ^ string_of_int !test_id, fun _ -> Ok x) :: !suites
