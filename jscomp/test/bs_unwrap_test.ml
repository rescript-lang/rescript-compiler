external log1 :
     ([`Pair of string * int | `Int of int | `String of string][@bs.unwrap])
  -> unit = "console.log"
  [@@bs.val]
...
external log2 : ([`Unit of unit][@bs.unwrap]) -> unit = "console.log"
  [@@bs.val]
...
let _ = log3 ~req:(`Int 7) ?opt:((fun _ -> print_endline "trace" ; None) ()) ()
...
external log4 :
     ([`String of string | `Options of [%bs.obj: < foo: int > ]][@bs.unwrap])
  -> unit = "console.log"
  [@@bs.val]
...
external log5 : ?h:([`A of int | `B of string][@bs.unwrap]) -> int -> unit
  = "console.log"
  [@@bs.val]
