...
let rec split x (tree : _ Set_gen.t) : _ Set_gen.t * bool * _ Set_gen.t =
  match tree with
...
let rec add x (tree : _ Set_gen.t) : _ Set_gen.t =
  ...
  | Node (l, v, r, _) as t ->
      let c = compare_elt x v in
      if c = 0 then t
      else if c < 0 then Set_gen.internal_bal (add x l) v r
      else Set_gen.internal_bal l v (add x r)
...
let rec remove x (tree : _ Set_gen.t) : _ Set_gen.t =
  match tree with
  | Empty -> Empty
  | Node (l, v, r, _) ->
      let c = compare_elt x v in
      if c = 0 then Set_gen.internal_merge l r
      else if c < 0 then Set_gen.internal_bal (remove x l) v r
      else Set_gen.internal_bal l v (remove x r)
...
let rec find x (tree : _ Set_gen.t) =
  match tree with
...
let of_array l = Array.fold_left (fun acc x -> add x acc) empty l
