...
let gg x =
  let a = match x with 0 | 1 | 2 -> 3 | 3 -> 4 | 4 -> 6 | 8 -> 7 | _ -> 8 in
  a + 3
...
Mt.from_pair_suites __MODULE__
  [ ("mutliple switch", fun _ -> Eq (9, ff "4"))
  ; ("int switch", fun _ -> Eq (9, gg 4))
  ; ("escape_normal", fun _ -> Eq ("haha", String.escaped "haha"))
  ; ( "escape_bytes"
    , fun _ ->
        Eq (Bytes.of_string "haha", Bytes.escaped (Bytes.of_string "haha")) )
  ; (* FIXME it used char pattern match *)
    ("escape_quote", fun _ -> Eq ("\\\"\\\"", String.escaped {|""|}))
  ; ( "rev_split_by_char"
    , fun _ -> Eq ([""; "bbbb"; "bbbb"], rev_split_by_char 'a' "bbbbabbbba") )
  ; (__LOC__, fun _ -> Eq (["aaaa"], rev_split_by_char ',' "aaaa"))
  ...
  ; ( "split_empty2"
    , fun _ ->
        Eq
          ...
          , Ext_string_test.split " test_unsafe_obj_ffi_ppx.cmi"
              ~keep_empty:false ' ' ) )
