  ...
  while(true) {
    ...
    switch (exit) {
      case 1 : 
          ...
          var match$2;
          var exit$1 = 0;
          if ($$static !== false && typeof match$1 === "number" && match$1 === 77) {
...
function _function(env) {
  ...
  var match$2;
  var exit = 0;
  if (match !== false && typeof match$1 === "number") {
    if (match$1 !== 3) {
      if (match$1 !== 89) {
        exit = 1;
      } else {
...
function _method(env, kind) {
  ...
  token$4(env, /* T_RPAREN */4);
  var returnType = wrap(annotation_opt, env);
  var match$1 = function_body(env, async, generator$1);
  var body = match$1[1];
...
function class_element(env) {
  ...
  var match = key(env);
  var exit = 0;
  if (async$1 !== false || generator$1 !== false) {
    exit = 1;
  } else {
    var key$1 = match[1];
    switch (key$1.tag | 0) {
...
function class_declaration(env, decorators) {
  ...
  var id = match !== false ? (
      match$1 !== false ? /* Some */[Curry._2(Parse[/* identifier */10], /* None */0, tmp_env)] : /* None */0
