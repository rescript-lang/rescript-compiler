(* type process
   ...
   external on : process -> [ `beforeExit | `exit ] -> unit Js.fn -> unit =
   "on" [@@bs.send]

   external p : process = "process" [@@bs.val]

   external on_hi : process -> [ `hello | `xx ] -> (unit*unit) Js.fn -> unit =
   "on" [@@bs.send]
   ...
   let () = on p `exit (Js.Internal.fn_mk0 (fun _ -> prerr_endline "hello
   world")); on_hi p `xx (Js.Internal.fn_mk1 (fun _ -> prerr_endline "hello
   world")) *)

let h0 x = Js.Internal.fn_run0 x

(* {[ function h0 (x){ return x () } ]} *)
...
let ocaml_run =
  Js.Internal.fn_run3 (Js.Internal.fn_mk3 (fun x y z -> x + y + z)) 1
...
let a4 =
  Js.Internal.fn_mk4 (fun x y z ->
      let u = (x * x) + (y * y) + (z * z) in
      fun d -> u + d)

let a44 =
  Js.Internal.fn_mk4 (fun x y z d ->
      let u = (x * x) + (y * y) + (z * z) in
      u + d)
