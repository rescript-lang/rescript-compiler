...
external empty : unit -> map = "OrderedMap"
  [@@bs.new] [@@bs.module "immutable"]

external set : map -> int -> int -> map = "set" [@@bs.send]

external get : map -> int -> int option = "get"
...
let should b = if not b then Js.Exn.raiseError "impossible"
let count = 1_000_000
