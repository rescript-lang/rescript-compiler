...
type u = [`D | `C | `f[@bs.as "x"]] [@@bs.deriving jsConverter]
...
type v = A0 | A1 [@bs.as 3] | A2 | A3 [@@bs.deriving jsConverter]
...
let s = function A0 -> "A0" | A1 -> "A1" | A2 -> "A2" | A3 -> "A3"

let () =
  eq __LOC__ (Array.map vToJs [|A0; A1; A2; A3|]) [|0; 3; 4; 5|] ;
  eq __LOC__
    (Array.map vFromJs [|0; 1; 2; 3; 4; 5; 6|])
    [|Some A0; None; None; Some A1; Some A2; Some A3; None|]

type v1 = B0 | B1 | B2 | B3 | B4 | B5 [@@bs.deriving jsConverter]

let () =
  eq __LOC__ (Array.map v1ToJs [|B0; B1; B2; B3; B4; B5|]) [|0; 1; 2; 3; 4; 5|] ;
  eq __LOC__
    (Array.map v1FromJs [|-1; 0; 1; 2; 3; 4; 5; 6|])
    [|None; Some B0; Some B1; Some B2; Some B3; Some B4; Some B5; None|]

(** TODO: add newType support *)
type v2 = C0 [@bs.as 2] | C1 | C2 | C3 | C4 | C5 [@@bs.deriving jsConverter]

let ( +> ) = Array.append

let () =
  eq __LOC__ (Array.map v2ToJs [|C0; C1; C2; C3; C4; C5|]) [|2; 3; 4; 5; 6; 7|] ;
  eq __LOC__
