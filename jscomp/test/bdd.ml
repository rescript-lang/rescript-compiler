    ...
    | Node (l2, v2, i2, r2) ->
        ...
        if i1 = andslot1.(h) && i2 = andslot2.(h) then andslot3.(h)
          ...
          let f =
            ...
            | GREATER -> mkNode (and2 n1 l2) v2 (and2 n1 r2) in
    ...
    | Node (l2, v2, i2, r2) ->
        ...
        if i1 = andslot1.(h) && i2 = andslot2.(h) then andslot3.(h)
          ...
          let f =
            ...
            | GREATER -> mkNode (xor n1 l2) v2 (xor n1 r2) in
          andslot1.(h) <- i1 ; andslot2.(h) <- i2 ; andslot3.(h) <- f ; f
    | Zero -> n1
    | One -> not n1 )
  | Zero -> n2
...
let bool_equal a b =
  match (a, b) with true, true | false, false -> true | _ -> false

let test_hwb bdd vars =
  (* We should have eval bdd vars = vars.(n-1) if n > 0 eval bdd vars = false
     if n = 0 where n is the number of "true" elements in vars. *)
  let ntrue = ref 0 in
