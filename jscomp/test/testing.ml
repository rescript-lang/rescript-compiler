...
let next_test () = incr test_num ; print_test_number ()
...
(* Applies f to x and checks that the evaluation indeed raises an exception
...
let test_raises_exc_p pred f x =
  ...
  try
    ...
    false
  with x -> pred x || (print_failure_test_fail () ; false)

(* Applies f to x and checks that the evaluation indeed raises some exception. *)
let test_raises_some_exc f = test_raises_exc_p (fun _ -> true) f
let test_raises_this_exc exc = test_raises_exc_p (fun x -> x = exc)

(* Applies f to x and checks that the evaluation indeed raises exception
   Failure s. *)

let test_raises_this_failure s f x =
  test_raises_exc_p (fun x -> x = Failure s) f x

(* Applies f to x and checks that the evaluation indeed raises the exception
   Failure. *)
