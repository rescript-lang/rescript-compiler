...
external config1 : stdio:(_[@bs.as "inherit"]) -> v:int -> unit -> _ = ""
  [@@bs.obj]
...
external on_exit : (_[@bs.as "exit"]) -> (int -> string) -> unit = "process.on"
  [@@bs.val]

let () = on_exit (fun exit_code -> string_of_int exit_code)

external on_exit_int : (_[@bs.as 1]) -> (int -> unit) -> unit = "process.on"
  [@@bs.val]

let () = on_exit_int (fun _ -> ())
...
external on_exit4 : (int -> string) -> (_[@bs.as 1]) -> unit = "process.on"
  [@@bs.val]

let () = on_exit4 (fun i -> string_of_int i)

external on_exit_slice :
  int -> (_[@bs.as 3]) -> (_[@bs.as "xxx"]) -> string array -> unit = "xx"
  [@@bs.val] [@@bs.splice]
...
external on_exit_slice1 : int -> int array -> unit = "xx" [@@bs.send.pipe: t]
...
external on_exit_slice5 :
  ...
  -> (_[@bs.as
         {json| [{ "arr" : ["你好",1,2,3], "encoding" : "utf8"}] |json}])
  -> (_[@bs.as
         {json| [{ "arr" : ["你好",1,2,3], "encoding" : "utf8"}] |json}])
  -> (_[@bs.as "xxx"])
  -> ([`a | `b | `c][@bs.int])
...
external on_exit : (_[@bs.as "exit"]) -> (int -> unit) -> unit = "on"
  [@@bs.send.pipe: process]

let register (p : process) = p |> on_exit (fun i -> Js.log i)

external io_config : stdio:(_[@bs.as "inherit"]) -> cwd:string -> unit -> _
  = ""
  [@@bs.obj]
