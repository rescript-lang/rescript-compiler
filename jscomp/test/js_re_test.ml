let suites =
  Mt.
    [ ( "captures"
      , fun _ ->
          ...
          | Some result ->
              ...
              Eq
                ( (Js.Nullable.return "3", Js.Nullable.null)
                , (defined, undefined) )
          | None -> Fail () )
    ; ( "fromString"
      , fun _ ->
    ...
    ; ( "exec_no_match"
      , fun _ ->
          match
            [%re "/https:\\/\\/(.*)/"] |. Js.Re.exec_ "http://xxx.domain.com"
          with
          | Some _ -> FailWith "regex should not match"
          | None -> Ok true )
    ; ( "test_str"
      , fun _ ->
          let res = "foo" |. Js.Re.fromString |. Js.Re.test_ "#foo#" in
          Eq (true, res) )
    ; ( "fromStringWithFlags"
      , fun _ ->
    ...
    ; ( "result_input"
      , fun _ ->
          ...
          | None -> Fail () )
    ; (* es2015 *) ("t_flags", fun _ -> Eq ("gi", [%re "/./ig"] |. Js.Re.flags))
    ; ("t_global", fun _ -> Eq (true, [%re "/./ig"] |. Js.Re.global))
    ; ("t_ignoreCase", fun _ -> Eq (true, [%re "/./ig"] |. Js.Re.ignoreCase))
    ; ( "t_lastIndex"
      , fun _ ->
          let re = [%re "/na/g"] in
    ...
    ; ("t_multiline", fun _ -> Eq (false, [%re "/./ig"] |. Js.Re.multiline))
    ; ("t_source", fun _ -> Eq ("f.+o", [%re "/f.+o/ig"] |. Js.Re.source))
    ; (* es2015 *)
      ("t_sticky", fun _ -> Eq (true, [%re "/./yg"] |. Js.Re.sticky))
    ; ("t_unicode", fun _ -> Eq (false, [%re "/./yg"] |. Js.Re.unicode)) ]
