...
let shift_right_logical_tests =
  ( Ext_array_test.range 0 31
    |> Array.map (fun x -> Int32.shift_right_logical (-1l) x)
  ...
  ref
    ...
    @ (let a, b = shift_right_logical_tests in
       Ext_array_test.map2i
         (fun i a b ->
           ( Format.asprintf "shift_right_logical_cases %d" i
           , fun _ -> Mt.Eq (a, b) ))
         a b
       |> Array.to_list)
    @ (let a, b = shift_right_tests in
       Ext_array_test.map2i
         (fun i a b ->
           (Format.asprintf "shift_right_cases %d" i, fun _ -> Mt.Eq (a, b)))
         a b
       |> Array.to_list)
    @
    ...
    Ext_array_test.map2i
      (fun i a b ->
        (Format.asprintf "shift_left_cases %d" i, fun _ -> Mt.Eq (a, b)))
      a b
    |> Array.to_list )
