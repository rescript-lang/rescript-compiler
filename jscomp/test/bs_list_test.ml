...
let () =
  ...
  let f i = eq __LOC__ (N.getExn u i) (i * i) in
  for i = 0 to 4 do
    f i
  done ;
...
let () =
  let ( =~ ) = eq "FLATTEN" in
  N.(flatten [[1]; [2]; [3]; []; makeBy 4 (fun i -> i)])
  =~ [1; 2; 3; 0; 1; 2; 3] ;
  N.flatten [] =~ [] ;
...
let () =
  ...
  N.(concatMany [|[1]; [2]; [3]; []; makeBy 4 (fun i -> i)|])
  =~ [1; 2; 3; 0; 1; 2; 3] ;
  N.concatMany [||] =~ [] ;
...
let () =
  eq __LOC__
    ( N.(concat (makeBy 100 (fun i -> i)) (makeBy 100 (fun i -> i)))
    |> N.toArray )
    A.(concat (makeBy 100 (fun i -> i)) (makeBy 100 (fun i -> i)))
...
let () =
  let ( =~ ) = eq "PARTITION" in
  N.partition [1; 2; 3; 2; 3; 4] mod2 =~ ([2; 2; 4], [1; 3; 3]) ;
  N.partition [2; 2; 2; 4] mod2 =~ ([2; 2; 2; 4], []) ;
  N.partition [2; 2; 2; 4] (fun x -> not (mod2 x)) =~ ([], [2; 2; 2; 4]) ;
  N.partition [] mod2 =~ ([], [])
...
let () =
  let ( =~ ) = eq "MAP" in
  N.map (N.makeBy 5 id) (fun x -> x * 2) =~ [0; 2; 4; 6; 8] ;
  N.map [] id =~ [] ;
  N.map [1] (fun x -> -x) =~ [-1]
...
let () =
  ...
  b __LOC__
    (N.hasAssoc [(1, "1"); (2, "2"); (3, "3")] 4 (fun x y -> x + 1 = y)) ;
  ...
  b __LOC__
    ( N.setAssoc [(1, "a"); (2, "b"); (3, "c")] 2 "x" ( = )
    = [(1, "a"); (2, "x"); (3, "c")] ) ;
  b __LOC__
    ( N.setAssoc [(1, "a"); (3, "c")] 2 "2" ( = )
    = [(2, "2"); (1, "a"); (3, "c")] ) ;
...
let () =
  ...
  eq __LOC__
    (N.reduce2 length_10_id length_8_id 0 (fun acc x y -> acc + x + y))
    56 ;
  eq __LOC__ (N.reduce2 [1; 2; 3] [2; 4; 6] 0 (fun a b c -> a + b + c)) 18 ;
  eq __LOC__
    (N.reduceReverse2 length_10_id length_8_id 0 (fun acc x y -> acc + x + y))
    56 ;
  eq __LOC__
    (N.reduceReverse2 length_10_id length_10_id 0 (fun acc x y -> acc + x + y))
    90 ;
  eq __LOC__
    (N.reduceReverse2 [1; 2; 3] [1; 2] 0 (fun acc x y -> acc + x + y))
    6 ;
...
let makeTest n = eq __LOC__ (N.make n 3) (N.makeBy n (fun _ -> 3))
let () = eq __LOC__ ([] |. N.add 3 |. N.add 2) [2; 3]
...
let () = makeTest 0 ; makeTest 1 ; makeTest 2 ; makeTest 3
...
let () =
  ...
  let u1 =
    u0 |. N.keepMap (fun x -> if x mod 7 = 0 then Some (x + 1) else None) in
  eq __LOC__ u1 [1; 8; 15] ;
  b __LOC__
    N.(
      [1; 2; 3; 4]
      |. keepMap (fun x -> if x mod 2 = 0 then Some (-x) else None)
      = [-2; -4]) ;
  b __LOC__
