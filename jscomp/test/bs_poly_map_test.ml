...
let mergeInter s1 s2 =
  setOfArray
  @@ M.keysToArray
       (M.merge s1 s2 (fun k v1 v2 ->
            match (v1, v2) with Some _, Some _ -> Some () | _, _ -> None))

let mergeUnion s1 s2 =
  setOfArray @@ M.keysToArray
  @@ M.merge s1 s2 (fun k v1 v2 ->
         match (v1, v2) with None, None -> None | _, _ -> Some ())
...
let randomRange i j = A.map (I.randomRange i j) (fun x -> (x, x))
...
let () =
  ...
  (* no 3 *)
  let a3 =
    M.update a2 3 (fun (k : int option) ->
        match k with Some (k : int) -> Some (k + 1) | None -> Some 11) in
  (* 3, 11 *)
  let a4 =
    M.update a2 3 (fun k ->
        match k with Some k -> Some (k + 1) | None -> None) in
  (* no 3 *)
...
let acc m i =
  M.update m i (fun n ->
      match n with None -> Some 1 | Some acc -> Some (acc + 1))

let acc m is : _ M.t = A.reduce is m (fun a i -> acc a i)
...
let () =
  ...
  b __LOC__
    (M.eq m1
       (mapOfArray
          (A.makeBy 31 (fun i -> (i, if i >= 10 && i <= 20 then 2 else 1))))
       (fun x y -> x = y))

let () =
  let v0 = emptyMap () in
  let v1 = M.mergeMany v0 (A.map (I.randomRange 0 10_000) (fun x -> (x, x))) in
  let v2 = mapOfArray (A.map (I.randomRange 0 10_000) (fun x -> (x, x))) in
  b __LOC__ (M.eq v1 v2 (fun x y -> x = y)) ;
  let inc x = match x with None -> Some 0 | Some v -> Some (v + 1) in
  ...
  b __LOC__
    (A.eq (M.keysToArray v6) (A.makeBy 5_000 (fun i -> 5_001 + i)) ( = )) ;
  ...
  b __LOC__
    (A.eq (M.keysToArray v9) (A.makeBy 5_000 (fun i -> 5_001 + i)) ( = ))
