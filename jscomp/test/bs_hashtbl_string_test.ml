...
module String =
(val Belt.Id.hashable ~eq:(fun (x : string) y -> x = y) ~hash:Hashtbl.hash)

module String1 =
(val Belt.Id.hashable ~eq:(fun (x : string) y -> x = y) ~hash:hashString)
...
module Int =
(val Belt.Id.hashable ~eq:(fun (x : int) y -> x = y) ~hash:Hashtbl.hash)
...
(* module B = Belt.Bag *)
(* (empty : _ Belt.HashMap.t) #.add (string_of_int i) i #.add (string_of_int i)
   i *)
module M = Belt.HashMap

let bench2 (type t) (m : (string, t) Belt.Id.hashable) =
  let empty = M.make ~id:m ~hintSize:initial_size in
  let module String = (val m) in
  (* let hash = String.hash in let eq = String.eq in *)
...
let bench3 (type t) (m : (string, t) Belt.Id.comparable) =
  ...
  for i = 0 to count do
    table := Md0.set ~cmp !table (string_of_int i) i
  done ;
  for i = 0 to count do
    assert (Md0.has ~cmp !table (string_of_int i))
  done ;
  for i = 0 to count do
...
let bench4 () =
  let table = H.make initial_size in
  for i = 0 to count do
...
let bench5 () =
  let table = H0.make ~id:(module Int) ~hintSize:initial_size in
  (* let table_data = M.getData table in *)
  (* let hash = Int.hash in let eq = Int.eq in *)
  [%time
    for i = 0 to count do
      H0.set table i i
    done] ;
...
let bench6 () =
  let table = HI.make initial_size in
  for i = 0 to count do
    HI.set table i i
  done ;
  for i = 0 to count do
...
(* ;; [%time bench4 ()] ;; [%time bench4 ()] ;; [%time bench2 (module String1)]
   ;; [%time bench2 (module String2)]

   ;; [%time bench3 (module S)] ;; [%time bench5()] *)
(* ;; [%time bench6 ()] *)

;;
[%time bench7 ()]

(* ;; [%time bench7 ()] ;; [%time bench7 ()] ;; [%time bench7 ()] ;; [%time
   bench7 ()] ;; [%time bench7 ()] *)
