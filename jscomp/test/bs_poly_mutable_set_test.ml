...
module A = Belt.Array
module IntCmp = (val Belt.Id.comparable ~cmp:(fun (x : int) y -> compare x y))
module L = Belt.List
...
let () =
  ...
  b __LOC__
    (L.eq (N.toList v) (L.makeBy 1_501 (fun i -> i + 500)) (fun x y -> x = y)) ;
...
let () =
  ...
  b __LOC__
    (N.eq
       (N.union (f (I.randomRange 0 20)) (f (I.randomRange 21 40)))
  ...
  b __LOC__
    ...
    =~ empty () ) ;
  b __LOC__
    ( N.intersect (f [|1; 3; 4; 5; 7; 9|]) (f [|2; 4; 5; 6; 8; 10|])
    =~ f [|4; 5|] ) ;
...
let () =
  ...
  let a1, a2 =
    (N.keep a0 (fun x -> x mod 2 = 0), N.keep a0 (fun x -> x mod 2 <> 0)) in
