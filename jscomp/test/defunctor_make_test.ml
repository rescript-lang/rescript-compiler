...
let rec add x data compare = function
  ...
  | Node (l, v, d, r, h) ->
      ...
      else if c < 0 then bal (add x data compare l) v d r
      else bal l v d (add x data compare r)

(* type ('k, 'id) compare = *)
(* (module C with type id = 'id and type key = 'k) *)

type ('k, 'v, 'id) t1 =
  {compare: ('k, 'id) Comparable.compare; data: ('v, 'k) t}

let add (type k v id) x data (v : (k, v, id) t1) =
  let module X = (val v.compare) in
  { compare= v.compare
  ; data= add x data (Comparable.getcompare X.compare) v.data }

let empty (v : _ Comparable.compare) = {compare= v; data= Empty}

(* module Make (X : sig type key end) = struct type nonrec key = X.key type id
   end module U = struct include Make ( struct type key = int end) let compare
   = Pervasives.compare end *)

module V0 = Comparable.Make (struct
  type key = int

  let compare = fun [@bs] (x : key) y -> Pervasives.compare (x : key) y
end)

module V1 = Comparable.Make (struct
  type key = int

  let compare = fun [@bs] (x : key) y -> Pervasives.compare x y
end)
...
(* let v1 = empty *)
(* (module (struct type id type key = int let compare = Pervasives.compare
   end)) *)
