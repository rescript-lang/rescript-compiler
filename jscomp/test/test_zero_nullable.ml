...
module Test_null = struct
  ...
  let f6 x y = x == y
  let f7 x = match Some x with None -> None | Some x -> x
  ...
  let f8 (x : 'a Js.Null.t Js.Null.t) =
    match Js.Null.toOption x with
    | Some x -> ( match Js.Null.toOption x with Some _ -> 0 | None -> 1 )
    | None -> 2
...
module Test_def = struct
  ...
  let f8 x =
    match Js.Undefined.toOption x with
    | Some x -> (
      match Js.Undefined.toOption x with Some _ -> 0 | None -> 1 )
    | None -> 2
...
module Test_null_def = struct
  ...
  let f8 (x : 'a t t) =
    match toOption x with
    | Some x -> ( match toOption x with Some _ -> 0 | None -> 1 )
    | None -> 2
