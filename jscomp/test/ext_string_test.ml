...
let digits_of_str s ~offset x =
  let rec aux i acc s x =
    if i >= x then acc
      ...
      aux (i + 1)
        ((10 * acc) + Char.code s.[offset + i] - 48 (* Char.code '0' *))
        s x in
  aux 0 0 s x

(* {[ starts_with_and_number "js_fn_mk_01" 0 "js_fn_mk_" = 1 ;;
   starts_with_and_number "js_fn_run_02" 0 "js_fn_mk_" = -1 ;;
   starts_with_and_number "js_fn_mk_03" 6 "mk_" = 3 ;; starts_with_and_number
   "js_fn_mk_04" 6 "run_" = -1;; starts_with_and_number "js_fn_run_04" 6 "run_"
   = 4;; (starts_with_and_number "js_fn_run_04" 6 "run_" = 3) = false ;; ]} *)
let starts_with_and_number s ~offset beg =
  ...
  if finish_delim > s_len then -1
    ...
    while
      !i < finish_delim
      && String.unsafe_get s !i = String.unsafe_get beg (!i - offset)
    do
      incr i
    done ;
    if !i = finish_delim then digits_of_str ~offset:finish_delim s 2 else -1
...
type check_result =
  ...
  | Invalid_module_name
  | Suffix_mismatch
      (** TODO: move to another module Make {!Ext_filename_test} not stateful *)
...
external compare : string -> string -> int
  = "caml_string_length_based_compare" "noalloc"
