...
let combine path1 path2 =
  ...
  else if Filename.is_relative path2 then path1 // path2
  else path2

(* Note that [.//] is the same as [./] *)
let path_as_directory x =
  if x = "" then x
  else if Ext_string_test.ends_with x Filename.dir_sep then x
  else x ^ Filename.dir_sep
...
let chop_extension ?(loc = "") name =
  ...
  with Invalid_argument _ ->
    Ext_pervasives_test.invalid_argf "Filename.chop_extension ( %s : %s )" loc
      name
...
(** path2: a/b path1: a result: ./b TODO: [Filename.concat] with care

    [file1] is currently compilation file [file2] is the dependency
  ...
  if node_modules_shorten && v >= 0 then
    let rec skip i =
      if i >= len then
        ...
        (* https://en.wikipedia.org/wiki/Path_(computing)) most path separator
           are a single char *)
        let curr_char = String.unsafe_get file2 i in
        if curr_char = os_path_separator_char || curr_char = '.' then
          skip (i + 1)
        else i
      (* TODO: we need do more than this suppose user input can be {[
         "xxxghsoghos/ghsoghso/node_modules/../buckle-stdlib/list.js" ]} This
         seems weird though *) in
...
let rec find_root_filename ~cwd filename =
  if Sys.file_exists (cwd // filename) then cwd
    ...
    if String.length cwd' < String.length cwd then
      ...
      Ext_pervasives_test.failwithf ~loc:__LOC__ "%s not found from %s"
        filename cwd
...
let module_name_of_file_if_any file =
  String.capitalize (chop_extension_if_any @@ Filename.basename file)

(** For win32 or case insensitve OS [".cmj"] is the same as [".CMJ"] *)

(* let has_exact_suffix_then_chop fname suf = *)

let combine p1 p2 =
  if p1 = "" || p1 = Filename.current_dir_name then p2
  else if p2 = "" || p2 = Filename.current_dir_name then p1
  else if Filename.is_relative p2 then Filename.concat p1 p2
  else p2
...
let split_aux p =
  let rec go p acc =
    ...
    if dir = p then (dir, acc)
      ...
      if Ext_string_test.equal new_path Filename.dir_sep then go dir acc
    ...
    let rec go xss yss =
      ...
      | x :: xs, y :: ys ->
          if Ext_string_test.equal x y then go xs ys
            ...
            let start =
              List.fold_left
                (fun acc _ -> acc // Ext_string_test.parent_dir_lit)
...
let get_extension x =
  ...
  if pos < 0 then "" else Ext_string_test.tail_from x pos

let simple_convert_node_path_to_os_path =
  if Sys.unix then fun x -> x
  else if Sys.win32 || Sys.cygwin then Ext_string_test.replace_slash_backward
