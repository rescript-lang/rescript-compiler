...
let suites =
  Mt.
    ...
    ; (* assert.notDeepEqual(undefined,null) raises ..*)
      ( "undefined_gadt_test"
      , fun _ -> Eq (Js.Types.test Js.undefined Undefined, true) )
    ; (* "null_gadt_test", (fun _ -> Neq (Js.Types.test Js.null Js.Null,
         Js.null )); *)
    ...
    ; ( "function_gadt_test"
      , fun _ -> Eq (Js.Types.test (fun x -> x) Function, true) )
    ; ( "object_gadt_test"
      , fun _ -> Eq (Js.Types.test [%bs.obj {x= 3}] Object, true) ) ]
