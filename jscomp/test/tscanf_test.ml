...
(* A testbed file for the module Scanf. Adapted by Hongbo Zhang *)
...
let f ib =
  ...
  bscanf ib " %i;" (fun i ->
      bscanf ib " %i;" (fun j ->
          bscanf ib " %i;" (fun k ->
              bscanf ib " %i;" (fun l -> bscanf ib " ]" () ; [i; j; k; l]))))
...
let rec scan_elems ib accu =
  bscanf ib " %c " (fun c ->
      match c with
      | '[' when accu = [] ->
          (* begginning of list: could find either - an int, if the list is not
             empty, - the char ], if the list is empty. *)
          bscanf ib "%[]]" (function
            | "]" -> accu
...
(* Should work but does not with this version of scan_int_list! scan_int_list
   (Scanning.from_string "[1;2;3;4; ]");; (* Should lead to a bad input error.
   *) scan_int_list (Scanning.from_string "[1;2;3;4 5]");; scan_int_list
   (Scanning.from_string "[1;2;3;4;;");; scan_int_list (Scanning.from_string
   "[1;2;3;4;; 5]");; scan_int_list (Scanning.from_string "[1;2;3;4;; 23]");; *)
...
(* The general HO list scanner. This version does not fix the separator, nor
   the spacing before and after the separator (it uses the functional argument
   [scan_elem] to parse the separator, its spacing, and the item). *)
...
(* To scan an OCaml string: the format is "\"%s@\"". A better way would be to
   add a %S (String.escaped), a %C (Char.escaped). This is now available. *)
...
(* In this version the [scan_elem] function should be a [kscanf] like scanning
   function: we give it an error continuation.
...
(* Using [kscanf] only.
   ...
   We use format values to stand for ``functional'' specifications to scan the
   elements of lists.
...
(* Using kscanf only.
   ...
   to scan items: we use scanners that scan elements of the list on the fly. *)
(* This version cannot handle empty lists! let rec scan_elems ib scan_elem accu
   = scan_elem ib (fun elem -> let accu = elem :: accu in kscanf ib (fun ib exc
...
(* We may also try a version with only one format: We also changed the type of
   [scan_elem] to partially apply it to its ``natural'' continuation. let rec
   scan_elems scan_elem accu ib = (* We use [kscanf], so that: if the element
   reader fails, we can return the list of elements read so far. *) kscanf ib
   (fun ib exc -> accu) (* The format string for [kscanf]: we read an element
   using [scan_elem], then find a semi-colon if any, in order to decide if we
   stop reading or go on with other elements. *) "%r %1[;] " (* The reader:
   once an element has been read it returns the new accu. *) (scan_elem
   (function elem -> elem :: accu)) (fun accu s -> (* Cannot find a semi-colon:
   no more elements to read. *) if s = "" then accu (* We found a semi-colon:
   go on with the new accu. *) else scan_elems scan_elem accu ib) ;;

   let scan_list scan_elem ib = bscanf ib "[ %r ]" (scan_elems scan_elem [])
   List.rev ;;

   (* For instance: let scan_float f ib = Scanf.bscanf ib "%f" f;; # scan_list
   scan_float;; - : Scanf.Scanning.scanbuf -> float list = <fun> *)

   (* The element scanner builder. *) let make_scan_elem fmt f ib =
   Scanf.bscanf ib fmt f;;

   (* Promote an element reader format to an element list reader. *) let
   list_scanner fmt = scan_list (make_scan_elem fmt);;

   let scan_float = make_scan_elem "%f";;

   scan_list scan_float;;

   list_scanner "%f";; - : Scanf.Scanning.scanbuf -> float list = <fun> *)

(* The prototype of a [scan_elem] function for the generic [scan_list]
   functional. This [scan_elem] scans a floating point number. *)
let scan_float ib = Scanf.bscanf ib "%f"
let scan_float_list = scan_list scan_float

(* In the following list scanners, we directly give the [scan_elem] function as
   an immediate function value argument to the polymorphic [scan_list]. *)
...
(* [scan_list] is truely polymorphic: scanning a list of lists of items is a
   one liner!
...
(* scan_List "%f";; - : Scanf.Scanning.scanbuf -> float list = <fun>

   (* To read a list as a succession of elements separated by a blank. *) let
   rec read_elems read_elem accu ib = kscanf ib (fun ib exc -> accu) "%r "
   (read_elem (function elem -> elem :: accu)) (fun accu -> read_elems
   read_elem accu ib) ;;

   let read_list read_elem ib = List.rev (read_elems read_elem [] ib) ;;

   (* Promote an element reader format to an element list reader. *) let
   scan_list fmt = read_list (make_read_elem fmt);;
...
let test39 () =
  let is_empty_buff ib =
    Scanning.beginning_of_input ib && Scanning.end_of_input ib in
  let ib = Scanning.from_string "" in
  is_empty_buff ib
  && (* Do it twice since testing empty buff could incorrectly thraw an
        exception or wrongly change the beginning_of_input condition. *)
     is_empty_buff ib
...
(* Testing buffers defined via functions + co-routines that read and write from
   the same buffers + range chars and proper handling of \n + the end of file
   condition. *)
let next_char ob () =
...
let send_string ob s = Buffer.add_string ob s ; Buffer.add_char ob '\n'
let send_int ob i = send_string ob (string_of_int i)

let rec reader =
  let count = ref 0 in
  fun ib ob ->
    if Scanf.Scanning.beginning_of_input ib then (
      count := 0 ;
      ...
      Scanf.bscanf ib "%[^\n]\n" (function
        ...
        | s ->
            ...
            count := l + !count ;
            if !count >= 100 then (send_string ob "stop" ; send_int ob !count)
            else send_int ob l ;
            writer ib ob)
...
(* Simple tests may also fail! Ensure this is not the case with the current
   version for module [Scanf]. *)
let test51 () =
...
(* Tests that indeed the [%s@c] format works properly. Also tests the
   difference between [\n] and [@\n] is correctly handled. In particular, tests
   that if no [c] character can be found in the input, then the token obtained
   for [%s@c] spreads to the end of input. *)
let test52 () =
  sscanf "Hello\n" "%s@\n" id = "Hello"
  && sscanf "Hello" "%s@\n" id = "Hello"
  && sscanf "Hello" "%s%s@\n" (fun s1 s2 -> s1 = "Hello" && s2 = "")
...
(* To be continued ... (* Trying to scan records. *) let rec scan_fields ib
   scan_field accu = kscanf ib (fun ib exc -> accu) scan_field (fun i -> let
   accu = i :: accu in kscanf ib (fun ib exc -> accu) " %1[;] " (fun s -> if s
   = "" then accu else scan_fields ib scan_field accu)) ;;

   let scan_record scan_field ib = bscanf ib "{ " (); let accu = scan_fields ib
   scan_field [] in bscanf ib " }" (); List.rev accu ;;

   let scan_field ib = bscanf ib "%s = %[^;]" (fun finame ficont -> finame,
   ficont);; *)
