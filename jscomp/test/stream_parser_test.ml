...
let parse (token : unit -> Genlex.token) = 
  ...
  let token () =
    if Queue.is_empty look_ahead then 
      try token () with _ -> Kwd "=="
    else Queue.pop look_ahead in
  ...
  let rec 
    ...
    | e -> push e ; raise (Parse_error "unexpected token")
  and
    parse_term () = 
    parse_term_aux (parse_atom ())
  and
    parse_term_aux e1 =
    match token () with 
    | Kwd "*" -> e1 * parse_term ()
  ...
  and
    parse_expr () = 
    parse_expr_aux (parse_term ())
  and
    parse_expr_aux e1 = 
    match token () with 
    | Kwd "+" -> e1 + parse_expr ()
    | Kwd "-" -> e1 - parse_expr ()
    | e -> (* [%debugger]; *) push e ; e1
  in 
  let r = parse_expr () in 
  r, Queue.fold (fun acc x  -> x :: acc) [] look_ahead 
let lexer = Genlex.make_lexer ["(";"*"; "/";  "+"; "-"; ")"];;
let token chars = let strm = (lexer chars) in fun () -> Stream.next  strm;;
