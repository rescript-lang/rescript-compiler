...
let () =
  ...
  S.push 1 s ;
  assert_ __LOC__ (S.to_list s = [1] && S.length s = 1) ;
  S.push 2 s ;
  assert_ __LOC__ (S.to_list s = [1; 2] && S.length s = 2) ;
  S.push 3 s ;
  assert_ __LOC__ (S.to_list s = [1; 2; 3] && S.length s = 3) ;
  S.push 4 s ;
  assert_ __LOC__ (S.to_list s = [1; 2; 3; 4] && S.length s = 4) ;
...
let () =
  ...
  assert_ __LOC__ (S.to_list s1 = []) ;
  let s2 = S.copy s1 in
  assert_ __LOC__ (S.length s1 = 0) ;
  assert_ __LOC__ (S.to_list s1 = []) ;
  assert_ __LOC__ (S.length s2 = 0) ;
  assert_ __LOC__ (S.to_list s2 = [])
...
let () =
  ...
  assert_ __LOC__ (S.to_list s1 = [1; 2; 3; 4]) ;
  let s2 = S.copy s1 in
  assert_ __LOC__ (S.length s1 = 4) ;
  assert_ __LOC__ (S.to_list s1 = [1; 2; 3; 4]) ;
  assert_ __LOC__ (S.length s2 = 4) ;
  assert_ __LOC__ (S.to_list s2 = [1; 2; 3; 4])
