...
include (
  struct
    ...
    let () =
      ...
      let a1, a2 =
        (N.keep a0 (fun x -> x mod 2 = 0), N.keep a0 (fun x -> x mod 2 <> 0))
      in
...
let () =
  ...
  N.checkInvariantInternal v ;
  b __LOC__ @@ R.every 0 1_00_000 (fun i -> N.has v i) ;
...
let () =
  let id loc x =
    ...
    b loc (A.every2 (N.toArray u) x ( = )) in
  id __LOC__ [||] ;
  id __LOC__ [|0|] ;
  id __LOC__ [|0; 1|] ;
  id __LOC__ [|0; 1; 2|] ;
