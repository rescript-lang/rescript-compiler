...
let suites =
  Mt.
    [ ("valueOf", fun _ -> Eq (195131516789., N.valueOf (date ())))
    ; ("make", fun _ -> Ok (N.make () |> N.getTime > 1487223505382.))
    ; ( "parseAsFloat"
      , fun _ ->
          Eq (N.parseAsFloat "1976-03-08T12:34:56.789+01:23", 195131516789.) )
    ; ( "parseAsFloat_invalid"
      , fun _ -> Ok (N.parseAsFloat "gibberish" |> Js_float.isNaN) )
    ; ( "fromFloat"
      , fun _ ->
          Eq
    ...
    ; ( "fromString_valid"
      , fun _ ->
          Eq
            ( 195131516789.
            , N.fromString "1976-03-08T12:34:56.789+01:23" |> N.getTime ) )
    ; ( "fromString_invalid"
      , fun _ -> Ok (N.fromString "gibberish" |> N.getTime |> Js_float.isNaN)
      )
    ; ( "makeWithYM"
      , fun _ ->
    ...
    ; ( "makeWithYMDH"
      , fun _ ->
          ...
          Eq
            ( (1984., 4., 6., 3.)
            , (N.getFullYear d, N.getMonth d, N.getDate d, N.getHours d) ) )
    ; ( "makeWithYMDHM"
      , fun _ ->
          let d =
            N.makeWithYMDHM ~year:1984. ~month:4. ~date:6. ~hours:3.
              ~minutes:59. () in
          Eq
            ( (1984., 4., 6., 3., 59.)
            , ( N.getFullYear d
    ...
    ; ( "makeWithYMDHMS"
      , fun _ ->
          let d =
            N.makeWithYMDHMS ~year:1984. ~month:4. ~date:6. ~hours:3.
              ~minutes:59. ~seconds:27. () in
          Eq
            ( (1984., 4., 6., 3., 59., 27.)
            , ( N.getFullYear d
              , N.getMonth d
    ...
    ; ( "utcWithYMD"
      , fun _ ->
          ...
          Eq
            ( (1984., 4., 6.)
            , (N.getUTCFullYear d, N.getUTCMonth d, N.getUTCDate d) ) )
    ; ( "utcWithYMDH"
      , fun _ ->
    ...
    ; ( "utcWithYMDHM"
      , fun _ ->
          let d =
            N.utcWithYMDHM ~year:1984. ~month:4. ~date:6. ~hours:3.
              ~minutes:59. () in
          let d = N.fromFloat d in
          Eq
            ( (1984., 4., 6., 3., 59.)
    ...
    ; ( "utcWithYMDHMS"
      , fun _ ->
          let d =
            N.utcWithYMDHMS ~year:1984. ~month:4. ~date:6. ~hours:3.
              ~minutes:59. ~seconds:27. () in
          let d = N.fromFloat d in
          Eq
            ( (1984., 4., 6., 3., 59., 27.)
            , ( N.getUTCFullYear d
              ...
              , N.getUTCMinutes d
              , N.getUTCSeconds d ) ) )
    ; (* locale dependent "getDate", (fun _ -> Eq(8., N.getDate (date ())));
         "getDay", (fun _ -> Eq(1., N.getDay (date ()))); *)
      ("getFullYear", fun _ -> Eq (1976., N.getFullYear (date ())))
    ; (* locale dependent "getHours", (fun _ -> Eq(12., N.getHours (date ()))); *)
      ("getMilliseconds", fun _ -> Eq (789., N.getMilliseconds (date ())))
    ; (* locale dependent "getMinutes", (fun _ -> Eq(11., N.getMinutes (date
         ()))); "getMonth", (fun _ -> Eq(2., N.getMonth (date ()))); *)
      ("getSeconds", fun _ -> Eq (56., N.getSeconds (date ())))
    ; ("getTime", fun _ -> Eq (195131516789., N.getTime (date ())))
    ; (* locale depdendent "getTimezoneOffset", (fun _ -> Eq(-60.,
         N.getTimezoneOffset (date ()))); *)
      ("getUTCDate", fun _ -> Eq (8., N.getUTCDate (date ())))
    ; ("getUTCDay", fun _ -> Eq (1., N.getUTCDay (date ())))
    ; ("getUTCFUllYear", fun _ -> Eq (1976., N.getUTCFullYear (date ())))
    ; ("getUTCHours", fun _ -> Eq (11., N.getUTCHours (date ())))
    ; ("getUTCMilliseconds", fun _ -> Eq (789., N.getUTCMilliseconds (date ())))
    ; ("getUTCMinutes", fun _ -> Eq (11., N.getUTCMinutes (date ())))
    ; ("getUTCMonth", fun _ -> Eq (2., N.getUTCMonth (date ())))
    ; ("getUTCSeconds", fun _ -> Eq (56., N.getUTCSeconds (date ())))
    ; ("getYear", fun _ -> Eq (1976., N.getFullYear (date ())))
    ; ( "setDate"
      , fun _ ->
          let d = date () in
    ...
    ; ( "setMinutesSMs"
      , fun _ ->
          let d = date () in
          let _ =
            N.setMinutesSMs d ~minutes:18. ~seconds:42. ~milliseconds:311. ()
          in
          Eq
            ( (18., 42., 311.)
            , (N.getMinutes d, N.getSeconds d, N.getMilliseconds d) ) )
    ; ( "setMonth"
      , fun _ ->
          let d = date () in
    ...
    ; ( "setUTCHoursMS"
      , fun _ ->
          ...
          Eq
            ( (22., 48., 54.)
            , (N.getUTCHours d, N.getUTCMinutes d, N.getUTCSeconds d) ) )
    ; ( "setUTCMilliseconds"
      , fun _ ->
          let d = date () in
    ...
    ; ( "setUTCMinutesSMs"
      , fun _ ->
          ...
          Eq
            ( (18., 42., 311.)
            , (N.getUTCMinutes d, N.getUTCSeconds d, N.getUTCMilliseconds d) )
      )
    ; ( "setUTCMonth"
      , fun _ ->
          let d = date () in
    ...
    ; ( "toDateString"
      , fun _ -> Eq ("Mon Mar 08 1976", N.toDateString (date ())) )
    ; ( "toGMTString"
      , fun _ -> Eq ("Mon, 08 Mar 1976 11:11:56 GMT", N.toUTCString (date ()))
      )
    ; ( "toISOString"
      , fun _ -> Eq ("1976-03-08T11:11:56.789Z", N.toISOString (date ())) )
    ; ("toJSON", fun _ -> Eq ("1976-03-08T11:11:56.789Z", N.toJSON (date ())))
    ; ( "toJSONUnsafe"
      , fun _ -> Eq ("1976-03-08T11:11:56.789Z", N.toJSONUnsafe (date ())) )
    ; (* locale dependent "toLocaleDateString", (fun _ -> Eq("3/8/1976",
         N.toLocaleDateString (date ()))); "toLocaleString", (fun _ ->
         Eq("3/8/1976, 12:11:56 PM", N.toLocaleString (date ())));
         "toLocaleTimeString", (fun _ -> Eq("12:11:56 PM", N.toLocaleTimeString
         (date ()))); "toString", (fun _ -> Eq("Mon Mar 08 1976 12:11:56
         GMT+0100 (CET)", N.toString (date ()))); "toTimeString", (fun _ ->
