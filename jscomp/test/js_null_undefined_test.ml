...
let suites =
  Mt.
    ...
    ; ("bind - null", fun _ -> StrictEq (null, bind null (fun [@bs] v -> v)))
    ; ( "bind - undefined"
      , fun _ -> StrictEq (undefined, bind undefined (fun [@bs] v -> v)) )
    ; ( "bind - empty"
      , fun _ -> StrictEq (undefined, bind undefined (fun [@bs] v -> v)) )
    ; ( "bind - 'a"
      , fun _ -> Eq (return 4, bind (return 2) (fun [@bs] n -> n * 2)) )
    ; ( "iter - null"
      , fun _ ->
          let hit = ref false in
          let _ = iter null (fun [@bs] _ -> hit := true) in
          Eq (false, !hit) )
    ; ( "iter - undefined"
      , fun _ ->
          let hit = ref false in
          let _ = iter undefined (fun [@bs] _ -> hit := true) in
          Eq (false, !hit) )
    ; ( "iter - empty"
      , fun _ ->
          let hit = ref false in
