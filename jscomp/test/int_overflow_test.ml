let hash_variant s =
  ...
  for i = 0 to String.length s - 1 do
    ...
    (* Here accu is 31 bits, times 223 will not be than 53 bits.. TODO: we can
       use `Sys.backend_type` for patching *)
  done ;
  (* reduce to 31 bits *)
...
Mt.from_pair_suites __MODULE__
  [ ( "plus_overflow"
    , fun _ -> Eq (true, Int32.(add max_int 1l) = Int32.min_int) )
  ; ( "minus_overflow"
    , fun _ -> Eq (true, Int32.(sub min_int one) = Int32.max_int) )
  ; ( "flow_again"
    , fun _ -> Eq (2147483646l, Int32.(add (add max_int max_int) min_int)) )
  ; ("flow_again", fun _ -> Eq (-2l, Int32.(add max_int max_int)))
  ; ( "hash_test"
    , fun _ -> Eq (hash_variant "xxyyzzuuxxzzyy00112233", 544087776) )
