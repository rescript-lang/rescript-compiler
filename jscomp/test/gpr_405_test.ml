...
module Make (G : G) = struct
  ...
  let find_default htbl x = try H.find htbl x with Not_found -> false
  ...
  let min_cutset gr first_node =
    ...
    and step3 successors top rest_of_stack =
      ...
      | successor :: other_successors ->
          if not (is_already_processed successor) (* step 4 *) then
            step2 successor ((top, successors) :: rest_of_stack) (* step 5 *)
          else
            let x =
              if is_on_the_stack successor then H.find n_labels successor
              else H.find l_labels successor in
            H.add l_labels top (max (H.find l_labels top) x) ;
            step3 other_successors top rest_of_stack
      | [] -> (
          (* step 7 *)
          if H.find l_labels top = H.find n_labels top then (
            cut_set := top :: !cut_set ;
            H.add l_labels top 0 ) ;
