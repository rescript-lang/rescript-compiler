let get_tokens lex str =
  ...
  let rec aux acc =
    let v = lex buf in
    if v = Arith_parser.EOF then List.rev acc else aux (v :: acc) in
  aux []
  ...
  Mt.
    [ ( "arith_token"
      , fun _ ->
          Eq
            ( f "x + 3 + 4 + y"
            , [IDENT "x"; PLUS; NUMERAL 3.; PLUS; NUMERAL 4.; PLUS; IDENT "y"]
            ) )
    ; ( "simple token"
      , fun _ -> Eq (Arith_lexer.lexeme (Lexing.from_string "10"), NUMERAL 10.)
      )
    ; ( "number_lexer"
      , fun _ ->
          let v = ref [] in
          ...
          Eq
            ( List.rev !v
            , [ "number"; "32"; "new line"; "+"; "new line"; "number"; "32"
    ...
    ; ( "simple number"
      , fun _ ->
          Eq
            ( Arith_syntax.str
                (Arith_parser.toplevel Arith_lexer.lexeme
                   (Lexing.from_string "10"))
            , "10." ) )
    ; ( "arith"
      , fun _ ->
          Eq
