...
let f2 x = if x then true else false
let f4 x = if x then true else false
let f3 = if true then true else false
let u : bool = [%bs.raw {| 1|}]
...
let suites =
  Mt.
    ...
    ; ( "js_bool_neq_acml_bool"
      , fun _ -> Eq (true, f true = [%bs.raw {|true|}] (* not type check*)) )
    ]

let ff u = if u = true then 1 else 2
let fi (x : int) y = x = y
let fb (x : bool) y = x = y
