...
let v5 =
  (object (self)
     ...
     method private hihi u = self##x + self##say ()

     method private bark () = Js.log "bark"
...
let v =
  (object (self)
     ...
     method private reset () = self ## y #= 0

     method incr () = self ## y #= (self##y + 1)

     (* TODO: the error message is confusing {[ self##y #= self##y + 1 ]} will
        be parsed as {[ (self##y #= self##y) + 1 ]} if we change `#=` into `:=`
        then it will not work if `self##y` is indeed a reference we need
...
let eventObj :
  ...
  (object (self)
     ...
     method push a : unit = Js.Vector.pushBack a self##events
