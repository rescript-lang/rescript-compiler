...
let v  = mapi [|1;2;3 |] (Js_fn.mk2 f)

let vv  = mapi [|1;2;3 |] (Js_fn.mk2 (+))
...
;; Mt.from_pair_suites __FILE__ Mt.[
    ...
    __LOC__, (fun _ -> Eq(  
         ...
         map (map [| 1;2;3|]  (Js_fn.mk1 (fun x -> fun y -> x + y))) 
          (Js_fn.mk1 @@ fun y -> (y 0)  * (y 1) ), [|2; 6 ; 12|]
      ));
    __LOC__, (fun _ -> Eq(
        mapi [|1;2;3|] (Js_fn.mk2 (fun x  -> let y =  x * x in fun i -> y + i )), 
        [|1; 5 ; 11|]        
      ))
]
