...
and translateSignatureItemFromTypes ~config ~outputFileRelative ~resolver
  ...
  | Types.Sig_module (id, moduleDeclaration, _) ->
    ...
    let config = moduleDeclaration.md_attributes |> Annotation.updateConfigForModule ~config in
  ...
  | Types.Sig_value (id, {val_attributes; val_loc; val_type}) ->
    ...
    typeEnv |> TypeEnv.updateModuleItem ~moduleItem;
    if val_attributes |> Annotation.fromAttributes ~config ~loc:val_loc = GenType then
      name
