...
(** Large, multi-dimensional, numerical arrays.
    ...
    implementation allows efficient sharing of large numerical arrays between
    OCaml code and C or Fortran numerical libraries.

    Concerning the naming conventions, users of this module are encouraged to
    do [open Bigarray] in their source, then refer to array types and
    operations via short dot notation, e.g. [Array1.t] or [Array2.sub].

    Big arrays support all the OCaml ad-hoc polymorphic
    operations:
    {ul
     {- comparisons ([=], [<>], [<=], etc, as well as {!Pervasives.compare});}
     {- hashing (module [Hash]);}
     {- and structured input-output (the functions from the {!Marshal} module,
        as well as {!Pervasives.output_value} and {!Pervasives.input_value}).}} *)

(** {1 Element kinds} *)

(** Big arrays can contain elements of the following
    kinds:
    {ul
    ...
    types defined below (defined with a single constructor instead of abstract
...
(** To each element kind is associated an OCaml type, which is the type of
    ...
    ['b] which represents the actual contents of the big array. Its
...
(** As shown by the types of the values above, big arrays of kind [float32_elt]
    ...
    complex kinds [complex32_elt], [complex64_elt] are accessed with the OCaml
    type {!Complex.t}. Big arrays of integer kinds are accessed using the
    ...
    bigarrays; [int32] for 32-bit integer bigarrays; [int64] for 64-bit integer
    bigarrays; and [nativeint] for platform-native integer bigarrays. Finally,
    big arrays of kind [int8_unsigned_elt] can also be accessed as arrays of
    characters instead of arrays of small integers, by using the kind value
...
(** To facilitate interoperability with existing C and Fortran code, this
    library supports two different memory layouts for big arrays, one
    compatible with the C conventions, the other compatible with the Fortran
    conventions.
    ...
    elements of row 1, etc. In other terms, the array elements at [(x,y)] and
    [(x, y+1)] are adjacent in memory.

    In the Fortran-style layout, array indices start at 1, and
    multi-dimensional arrays are laid out in column-major format. That is, for
    a two-dimensional array, all elements of column 0 are contiguous in memory,
    followed by all elements of column 1, etc. In other terms, the array
...
module Genarray : sig
  ...
  (** [Genarray.create kind layout dimensions] returns a new big array whose
      ...
      parameter [layout] (one of [c_layout] or [fortran_layout]). The
      [dimensions] parameter is an array of integers that indicate the size of
      the big array in each dimension. The length of [dimensions] determines
      the number of dimensions of the bigarray.

      For instance, [Genarray.create int32 c_layout \[|4;6;8|\]] returns a
      fresh big array of 32-bit integers, in C layout, having three dimensions,
      the three dimensions being 4, 6 and 8 respectively.
  ...
  (** [Genarray.dims a] returns all dimensions of the big array [a], as an
  ...
  (** [Genarray.nth_dim a n] returns the [n]-th dimension of the big array [a].
      The first dimension corresponds to [n = 0]; the second dimension
      corresponds to [n = 1]; the last dimension, to
      [n = Genarray.num_dims a - 1]. Raise [Invalid_argument] if [n] is less
  ...
  (** [Genarray.change_layout a layout] returns a bigarray with the specified
      ...
      original array share the same storage space. The dimensions are reversed,
      such that [get v \[| a; b |\]] in C layout becomes
  ...
  (** Read an element of a generic big array.
      [Genarray.get a \[|i1; ...; iN|\]] returns the element of [a] whose
      ...
      strictly less than the corresponding dimensions of [a]. If [a] has
      Fortran layout, the coordinates must be greater or equal than 1 and less
      or equal than the corresponding dimensions of [a]. Raise
      [Invalid_argument] if the array [a] does not have exactly [N] dimensions,
      or if the coordinates are outside the array bounds.

      If [N > 3], alternate syntax is provided: you can write
      [a.{i1, i2, ..., iN}] instead of [Genarray.get a \[|i1; ...; iN|\]]. (The
      syntax [a.{...}] with one, two or three coordinates is reserved for
  ...
  (** Assign an element of a generic big array.
      [Genarray.set a \[|i1; ...; iN|\] v] stores the value [v] in the element
      of [a] whose coordinates are [i1] in the first dimension, [i2] in the
      ...
      [Genarray.set a \[|i1; ...; iN|\] v]. (The syntax [a.{...} <- v] with
      one, two or three coordinates is reserved for updating one-, two- and
      three-dimensional arrays as described below.) *)

  external sub_left :
    ('a, 'b, c_layout) t -> int -> int -> ('a, 'b, c_layout) t = "caml_ba_sub"
  (** Extract a sub-array of the given big array by restricting the first
      ...
      with the same number of dimensions as [a], and the same dimensions as
      [a], except the first dimension, which corresponds to the interval
      [\[ofs ... ofs + len - 1\]] of the first dimension of [a]. No copying of
      elements is involved: the sub-array and the original array share the same
      storage space. In other terms, the element at coordinates
      [\[|i1; ...; iN|\]] of the sub-array is identical to the element at
      coordinates [\[|i1+ofs; ...; iN|\]] of the original array [a].

      [Genarray.sub_left] applies only to big arrays in C layout. Raise
      [Invalid_argument] if [ofs] and [len] do not designate a valid sub-array
      of [a], that is, if [ofs < 0], or [len < 0], or
  ...
  (** Extract a sub-array of the given big array by restricting the last
      ...
      [Invalid_argument] if [ofs] and [len] do not designate a valid sub-array
      of [a], that is, if [ofs < 1], or [len < 0], or
      [ofs + len > Genarray.nth_dim a (Genarray.num_dims a - 1)]. *)

  external slice_left :
    ('a, 'b, c_layout) t -> int array -> ('a, 'b, c_layout) t = "caml_ba_slice"
  (** Extract a sub-array of lower dimension from the given big array by fixing
      one or several of the first (left-most) coordinates.
      [Genarray.slice_left a \[|i1; ... ; iM|\]] returns the 'slice' of [a]
      obtained by setting the first [M] coordinates to [i1], ..., [iM]. If [a]
      has [N] dimensions, the slice has dimension [N - M], and the element at
      coordinates [\[|j1; ...; j(N-M)|\]] in the slice is identical to the
      element at coordinates [\[|i1; ...; iM; j1; ...; j(N-M)|\]] in the
      original array [a]. No copying of elements is involved: the slice and the
  ...
  (** Extract a sub-array of lower dimension from the given big array by fixing
      ...
      [Genarray.slice_right a \[|i1; ... ; iM|\]] returns the 'slice' of [a]
      obtained by setting the last [M] coordinates to [i1], ..., [iM]. If [a]
      has [N] dimensions, the slice has dimension [N - M], and the element at
      coordinates [\[|j1; ...; j(N-M)|\]] in the slice is identical to the
      element at coordinates [\[|j1; ...; j(N-M); i1; ...; iM|\]] in the
      original array [a]. No copying of elements is involved: the slice and the
      original array share the same storage space.

      [Genarray.slice_right] applies only to big arrays in Fortran layout.
      Raise [Invalid_argument] if [M >= N], or if [\[|i1; ... ; iM|\]] is
  ...
  (** Copy all elements of a big array in another big array.
      ...
      arrays [src] and [dst] must have the same number of dimensions and equal
      dimensions. Copying a sub-array of [src] to a sub-array of [dst] can be
      achieved by applying [Genarray.blit] to sub-array or slices of [src] and
      [dst]. *)
  ...
  (** Set all elements of a big array to a given value. [Genarray.fill a v]
      ...
      some elements of [a] to [v] can be achieved by applying [Genarray.fill]
...
(** Zero-dimensional arrays. The [Array0] structure provides operations similar
    to those of {!Bigarray.Genarray}, but specialized to the case of
...
module Array0 : sig
  ...
  (** [Array0.create kind layout] returns a new bigarray of zero dimension.
      [kind] and [layout] determine the array element kind and the array layout
  ...
  (** [Array0.change_layout a layout] returns a big array with the specified
      [layout], sharing the data with [a]. No copying of elements is involved:
      the new array and the original array share the same storage space.
      @since 4.06.0 *)
...
(** One-dimensional arrays. The [Array1] structure provides operations similar
    to those of {!Bigarray.Genarray}, but specialized to the case of
    one-dimensional arrays. (The {!Array2} and {!Array3} structures below
...
module Array1 : sig
  ...
  (** [Array1.get a x], or alternatively [a.{x}], returns the element of [a] at
      ...
      [Array1.dim a] if [a] has C layout. If [a] has Fortran layout, [x] must
      be greater or equal than [1] and less or equal than [Array1.dim a].
  ...
  (** Extract a scalar (zero-dimensional slice) of the given one-dimensional
      ...
      See {!Bigarray.Genarray.slice_left} and {!Bigarray.Genarray.slice_right}
      for more details.
      @since 4.05.0 *)
...
(** Two-dimensional arrays. The [Array2] structure provides operations similar
    to those of {!Bigarray.Genarray}, but specialized to the case of
...
module Array2 : sig
  ...
  (** [Array2.create kind layout dim1 dim2] returns a new bigarray of two
      ...
      second dimension. [kind] and [layout] determine the array element kind
  ...
  (** [Array2.get a x y], also written [a.{x,y}], returns the element of [a] at
      coordinates ([x], [y]). [x] and [y] must be within the bounds of [a], as
      described for {!Bigarray.Genarray.get}; otherwise, [Invalid_argument] is
      raised. *)

  external set : ('a, 'b, 'c) t -> int -> int -> 'a -> unit = "%caml_ba_set_2"
  (** [Array2.set a x y v], or alternatively [a.{x,y} <- v], stores the value
      [v] at coordinates ([x], [y]) in [a]. [x] and [y] must be within the
  ...
  (** Extract a two-dimensional sub-array of the given two-dimensional big
      ...
      {!Bigarray.Genarray.sub_left} for more details. [Array2.sub_left] applies
  ...
  (** Extract a two-dimensional sub-array of the given two-dimensional big
      ...
      {!Bigarray.Genarray.sub_right} for more details. [Array2.sub_right]
  ...
  (** Extract a row (one-dimensional slice) of the given two-dimensional big
      ...
      {!Bigarray.Genarray.slice_left} for more details. [Array2.slice_left]
      applies only to arrays with C layout. *)

  val slice_right :
    ('a, 'b, fortran_layout) t -> int -> ('a, 'b, fortran_layout) Array1.t
  (** Extract a column (one-dimensional slice) of the given two-dimensional big
      array. The integer parameter is the index of the column to extract. See
      {!Bigarray.Genarray.slice_right} for more details. [Array2.slice_right]
      applies only to arrays with Fortran layout. *)
...
(** Three-dimensional arrays. The [Array3] structure provides operations
    similar to those of {!Bigarray.Genarray}, but specialized to the case of
...
module Array3 : sig
  ...
  (** [Array3.create kind layout dim1 dim2 dim3] returns a new bigarray of
      three dimension, whose size is [dim1] in the first dimension, [dim2] in
  ...
  (** [Array3.change_layout a layout] returns a bigarray with the specified
      ...
      as [a]). No copying of elements is involved: the new array and the
      original array share the same storage space. The dimensions are reversed,
      such that [get v \[| a; b; c |\]] in C layout becomes
  ...
  (** [Array3.get a x y z], also written [a.{x,y,z}], returns the element of
      ...
      bounds of [a], as described for {!Bigarray.Genarray.get}; otherwise,
      [Invalid_argument] is raised. *)

  external set : ('a, 'b, 'c) t -> int -> int -> int -> 'a -> unit
    = "%caml_ba_set_3"
  (** [Array3.set a x y v], or alternatively [a.{x,y,z} <- v], stores the value
      [v] at coordinates ([x], [y], [z]) in [a]. [x], [y] and [z] must be
      within the bounds of [a], as described for {!Bigarray.Genarray.set};
      otherwise, [Invalid_argument] is raised. *)
  ...
  (** Extract a three-dimensional sub-array of the given three-dimensional big
      array by restricting the first dimension. See
      {!Bigarray.Genarray.sub_left} for more details. [Array3.sub_left] applies
      only to arrays with C layout. *)
  ...
  (** Extract a two-dimensional slice of the given three-dimensional big array
      ...
      the slice to extract. See {!Bigarray.Genarray.slice_right} for more
      details. [Array3.slice_right_2] applies only to arrays with Fortran
      layout. *)
...
(** Return the one-dimensional big array corresponding to the given generic big
    array. Raise [Invalid_argument] if the generic big array does not have
...
(** [reshape b \[|d1;...;dN|\]] converts the big array [b] to a [N]-dimensional
    array of dimensions [d1]...[dN]. The returned array and the original array
    [b] share their data and have the same layout. For instance, assuming that
    [b] is a one-dimensional array of dimension 12, [reshape b \[|3;4|\]]
    returns a two-dimensional array [b'] of dimensions 3 and 4. If [b] has C
    layout, the element [(x,y)] of [b'] corresponds to the element [x * 3 + y]
    of [b]. If [b] has Fortran layout, the element [(x,y)] of [b'] corresponds
    to the element [x + (y - 1) * 4] of [b]. The returned big array must have
    exactly the same number of elements as the original big array [b]. That is,
    the product of the dimensions of [b] must be equal to [i1 * ... * iN].
    Otherwise, [Invalid_argument] is raised. *)
...
(** Specialized version of {!Bigarray.reshape} for reshaping to one-dimensional
    arrays. *)

val reshape_2 : ('a, 'b, 'c) Genarray.t -> int -> int -> ('a, 'b, 'c) Array2.t
(** Specialized version of {!Bigarray.reshape} for reshaping to two-dimensional
    arrays. *)
