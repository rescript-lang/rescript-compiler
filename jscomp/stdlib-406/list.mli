...
(** List operations.
    ...
    space proportional to the length of its list argument, which can be a
    problem with very long lists. When the function takes several list
    arguments, an approximate formula giving stack usage (in some unspecified
...
(** Compare the length of a list to an integer. [compare_length_with l n] is
    equivalent to [compare (length l) n], except that the computation stops
    after at most [n] iterations on the list.
    @since 4.05.0 *)
...
(** [List.iter2 f \[a1; ...; an\] \[b1; ...; bn\]] calls in turn
    [f a1 b1; ...; f an bn]. Raise [Invalid_argument] if the two lists are
...
(** [List.rev_map2 f l1 l2] gives the same result as {!List.rev} [(] 
    {!List.map2} [f l1 l2)], but is tail-recursive and more efficient. *)
...
(** [List.fold_right2 f \[a1; ...; an\] \[b1; ...; bn\] c] is
    ...
    two lists are determined to have different lengths. Not tail-recursive. *)
...
(** Same as {!List.for_all}, but for a two-argument predicate. Raise
    [Invalid_argument] if the two lists are determined to have different
    lengths. *)
...
(** Sort a list in increasing order according to a comparison function. The
    ...
    example, {!Pervasives.compare} is a suitable comparison function. The
    resulting list is sorted in increasing order. [List.sort] is guaranteed to
    run in constant heap space (in addition to the size of the result list) and
