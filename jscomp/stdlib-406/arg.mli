...
(** Parsing of command line arguments.
    ...
    [-]. An option is a keyword alone or followed by an argument. The types of
    keywords are: [Unit], [Bool], [Set], [Clear], [String], [Set_string],
    [Int], [Set_int], [Float], [Set_float], [Tuple], [Symbol], and [Rest].
    [Unit], [Set] and [Clear] keywords take no argument. A [Rest] keyword takes
    the remaining of the command line as arguments. Every other keyword takes
    the following word on the command line as argument. For compatibility with
    GNU getopt_long, [keyword=arg] is also allowed. Arguments not preceded by a
    keyword are called anonymous arguments.

    Examples ([cmd] is assumed to be the command
    name):
...
type spec =
  ...
  | Expand of (string -> string array)
      (** If the remaining arguments to process are of the form
          [\["-foo"; "arg"\] @ rest] where "foo" is registered as [Expand f],
          then the arguments [f "arg" @ rest] are processed. Only allowed in
...
(** [Arg.parse speclist anon_fun usage_msg] parses the command line. [speclist]
    is a list of triples [(key, spec, doc)]. [key] is the option keyword, it
    must start with a ['-'] character. [spec] gives the option type and the
    function to call when this option is found on the command line. [doc] is a
    one-line description of this option. [anon_fun] is called on anonymous
    arguments. The functions in [spec] and [anon_fun] are called in the same
    order as their arguments appear on the command line.

    If an error occurs, [Arg.parse] exits the program, after printing to
    standard error an error message as
    follows:
...
(** [Arg.parse_argv ~current args speclist anon_fun usage_msg] parses the array
    ...
    [parse_argv]. The initial value of [current] is the index of the program
    name (argument 0) in the array. If an error occurs, [Arg.parse_argv] raises
    {!Arg.Bad} with the error message as argument. If option [-help] or
    [--help] is given, [Arg.parse_argv] raises {!Arg.Help} with the help
    message as argument. *)
...
(** Same as {!Arg.parse_argv}, except that the [speclist] argument is a
    reference and may be updated during the parsing. See {!Arg.parse_dynamic}.
    @since 4.01.0 *)
...
(** [Arg.usage speclist usage_msg] prints to standard error an error message
    ...
    {!Arg.parse} prints in case of error. [speclist] and [usage_msg] are the
    same as for {!Arg.parse}. *)

val usage_string : (key * spec * doc) list -> usage_msg -> string
(** Returns the message that would have been printed by {!Arg.usage}, if
...
(** Align the documentation strings by inserting spaces at the first alignment
    ...
    @param limit
    options with keyword and message longer than [limit] will not be used to
    compute the alignment. *)

val current : int ref
(** Position (in {!Sys.argv}) of the argument being processed. You can change
    this value, e.g. to force {!Arg.parse} to skip some arguments. {!Arg.parse}
    uses the initial value of {!Arg.current} as the index of argument 0 (the
