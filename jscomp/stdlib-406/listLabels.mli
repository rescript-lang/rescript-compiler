...
(** [List.rev_append l1 l2] reverses [l1] and concatenates it with [l2]. This
    is equivalent to [(] {!List.rev} [l1) @ l2], but [rev_append] is
...
(** [find p l] returns the first element of the list [l] that satisfies the
    predicate [p]. Returns [None] if there is no value that satisfies [p] in
    the list [l].
    @since 4.05 *)
...
(** [assoc_opt a l] returns the value associated with key [a] in the list of
    ...
    leftmost binding of [a] in list [l]. Returns [None] if there is no value
    associated with [a] in the list [l].
    @since 4.05 *)
