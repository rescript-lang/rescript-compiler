...
(** [extension name] is the shortest suffix [ext] of [name0] where:
    ...
    {ul
     {- [name0] is the longest suffix of [name] that does not contain a
        directory separator;}
...
(** Return the given file name without its extension, as defined in
    {!Filename.extension}. If the extension is empty, the function returns the
    given file name.
...
(** Split a file name into directory name / base file name. If [name] is a
    ...
    [basename name] (which is a relative file name) designate the same file as
...
(** [temp_file prefix suffix] returns the name of a fresh temporary file in the
    ...
    concatenating [prefix], then a suitably chosen integer number, then
    [suffix]. The optional argument [temp_dir] indicates the temporary
    directory to use, defaulting to the current result of
    {!Filename.get_temp_dir_name}. The temporary file is created empty, with
    permissions [0o600] (readable and writable only by the file owner). The
    file is guaranteed to be different from any other file that existed when
...
(** Same as {!Filename.temp_file}, but returns both the name of a fresh
    temporary file, and an output channel opened (atomically) on this file.
    This function is more secure than [temp_file]: there is no risk that the
    temporary file will be modified (e.g. replaced by a symbolic link) before
    the program opens it. The optional argument [mode] is a list of additional
    flags to control the opening of the file. It can contain one or several of
    [Open_append], [Open_binary], and [Open_text]. The default is
    [\[Open_text\]] (open in text mode). The file is created with permissions
    [perms] (defaults to readable and writable only by the file owner,
    [0o600]).
...
(** The name of the initial temporary directory: Under Unix, the value of the
    [TMPDIR] environment variable, or "/tmp" if the variable is not set. Under
