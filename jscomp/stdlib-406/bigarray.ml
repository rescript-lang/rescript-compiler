...
(* Keep those constants in sync with the caml_ba_kind enumeration in bigarray.h *)
...
(* Keep those constants in sync with the caml_ba_layout enumeration in
   bigarray.h *)
...
module Genarray = struct
  ...
  external map_internal :
    ...
    -> int array
    -> int64
    -> ('a, 'b, 'c) t = "caml_ba_map_file_bytecode" "caml_ba_map_file"
...
module Array0 = struct
  ...
  let create kind layout = Genarray.create kind layout [||]
...
module Array1 = struct
  ...
  let create kind layout dim = Genarray.create kind layout [|dim|]
  ...
  let size_in_bytes arr = kind_size_in_bytes (kind arr) * dim arr
...
module Array2 = struct
  ...
  let create kind layout dim1 dim2 = Genarray.create kind layout [|dim1; dim2|]
  ...
  let size_in_bytes arr = kind_size_in_bytes (kind arr) * dim1 arr * dim2 arr
...
(* Force caml_ba_get_{1,2,3,N} to be linked in, since we don't refer to those
   primitives directly in this file *)
