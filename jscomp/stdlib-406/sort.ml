...
(* There is a known performance bug in the code below. If you find it, don't
   bother reporting it. You're not supposed to use this module anyway. *)
let array cmp arr =
  let rec qsort lo hi =
    if hi - lo >= 6 then (
      let mid = (lo + hi) lsr 1 in
      (* Select median value from among LO, MID, and HI. Rearrange LO and HI so
         the three values are sorted. This lowers the probability of picking a
         pathological pivot. It also avoids extra comparisons on i and j in the
         two tight "while" loops below. *)
      if cmp (unsafe_get arr mid) (unsafe_get arr lo) then swap arr mid lo ;
      if cmp (unsafe_get arr hi) (unsafe_get arr mid) then (
        swap arr mid hi ;
        if cmp (unsafe_get arr mid) (unsafe_get arr lo) then swap arr mid lo ) ;
      ...
      while !i < !j do
        while not (cmp pivot (unsafe_get arr !i)) do
          incr i
        done ;
        while not (cmp (unsafe_get arr !j) pivot) do
          decr j
        done ;
      ...
      if !j - lo <= hi - !i then (qsort lo !j ; qsort !i hi)
      else (qsort !i hi ; qsort lo !j) ) in
  qsort 0 (Array.length arr - 1) ;
  (* Finish sorting by insertion sort *)
  for i = 1 to Array.length arr - 1 do
    let val_i = unsafe_get arr i in
    if not (cmp (unsafe_get arr (i - 1)) val_i) then (
