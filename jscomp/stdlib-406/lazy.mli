...
(** A value of type ['a Lazy.t] is a deferred computation, called a suspension,
    ...
    makes a suspension of the computation of [expr], without computing [expr]
    itself yet. "Forcing" the suspension will then compute [expr] and return
    its result.
    ...
    recursive definitions of the form [let rec x = lazy x] or
    [let rec x = lazy(lazy(...(lazy x)))] are accepted by the type-checker and
    lead, when forced, to ill-formed values that trigger infinite loops in the
    garbage collector and other parts of the run-time system. Without the
    [-rectypes] option, such ill-founded recursive definitions are rejected by
    the type-checker. *)
...
(** [force x] forces the suspension [x] and returns its result. If [x] has
    ...
    again. Raise {!Undefined} if the forcing of [x] tries to force [x] itself
    recursively. *)
...
val is_val : 'a t -> bool
(** [is_val x] returns [true] if [x] has already been forced and did not raise
    an exception.
    @since 4.00.0 *)
