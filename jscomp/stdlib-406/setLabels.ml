...
module Make(Ord: OrderedType) =
  struct
    ...
    *)
    ...
    let rec filter ~f:p = function
        Empty -> Empty
      | (Node{l; v; r}) as t ->
          (* call [p] in the expected left-to-right order *)
          let l' = filter ~f:p l in
          let pv = p v in
          let r' = filter ~f:p r in
          if pv then
            if l==l' && r==r' then t else join l' v r'
          else concat l' r'

    let rec partition ~f:p = function
        Empty -> (Empty, Empty)
      | Node{l; v; r} ->
          (* call [p] in the expected left-to-right order *)
          let (lt, lf) = partition ~f:p l in
          let pv = p v in
          let (rt, rf) = partition ~f:p r in
          if pv
