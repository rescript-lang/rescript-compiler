...
let set_data tables v =
  match tables with Empty -> assert false | Cons tables -> tables.data <- v

let set_next tables v =
  match tables with Empty -> assert false | Cons tables -> tables.next <- v
...
let set_methods table methods =
  ...
  let i = ref 0 in
  while !i < len do
    let label = methods.(!i) in
    let clo = method_impl table i methods in
