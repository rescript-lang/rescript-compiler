...
(** Profiling of a program's space behaviour over time. Currently only
    ...
    To use the functions in this module you
    must:
    ...
    abnormally terminated by a signal) so that the snapshot file is correctly
    completed.

    When using the automatic snapshot mode the profiling output is written to a
    file called "spacetime-<pid>" where <pid> is the process ID of the program.
    (If the program forks and continues executing then multiple files may be
    produced with different pid numbers.) The profiling output is by default
    written to the current working directory when the program starts. This may
    be customised by setting the OCAML_SPACETIME_SNAPSHOT_DIR environment
...
val enabled : bool
(** [enabled] is [true] if the compiler is configured with spacetime and
    [false] otherwise *)
...
module Series : sig
  ...
  (** [save_event] writes an event, which is an arbitrary string, into the
      ...
      parameter is as for {!Snapshot.take}. *)
  ...
  (** [save_and_close series] writes information into [series] required for
      ...
      file. The optional [time] parameter is as for {!Snapshot.take}. *)
end
...
module Snapshot : sig
  ...
  (** [take series] takes a snapshot of the profiling annotations on the values
      ...
      not allocate any memory itself. If the optional [time] is specified, it
      will be used instead of the result of {!Sys.time} as the timestamp of the
      snapshot. Such [time]s should start from zero and be monotonically
...
(** Like {!Series.save_event}, but writes to the automatic snapshot file. This
    function is a no-op if OCAML_SPACETIME_INTERVAL was not set. *)
