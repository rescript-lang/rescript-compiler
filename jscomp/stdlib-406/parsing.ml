...
let yyparse tables start lexer lexbuf =
  let rec loop cmd arg =
    ...
    | Compute_semantic_action ->
        let action, value =
          try (Semantic_action_computed, tables.actions.(env.rule_number) env)
          with Parse_error -> (Error_detected, Obj.repr ()) in
        loop action value
    | Grow_stacks_1 ->
        grow_stacks () ;
...
let symbol_start_pos () =
  let rec loop i =
    if i <= 0 then env.symb_end_stack.(env.asp)
      ...
      if st <> en then st else loop (i - 1) in
  loop env.rule_len
