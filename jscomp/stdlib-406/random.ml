...
(* Pseudo-random number generator This is a lagged-Fibonacci F(55, 24, +) with
   a modified addition function to enhance the mixing of bits. If we use normal
...
module State = struct
  ...
  let make seed =
    ...
    full_init result seed ; result

  let make_self_init () = make (random_seed ())

  let copy s =
    let result = new_state () in
    assign result s ; result
  ...
  let int s bound =
    if bound > 0x3FFFFFFF || bound <= 0 then invalid_arg "Random.int"
    else intaux s bound
  ...
  let rec int32aux s n =
    ...
    if Int32.sub r v > Int32.add (Int32.sub Int32.max_int n) 1l then
      int32aux s n
    else v
  ...
  let rec int64aux s n =
    ...
    if Int64.sub r v > Int64.add (Int64.sub Int64.max_int n) 1L then
      int64aux s n
    else v
  ...
  let nativeint =
    if Nativeint.size = 32 then fun s bound ->
  ...
  let rawfloat s =
    ...
    and r2 = Pervasives.float (bits s) in
    ((r1 /. scale) +. r2) /. scale
...
(********************
  ...
  should be called with n > 10r. It returns a triple (low, actual, high). If
  low <= actual <= high, the [g] function passed the test, otherwise it failed.
  ...
  100; chisquare diff 100000 100 init 27182818; init_diff2 1024; chisquare
  diff2 100000 1024 init 27182818; init_diff2 100; chisquare diff2 100000 100
  init 14142136; init_diff2 100; chisquare diff2 100000 100 init 299792643;
  ...
  float * float = (960., 1019.19744000000355, 1088.) # - : float * float *
  float = (960., 1059.31776000000536, 1088.) # - : float * float * float =
  (960., 1039.98463999999512, 1088.) # - : float * float * float = (960.,
  ...
  *)
  ...
  >= i1 then 0.0 else if i1 = i0 + 1 then Pervasives.float v.(i0) *.
  Pervasives.float v.(i0) else sumsq v i0 ((i0+i1)/2) +. sumsq v ((i0+i1)/2) i1
  ...
  let t = sumsq f 0 r and r = Pervasives.float r and n = Pervasives.float n in
  let sr = 2.0 *. sqrt r in (r -. sr, (r *. t /. n) -. n, r +. sr)
  ...
  *) let st = ref 0 let init_diff r = st := int r let diff r = let x1 = !st and
  ...
  numbers. *) let init_diff2 r = st1 := int r; st2 := int r let diff2 r = let
  x1 = !st1 and x2 = !st2 and x3 = int r in st1 := x2; st2 := x3; (x3 - x2 - x2
  + x1 + 2*r) mod r
