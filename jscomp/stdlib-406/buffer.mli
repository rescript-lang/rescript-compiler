...
(** [create n] returns a fresh buffer, initially empty. The [n] parameter is
    ...
    [n] characters are stored in the buffer, but shrinks back to [n] characters
    when [reset] is called. For best performance, [n] should be of the same
    order of magnitude as the number of characters that are expected to be
    stored in the buffer (for instance, 80 for a buffer that holds one output
    line). Nothing bad will happen if the buffer grows beyond that limit,
    however. In doubt, take [n = 16] for instance. If [n] is not between 1 and
...
(** [Buffer.sub b off len] returns a copy of [len] bytes from the current
    contents of the buffer [b], starting at offset [off].
...
(** Empty the buffer and deallocate the internal byte sequence holding the
    ...
    length [n] that was allocated by {!Buffer.create} [n]. For long-lived
...
(** [add_utf_8_uchar b u] appends the
    {{:https://tools.ietf.org/html/rfc3629} UTF-8} encoding of [u] at the end
    of buffer [b].
    @since 4.06.0 *)

val add_utf_16le_uchar : t -> Uchar.t -> unit
(** [add_utf_16le_uchar b u] appends the
    {{:https://tools.ietf.org/html/rfc2781} UTF-16LE} encoding of [u] at the
    end of buffer [b].
    @since 4.06.0 *)

val add_utf_16be_uchar : t -> Uchar.t -> unit
(** [add_utf_16be_uchar b u] appends the
    {{:https://tools.ietf.org/html/rfc2781} UTF-16BE} encoding of [u] at the
    end of buffer [b].
    @since 4.06.0 *)
...
(** [add_substitute b f s] appends the string pattern [s] at the end of buffer
    ...
    the pattern and substitutes each variable name by its value, as obtained by
    ...
    - an arbitrary sequence of characters enclosed by a pair of matching
      ...
      [$]. Raise [Not_found] if the closing character of a parenthesized
...
(** [add_channel b ic n] reads at most [n] characters from the input channel
    [ic] and stores them at the end of buffer [b]. Raise [End_of_file] if the
    channel contains fewer than [n] characters. In this case, the characters
    are still added to the buffer, so as to avoid loss of data. *)
...
(** [truncate b len] truncates the length of [b] to [len] Note: the internal
    byte sequence is not shortened. Raise [Invalid_argument] if [len < 0] or
    [len > length b].
    @since 4.05.0 *)
