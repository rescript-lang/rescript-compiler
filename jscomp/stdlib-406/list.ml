...
let nth l n =
  if n < 0 then invalid_arg "List.nth"
    ...
    let rec nth_aux l n =
      match l with
      | [] -> failwith "nth"
      | a :: l -> if n = 0 then a else nth_aux l (n - 1) in
    nth_aux l n
...
let nth_opt l n =
  if n < 0 then invalid_arg "List.nth"
    ...
    let rec nth_aux l n =
      match l with
      | [] -> None
      | a :: l -> if n = 0 then Some a else nth_aux l (n - 1) in
    nth_aux l n
...
let init len f =
  ...
  else if len > 10_000 then rev (init_tailrec_aux [] 0 len f)
...
(* Note: on a list of length between about 100000 (depending on the minor heap
   ...
   p = if p <= 0 then accu else begin if p = n then begin obj_truncate a p;
   loop (a.(p-1) :: accu) (n-1000) (p-1) end else begin loop (a.(p-1) :: accu)
   n (p-1) end end in loop [] (l-1000) l
