...
val catch : ('a -> 'b) -> 'a -> 'b
(** [Printexc.catch fn x] is similar to {!Printexc.print}, but aborts the
...
(** [Printexc.get_backtrace ()] returns a string containing the same exception
    backtrace that [Printexc.print_backtrace] would print. Same restriction
    usage than {!print_backtrace}.
    @since 3.11.0 *)
...
(** [Printexc.record_backtrace b] turns recording of exception backtraces on
    ...
    recorded, unless the [b] flag is given to the program through the
...
(** [Printexc.register_printer fn] registers [fn] as an exception printer. The
    printer should return [None] or raise an exception if it does not know how
    to convert the passed exception, and [Some s] with [s] the resulting string
    if it can convert the passed exception. Exceptions raised by the printer
    are ignored.

    When converting an exception into a string, the printers will be invoked in
    the reverse order of their registrations, until a printer returns a
    [Some s] value (if no such printer exists, the runtime will use a generic
    printer).
...
(** The abstract type [raw_backtrace] stores a backtrace in a low-level format,
    instead of directly exposing them as string as the [get_backtrace()]
    function does.
    ...
    use the array returned by the [backtrace_slots] function of the next
    section.
    @since 4.01.0 *)

val get_raw_backtrace : unit -> raw_backtrace
(** [Printexc.get_raw_backtrace ()] returns the same exception backtrace that
    [Printexc.print_backtrace] would print, but in a raw format. Same
...
(** [Printexc.get_callstack n] returns a description of the top of the call
    stack on the current program point (for the current thread), with at most
...
(** Returns the slots of a raw backtrace, or [None] if none of them contain
    ...
    Some possible reasons for returning [None] are as
    follow:
...
module Slot : sig
  ...
  (** [location slot] returns the location information of the slot, if
      ...
      Some possible reasons for failing to return a location are as
      follow:
...
(** This type allows direct access to raw backtrace slots, without any
    ...
    are equal, then they represent the same source location (the converse is
    not necessarily true in presence of inlining, for example).
    @since 4.02.0 *)
...
(** [Printexc.exn_slot_id] returns an integer which uniquely identifies the
    constructor used to create the exception value [exn] (in the current
    runtime).
    @since 4.02.0 *)
