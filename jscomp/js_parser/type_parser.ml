...
module type TYPE = sig
  ...
  val interface_helper :
    env ->
    (Loc.t * (Loc.t, Loc.t) Ast.Type.Generic.t) list
    * (Loc.t * (Loc.t, Loc.t) Ast.Type.Object.t)
  ...
  val annotation_and_predicate_opt :
    env ->
    (Loc.t, Loc.t) Ast.Type.annotation_or_hint
    * (Loc.t, Loc.t) Ast.Type.Predicate.t option
end
...
module Type (Parse : Parser_common.PARSER) : TYPE = struct
  ...
  and annotation env =
    if not (should_parse_types env) then
      error env Parse_error.UnexpectedTypeAnnotation;
    with_loc
      (fun env ->
  ...
  and postfix_with ?(in_optional_indexed_access = false) env t =
    if Peek.is_line_terminator env then t
    else
      match Peek.token env with
      | T_PLING_PERIOD ->
          Eat.token env;
          if Peek.token env <> T_LBRACKET then
            error env Parse_error.InvalidOptionalIndexedAccess;
          Expect.token env T_LBRACKET;
          postfix_brackets ~in_optional_indexed_access:true
            ~optional_indexed_access:true env t
      | T_LBRACKET ->
          Eat.token env;
          postfix_brackets ~in_optional_indexed_access
            ~optional_indexed_access:false env t
      | T_PERIOD -> (
          match Peek.ith_token ~i:1 env with
          | T_LBRACKET ->
              error env
                (Parse_error.InvalidIndexedAccess { has_bracket = true });
          ...
          | _ ->
              error env
                (Parse_error.InvalidIndexedAccess { has_bracket = false });
              t)
      | _ -> t

  and postfix_brackets ~in_optional_indexed_access ~optional_indexed_access env
      t =
    let t =
      with_loc ~start_loc:(fst t)
        (fun env ->
          if (not optional_indexed_access) && Eat.maybe env T_RBRACKET then
            let trailing = Eat.trailing_comments env in
            Type.Array
              {
                Type.Array.argument = t;
            ...
            else Type.IndexedAccess indexed_access)
        env
    in
  ...
  and primary env =
    ...
    | T_LCURLY | T_LCURLYBAR ->
        let loc, o =
          _object env ~is_class:false ~allow_exact:true ~allow_spread:true
        in
        (loc, Type.Object o)
    | T_INTERFACE ->
        with_loc
  ...
  and is_primitive = function
    | T_ANY_TYPE | T_MIXED_TYPE | T_EMPTY_TYPE | T_BOOLEAN_TYPE _
    | T_NUMBER_TYPE | T_BIGINT_TYPE | T_STRING_TYPE | T_SYMBOL_TYPE
    | T_VOID_TYPE | T_NULL ->
        true
    | _ -> false
  ...
  and primitive env =
    ...
    | T_BOOLEAN_TYPE _ ->
        ...
        let trailing = Eat.trailing_comments env in
        Some
          (Type.Boolean (Flow_ast_utils.mk_comments_opt ~leading ~trailing ()))
    | T_NUMBER_TYPE ->
        Eat.token env;
        let trailing = Eat.trailing_comments env in
        Some
          (Type.Number (Flow_ast_utils.mk_comments_opt ~leading ~trailing ()))
    | T_BIGINT_TYPE ->
        Eat.token env;
        let trailing = Eat.trailing_comments env in
        Some
          (Type.BigInt (Flow_ast_utils.mk_comments_opt ~leading ~trailing ()))
    | T_STRING_TYPE ->
        Eat.token env;
        let trailing = Eat.trailing_comments env in
        Some
          (Type.String (Flow_ast_utils.mk_comments_opt ~leading ~trailing ()))
    | T_SYMBOL_TYPE ->
        Eat.token env;
        let trailing = Eat.trailing_comments env in
        Some
          (Type.Symbol (Flow_ast_utils.mk_comments_opt ~leading ~trailing ()))
    | T_VOID_TYPE ->
        Eat.token env;
        let trailing = Eat.trailing_comments env in
  ...
  and anonymous_function_param _env annot =
    ( fst annot,
      ...
      { name = None; annot; optional = false } )
  ...
  and function_param_list_without_parens =
    let param env =
      match Peek.ith_token ~i:1 env with
      | T_COLON | T_PLING -> function_param_with_id env
      | _ ->
          let annot = _type env in
    ...
    let rec param_list env this_ acc =
      ...
      | (T_EOF | T_ELLIPSIS | T_RPAREN) as t ->
          ...
          in
          {
            Ast.Type.Function.Params.params = List.rev acc;
            rest;
            this_;
            comments = None;
          }
      | T_IDENTIFIER { raw = "this"; _ }
        when Peek.ith_token ~i:1 env == T_COLON
             || Peek.ith_token ~i:1 env == T_PLING ->
          if this_ <> None || acc <> [] then
            error env Parse_error.ThisParamMustBeFirst;
          let this_ =
            with_loc
              (fun env ->
                let leading = Peek.comments env in
                ...
                if Peek.token env == T_PLING then
                  error env Parse_error.ThisParamMayNotBeOptional;
                {
      ...
      (fun env ->
        ...
        let trailing = Eat.trailing_comments env in
        {
          ...
          Ast.Type.Function.Params.comments =
            Flow_ast_utils.mk_comments_with_internal_opt ~leading ~trailing
              ~internal ();
        })
      env
  ...
  and param_list_or_type env =
    ...
    let ret =
      ...
      | T_RPAREN ->
          ParamList
            {
              Ast.Type.Function.Params.this_ = None;
              params = [];
              rest = None;
              comments = None;
            }
      | T_IDENTIFIER _ | T_STATIC -> function_param_or_generic_type env
  ...
  and function_param_or_generic_type env =
    match Peek.ith_token ~i:1 env with
    | T_PLING | T_COLON -> ParamList (function_param_list_without_parens env [])
    | _ ->
  ...
  and function_with_params env start_loc tparams
      (params : (Loc.t, Loc.t) Ast.Type.Function.Params.t) =
    with_loc ~start_loc
      (fun env ->
        Expect.token env T_ARROW;
        let return = _type env in
        let open Type in
        Function { Function.params; return; tparams; comments = None })
      env
  ...
  and _object =
      ...
      let prop =
        with_loc ~start_loc
          (fun env ->
            ...
            let tparams = type_params_remove_trailing env (type_params env) in
            let value = methodish env start_loc tparams in
            let open Type.Object.CallProperty in
            {
              value;
              static = static <> None;
              comments = Flow_ast_utils.mk_comments_opt ~leading ();
      ...
      let prop =
        with_loc ~start_loc
          (fun env ->
            ...
            let open Type.Object.Property in
            {
              key;
              value = Init value;
              optional;
          ...
          (fun env ->
            ...
            let optional, _method, value, trailing =
              ...
              | T_LESS_THAN | T_LPAREN ->
                  let tparams =
                    type_params_remove_trailing env (type_params env)
                  in
                  let value =
    ...
    let error_invalid_property_name env is_class static key =
      ...
      | Expression.Object.Property.Identifier
          (loc, { Identifier.name; comments = _ })
        when is_class
             && (is_constructor name || (is_static && is_prototype name)) ->
          error_at env
            ( loc,
      ...
      | _ -> ()
    in
    let rec properties ~is_class ~allow_inexact ~allow_spread ~exact env
        ((props, inexact, internal) as acc) =
      ...
      | T_ELLIPSIS when allow_spread -> (
          ...
          | T_COMMA | T_SEMICOLON | T_RCURLY | T_RCURLYBAR -> (
              ...
              | _ ->
                  error_at env
                    (start_loc, Parse_error.UnexpectedExplicitInexactInObject);
                  properties ~is_class ~allow_inexact ~allow_spread ~exact env
                    acc)
          | _ ->
      ...
      | _ ->
          let prop =
            property env start_loc ~is_class ~allow_static:is_class
              ~allow_proto:is_class ~variance:None ~static:None ~proto:None
              ~leading:[]
          in
          semicolon exact env;
          properties ~is_class ~allow_inexact ~allow_spread ~exact env
            (prop :: props, inexact, internal)
    and property env ~is_class ~allow_static ~allow_proto ~variance ~static
        ~proto ~leading start_loc =
      match Peek.token env with
      | (T_PLUS | T_MINUS) when variance = None ->
          let variance = maybe_variance env in
          property env ~is_class ~allow_static:false ~allow_proto:false
            ~variance ~static ~proto ~leading start_loc
      ...
      | token -> (
          ...
          | Some _, Some _, _ ->
              failwith "Can not have both `static` and `proto`"
          | Some static_loc, None, (T_PLING | T_COLON) ->
              ...
              in
              let static = None in
              init_property env start_loc ~variance ~static ~proto ~leading:[]
                key
          ...
          | _ -> (
              ...
              | ( _,
                  (Expression.Object.Property.Identifier
                     ( _,
                       {
                         Identifier.name = ("get" | "set") as name;
                         comments = _;
                       } ) as key) ) -> (
                  match Peek.token env with
                  | T_LESS_THAN | T_LPAREN ->
                  ...
                  | T_COLON | T_PLING ->
                      init_property env start_loc ~variance ~static ~proto
                        ~leading key
                  | _ ->
                      ...
                      error_unexpected_proto env proto;
                      error_unexpected_variance env variance;
                      getter_or_setter ~is_getter ~leading env start_loc static
                        key)
              | _, key -> (
                  match Peek.token env with
                  | T_LESS_THAN | T_LPAREN ->
      ...
      with_loc
        (fun env ->
          ...
          let properties, inexact, internal =
            let env = with_no_anon_function_type false env in
            properties ~is_class ~allow_inexact ~exact ~allow_spread env
              ([], false, [])
          in
  ...
  and interface_helper =
    ...
    fun env ->
      ...
      in
      let body =
        _object env ~allow_exact:false ~allow_spread:false ~is_class:false
      in
      (extends, body)

  and type_identifier env =
    let loc, { Identifier.name; comments } = identifier_name env in
    if is_reserved_type name then
  ...
  and bounded_type env =
    with_loc
      (fun env ->
        ...
        let bound =
          if Peek.token env = T_COLON then Ast.Type.Available (annotation env)
          else Ast.Type.Missing (Peek.loc_skip_lookahead env)
        in
        (name, bound))
      env
  ...
  and type_params =
    let rec params env ~require_default acc =
            ...
            let default, require_default =
              ...
              | _ ->
                  if require_default then
                    error_at env (loc, Parse_error.MissingTypeParamDefault);
      ...
      match Peek.token env with
      | T_EOF | T_GREATER_THAN -> List.rev acc
      | _ ->
          Expect.token env T_COMMA;
          if Peek.token env = T_GREATER_THAN then List.rev acc
  ...
  and add_comments (loc, t) leading trailing =
    ...
    ( loc,
      ...
      | Object ({ Object.comments; _ } as t) ->
          Object
            { t with Object.comments = merge_comments_with_internal comments }
      ...
      | IndexedAccess ({ IndexedAccess.comments; _ } as t) ->
          IndexedAccess
            { t with IndexedAccess.comments = merge_comments comments }
      | OptionalIndexedAccess
          {
            OptionalIndexedAccess.indexed_access =
              { IndexedAccess.comments; _ } as indexed_access;
            optional;
          } ->
          OptionalIndexedAccess
            {
              OptionalIndexedAccess.indexed_access =
                {
                  indexed_access with
                  IndexedAccess.comments = merge_comments comments;
                };
              optional;
            }
      ...
      | Intersection ({ Intersection.comments; _ } as t) ->
          Intersection
            { t with Intersection.comments = merge_comments comments }
      ...
      | StringLiteral ({ StringLiteral.comments; _ } as t) ->
          StringLiteral
            { t with StringLiteral.comments = merge_comments comments }
      | NumberLiteral ({ NumberLiteral.comments; _ } as t) ->
          NumberLiteral
            { t with NumberLiteral.comments = merge_comments comments }
      | BigIntLiteral ({ BigIntLiteral.comments; _ } as t) ->
          BigIntLiteral
            { t with BigIntLiteral.comments = merge_comments comments }
      | BooleanLiteral ({ BooleanLiteral.comments; _ } as t) ->
          BooleanLiteral
            { t with BooleanLiteral.comments = merge_comments comments } )
  ...
  let predicate_opt env =
    let env = with_no_anon_function_type false env in
    match Peek.token env with T_CHECKS -> Some (predicate env) | _ -> None
