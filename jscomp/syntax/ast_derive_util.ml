...
let mk_fun ~loc (typ : Parsetree.core_type) (value : string) body :
    Parsetree.expression =
  Ast_compatible.fun_ (Pat.constraint_ (Pat.var {txt= value; loc}) typ) body

let destruct_label_declarations ~loc (arg_name : string)
  ...
  Ext_list.fold_right labels ([], [])
    (fun {pld_name= {txt}; pld_type} (core_type_exps, labels) ->
      ( ( pld_type
        , Exp.field
            (Exp.ident {txt= Lident arg_name; loc})
            {txt= Lident txt; loc} )
        :: core_type_exps
      , txt :: labels ))
