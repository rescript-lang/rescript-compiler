              ...
              (false, false) (fun (null, undefined) ({txt; loc}, opt_expr) ->
                ...
                | "nullable" -> (
                  ...
                  | Some e ->
                      ...
                      (v, v) )
                | _ -> Bs_syntaxerr.err loc Unsupported_predicates) in
          ...
          let result =
            Ext_list.fold_left
              ...
              (fun st ({txt; loc}, opt_expr) ->
                ...
                else Bs_syntaxerr.err loc Unsupported_predicates) in
          (* properties -- void [@@bs.set{only}] *)
...
let process_attributes_rev (attrs : t) : attr_kind * t =
  Ext_list.fold_left attrs (Nothing, [])
    (fun (st, acc) (({txt; loc}, _) as attr) ->
      ...
      | "bs.this", (Nothing | Meth_callback _) -> (Meth_callback attr, acc)
      | "bs.meth", (Nothing | Method _) -> (Method attr, acc)
      | "bs", _ | "bs.this", _ ->
...
let external_needs_to_be_encoded (attrs : t) =
  Ext_list.exists_fst attrs (fun {txt} ->
...
let has_inline_payload_in_sig (attrs : t) =
  Ext_list.find_first attrs (fun (({txt}, _) as attr) ->
      if txt = "bs.inline" then (
        Bs_ast_invariant.mark_used_bs_attribute attr ;
        true )
      else false)
...
type derive_attr =
  {explict_nonrec: bool; bs_deriving: Ast_payload.action list option}

let process_derive_type (attrs : t) : derive_attr * t =
  Ext_list.fold_left attrs
    ({explict_nonrec= false; bs_deriving= None}, [])
    (fun (st, acc) (({txt; loc}, payload) as attr) ->
      match (st, txt) with
      ...
      | _, _ ->
          let st =
            if txt = "nonrec" then {st with explict_nonrec= true} else st in
          (st, attr :: acc))
...
let iter_process_derive_type (attrs : t) =
  ...
  Ext_list.iter attrs (fun (({txt; loc}, payload) as attr) ->
      ...
      | "bs.deriving" -> (
          ...
          | {bs_deriving= Some _} ->
              Bs_syntaxerr.err loc Duplicated_bs_deriving )
      | "nonrec" -> st := {!st with explict_nonrec= true}
      (* non bs attribute, no need to mark its use *)
      | _ -> ()) ;
  !st

(* duplicated [bs.uncurry] [bs.string] not allowed, it is worse in bs.uncurry
   since it will introduce inconsistency in arity *)
let iter_process_bs_string_int_unwrap_uncurry (attrs : t) =
  ...
  Ext_list.iter attrs (fun (({txt; loc}, (payload : _)) as attr) ->
      match txt with
      | "bs.string" -> assign `String attr
      | "bs.int" -> assign `Int attr
      | "bs.ignore" -> assign `Ignore attr
      | "bs.unwrap" -> assign `Unwrap attr
      | "bs.uncurry" ->
  ...
  Ext_list.iter attrs (fun (({txt; loc}, payload) as attr) ->
      ...
      | "bs.as" ->
          if !st = None then (
            ...
            | None -> (
              match Ast_payload.is_single_string payload with
              | Some (s, None) -> st := Some (`Str s)
              | Some (s, Some "json") -> st := Some (`Json_str s)
              | None | Some (_, Some _) ->
...
let bs : attr = ({txt= "bs"; loc= locg}, Ast_payload.empty)

let is_bs (attr : attr) =
  match attr with {Location.txt= "bs"; _}, _ -> true | _ -> false

let is_optional (attr : attr) =
  match attr with {Location.txt= "bs.optional"; _}, _ -> true | _ -> false

let is_bs_as (attr : attr) =
  match attr with {Location.txt= "bs.as"; _}, _ -> true | _ -> false
...
let deprecated s : attr =
  ...
  , PStr
      [ { pstr_desc= Pstr_eval (Ast_compatible.const_exp_string ~loc:locg s, [])
        ; pstr_loc= locg } ] )
