...
let is_any (ty : t) = ty.ptyp_desc = Ptyp_any
...
let replace_result (ty : t) (result : t) : t =
  let rec aux (ty : Parsetree.core_type) =
    match ty with
    | {ptyp_desc= Ptyp_arrow (label, t1, t2)} ->
  ...
  let result_type =
    Ast_comb.to_js_type loc
      (Ast_compatible.object_ ~loc
         (Ext_list.map2 labels tyvars (fun x y -> (x.Asttypes.txt, [], y)))
         Closed)
  in
...
let make_obj ~loc xs =
  Ast_comb.to_js_type loc (Ast_compatible.object_ ~loc xs Closed)
...
let get_uncurry_arity (ty : t) =
  match ty.ptyp_desc with
  | Ptyp_arrow
      (arg_label, {ptyp_desc= Ptyp_constr ({txt= Lident "unit"}, [])}, rest)
...
let list_of_arrow (ty : t) : t * Ast_compatible.param_type list =
  let rec aux (ty : t) acc =
    match ty.ptyp_desc with
    | Ptyp_arrow (label, t1, t2) ->
        aux t2
          ( ( {label; ty= t1; attr= ty.ptyp_attributes; loc= ty.ptyp_loc}
              : Ast_compatible.param_type )
          :: acc )
    | Ptyp_poly (_, ty) ->
        (* should not happen? *)
