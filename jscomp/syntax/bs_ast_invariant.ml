...
let emit_external_warnings : iterator =
  ...
  ; expr=
      (fun self a ->
        ...
        | Pexp_constant (Pconst_integer (s, None)) -> (
          try
            (* range check using int32 It is better to give a warning instead
               of error to avoid make people unhappy. It also has restrictions
               in which platform bsc is running on since it will affect int
               ranges *)
            ignore
              ( if String.length s = 0 || s.[0] = '-' then Int32.of_string s
              else Int32.of_string ("-" ^ s) )
          with _ -> Bs_warnings.warn_literal_overflow a.pexp_loc )
  ...
  ; value_description=
      (fun self v ->
        ...
        | _ -> default_iterator.value_description self v)
  ; pat=
      (fun self (pat : Parsetree.pattern) ->
        match pat.ppat_desc with
        | Ppat_constant (Pconst_string (_, Some "j")) ->
