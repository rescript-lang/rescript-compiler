...
(* [@@@ocaml.warning "+9"] (* Ensure that record patterns don't miss any field.
   *) *)
...
module MT = struct
  ...
  let map_signature_item sub {psig_desc= desc; psig_loc= loc} =
    ...
    | Psig_type (rf, l) ->
        (* #if false then type_ ~loc rf (List.map (sub.type_declaration sub) l)
           #else *)
...
module M = struct
  ...
  let map_structure_item sub {pstr_loc= loc; pstr_desc= desc} =
    ...
    | Pstr_value (r, vbs) ->
        (* #if false then value ~loc r (List.map (sub.value_binding sub) vbs)
           #else *)
        value ~loc r
...
module E = struct
  ...
  let map sub {pexp_loc= loc; pexp_desc= desc; pexp_attributes= attrs} =
    ...
    | Pexp_let (r, vbs, e) ->
        (* #if false then let_ ~loc ~attrs r (List.map (sub.value_binding sub)
           vbs) (sub.expr sub e) #else *)
        let_ ~loc ~attrs r
          (( if r = Recursive then sub.value_bindings_rec
           else sub.value_bindings )
             sub vbs)
...
module CE = struct
  ...
  let map sub {pcl_loc= loc; pcl_desc= desc; pcl_attributes= attrs} =
    ...
    | Pcl_let (r, vbs, ce) ->
        (* #if false then let_ ~loc ~attrs r (List.map (sub.value_binding sub)
           vbs) (sub.class_expr sub ce) #else *)
        let_ ~loc ~attrs r
          (( if r = Recursive then sub.value_bindings_rec
           else sub.value_bindings )
             sub vbs)
