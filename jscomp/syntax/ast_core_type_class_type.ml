...
let process_getter_setter ~not_getter_setter
    ~(get : Parsetree.core_type -> _ -> Parsetree.attributes -> _) ~set loc
    name (attrs : Ast_attributes.t) (ty : Parsetree.core_type) (acc : _ list) =
  ...
  | st, pctf_attributes ->
      let get_acc =
        ...
        | None | Some `Get ->
            ...
            get ty name pctf_attributes :: acc in
      ...
      let not_getter_setter (ty : Parsetree.core_type) =
        let ty =
          ...
          | Ptyp_arrow (label, args, body) ->
              Ast_util.to_method_type ty.ptyp_loc self label args body
          | Ptyp_poly
              ...
              { ty with
                ptyp_desc=
                  Ptyp_poly
                    ( strs
                    , Ast_util.to_method_type ptyp_loc self label args body )
              }
          | _ -> self.typ self ty in
        {ctf with pctf_desc= Pctf_method (name, private_flag, virtual_flag, ty)}
      in
        ...
        { ctf with
          pctf_desc=
            Pctf_method
              ...
              , private_flag
              , virtual_flag
              , Ast_util.to_method_type loc self Ast_compatible.no_label ty
...
(* Attributes are very hard to attribute (since ptyp_attributes could happen in
   so many places), and write ppx extensions correctly, we can only use it
   locally *)

let typ_mapper record_as_js_object (self : Bs_ast_mapper.mapper)
    (ty : Parsetree.core_type) =
  match ty with
  | {ptyp_desc= Ptyp_extension ({txt= "bs.obj" | "obj"}, PTyp ty)} ->
      Ext_ref.non_exn_protect record_as_js_object true (fun _ ->
          self.typ self ty)
  | { ptyp_attributes
    ; ptyp_desc= Ptyp_arrow (label, args, body)
    ; (* let it go without regard label names, it will report error later when
         the label is not empty *)
      ptyp_loc= loc } -> (
    match Ast_attributes.process_attributes_rev ptyp_attributes with
    ...
    | Nothing, _ -> Bs_ast_mapper.default_mapper.typ self ty )
            ...
            | Parsetree.Otag (label, ptyp_attrs, core_type) ->
                  ...
                  let attrs, core_type =
                    ...
                    | Method _, _ ->
                        Location.raise_errorf ~loc
                          "bs.get/set conflicts with bs.meth"
                    | Meth_callback attr, attrs -> (attrs, attr +> ty) in
                  Ast_compatible.object_field name attrs
                    (self.typ self core_type) in
                let set ty name attrs =
                  let attrs, core_type =
                    ...
                    | Meth_callback attr, attrs -> (attrs, attr +> ty) in
                  Ast_compatible.object_field name attrs
                    (Ast_util.to_method_type loc self Ast_compatible.no_label
                       core_type
                ...
                process_getter_setter ~not_getter_setter ~get ~set loc label
                  ptyp_attrs core_type acc) in
      let inner_type =
        {ty with ptyp_desc= Ptyp_object (new_methods, closed_flag)} in
      if !record_as_js_object then Ast_comb.to_js_type loc inner_type
      else inner_type
