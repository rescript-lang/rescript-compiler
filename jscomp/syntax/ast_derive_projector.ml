...
(* #if OCAML_VERSION =~ ">4.03.0" then let constructor_arguments_length (xs :
   Parsetree.constructor_arguments) = match xs with | Pcstr_tuple xs ->
   List.length xs | Pcstr_record xs -> List.length xs (* inline record FIXME*)
   #else let constructor_arguments_length = List.length #end *)
      ...
      { structure_gen=
          (fun (tdcls : tdcls) _explict_nonrec ->
            let handle_tdcl tdcl =
              let core_type =
                Ast_derive_util.core_type_of_type_declaration tdcl in
              ...
              | Ptype_variant constructor_declarations ->
                  Ext_list.map constructor_declarations
                    (fun { pcd_name= {loc; txt= con_name}
                      ...
                      let little_con_name =
                        Ext_string.uncapitalize_ascii con_name in
