...
let search upper polyvar array =
  app3
    (Exp.ident {loc= noloc; txt= Longident.Ldot (jsMapperRt, "binarySearch")})
    upper (eraseType polyvar) array

let revSearch len constantArray exp =
  app3
    (Exp.ident {loc= noloc; txt= Longident.Ldot (jsMapperRt, "revSearch")})
    len constantArray exp

let revSearchAssert len constantArray exp =
  app3
    (Exp.ident {loc= noloc; txt= Longident.Ldot (jsMapperRt, "revSearchAssert")})
    len constantArray exp

let toInt exp array =
  app2
    (Exp.ident {loc= noloc; txt= Longident.Ldot (jsMapperRt, "toInt")})
    (eraseType exp) array
...
let fromIntAssert len array exp =
  app3
    (Exp.ident {loc= noloc; txt= Longident.Ldot (jsMapperRt, "fromIntAssert")})
    len array exp

let assertExp e =
  Exp.extension ({Asttypes.loc= noloc; txt= "assert"}, PStr [Str.eval e])

let derivingName = "jsConverter"

(* let notApplicable loc = Location.prerr_warning loc
   (Warnings.Bs_derive_warning ( derivingName ^ " not applicable to this
   type")) *)
          ...
          (fun (tdcls : tdcls) _ ->
            let handle_tdcl (tdcl : Parsetree.type_declaration) =
              ...
              let coerceResultToNewType e =
                if createType then e +> newType else e in
                  ...
                  let fromJs =
                    Ast_comb.single_non_rec_value patFromJs
                      (Ast_compatible.fun_ (Pat.var pat_param)
                         ...
                         else Exp.constraint_ obj_exp core_type )) in
                    ...
                    let result : _ list =
                      Ext_list.map row_fields (fun tag ->
                          ...
                          | Rtag (label, attrs, _, []) -> (
                              ...
                              , match
                                  Ast_attributes.iter_process_bs_string_as_ast
                                    attrs
                                with
                                | Some name -> name
                                | None ->
                                    Ast_compatible.const_exp_string
                                      (Ast_compatible.label_of_name label) ) )
                          | _ -> assert false
                          (* checked by [is_enum_polyvar] *)) in
                    let result_len = List.length result in
                    let exp_len = Ast_compatible.const_exp_int result_len in
                    let v =
                      [ eraseTypeStr
                      ; Ast_comb.single_non_rec_value {loc; txt= constantArray}
                          (Exp.array
                             (Ext_list.map
                                (List.sort
                                ...
                                (fun (i, str) ->
                                  Exp.tuple
                                    [Ast_compatible.const_exp_int i; str])))
                      ; toJsBody
                          (coerceResultToNewType
                             (search exp_len exp_param expConstantArray))
                      ; Ast_comb.single_non_rec_value patFromJs
                          (Ast_compatible.fun_ (Pat.var pat_param)
                             ( if createType then
                               revSearchAssert exp_len expConstantArray
                                 (exp_param +: newType)
                               +> core_type
                             else
                               revSearch exp_len expConstantArray exp_param
                  ...
                  if Ast_polyvar.is_enum_constructors ctors then
                    ...
                    | `New xs ->
                        let constantArrayExp =
                          Exp.ident {loc; txt= Lident constantArray} in
                        let exp_len =
                        ...
                        let v =
                          ...
                          ; toJsBody
                              ( coerceResultToNewType
                              @@ toInt exp_param constantArrayExp )
                          ; Ast_comb.single_non_rec_value patFromJs
                              (Ast_compatible.fun_ (Pat.var pat_param)
                                 ( if createType then
                    ...
                    | `Offset offset ->
                        let v =
                          ...
                          ; (let len = List.length ctors in
                             let range_low =
                               Ast_compatible.const_exp_int (offset + 0) in
                             let range_upper =
            ...
            let handle_tdcl tdcl =
              ...
              let toJsType result =
                Ast_comb.single_non_rec_val patToJs
                  (Ast_compatible.arrow core_type result) in
              let newType, newTdcl =
                U.new_type_of_type_declaration tdcl ("abs_" ^ name) in
              ...
              | Ptype_abstract -> (
                ...
                | Some _ ->
                    let ty1 =
                      ...
                      else Ast_literal.type_string () in
                    let ty2 =
