...
let strip_option arg_name =
  if arg_name.[0] = '?' then String.sub arg_name 1 (String.length arg_name - 1)
  else arg_name
...
let handleTdcl light (tdcl : Parsetree.type_declaration) =
  ...
  | Ptype_record label_declarations ->
      ...
      let has_optional_field, new_label_declarations =
        Ext_list.fold_right label_declarations (false, [])
          (fun ({pld_type; pld_loc; pld_attributes} as dcl :
                 Parsetree.label_declaration)
               (has_optional_field, acc)
               ->
            let has_optional_field_local =
              Ast_attributes.has_bs_optional pld_attributes in
            let acc =
              if has_optional_field_local then
                { dcl with
                  pld_type=
                    { dcl.pld_type with
                      ptyp_desc=
                        Ptyp_constr
                          ({txt= Lident "option"; loc= pld_loc}, [pld_type])
                    ; ptyp_loc= pld_loc }
                ; pld_attributes=
                    Ext_list.exclude pld_attributes (fun x ->
                        Ast_attributes.is_optional x
                        || Ast_attributes.is_bs_as x) }
                :: acc
              else dcl :: acc in
            (has_optional_field || has_optional_field_local, acc)) in
      let newTdcl =
        { tdcl with
      ...
      let setter_accessor, makeType, labels =
        Ext_list.fold_right label_declarations
          ...
          (fun ({ pld_name= {txt= label_name; loc= label_loc} as pld_name
            ...
            let newLabel = assert false (* FIXME *) in
            let maker, getter_type =
              if is_optional then
                let maker_optional_type =
                  Ast_core_type.lift_option_type pld_type in
            ...
            let makeGetter light deprec pld_name =
              Str.value Nonrecursive
                [ Vb.mk ~loc:pld_loc
                    ~attrs:
                      ( if deprec then [deprecated pld_name.Asttypes.txt]
                      else [] )
                    (Pat.var
                       { pld_name with
                         txt= (if light then label_name else label_name ^ "Get")
                       })
                    (Exp.constraint_
                       (Ast_compatible.fun_ ~loc:pld_loc
                          ...
                          (Exp.field
                             (Exp.ident
                                { Location.txt= Longident.Lident "o"
                                ; loc= pld_loc })
                             { txt= Longident.Lident pld_name.Location.txt
                             ; loc= pld_loc }))
                       getter_type) ] in
            let acc =
              if not light then
                makeGetter true true pld_name
                :: makeGetter false false pld_name
                :: acc
            ...
            let acc =
              if is_current_field_mutable then
                ...
                let variable =
                  Exp.ident {Location.txt= Longident.Lident "v"; loc= pld_loc}
                in
                let setter =
                  Str.value Nonrecursive
                    [ Vb.mk
                              ...
                              (Ast_compatible.fun_ ~loc:pld_loc
                                 ...
                                 (Exp.setfield
                                    (Exp.ident
                                       ...
                                       ; loc= pld_loc })
                                    { txt=
                                        Longident.Lident pld_name.Location.txt
                                    ; loc= pld_loc }
                                    ( if is_optional then
                                      Exp.construct
                                        {txt= Lident "Some"; loc= pld_loc}
                                        (Some variable)
                                    else variable ))))
                           setter_type) ] in
                setter :: acc
              else acc in
            (acc, maker, newLabel :: labels)) in
      ( newTdcl
      , if is_private then setter_accessor
          ...
          let my_loc =
            match labels with
            | [] -> !default_loc
            | {Asttypes.loc= label_loc} :: _ -> label_loc in
          let maker_body =
            Exp.record
              (Ext_list.fold_right labels []
                 (fun {Asttypes.txt; loc= label_loc} rest ->
                   let field_name =
                     { Asttypes.txt= Longident.Lident (strip_option txt)
                     ; loc= label_loc } in
                   (field_name, Exp.ident field_name) :: rest))
              None in
          (* This is to support bs.optional, which makes certain args of the
             function optional so we add a unit at the end to prevent
             auto-currying issues. *)
          let body_with_extra_unit_fun =
            if has_optional_field then
              Ast_compatible.fun_ ~loc:my_loc
                (Pat.var {txt= "()"; loc= my_loc})
                maker_body
            else maker_body in
          let myMaker = assert false in
    ...
    ; pstr_desc=
        Pstr_value
          ...
          , [ ( { pvb_pat= {ppat_desc= Ppat_var name}
                ; pvb_expr= {pexp_desc= Pexp_constraint (_, typ)} } as _makerVb
              ) ] ) } ->
...
let handleTdclsInStr ~light tdcls =
  let tdcls, tdcls_sig, code, code_sig =
    Ext_list.fold_right tdcls ([], [], [], [])
      (fun tdcl (tdcls, tdcls_sig, sts, code_sig) ->
        ...
        | ntdcl, value_descriptions ->
            ...
            , Ext_list.map_append value_descriptions code_sig
                code_sig_transform )) in
