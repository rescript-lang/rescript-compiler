...
let isAbstract (xs : Ast_payload.action list) =
  match xs with
  | [({loc; txt= "abstract"}, None)] -> Complex_abstract
  | [ ( {loc; txt= "abstract"}
...
(* let handle_config (config : Parsetree.expression option) = match config with
   | Some config -> U.invalid_config config | None -> () *)

(** For this attributes, its type was wrapped as an option, so we can still
    reuse existing frame work *)
...
let deprecated name =
  Ast_attributes.deprecated
    ( "use " ^ name
    ^ "Get instead, or swith to the new [@bs.deriving {abstract: light}] for \
       the type declaration (OCaml syntax: [@@bs.deriving {abstract = light}])"
    )
...
let handleTdcl light (tdcl : Parsetree.type_declaration) :
  ...
  let newTdcl =
    ...
    ; ptype_attributes= [] (* avoid non-terminating*) } in
  ...
  | Ptype_record label_declarations ->
      ...
      let has_optional_field =
        Ext_list.exists label_declarations (fun {pld_type; pld_attributes} ->
            Ast_attributes.has_bs_optional pld_attributes) in
      let setter_accessor, makeType, labels =
        Ext_list.fold_right label_declarations
          ...
          , [] )
          (fun ({ pld_name= {txt= label_name; loc= label_loc} as pld_name
                ; pld_type
                ; pld_mutable
                ; pld_attributes
                ; pld_loc } :
            ...
            let prim_as_name, newLabel =
              match
                Ast_attributes.iter_process_bs_string_as pld_attributes
              with
            ...
            let maker, acc =
              if is_optional then
                ...
                ( Ast_compatible.opt_arrow ~loc:pld_loc label_name pld_type
                    maker
                , let aux light deprec pld_name : Parsetree.value_description =
                    Val.mk ~loc:pld_loc
                      ...
                      ~attrs:
                        ( if deprec then
                          deprecated pld_name.Asttypes.txt
                          :: get_optional_attrs
                        else get_optional_attrs )
                      ~prim
                      (Ast_compatible.arrow ~loc core_type optional_type) in
                  if not light then
                ...
                , let aux light deprec pld_name =
                    Val.mk ~loc:pld_loc
                      ...
                      ~attrs:
                        ( if deprec then
                          deprecated pld_name.Asttypes.txt :: get_attrs
                        else get_attrs )
                      ~prim:
                        [ ""
                        ; (* Not needed actually*)
                          External_ffi_types.to_string
                            (Ffi_bs
                               ( [ { arg_type= Nothing
                                   ; arg_label= External_arg_spec.empty_label
                                   } ]
                               , Return_identity
                               , Js_get
                                   { js_get_name= prim_as_name
                                   ; js_get_scopes= [] } )) ]
                      (Ast_compatible.arrow ~loc core_type pld_type) in
                  if not light then
                    aux true true pld_name :: aux false false pld_name :: acc
                  else aux true false pld_name :: acc ) in
            ...
            (acc, maker, (is_optional, newLabel) :: labels)) in
      ( newTdcl
      , if is_private then setter_accessor
        else
          let myPrims =
            Ast_external_process.pval_prim_of_option_labels labels
              has_optional_field in
          let myMaker =
            Val.mk ~loc {loc; txt= type_name} ~prim:myPrims makeType in
