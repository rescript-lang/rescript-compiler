...
let mapU q f =
  copyMapAux
    (t ~length:(lengthGet q) ~first:null ~last:null)
    null (firstGet q) f

let map q f = mapU q (fun [@bs] a -> f a)
let isEmpty q = lengthGet q = 0
let size q = lengthGet q
...
let forEachU q f = iterAux (firstGet q) f
...
let reduceU q accu f = foldAux f accu (firstGet q)
