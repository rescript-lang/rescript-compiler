...
let remove d v =
  ...
  | Some oldRoot2 ->
      let newRoot =
        remove0 ~cmp:(Belt_Id.getCmpInternal (S.cmpGet d)) oldRoot2 v in
...
let removeMany d xs =
  ...
  | Some nt ->
      ...
      S.dataSet d
        (removeMany0 nt xs 0 len ~cmp:(Belt_Id.getCmpInternal (S.cmpGet d)))
...
let addCheck m e =
  ...
  let newRoot =
    addCheck0 ~cmp:(Belt_Id.getCmpInternal (S.cmpGet m)) oldRoot e added in
...
let isEmpty d = N.isEmpty (S.dataGet d)
let minimum d = N.minimum (S.dataGet d)
let minUndefined d = N.minUndefined (S.dataGet d)
let maximum d = N.maximum (S.dataGet d)
let maxUndefined d = N.maxUndefined (S.dataGet d)
...
let size d = N.size (S.dataGet d)
let toList d = N.toList (S.dataGet d)
let toArray d = N.toArray (S.dataGet d)
...
let checkInvariantInternal d = N.checkInvariantInternal (S.dataGet d)
...
let cmp d0 d1 = N.cmp ~cmp:(S.cmpGet d0) (S.dataGet d0) (S.dataGet d1)
let eq d0 d1 = N.eq ~cmp:(S.cmpGet d0) (S.dataGet d0) (S.dataGet d1)
let get d x = N.get ~cmp:(S.cmpGet d) (S.dataGet d) x
let getUndefined d x = N.getUndefined ~cmp:(S.cmpGet d) (S.dataGet d) x
let getExn d x = N.getExn ~cmp:(S.cmpGet d) (S.dataGet d) x
...
let keepU d p = S.t ~data:(N.keepCopyU (S.dataGet d) p) ~cmp:(S.cmpGet d)
...
let subset a b = N.subset ~cmp:(S.cmpGet a) (S.dataGet a) (S.dataGet b)
