...
let remove m e =
  ...
  if newData == data then m else t ~cmp ~data:newData
...
let eq m n = Dict.eq ~cmp:(cmpGet m) (dataGet m) (dataGet n)
...
let keepU m f = t ~cmp:(cmpGet m) ~data:(Dict.keepU (dataGet m) f)
...
let get m e = Dict.get ~cmp:(cmpGet m) (dataGet m) e
let getUndefined m e = Dict.getUndefined ~cmp:(cmpGet m) (dataGet m) e
let getExn m e = Dict.getExn ~cmp:(cmpGet m) (dataGet m) e
let has m e = Dict.has ~cmp:(cmpGet m) (dataGet m) e
...
let getId (type value identity) (m : (value, identity) t) :
    (value, identity) id =
  let module T = struct
