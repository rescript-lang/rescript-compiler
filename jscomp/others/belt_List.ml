...
(* perf is not everything, there are better memory represenations {[ type 'a
   cell = { mutable head : 'a; mutable tail : 'a opt_cell }

   and 'a opt_cell = 'a cell Js.null

   and 'a t = { length : int ; data : 'a opt_cell } ]} However, - people use
   List not because of its perf, but its convenencie, in that case, pattern
   match and compatibility seems more attractive, we could keep a mutable list
   - The built in types would indicate that its construtor is immutable, a
   better optimizer would break such code

   {[ type 'a t = { head : 'a; mutable tail : 'a t | int } ]} In the future, we
   could come up with a safer version {[ type 'a t = | Nil | Cons of { hd : 'a
   ; mutable tail : 'a t } ]} *)
...
(* [mutableCell x []] == [x] but tell the compiler that is a mutable cell, so
   it wont be mis-inlined in the future dont inline a binding to mutable cell,
   it is mutable *)
external unsafeMutateTail : 'a t -> 'a t -> unit = "#setfield1"

(* - the cell is not empty - it is mutated *)
external unsafeTail : 'a t -> 'a t = "%field1"

(* - the cell is not empty *)

let head x = match x with [] -> None | x :: _ -> Some x
let headExn x = match x with [] -> [%assert "headExn"] | x :: _ -> x
let tail x = match x with [] -> None | _ :: xs -> Some xs
let tailExn x = match x with [] -> [%assert "tailExn"] | _ :: t -> t
let add xs x = x :: xs
...
let getExn x n = if n < 0 then [%assert "getExn"] else nthAuxAssert x n
...
(* return the tail pointer so it can continue copy other list *)
...
let rec removeAssocAuxWithMap cellX x prec f =
  ...
  | [] -> false
  | ((a, _) as h) :: t ->
      if (f a x [@bs]) then (unsafeMutateTail prec t ; true)
      else
        let next = mutableCell h [] in
        unsafeMutateTail prec next ;
...
let mapWithIndexU xs f =
  ...
  | h :: t ->
      ...
      copyAuxWithMapI f 1 t cell ; cell
...
let shuffle xs =
  let v = toArray xs in
  A.shuffleInPlace v ; fromArray v
...
(* TODO: best practice about raising excpetion 1. raise OCaml exception, no
   stacktrace 2. raise JS exception, how to pattern match *)
...
let rec forEachU xs f =
  match xs with [] -> () | a :: l -> (f a [@bs]) ; forEachU l f
...
let rec keepU xs p =
  ...
  | h :: t ->
      if (p h [@bs]) then (
        let cell = mutableCell h [] in
        copyAuxWitFilter p t cell ; cell )
      else keepU t p
  ...
  let rec auxKeepWithIndex xs p i =
    ...
    | h :: t ->
        if (p h i [@bs]) then (
          ...
          cell )
        else auxKeepWithIndex t p (i + 1) in
