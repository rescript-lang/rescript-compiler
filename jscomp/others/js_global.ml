...
(** Contains functions available in the global scope ([window] in a browser
    context) *)
...
external setInterval : (unit -> unit) -> int -> intervalId = "setInterval"
  [@@bs.val]
(** {i Repeatedly} executes a callback with a specified interval (in
    milliseconds) between calls

    {b Return} an {!intervalId} that can be passed to {!clearInterval} to
    cancel the timeout
    @see
...
(** Execute a callback after a specified delay (in milliseconds)
    ...
    {b returns} a {!timeoutId} that can be passed to {!clearTimeout} to cancel
    the timeout
    @see
