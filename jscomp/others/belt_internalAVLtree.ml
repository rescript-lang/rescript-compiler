...
and ('key, 'a) t = ('key, 'a) node Js.null [@@bs.deriving abstract]
...
let rec copy n =
  ...
  | Some n ->
      ...
      @@ node ~left:(copy l) ~right:(copy r) ~value:(valueGet n)
           ~key:(keyGet n) ~height:(heightGet n)
...
let updateValue n newValue =
  if valueGet n == newValue then n
    ...
    node ~left:(leftGet n) ~right:(rightGet n) ~key:(keyGet n) ~value:newValue
      ~height:(heightGet n)
...
let bal l x d r =
  ...
  if hl > hr + 2 then
    let ll, lv, ld, lr =
      l |. unsafeCoerce |. (leftGet, keyGet, valueGet, rightGet) in
    if treeHeight ll >= treeHeight lr then create ll lv ld (create lr x d r)
    else
      let lrl, lrv, lrd, lrr =
        lr |. unsafeCoerce |. (leftGet, keyGet, valueGet, rightGet) in
      create (create ll lv ld lrl) lrv lrd (create lrr x d r)
  else if hr > hl + 2 then
    let rl, rv, rd, rr =
      r |. unsafeCoerce |. (leftGet, keyGet, valueGet, rightGet) in
    if treeHeight rr >= treeHeight rl then create (create l x d rl) rv rd rr
    else
      let rll, rlv, rld, rlr =
        rl |. unsafeCoerce |. (leftGet, keyGet, valueGet, rightGet) in
...
let rec minKey0Aux n =
  match toOpt (leftGet n) with None -> keyGet n | Some n -> minKey0Aux n
...
let rec maxKey0Aux n =
  match toOpt (rightGet n) with None -> keyGet n | Some n -> maxKey0Aux n
...
let rec mapU n f =
  ...
  | Some n ->
      ...
      return
      @@ node ~left:newLeft ~key:(keyGet n) ~value:newD ~right:newRight
           ~height:(heightGet n)
...
let rec join ln v d rn =
  ...
  | Some l, Some r ->
      let ll, lv, ld, lr, lh =
        l |. (leftGet, keyGet, valueGet, rightGet, heightGet) in
      let rl, rv, rd, rr, rh =
        r |. (leftGet, keyGet, valueGet, rightGet, heightGet) in
...
let rec eqAux e1 e2 ~kcmp ~veq =
  ...
  | h1 :: t1, h2 :: t2 ->
      if
        ((Belt_Id.getCmpInternal kcmp) (keyGet h1) (keyGet h2) [@bs]) = 0
        && (veq (valueGet h1) (valueGet h2) [@bs])
      then
        eqAux ~kcmp ~veq
          (stackAllLeft (rightGet h1) t1)
          (stackAllLeft (rightGet h2) t2)
...
let rec updateMutate (t : _ t) x data ~cmp =
  ...
  | Some nt ->
      ...
      if c = 0 then (valueSet nt data ; return nt)
      else
        let l, r = (leftGet nt, rightGet nt) in
        if c < 0 then
    ...
    let next =
      ref
        (S.strictlySortedLengthU xs (fun [@bs] (x0, _) (y0, _) ->
             ((Belt_Id.getCmpInternal cmp) x0 y0 [@bs]) < 0)) in
    let result =
      ref
