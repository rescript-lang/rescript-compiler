...
and 'value t = 'value node Js.null [@@bs.deriving abstract]
...
let treeHeight (n : _ t) =
  match toOpt n with None -> 0 | Some n -> heightGet n
...
let rec copy n =
  ...
  | Some n ->
      ...
      node ~left:(copy l) ~right:(copy r) ~value:(valueGet n)
        ~height:(heightGet n)
      |. return
...
(* Creates a new node with leftGet son l, value v and right son r. We must have
   ...
   treeHeight l - treeHeight r | <= 2. Inline expansion of treeHeight for
   better speed. *)
...
(* Same as create, but performs one step of rebalancing if necessary. Assumes l
   and r balanced and | treeHeight l - treeHeight r | <= 3. Inline expansion of
...
let rec min0Aux n =
  match toOpt (leftGet n) with None -> valueGet n | Some n -> min0Aux n
...
let rec max0Aux n =
  match toOpt (rightGet n) with None -> valueGet n | Some n -> max0Aux n
...
let rec stackAllLeft v s =
  match toOpt v with None -> s | Some x -> stackAllLeft (leftGet x) (x :: s)

let rec forEachU n f =
  match toOpt n with
  | None -> ()
  | Some n ->
      forEachU (leftGet n) f ;
      (f (valueGet n) [@bs]) ;
      forEachU (rightGet n) f
...
let rec reduceU s accu f =
  ...
  | Some n ->
      ...
      reduceU r (f (reduceU l accu f) k [@bs]) f
...
(* [addMinElement v n] and [addMaxElement v n] assume that the added v is
   *strictly* smaller (or bigger) than all the present elements in the tree.
   They are only used during the "join" operation which respects this
   precondition. *)

let rec addMinElement n v =
  match toOpt n with
  | None -> singleton v
  | Some n -> bal (addMinElement (leftGet n) v) (valueGet n) (rightGet n)

let rec addMaxElement n v =
  match toOpt n with
  | None -> singleton v
  | Some n -> bal (leftGet n) (valueGet n) (addMaxElement (rightGet n) v)

(* [join ln v rn] return a balanced tree simliar to [create ln v rn] bal, but
   no assumptions are made on the relative heights of [ln] and [rn]. *)
...
let rec joinShared ln v rn =
  ...
  | Some l, Some r ->
      ...
      if lh > rh + 2 then
        bal (leftGet l) (valueGet l) (joinShared (rightGet l) v rn)
      else if rh > lh + 2 then
        bal (joinShared ln v (leftGet r)) (valueGet r) (rightGet r)
      else create ln v rn

(* [concat l r] No assumption on the heights of l and r. *)
...
let rec toListAux n accu =
  match toOpt n with
  | None -> accu
  | Some n -> toListAux (leftGet n) (valueGet n :: toListAux (rightGet n) accu)
...
let rec fillArrayWithFilter n i arr p =
  ...
  in
  let rnext =
    if (p v [@bs]) then (A.setUnsafe arr next v ; next + 1) else next in
  match toOpt r with
  | None -> rnext
...
(* ATT: functional methods in general can be shared with imperative methods,
   however, it does not apply when functional methods makes use of referential
   equality *)
...
let rec compareAux e1 e2 ~cmp =
  ...
  | h1 :: t1, h2 :: t2 ->
      let c =
        ((Belt_Id.getCmpInternal cmp) (valueGet h1) (valueGet h2) [@bs]) in
      if c = 0 then
        compareAux ~cmp
...
let rotateWithLeftChild k2 =
  ...
  let hlk2, hrk2 =
    (k2 |. leftGet |. treeHeight, k2 |. rightGet |. treeHeight) in
...
let rotateWithRightChild k1 =
  ...
  let hlk1, hrk1 =
    (k1 |. leftGet |. treeHeight, k1 |. rightGet |. treeHeight) in
    ...
    let next =
      ref
        (S.strictlySortedLengthU xs (fun [@bs] x y ->
             ((Belt_Id.getCmpInternal cmp) x y [@bs]) < 0)) in
    let result =
      ref
