...
(* For JS backends, we use [undefined] as default value, so that buckets could
   be allocated lazily *)
...
(* TODO: the current implementation relies on the fact that bucket empty value
   is [undefined] in both places, in theory, it can be different *)
type ('a, 'b) bucket =
  {mutable key: 'a; mutable value: 'b; mutable next: ('a, 'b) bucket C.opt}
...
let rec copy (x : _ t) : _ t =
  C.container ~hash:(C.hashGet x) ~eq:(C.eqGet x) ~size:(C.sizeGet x)
...
and copyBucket c =
  ...
  | None -> c
  | Some c ->
      let head = bucket ~key:(keyGet c) ~value:(valueGet c) ~next:C.emptyOpt in
...
let rec do_bucket_iter ~f buckets =
  ...
  | None -> ()
  | Some cell ->
      (f (keyGet cell) (valueGet cell) [@bs]) ;
      do_bucket_iter ~f (nextGet cell)
...
let rec do_bucket_fold ~f b accu =
  ...
  | None -> accu
  | Some cell ->
      do_bucket_fold ~f (nextGet cell)
        (f accu (keyGet cell) (valueGet cell) [@bs])
...
let getMaxBucketLength h =
  A.reduceU (C.bucketsGet h) 0 (fun [@bs] m b ->
...
let getBucketHistogram h =
  ...
  A.forEachU (C.bucketsGet h) (fun [@bs] b ->
...
let toArray h =
  ...
  for i = 0 to A.length d - 1 do
    ...
    | None -> ()
    | Some cell -> current := fillArray !current arr cell
  done ;
  arr
...
let linear h f =
  ...
  for i = 0 to A.length d - 1 do
    ...
    match C.toOpt cell with
    | None -> ()
    | Some cell -> current := fillArrayMap !current arr cell f
  done ;
  arr
