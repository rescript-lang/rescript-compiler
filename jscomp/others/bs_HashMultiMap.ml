...
let rec replaceFirst ~eq  key info cell = 
  ...
  then
    ...
    | None -> true 
    | Some cell -> 
      replaceFirst ~eq key info cell

(*
    Since we are replacing the first one in the bucket, 
    it requires when adding new element, the newer comes in
    the head, so that replace will replace the latest one
    [add tbl 10 '0'; add tbl 10 '1']
    the replacement will replace [10,'1']
*)
let replaceBy h key value ~eq ~hash =       
  let h_buckets = C.buckets h in 
  let buckets_len = A.length h_buckets in 
  let hash, eq = Bs_Dict.getHashInternal hash , Bs_Dict.getEqInternal eq in 
  ...
  | Some bucket -> 
      if replaceFirst ~eq key value bucket then begin
...
let replace h key value =
  replaceBy h key value ~eq:(C.eq h) ~hash:(C.hash h)
...
(*TODO: getAll stack-safe *)
(* let rec findAlInBucket buckets key ~eq = 
    match C.toOpt buckets with 
    | None ->
      []
    | Some cell ->
      if eq (N.key cell) key [@bs]
      then (N.value cell) :: findAlInBucket (N.next cell)
      else findAlInBucket (N.next cell) 
              
let getAll0 ~hash ~eq h key =
  let h_buckets = C.buckets h in     
  let nid = (Bs_Dict.getHashInternal hash) key [@bs] land (A.length h_buckets - 1) in 
  findAlInBucket (A.getUnsafe h_buckets nid)
...
let add0 h key value ~hash  =
  let h_buckets = C.buckets h in  
  let buckets_lenth = A.length h_buckets in 
  let i = hash key [@bs] land (buckets_lenth - 1) in 
  let bucket = 
  ...
  if h_new_size > buckets_lenth lsl 1 then resize ~hash  h


let add  h key info  = 
  add0 ~hash:(Bs_Dict.getHashInternal (C.hash h)) h key info     
...
let ofArray (type a) (type id) arr ~dict:(dict:(a,id) dict) =     
  ...
  let hash = Bs_Dict.getHashInternal hash in   
  for i = 0 to len - 1 do 
    let key,value = (A.getUnsafe arr i) in 
    add0 ~hash v key value
  done ;
  v
      
let mergeMany h arr = 
  let hash = Bs_Dict.getHashInternal ( C.hash h)  in 
  let len = A.length arr in 
  for i = 0 to len - 1 do 
    let key,value = (A.getUnsafe arr i) in 
    add0 h  ~hash key value
  done
