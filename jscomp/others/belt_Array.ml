...
external getUndefined : 'a array -> int -> 'a Js.undefined
  = "%array_unsafe_get"
...
(* See #6575. We could also check for maximum array size, but this depends on
   whether we create a float array or a regular one... *)
let makeByU l f =
  if l <= 0 then [||]
  else
    let res = makeUninitializedUnsafe l in
...
(* We don't need check [blitLength] since when [blitLength < 0] the for loop
   will be nop *)
...
let keepU a f =
  ...
  for i = 0 to l - 1 do
    let v = getUnsafe a i in
    if (f v [@bs]) then (setUnsafe r !j v ; incr j)
  done ;
...
let keepWithIndexU a f =
  ...
  for i = 0 to l - 1 do
    let v = getUnsafe a i in
    if (f v i [@bs]) then (setUnsafe r !j v ; incr j)
  done ;
  truncateToLengthUnsafe r !j ;
  r
...
let keepMapU a f =
  ...
  for i = 0 to l - 1 do
    let v = getUnsafe a i in
    match (f v [@bs]) with None -> () | Some v -> setUnsafe r !j v ; incr j
  done ;
  truncateToLengthUnsafe r !j ;
  r
...
let rec everyAux arr i b len =
  if i = len then true
  else if (b (getUnsafe arr i) [@bs]) then everyAux arr (i + 1) b len
  else false

let rec someAux arr i b len =
  if i = len then false
  else if (b (getUnsafe arr i) [@bs]) then true
...
let rec someAux2 arr1 arr2 i b len =
  if i = len then false
  else if (b (getUnsafe arr1 i) (getUnsafe arr2 i) [@bs]) then true
...
let partitionU a f =
  ...
  for ii = 0 to l - 1 do
    ...
    if (f v [@bs]) then (setUnsafe a1 !i v ; incr i)
...
let unzip a =
  ...
  for i = 0 to l - 1 do
    ...
    setUnsafe a1 i v1 ; setUnsafe a2 i v2
  done ;
  (a1, a2)
