...
(** This module is {!Belt.MutableSet} specialized with key type to be a
    primitive type. It is more efficient in general, the API is the same with
    {!Belt_MutableSet} except its key type is fixed, and identity is not
    needed(using the built-in one) *)
...
type t = {mutable data: I.t} [@@bs.deriving abstract]
(** The type of sets. *)
...
let mergeMany d arr = dataSet d (addArrayMutate (dataGet d) arr)
let make () = t ~data:N.empty
let isEmpty d = N.isEmpty (dataGet d)
let minimum d = N.minimum (dataGet d)
let minUndefined d = N.minUndefined (dataGet d)
...
let cmp d0 d1 = I.cmp (dataGet d0) (dataGet d1)
let eq d0 d1 = I.eq (dataGet d0) (dataGet d1)
...
let keepU d p = t ~data:(N.keepCopyU (dataGet d) p)
