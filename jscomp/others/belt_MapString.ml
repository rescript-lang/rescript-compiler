...
let rec set t (newK : key) (newD : _) =
  ...
  | Some n ->
      ...
      if newK = k then N.return (N.updateValue n newD)
        ...
        if newK < k then N.bal (set (N.leftGet n) newK newD) k v (N.rightGet n)
        else N.bal (N.leftGet n) k v (set (N.rightGet n) newK newD)

let rec updateU t (x : key) f =
  match N.toOpt t with
  | None -> (
    match (f None [@bs]) with None -> t | Some data -> N.singleton x data )
  | Some n ->
      let k = N.keyGet n in
      if x = k then
...
let rec removeAux n (x : key) =
  ...
  if x = v then
    ...
    | Some left ->
        ...
        if ll == l then N.return n else N.(bal ll v (valueGet n) r)
  else
