...
let keysToArray d = N.keysToArray (S.dataGet d)
let valuesToArray d = N.valuesToArray (S.dataGet d)
...
let mapU m f = S.t ~cmp:(S.cmpGet m) ~data:(N.mapU (S.dataGet m) f)
...
let get m x = N.get ~cmp:(S.cmpGet m) (S.dataGet m) x
let getUndefined m x = N.getUndefined ~cmp:(S.cmpGet m) (S.dataGet m) x

let getWithDefault m x def =
  N.getWithDefault ~cmp:(S.cmpGet m) (S.dataGet m) x def

let getExn m x = N.getExn ~cmp:(S.cmpGet m) (S.dataGet m) x
let has m x = N.has ~cmp:(S.cmpGet m) (S.dataGet m) x
