...
let rec replaceInBucket ~eq key info cell =
  if (eq (N.keyGet cell) key [@bs]) then (N.valueSet cell info ; false)
  else
...
(* if [key] already exists, replace it, otherwise add it Here we add it to the
   head, it could be tail *)
let set h key value =
  set0 h key value
...
let remove h key =
  ...
  | Some cell ->
      ...
      if (eq (N.keyGet cell) key [@bs]) then (
        ...
        C.sizeSet h (C.sizeGet h - 1) )
      else removeInBucket ~eq h h_buckets i key cell (N.nextGet cell)
...
let get h key =
  ...
  | Some cell1 -> (
      let eq = Belt_Id.getEqInternal (C.eqGet h) in
      if (eq key (N.keyGet cell1) [@bs]) then Some (N.valueGet cell1)
      else
        match C.toOpt (N.nextGet cell1) with
        | None -> None
        | Some cell2 -> (
            if (eq key (N.keyGet cell2) [@bs]) then Some (N.valueGet cell2)
            else
              ...
              | Some cell3 ->
                  ...
                  else getAux ~eq key (N.nextGet cell3) ) )
...
let mergeMany h arr =
  let hash, eq =
    (Belt_Id.getHashInternal (C.hashGet h), Belt_Id.getEqInternal (C.eqGet h))
  in
  let len = A.length arr in
  for i = 0 to len - 1 do
    let key, value = A.getUnsafe arr i in
