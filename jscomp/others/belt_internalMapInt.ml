...
let rec add t (x : key) (data : _) =
  ...
  | Some n ->
      ...
      if x = k then N.return (N.updateValue n data)
        ...
        if x < k then N.bal (add (N.leftGet n) x data) k v (N.rightGet n)
        else N.bal (N.leftGet n) k v (add (N.rightGet n) x data)
  ...
  | Some n ->
      ...
      if x = v then
        ...
        | _, Some rn ->
            ...
            N.bal l !kr !vr r
      else if x < v then N.(bal (remove l x) v (valueGet n) r)
      else N.(bal l v (valueGet n) (remove r x))
...
let rec eqAux e1 e2 eq =
  ...
  | h1 :: t1, h2 :: t2 ->
      if
        (N.keyGet h1 : key) = N.keyGet h2
        && (eq (N.valueGet h1) (N.valueGet h2) [@bs])
      then
        eqAux
          (N.stackAllLeft (N.rightGet h1) t1)
          (N.stackAllLeft (N.rightGet h2) t2)
...
let rec addMutate (t : _ t) x data : _ t =
  ...
  | Some nt ->
      ...
      if x = k then (N.keySet nt x ; N.valueSet nt data ; N.return nt)
      else
...
let fromArray (xs : (key * _) array) =
  ...
  if len = 0 then N.empty
    ...
    let next =
      ref (S.strictlySortedLengthU xs (fun [@bs] (x0, _) (y0, _) -> x0 < y0))
    in
    let result =
      ref
