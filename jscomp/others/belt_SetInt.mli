...
(** This module is {!Belt.Set} specialized with value type to be a primitive
    type. It is more efficient in general, the API is the same with {!Belt_Set}
    except its value type is fixed, and identity is not needed(using the
    built-in one)
...
(** [forEach s f] applies [f] in turn to all elements of [s]. In increasing
    order *)
...
(** [some p s] checks if at least one element of the set satisfies the
    predicate [p]. Oder unspecified. *)

val keepU : t -> ((value -> bool)[@bs]) -> t

val keep : t -> (value -> bool) -> t
(** [keep p s] returns the set of all elements in [s] that satisfy predicate
    [p]. *)
...
(** [split x s] returns a triple [(l, present, r)], where [l] is the set of
    ...
    contains no element equal to [x], or [true] if [s] contains an element
    equal to [x]. *)
