...
(** [Belt.Result] is a data type with two variants: [Ok] and [Error]. Each of
    these variants can contain data, and those two pieces of data need not have
    the same data type. [Belt.Result] is useful when you need to not only
    determine whether some data is valid or not (use [Belt.Option] for that),
    but also keep information about the invalid data.
...
(** [getExn res]
    ...
    when [res] is [Ok n], returns [n] when [res] is [Error m], {b raise} an
    exception
    @example {[
    getExn good = 42
...
(** [map res f]
    ...
    When [res] is [Ok n], returns [Ok (f n)]. Otherwise returns [res]
    unchanged. Function [f] takes a value of the same type as [n] and returns
    an ordinary value.
    @example
    {[
...
(** [flatMap res f]
    ...
    Function [f] takes a value of the same type as [n] and returns a
    [Belt.Result].
    @example
    {[
let recip x = 
...
(** [isOk res]
    ...
    Returns [true] if [res] is of the form [Ok n], [false] if it is the
    [Error e] variant. *)

val isError : ('a, 'b) t -> bool
(** [isError res]

    Returns [true] if [res] is of the form [Error e], [false] if it is the
    [Ok n] variant. *)
...
(** [eq res1 res2 f]
    ...
    Determine if two [Belt.Result] variables are equal with respect to an
    equality function. If [res1] and [res2] are of the form [Ok n] and [Ok m],
    return the result of [f n m]. If one of [res1] and [res2] are of the form
    [Error e], return false If both [res1] and [res2] are of the form
    [Error e], return true
    @example
    {[
    let good1 = Ok 42
...
(** [cmp res1 res2 f]
    ...
    The comparison function returns -1 if the first variable is "less than" the
    second, 0 if the two variables are equal, and 1 if the first is "greater
    than" the second.

    If [res1] and [res2] are of the form [Ok n] and [Ok m], return the result
    of [f n m]. If [res1] is of the form [Error e] and [res2] of the form
    [Ok n], return -1 (nothing is less than something) If [res1] is of the form
    [Ok n] and [res2] of the form [Error e], return 1 (something is greater
    than nothing) If both [res1] and [res2] are of the form [Error e], return 0
    (equal)
    @example
    {[
    let good1 = Ok 59
