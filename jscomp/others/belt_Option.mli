...
(** [getExn optionalValue] Returns [value] if [optionalValue] is [Some value],
    otherwise raises [getExn]
    @example
    {[
    getExn (Some 3) = 3;;
...
(** [map optionValue f]
    ...
    If [optionValue] is [Some value], returns [Some (f value)]; otherwise
    returns [None]
    @example
    {[
...
(** [flatMap optionValue f]
    ...
    If [optionValue] is [Some value], returns [f value]; otherwise returns
    [None] The function [f] must have a return type of ['a option]
    @example
    {[
    let f (x : float) =
        if x >= 0.0 then
...
(** [cmp optValue1 optvalue2 comparisonFcn]
    ...
    [comparisonFcn value1 value2]; [comparisonFcn] takes two arguments and
    returns -1 if the first argument is less than the second, 0 if the
    arguments are equal, and 1 if the first argument is greater than the
    second.
    @example
    {[
