...
let rec replaceInBucket (key : key) info cell =
  if N.keyGet cell = key then (N.valueSet cell info ; false)
  else
...
let remove h key =
  ...
  | Some cell ->
      if N.keyGet cell = key then (
        ...
        C.sizeSet h (C.sizeGet h - 1) )
      else removeInBucket h h_buckets i key cell (N.nextGet cell)
...
let get h (key : key) =
  ...
  | None -> None
  | Some cell1 -> (
      if key = N.keyGet cell1 then Some (N.valueGet cell1)
      else
        match C.toOpt (N.nextGet cell1) with
        | None -> None
        | Some cell2 -> (
            if key = N.keyGet cell2 then Some (N.valueGet cell2)
            else
