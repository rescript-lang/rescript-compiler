...
(** A stdlib shipped with BuckleScript
    ...
    {ol
     ...
     {- Exception thrown functions are all suffixed with {i Exn}, e.g,
        {i getExn}}
    ...
    to people who have a background in functional programming.

    {b A special encoding for collection safety}

    When we create a collection library for a custom data type we need a way to
    provide a comparator function. Take {i Set} for example, suppose its
    element type is a pair of ints, it needs a custom {i compare} function that
    takes two tuples and returns their order. The {i Set} could not just be
    typed as [Set.t (int * int)], its customized {i compare} function needs to
    manifest itself in the signature, otherwise, if the user creates another
    ...
    creates a big closure at runtime and makes dead code elimination much
    harder. We use a phantom type to solve the problem:
    ...
    import, {i Belt.Set}, we may hide the internal, {i i.e}, {i Belt_Set} in
    the future *)
...
(** {!Belt.Id}
    ...
    It create a unique identifier per module of functions so that different
    data structures with slightly different comparison functions won't mix *)
...
(** {!Belt.SortArray}
    ...
    It also has two specialized inner modules {!Belt.SortArray.Int} and
...
(** {!Belt.Set}
    ...
    It also has three specialized inner modules {!Belt.Set.Int},
    {!Belt.Set.String} and

    {!Belt.Set.Dict}: This module separates data from function which is more
    verbose but slightly more efficient *)
...
(** {!Belt.Map},
    ...
    {!Belt.Map.Dict}: This module separates data from function which is more
    verbose but slightly more efficient *)
...
(** {!Belt.MutableSet}
    ...
    It also has two specialized inner modules {!Belt.MutableSet.Int} and
...
(** {!Belt.MutableMap}
    ...
    It also has two specialized inner modules {!Belt.MutableMap.Int} and
...
(** {!Belt.HashSet}
    ...
    It also has two specialized inner modules {!Belt.HashSet.Int} and
    {!Belt.HashSet.String} *)
...
(** {!Belt.HashMap}
    ...
    It also has two specialized inner modules {!Belt.HashMap.Int} and
    {!Belt.HashMap.String} *)
