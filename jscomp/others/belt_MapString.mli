...
(** [eq m1 m2] tests whether the maps [m1] and [m2] are equal, that is, contain
...
(** [findFirstBy m p] uses funcion [f] to find the first key value pair to
    match predicate [p].
    @example
    {[
...
(** [forEach m f] applies [f] to all bindings in map [m]. [f] receives the key
...
(** [remove m x] returns a map containing the same bindings as [m], except for
...
val mergeU :
  'v t -> 'v2 t -> ((key -> 'v option -> 'v2 option -> 'c option)[@bs]) -> 'c t

val merge :
  'v t -> 'v2 t -> (key -> 'v option -> 'v2 option -> 'c option) -> 'c t
(** [merge m1 m2 f] computes a map whose keys is a subset of keys of [m1] and
    of [m2]. The presence of each such binding, and the corresponding value, is
...
val partitionU : 'v t -> ((key -> 'v -> bool)[@bs]) -> 'v t * 'v t

val partition : 'v t -> (key -> 'v -> bool) -> 'v t * 'v t
(** [partition m p] returns a pair of maps [(m1, m2)], where [m1] contains all
