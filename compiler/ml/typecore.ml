...
let rec type_approx env sexp =
  ...
  | Pexp_fun (p, _, _, e) ->
    ...
    newty (Tarrow (p, ty, type_approx env e, Cok, None))
  | Pexp_function ({pc_rhs = e} :: _) ->
    newty (Tarrow (Nolabel, newvar (), type_approx env e, Cok, None))
  ...
  re
    {
      ...
      exp_loc = loc;
      exp_extra = [];
      exp_type = instance env (newgenty (Tarrow (l, ty_arg, ty_res, Cok, None)));
...
and type_argument ?type_clash_context ?recarg env sarg ty_expected' ty_expected
  ...
  in
  match expand_head env ty_expected' with
  | {desc = Tarrow (Nolabel, ty_arg, ty_res, _, _); level = _}
