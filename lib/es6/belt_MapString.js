...
function updateU(t, x, f) {
  if (t !== undefined) {
    let k = t.k;
    if (x === k) {
      let data = Curry._1(f, Caml_option.some(t.v));
      if (data !== undefined) {
  ...
  let data$1 = Curry._1(f, undefined);
  if (data$1 !== undefined) {
...
let cmpU = Belt_internalMapString.cmpU;

let cmp = Belt_internalMapString.cmp;

let eqU = Belt_internalMapString.eqU;
...
let mergeU = Belt_internalMapString.mergeU;
