...
import * as PervasivesU from "./pervasivesU.js";
...
let prng = Lazy.from_fun(function () {
...
function reset(h) {
  let len = h.data.length;
  if (len === PervasivesU.abs(h.initial_size)) {
    return clear(h);
  } else {
    h.size = 0;
    h.data = Caml_array.make(PervasivesU.abs(h.initial_size), "Empty");
    return;
  }
}
