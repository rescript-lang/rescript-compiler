...
let prng = CamlinternalLazy.from_fun(() => Random.State.make_self_init());
...
function stats(h) {
  let mbl = $$Array.fold_left(((m, b) => Caml.int_max(m, bucket_length(0, b))), 0, h.data);
...
function MakeSeeded(H) {
  let key_index = (h, key) => H.hash(h.seed, key) & (h.data.length - 1 | 0);
  let add = (h, key, data) => {
    let i = key_index(h, key);
    let bucket = {
      TAG: "Cons",
  ...
  let replace_bucket = (key, data, _param) => {
    while (true) {
      let param = _param;
      if (typeof param !== "object") {
        return true;
...
function Make(H) {
  let equal = H.equal;
  let key_index = (h, key) => H.hash(key) & (h.data.length - 1 | 0);
  let add = (h, key, data) => {
    let i = key_index(h, key);
    let bucket = {
      TAG: "Cons",
