...
function succ_aux(x_lo, x_hi) {
  var lo = x_lo + 1 & 4294967295;
  var overflow = neg_signed(x_lo) && !neg_signed(lo) ? 1 : 0;
  var hi = x_hi + overflow & 4294967295;
  return /* Int64 */[
          /* hi */hi,
          /* lo */(lo >>> 0)
        ];
...
function succ(param) {
  return succ_aux(param[/* lo */1], param[/* hi */0]);
}

function add_lo(param, y_lo) {
  var x_lo = param[/* lo */1];
  var y_lo$1 = (y_lo >>> 0);
  var lo = x_lo + y_lo$1 & 4294967295;
  var overflow = neg_signed(x_lo) && (neg_signed(y_lo$1) || !neg_signed(lo)) || neg_signed(y_lo$1) && !neg_signed(lo) ? 1 : 0;
...
function neg(x) {
  if (eq(x, min_int)) {
    ...
    return succ_aux(x[/* lo */1] ^ -1, x[/* hi */0] ^ -1);
  }
}
...
function to_string(self) {
  ...
  if (self_hi === 0) {
    ...
    var approx_div1_times8 = lsl_(approx_div1, 3);
    var approx_div1_times2 = lsl_(approx_div1, 1);
    var x = add(self, neg(approx_div1_times8));
    var rem = add(x, neg(approx_div1_times2));
    var rem_lo = rem[/* lo */1];
    var rem_hi = rem[/* hi */0];
