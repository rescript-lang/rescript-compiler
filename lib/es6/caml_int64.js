...
function lsr_(x, numBits) {
  ...
  if (offset === 0) {
    ...
    return mk((hi << (-offset | 0)) | (x.lo >>> numBits), (hi >>> numBits));
  }
}
...
function xor(param, param$1) {
  return mk(param.lo ^ param$1.lo, param.hi ^ param$1.hi);
}
...
function float_of_bits(x) {
  return (function(lo,hi){ return (new Float64Array(new Int32Array([lo,hi]).buffer))[0]})(x.lo, x.hi);
}
