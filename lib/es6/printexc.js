...
function print_raw_backtrace(outchan, raw_backtrace) {
  ...
  for(var i = 0 ,i_finish = backtrace.length - 1 | 0; i <= i_finish; ++i){
    var match = format_backtrace_slot(i, Caml_array.caml_array_get(backtrace, i));
    if (match !== undefined) {
...
function backtrace_slots(raw_backtrace) {
  ...
  var exists_usable = function (_i) {
    while(true) {
      ...
      if (usable_slot(Caml_array.caml_array_get(match, i))) {
        return true;
      }
      _i = i - 1 | 0;
      continue ;
  ...
  if (exists_usable(match.length - 1 | 0)) {
    return match;
  }
