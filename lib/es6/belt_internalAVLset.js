...
function reduceU(_s, _accu, f) {
  while(true) {
    ...
    _accu = Curry._2(f, reduceU(s.l, accu, f), s.v);
    _s = s.r;
    continue;
  };
}
...
function fillArrayWithFilter(_n, _i, arr, p) {
  while(true) {
    ...
    let rnext = Curry._1(p, v) ? (arr[next] = v, next + 1 | 0) : next;
    if (r === undefined) {
      return rnext;
    }
    _i = rnext;
...
function cmp(s1, s2, cmp$1) {
  ...
  if (len1 === len2) {
    ...
    while(true) {
      ...
      let h2 = e2.hd;
      let h1 = e1.hd;
      let c = Curry._2(cmp$1, h1.v, h2.v);
      if (c !== 0) {
        return c;
      }
