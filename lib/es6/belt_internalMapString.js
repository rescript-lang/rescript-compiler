...
function merge(s1, s2, f) {
  if (s1 !== undefined) {
    if (s1.h >= (
      ...
      return Belt_internalAVLtree.concatOrJoin(merge(l1, match[0], f), v1, f(v1, Caml_option.some(d1), match[1]), merge(r1, match[2], f));
    }
  ...
  return Belt_internalAVLtree.concatOrJoin(merge(match$1[0], l2, f), v2, f(v2, match$1[1], Caml_option.some(d2)), merge(match$1[2], r2, f));
}
...
function fromArray(xs) {
  ...
  let next = Belt_SortArray.strictlySortedLength(xs, ((param, param$1) => {
