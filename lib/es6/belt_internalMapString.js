...
function mergeU(s1, s2, f) {
  if (s1 !== undefined) {
    if (s1.h >= (
      ...
      return Belt_internalAVLtree.concatOrJoin(mergeU(l1, match[0], f), v1, Curry._3(f, v1, Caml_option.some(d1), match[1]), mergeU(r1, match[2], f));
    }
  ...
  return Belt_internalAVLtree.concatOrJoin(mergeU(match$1[0], l2, f), v2, Curry._3(f, v2, match$1[1], Caml_option.some(d2)), mergeU(match$1[2], r2, f));
}
...
function compareAux(_e1, _e2, vcmp) {
  while(true) {
    ...
    let h2 = e2.hd;
    let h1 = e1.hd;
    let c = Caml.string_compare(h1.k, h2.k);
    if (c !== 0) {
      return c;
    }
    let cx = Curry._2(vcmp, h1.v, h2.v);
    if (cx !== 0) {
      return cx;
    }
...
function eqAux(_e1, _e2, eq) {
  while(true) {
    ...
    let h2 = e2.hd;
    let h1 = e1.hd;
    if (!(h1.k === h2.k && Curry._2(eq, h1.v, h2.v))) {
      return false;
    }
    _e2 = Belt_internalAVLtree.stackAllLeft(h2.r, e2.tl);
    _e1 = Belt_internalAVLtree.stackAllLeft(h1.r, e1.tl);
