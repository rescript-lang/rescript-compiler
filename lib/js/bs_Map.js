...
function update0(t, newK, newD, cmp) {
  if (t !== null) {
    var k = t.key;
    var c = cmp(newK, k);
    if (c) {
      ...
      var v = t.value;
      if (c < 0) {
        return Bs_internalAVLtree.bal(update0(l, newK, newD, cmp), k, v, r);
      } else {
        return Bs_internalAVLtree.bal(l, k, v, update0(r, newK, newD, cmp));
      }
    } else {
...
function updateWithOpt0(t, newK, f, cmp) {
  if (t !== null) {
    var k = t.key;
    var c = cmp(newK, k);
    if (c) {
      ...
      var v = t.value;
      if (c < 0) {
        return Bs_internalAVLtree.bal(updateWithOpt0(l, newK, f, cmp), k, v, r);
      } else {
        return Bs_internalAVLtree.bal(l, k, v, updateWithOpt0(r, newK, f, cmp));
      }
    } else {
      var match = f(/* Some */[k]);
      if (match) {
        return Bs_internalAVLtree.updateKV(t, newK, match[0]);
      } else {
        return t;
      }
    }
    ...
    if (c) {
      if (c < 0) {
        ...
        if (ll === l) {
          ...
          return Bs_internalAVLtree.bal(ll, v, t.value, r);
        }
      } else {
        ...
        if (rr === r) {
          ...
          return Bs_internalAVLtree.bal(l, v, t.value, rr);
        }
      }
    } else if (l !== null) {
      if (r !== null) {
...
function updateArray0(h, arr, cmp) {
  var len = arr.length;
  var v = h;
  for(var i = 0 ,i_finish = len - 1 | 0; i <= i_finish; ++i){
    var match = arr[i];
    v = update0(v, match[0], match[1], cmp);
  }
  return v;
}
...
function remove(m, x) {
  var dict = m.dict;
  var data = m.data;
  var newData = remove0(data, x, dict[/* cmp */0]);
  if (newData === data) {
    return m;
  } else {
    return {
...
function update(map, key, data) {
  var dict = map.dict;
  var map$1 = map.data;
  return {
          dict: dict,
          data: update0(map$1, key, data, dict[/* cmp */0])
        };
}

function updateArray(m, e) {
  var dict = m.dict;
  var data = m.data;
  var newData = updateArray0(data, e, dict[/* cmp */0]);
  return {
          dict: dict,
          data: newData
        };
...
function updateWithOpt(map, key, f) {
  var dict = map.dict;
  var map$1 = map.data;
  return {
          dict: dict,
          data: updateWithOpt0(map$1, key, f, dict[/* cmp */0])
        };
}
