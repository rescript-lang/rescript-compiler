...
var randomized = /* record */[/* contents */randomized_default];
...
function find_opt(h, key) {
  ...
  if (match) {
    ...
    var next1 = match[/* next */2];
    if (Caml_obj.caml_equal(key, k1)) {
      return Js_primitive.some(d1);
    } else if (next1) {
      var k2 = next1[/* key */0];
      var d2 = next1[/* data */1];
      var next2 = next1[/* next */2];
      if (Caml_obj.caml_equal(key, k2)) {
        return Js_primitive.some(d2);
      } else if (next2) {
        var k3 = next2[/* key */0];
        var d3 = next2[/* data */1];
        var next3 = next2[/* next */2];
        if (Caml_obj.caml_equal(key, k3)) {
          return Js_primitive.some(d3);
        } else {
          var key$1 = key;
          var _param = next3;
          while(true) {
  ...
  while(true) {
    ...
    if (slot) {
      ...
      if (match !== undefined) {
        ...
        slot[/* data */1] = Js_primitive.valFromOption(match);
        _slot = next;
        _prec = slot;
        continue ;
      } else {
