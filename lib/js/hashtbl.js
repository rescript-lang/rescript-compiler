...
function resize(indexfun, h) {
  ...
  if (nsize < Sys.max_array_length) {
    ...
    var insert_bucket = function (param) {
      if (param) {
        ...
        ndata[nidx] = /* Cons */[
          key,
          param[1],
          Caml_io.caml_array_safe_get(ndata, nidx)
        ];
        return /* () */0;
      }
      else {
    ...
    for(var i = 0 ,i_finish = osize - 1 | 0; i <= i_finish; ++i){
      insert_bucket(Caml_io.caml_array_safe_get(odata, i));
    }
    return /* () */0;
  }
  else {
...
function add(h, key, info) {
  ...
  var bucket_002 = Caml_io.caml_array_safe_get(h[/* data */1], i);
  var bucket = /* Cons */[
    key,
    info,
    bucket_002
...
function remove(h, key) {
  ...
  var i = key_index(h, key);
  h[/* data */1][i] = remove_bucket(Caml_io.caml_array_safe_get(h[/* data */1], i));
  return /* () */0;
}

function find(h, key) {
  var match = Caml_io.caml_array_safe_get(h[/* data */1], key_index(h, key));
  if (match) {
...
function find_all(h, key) {
  ...
  return find_in_bucket(Caml_io.caml_array_safe_get(h[/* data */1], key_index(h, key)));
}
...
function replace(h, key, info) {
  ...
  var i = key_index(h, key);
  var l = Caml_io.caml_array_safe_get(h[/* data */1], i);
  try {
...
function mem(h, key) {
  var _param = Caml_io.caml_array_safe_get(h[/* data */1], key_index(h, key));
  while(true) {
    var param = _param;
    if (param) {
      if (Caml_obj.caml_compare(param[0], key)) {
...
function iter(f, h) {
  ...
  for(var i = 0 ,i_finish = d.length - 1 | 0; i <= i_finish; ++i){
    do_bucket(Caml_io.caml_array_safe_get(d, i));
  }
  return /* () */0;
}
...
function fold(f, h, init) {
  ...
  var accu = init;
  for(var i = 0 ,i_finish = d.length - 1 | 0; i <= i_finish; ++i){
    accu = do_bucket(Caml_io.caml_array_safe_get(d, i), accu);
  }
  return accu;
}
...
function stats(h) {
  ...
  $$Array.iter(function (b) {
        ...
        histo[l] = Caml_io.caml_array_safe_get(histo, l) + 1 | 0;
        return /* () */0;
      }, h[/* data */1]);
  return /* record */[
