...
function find(h, key) {
  ...
  if (match) {
    if (Caml_obj.caml_compare(key, match[0]) === 0) {
      return match[1];
    } else {
      var rest1 = match[2];
      if (rest1) {
        if (Caml_obj.caml_compare(key, rest1[0]) === 0) {
          return rest1[1];
        } else {
          var rest2 = rest1[2];
          if (rest2) {
            if (Caml_obj.caml_compare(key, rest2[0]) === 0) {
              return rest2[1];
            } else {
              var key$1 = key;
              var _param = rest2[2];
              while(true) {
                var param = _param;
                if (param) {
                  if (Caml_obj.caml_compare(key$1, param[0]) === 0) {
                    return param[1];
                  } else {
                    _param = param[2];
                    continue ;
...
function find_all(h, key) {
  var find_in_bucket = function (_param) {
    while(true) {
      ...
      if (param) {
        var rest = param[2];
        if (Caml_obj.caml_compare(param[0], key) === 0) {
          return /* :: */[
                  param[1],
                  find_in_bucket(rest)
                ];
