...
function reset(h) {
  var len = h.data.length;
  if (h.length < 4 || len === h.initial_size) {
    return clear(h);
  } else {
    h.size = 0;
...
function key_index(h, key) {
  if (true || h.length >= 3) {
    ...
    return Caml_external_polyfill.resolve("caml_hash_univ_param")(10, 100, key) % h.data.length;
  }
}
...
function replace(h, key, info) {
  ...
  try {
    return Caml_array.caml_array_set(h.data, i, replace_bucket(l));
  }
  catch (exn){
...
function MakeSeeded(H) {
  var key_index = function (h, key) {
    return Curry._2(H[/* hash */1], h.seed, key) & (h.data.length - 1 | 0);
  };
...
function Make(H) {
  var equal = H[/* equal */0];
  var key_index = function (h, key) {
    return Curry._1(H[/* hash */1], key) & (h.data.length - 1 | 0);
  };
  var add = function (h, key, info) {
    var i = key_index(h, key);
    var bucket_002 = Caml_array.caml_array_get(h.data, i);
