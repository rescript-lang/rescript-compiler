...
function randomize(param) {
  randomized[/* contents */0] = true;
  return /* () */0;
}

function is_randomized(param) {
  return randomized[/* contents */0];
}
...
function reset(h) {
  var len = h[/* data */1].length;
  if (len === Pervasives.abs(h[/* initial_size */3])) {
    return clear(h);
  } else {
    h[/* size */0] = 0;
...
function resize(indexfun, h) {
  ...
  if (nsize >= osize) {
    ...
    var inplace = h[/* initial_size */3] >= 0;
    h[/* data */1] = ndata;
...
function iter(f, h) {
  ...
  var old_trav = h[/* initial_size */3] < 0;
  if (!old_trav) {
