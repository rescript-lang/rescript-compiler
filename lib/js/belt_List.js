...
function unsafeMutateTail0 (l,x){
  l.Arg1 = x;
};

function unsafeTail0 (l){
  return l.Arg1
};

function head(x) {
  if (x !== "[]") {
    return Caml_option.some(x.Arg0);
  }
...
function get(x, n) {
  if (n < 0) {
    ...
    while(true) {
      ...
      if (x$1 !== "[]") {
        if (n$1 === 0) {
          return Caml_option.some(x$1.Arg0);
        } else {
          _n = n$1 - 1 | 0;
          _x = x$1.Arg1;
          continue ;
...
function partitionAux(p, _cell, _precX, _precY) {
  while(true) {
    ...
    if (cell !== "[]") {
      ...
      if (p(h)) {
        unsafeMutateTail0(precX, next);
        _precX = next;
        _cell = t;
        continue ;
      } else {
        unsafeMutateTail0(precY, next);
        _precY = next;
        _cell = t;
        continue ;
      }
...
function splitAux(_cell, _precX, _precY) {
  while(true) {
    ...
    if (cell !== "[]") {
      ...
      unsafeMutateTail0(precX, nextA);
      unsafeMutateTail0(precY, nextB);
      _precY = nextB;
      _precX = nextA;
      _cell = cell.Arg1;
      continue ;
...
function setAssocAuxWithMap(_cellX, x, k, _prec, eq) {
  while(true) {
    ...
    if (cellX !== "[]") {
      ...
      var h = cellX.Arg0;
      if (eq(h[0], x)) {
        unsafeMutateTail0(prec, /* constructor */{
              tag: "::",
              Arg0: /* tuple */[
                x,
                k
...
function take(lst, n) {
  if (n < 0) {
    ...
    var has = takeAux(n - 1 | 0, lst.Arg1, cell);
    if (has) {
      return cell;
    } else {
      return ;
...
function splitAt(lst, n) {
  if (n < 0) {
    ...
    var rest = splitAtAux(n - 1 | 0, lst.Arg1, cell);
    if (rest !== undefined) {
      return /* tuple */[
              cell,
              rest
...
function concat(xs, ys) {
  if (xs !== "[]") {
    ...
    unsafeMutateTail0(copyAuxCont(xs.Arg1, cell), ys);
    return cell;
  } else {
    return ys;
  }
...
function zipByU(l1, l2, f) {
  if (l1 !== "[]" && l2 !== "[]") {
    var cell = /* constructor */{
      tag: "::",
      Arg0: f(l1.Arg0, l2.Arg0),
      Arg1: "[]"
    };
    copyAuxWithMap2(f, l1.Arg1, l2.Arg1, cell);
    return cell;
  } else {
    return "[]";
  }
...
function mapWithIndexU(xs, f) {
  if (xs !== "[]") {
    ...
    copyAuxWithMapI(f, 1, xs.Arg1, cell);
    return cell;
  } else {
    return "[]";
  }
...
function flattenAux(_prec, _xs) {
  while(true) {
    ...
    if (xs !== "[]") {
      _xs = xs.Arg1;
      _prec = copyAuxCont(xs.Arg0, prec);
      continue ;
    } else {
      return unsafeMutateTail0(prec, "[]");
    }
  };
}
  ...
  while(true) {
    ...
    if (xs !== "[]") {
      ...
      if (match !== "[]") {
        ...
        flattenAux(copyAuxCont(match.Arg1, cell), xs.Arg1);
        return cell;
      } else {
        _xs = xs.Arg1;
        continue ;
...
function reduceReverseUnsafeU(l, accu, f) {
  if (l !== "[]") {
    return f(reduceReverseUnsafeU(l.Arg1, accu, f), l.Arg0);
  } else {
    return accu;
  }
}
...
function reduceReverse2UnsafeU(l1, l2, accu, f) {
  if (l1 !== "[]" && l2 !== "[]") {
    return f(reduceReverse2UnsafeU(l1.Arg1, l2.Arg1, accu, f), l1.Arg0, l2.Arg0);
  } else {
    return accu;
  }
}
...
function partitionU(l, p) {
  if (l !== "[]") {
    ...
    var b = p(h);
    partitionAux(p, l.Arg1, nextX, nextY);
    if (b) {
      return /* tuple */[
              nextX,
              unsafeTail0(nextY)
