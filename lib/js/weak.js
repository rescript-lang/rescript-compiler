...
function fill(ar, ofs, len, x) {
  if (ofs < 0 || len < 0 || (ofs + len | 0) > (ar.length - 1 | 0)) {
    throw [
          Caml_builtin_exceptions.invalid_argument,"Weak.fill"
        ];
  }
  else {
...
function Make(H) {
  ...
  var create = function (sz) {
    ...
    return /* record */[
            /* table */Caml_array.caml_make_vect(sz$2, emptybucket),/* hashes */Caml_array.caml_make_vect(sz$2, /* int array */[]),/* limit */7,/* oversize */0,/* rover */0
          ];
  };
  ...
  var add_aux = function (t, setter, d, h, index) {
    ...
    while(true) {
      ...
      if (i >= sz) {
        ...
        if (newsz <= sz) {
          throw [
                Caml_builtin_exceptions.failure,"Weak.Make: hash bucket cannot grow more"
              ];
        }
  ...
  var stats = function (t) {
    ...
    return /* tuple */[
            len,count(t),totlen,lens[0],lens[len / 2 | 0],lens[len - 1 | 0]
          ];
  };
  return /* module */[
          /* create */create,/* clear */clear,/* merge */merge,/* add */add,/* remove */remove,/* find */find,/* find_all */find_all,/* mem */mem,/* iter */iter,/* fold */fold,/* count */count,/* stats */stats
        ];
}
