...
function create(n) {
  ...
  return /* record */[
          /* buffer */s,/* position */0,/* length */n$2,/* initial_buffer */s
        ];
}
...
function sub(b, ofs, len) {
  if (ofs < 0 || len < 0 || ofs > (b[/* position */1] - len | 0)) {
    throw [
          Caml_builtin_exceptions.invalid_argument,"Buffer.sub"
        ];
  }
  else {
...
function blit(src, srcoff, dst, dstoff, len) {
  if (len < 0 || srcoff < 0 || srcoff > (src[/* position */1] - len | 0) || dstoff < 0 || dstoff > (dst.length - len | 0)) {
    throw [
          Caml_builtin_exceptions.invalid_argument,"Buffer.blit"
        ];
  }
  else {
...
function nth(b, ofs) {
  if (ofs < 0 || ofs >= b[/* position */1]) {
    throw [
          Caml_builtin_exceptions.invalid_argument,"Buffer.nth"
        ];
  }
  else {
...
function resize(b, more) {
  ...
  if (new_len > Sys.max_string_length) {
    if ((b[/* position */1] + more | 0) <= Sys.max_string_length) {
      ...
      throw [
            Caml_builtin_exceptions.failure,"Buffer.add: cannot grow buffer"
          ];
    }
  }
...
function add_substring(b, s, offset, len) {
  if (offset < 0 || len < 0 || (offset + len | 0) > s.length) {
    throw [
          Caml_builtin_exceptions.invalid_argument,"Buffer.add_substring/add_subbytes"
        ];
  }
...
function add_channel(b, ic, len) {
  if (len < 0 || len > Sys.max_string_length) {
    throw [
          Caml_builtin_exceptions.invalid_argument,"Buffer.add_channel"
        ];
  }
  ...
  if (start >= lim) {
    ...
    if (c !== 40) {
      if (c !== 123) {
        ...
        return /* tuple */[
                $$String.sub(s, start, stop - start | 0),stop
              ];
      }
      else {
        exit = 1;
    ...
    if (exit === 1) {
      ...
      return /* tuple */[
              $$String.sub(s, new_start, (stop$1 - start | 0) - 1 | 0),stop$1 + 1 | 0
            ];
    }
