...
function insert_bucket(hash, h_buckets, ndata_tail, _, _old_bucket) {
  while(true) {
    ...
    if (old_bucket !== undefined) {
      var nidx = hash(old_bucket[/* key */0]) & (h_buckets.length - 1 | 0);
...
function remove0(hash, eq, h, key) {
  ...
  while(true) {
    ...
    if (buckets !== undefined) {
      var cell_next = buckets[/* next */2];
      if (eq$1(buckets[/* key */0], key$1)) {
        if (prec !== undefined) {
          prec[/* next */2] = cell_next;
        } else {
...
function findOpt0(hash, eq, h, key) {
  ...
  if (match !== undefined) {
    if (eq(key, match[/* key */0])) {
      return /* Some */[match[/* value */1]];
    } else {
      var match$1 = match[/* next */2];
      if (match$1 !== undefined) {
        if (eq(key, match$1[/* key */0])) {
          return /* Some */[match$1[/* value */1]];
        } else {
          var match$2 = match$1[/* next */2];
          if (match$2 !== undefined) {
            if (eq(key, match$2[/* key */0])) {
              return /* Some */[match$2[/* value */1]];
            } else {
              var eq$1 = eq;
              var key$1 = key;
              var _buckets = match$2[/* next */2];
              while(true) {
                var buckets = _buckets;
                if (buckets !== undefined) {
                  var k = buckets[/* key */0];
...
function findAll0(hash, eq, h, key) {
  var find_in_bucket = function (_buckets) {
    while(true) {
      var buckets = _buckets;
      if (buckets !== undefined) {
        if (eq(buckets[/* key */0], key)) {
          return /* :: */[
                  buckets[/* value */1],
                  find_in_bucket(buckets[/* next */2])
                ];
        } else {
