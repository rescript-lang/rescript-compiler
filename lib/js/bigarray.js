...
function of_array$1(kind, layout, data) {
  ...
  var dim2 = dim1 ? Caml_array.caml_array_safe_get(data, 0).length : 0;
  var ba = create$1(kind, layout, dim1, dim2);
  layout !== 0 ? 1 : 0;
  for(var i = 0 ,i_finish = dim1 - 1 | 0; i <= i_finish; ++i){
    var row = Caml_array.caml_array_safe_get(data, i);
    if (row.length !== dim2) {
      throw [
...
function of_array$2(kind, layout, data) {
  var dim1 = data.length;
  var dim2 = dim1 ? Caml_array.caml_array_safe_get(data, 0).length : 0;
  var dim3 = dim2 ? Caml_array.caml_array_safe_get(Caml_array.caml_array_safe_get(data, 0), 0).length : 0;
  ...
  for(var i = 0 ,i_finish = dim1 - 1 | 0; i <= i_finish; ++i){
    ...
    for(var j = 0 ,j_finish = dim2 - 1 | 0; j <= j_finish; ++j){
      var col = Caml_array.caml_array_safe_get(row, j);
      if (col.length !== dim3) {
        throw [
              Caml_builtin_exceptions.invalid_argument,
              "Bigarray.Array3.of_array: non-cubic data"
