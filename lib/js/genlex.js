...
function make_lexer(keywords) {
  ...
  List.iter(s => Hashtbl.add(kwd_table, s, {
    TAG: "Kwd",
    _0: s
  }), keywords);
  ...
  let string = strm__ => {
    while (true) {
      ...
      throw Caml_js_exceptions.internalMakeExn(Stream.Failure);
    };
  };
  let char = strm__ => {
    let c = Stream.peek(strm__);
  ...
  return input => Stream.from(_count => next_token(input));
}
