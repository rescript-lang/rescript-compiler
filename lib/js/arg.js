...
function parse_argv_dynamic($staropt$star, argv, speclist, anonfun, errmsg) {
  var current$1 = $staropt$star !== undefined ? Js_primitive.valFromOption($staropt$star) : current;
  var l = argv.length;
  var b = $$Buffer.create(200);
...
function parse_argv($staropt$star, argv, speclist, anonfun, errmsg) {
  var current$1 = $staropt$star !== undefined ? Js_primitive.valFromOption($staropt$star) : current;
  return parse_argv_dynamic(Js_primitive.some(current$1), argv, [speclist], anonfun, errmsg);
}
