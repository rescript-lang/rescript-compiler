...
function remove(d, v) {
  var oldRoot = d.data;
  if (oldRoot !== null) {
    var newRoot = removeMutateAux(oldRoot, v, d.cmp);
    if (newRoot !== oldRoot) {
      d.data = newRoot;
      return /* () */0;
    } else {
...
function removeCheck(d, v) {
  ...
  if (oldRoot !== null) {
    var removed = [/* false */0];
    var newRoot = removeMutateCheckAux(oldRoot, v, removed, d.cmp);
    if (newRoot !== oldRoot) {
      d.data = newRoot;
    }
    return removed[0];
...
function addCheck(m, e) {
  var oldRoot = m.data;
  var added = [/* false */0];
  var newRoot = addMutateCheckAux(oldRoot, e, added, m.cmp);
  if (newRoot !== oldRoot) {
    m.data = newRoot;
  }
  return added[0];
...
function split(d, key) {
  ...
  var cmp = d.cmp;
  var i = Bs_Sort.binarySearchBy(arr, key, cmp);
  var len = arr.length;
  if (i < 0) {
    var next = (-i | 0) - 1 | 0;
    return /* tuple */[
  ...
  if (match !== null) {
    if (match$1 !== null) {
      ...
      if (cmp(tmp[sizea - 1 | 0], tmp[sizea]) < 0 || cmp(tmp[totalSize - 1 | 0], tmp[0]) < 0) {
        ...
        var k = Bs_Sort.intersect(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0, cmp);
        return {
                cmp: cmp,
  ...
  if (dataa !== null) {
    if (match !== null) {
      ...
      if (cmp(tmp[sizea - 1 | 0], tmp[sizea]) < 0 || cmp(tmp[totalSize - 1 | 0], tmp[0]) < 0) {
        ...
        var k = Bs_Sort.diff(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0, cmp);
        return {
                cmp: cmp,
                data: Bs_internalAVLset.ofSortedArrayAux(tmp2, 0, k)
              };
  ...
  if (dataa !== null) {
    if (datab !== null) {
      ...
      if (cmp(tmp[sizea - 1 | 0], tmp[sizea]) < 0) {
        ...
        var k = Bs_Sort.union(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0, cmp);
        return {
                cmp: cmp,
                data: Bs_internalAVLset.ofSortedArrayAux(tmp2, 0, k)
              };
