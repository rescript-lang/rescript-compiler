...
function split(d, key) {
  ...
  var i = Bs_Sort.binarySearchBy(arr, key, dict[/* cmp */0]);
  var len = arr.length;
  if (i < 0) {
    var next = (-i | 0) - 1 | 0;
    return /* tuple */[
...
function get(d, x) {
  var M = d.dict;
  return Bs_internalAVLset.get0(d.data, x, M[/* cmp */0]);
}

function getUndefined(d, x) {
  var M = d.dict;
  return Bs_internalAVLset.getUndefined0(d.data, x, M[/* cmp */0]);
}

function getExn(d, x) {
  var dict = d.dict;
  return Bs_internalAVLset.getExn0(d.data, x, dict[/* cmp */0]);
}
