...
function new_table(pub_labels) {
  ...
  for(var i = 0 ,i_finish = len - 1 | 0; i <= i_finish; ++i){
    methods[(i << 1) + 3 | 0] = Caml_io.caml_array_safe_get(pub_labels, i);
  }
  return /* record */[
          /* size */2,
          /* methods */methods,
...
function get_method(table, label) {
  try {
    ...
    if (exn === Caml_builtin_exceptions.not_found) {
      return Caml_io.caml_array_safe_get(table[/* methods */1], label);
    }
    else {
      throw exn;
    }
...
function new_methods_variables(table, meths, vals) {
  ...
  for(var i = 0 ,i_finish = nmeths - 1 | 0; i <= i_finish; ++i){
    res[i] = get_method_label(table, Caml_io.caml_array_safe_get(meths$1, i));
  }
  for(var i$1 = 0 ,i_finish$1 = nvals - 1 | 0; i$1 <= i_finish$1; ++i$1){
    res[i$1 + nmeths | 0] = new_variable(table, Caml_io.caml_array_safe_get(vals, i$1));
  }
  return res;
}
...
function init_class(table) {
  ...
  return resize(table, 3 + ((Caml_io.caml_array_safe_get(table[/* methods */1], 1) << 4) / Sys.word_size | 0) | 0);
}
...
function build_path(n, keys, tables) {
  ...
  for(var i = 0; i <= n; ++i){
    r = /* Cons */[
      Caml_io.caml_array_safe_get(keys, i),
      r,
      /* Empty */0
    ];
  }
...
function lookup_keys(i, keys, tables) {
  if (i < 0) {
    ...
    var key = Caml_io.caml_array_safe_get(keys, i);
    var _tables = tables;
    while(true) {
...
function new_cache(table) {
  ...
  var n$1 = n % 2 === 0 || n > (2 + ((Caml_io.caml_array_safe_get(table[/* methods */1], 1) << 4) / Sys.word_size | 0) | 0) ? n : new_method(table);
...
function method_impl(table, i, arr) {
  var next = function () {
    i[0] = i[0] + 1 | 0;
    return Caml_io.caml_array_safe_get(arr, i[0]);
  };
  var clo = next(/* () */0);
...
function set_methods(table, methods) {
  ...
  var i = [0];
  while(i[0] < len) {
    var label = Caml_io.caml_array_safe_get(methods, i[0]);
