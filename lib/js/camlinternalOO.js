...
function new_table(pub_labels) {
  ...
  for(var i = 0 ,i_finish = len - 1 | 0; i <= i_finish; ++i){
    methods[(i << 1) + 3 | 0] = Caml_array.caml_array_get(pub_labels, i);
  }
  return /* record */[
          /* size */2,
          /* methods */methods,
...
function new_methods_variables(table, meths, vals) {
  ...
  for(var i = 0 ,i_finish = nmeths - 1 | 0; i <= i_finish; ++i){
    res[i] = get_method_label(table, Caml_array.caml_array_get(meths$1, i));
  }
  for(var i$1 = 0 ,i_finish$1 = nvals - 1 | 0; i$1 <= i_finish$1; ++i$1){
    res[i$1 + nmeths | 0] = new_variable(table, Caml_array.caml_array_get(vals, i$1));
  }
  return res;
}
