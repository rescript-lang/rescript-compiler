...
function copyBucketReHash(hash, h_buckets, ndata_tail, _old_bucket) {
  while(true) {
    ...
    let nidx = Curry._1(hash, old_bucket.key) & (h_buckets.length - 1 | 0);
...
function set0(h, key, value, eq, hash) {
  ...
  let i = Curry._1(hash, key) & (buckets_len - 1 | 0);
  let l = h_buckets[i];
  if (l !== undefined) {
...
function remove(h, key) {
  let h_buckets = h.buckets;
  let i = Curry._1(h.hash, key) & (h_buckets.length - 1 | 0);
  let bucket = h_buckets[i];
  if (bucket === undefined) {
    return;
  }
  let eq = h.eq;
  if (Curry._2(eq, bucket.key, key)) {
    ...
    while(true) {
      ...
      if (Curry._2(eq, bucket$1.key, key)) {
        prec.next = cell_next;
        h.size = h.size - 1 | 0;
        return;
      }
...
function get(h, key) {
  let h_buckets = h.buckets;
  let nid = Curry._1(h.hash, key) & (h_buckets.length - 1 | 0);
  let cell1 = h_buckets[nid];
  if (cell1 === undefined) {
    return;
  }
  let eq = h.eq;
  if (Curry._2(eq, key, cell1.key)) {
  ...
  if (Curry._2(eq, key, cell2.key)) {
    return Caml_option.some(cell2.value);
  }
  let cell3 = cell2.next;
  if (cell3 !== undefined) {
    if (Curry._2(eq, key, cell3.key)) {
      ...
      while(true) {
        ...
        if (Curry._2(eq, key, buckets.key)) {
