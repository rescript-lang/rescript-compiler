...
function reduceU(_m, _accu, f) {
  while(true) {
    ...
    let l = m.l;
    let r = m.r;
    _accu = Curry._3(f, reduceU(l, accu, f), v, d);
    _m = r;
    continue;
  };
}
...
function eqU(s1, s2, kcmp, veq) {
  ...
  if (len1 === len2) {
    ...
    while(true) {
      ...
      let h2 = e2.hd;
      let h1 = e1.hd;
      if (!(Curry._2(kcmp, h1.k, h2.k) === 0 && Curry._2(veq, h1.v, h2.v))) {
        return false;
      }
      _e2 = stackAllLeft(h2.r, e2.tl);
      _e1 = stackAllLeft(h1.r, e1.tl);
...
function fromArray(xs, cmp) {
  ...
  let next = Belt_SortArray.strictlySortedLengthU(xs, (function (param, param$1) {
    return Curry._2(cmp, param[0], param$1[0]) < 0;
  }));
  let result;
  if (next >= 0) {
