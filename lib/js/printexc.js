...
function print_raw_backtrace(outchan, raw_backtrace) {
  ...
  if (backtrace) {
    ...
    for(var i = 0 ,i_finish = a.length - 1 | 0; i <= i_finish; ++i){
      var match = format_backtrace_slot(i, Caml_io.caml_array_safe_get(a, i));
      if (match) {
  ...
  if (match) {
    ...
    var exists_usable = function (_i) {
      while(true) {
        var i = _i;
        if (i !== -1) {
          if (usable_slot(Caml_io.caml_array_safe_get(backtrace, i))) {
            return /* true */1;
          }
          else {
            _i = i - 1 | 0;
...
var get_raw_backtrace_slot = Caml_io.caml_array_safe_get;
