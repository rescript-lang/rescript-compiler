...
function resize(hash, h) {
  ...
  if (nsize >= osize) {
    ...
    for(var i = 0 ,i_finish = osize - 1 | 0; i <= i_finish; ++i){
      copyBucketReHash(hash, h_buckets, ndata_tail, odata[i]);
    }
...
function replaceFirst(eq, key, info, _cell) {
  while(true) {
    var cell = _cell;
    if (eq(cell.key, key)) {
      cell.value = info;
...
function replaceBy(h, key, value, eq, hash) {
  var h_buckets = h.buckets;
  var buckets_len = h_buckets.length;
  var i = hash(key) & (buckets_len - 1 | 0);
  var l = h_buckets[i];
  if (l !== undefined) {
    if (replaceFirst(eq, key, value, l)) {
      h_buckets[i] = {
        key: key,
        value: value,
        next: l
...
function replace(h, key, value) {
  return replaceBy(h, key, value, h.eq, h.hash);
}
...
function add0(h, key, value, hash) {
  var h_buckets = h.buckets;
  var buckets_lenth = h_buckets.length;
  var i = hash(key) & (buckets_lenth - 1 | 0);
  var bucket = {
    key: key,
    value: value,
    next: h_buckets[i]
