...
function update(t, x, f) {
  var oldRoot = t.data;
  var M = t.dict;
  var newRoot = updateDone(oldRoot, x, f, M[/* cmp */0]);
  if (newRoot !== oldRoot) {
    t.data = newRoot;
    return /* () */0;
  } else {
...
function cmp(m1, m2, cmp$1) {
  var X = m1.dict;
  return Bs_internalAVLtree.cmp(m1.data, m2.data, X[/* cmp */0], cmp$1);
}

function eq(m1, m2, cmp) {
  var M = m1.dict;
  return Bs_internalAVLtree.eq(m1.data, m2.data, M[/* cmp */0], cmp);
}
...
function get(m, x) {
  var X = m.dict;
  return Bs_internalAVLtree.get(m.data, x, X[/* cmp */0]);
}

function getUndefined(m, x) {
  var X = m.dict;
  return Bs_internalAVLtree.getUndefined(m.data, x, X[/* cmp */0]);
}

function getWithDefault(m, x, def) {
  var X = m.dict;
  return Bs_internalAVLtree.getWithDefault(m.data, x, def, X[/* cmp */0]);
}

function getExn(m, x) {
  var X = m.dict;
  return Bs_internalAVLtree.getExn(m.data, x, X[/* cmp */0]);
}

function has(m, x) {
  var X = m.dict;
  return Bs_internalAVLtree.has(m.data, x, X[/* cmp */0]);
}

function ofArray(data, dict) {
  return {
          dict: dict,
          data: Bs_internalAVLtree.ofArray(data, dict[/* cmp */0])
        };
}
