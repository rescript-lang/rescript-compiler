...
function ofSortedArrayUnsafe(xs) {
  return {
          root: Bs_internalAVLset.ofSortedArrayUnsafe0(xs)
        };
}
...
function cmp(d0, d1) {
  return Bs_internalSetString.cmp(d0.root, d1.root);
}

function diff(d0, d1) {
  return {
          root: Bs_internalSetString.diff(d0.root, d1.root)
        };
}

function eq(d0, d1) {
  return Bs_internalSetString.eq(d0.root, d1.root);
}

function findOpt(d, x) {
  return Bs_internalSetString.findOpt(d.root, x);
}

function split(d, p) {
  var match = Bs_internalSetString.split(d.root, p);
  return /* tuple */[
          {
            root: match[0]
          },
...
function subset(a, b) {
  return Bs_internalSetString.subset(a.root, b.root);
}

function inter(a, b) {
  return {
          root: Bs_internalSetString.inter(a.root, b.root)
        };
}

function union(a, b) {
  return {
          root: Bs_internalSetString.union(a.root, b.root)
        };
}
