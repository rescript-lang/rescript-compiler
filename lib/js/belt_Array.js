...
function getExn(arr, i) {
  if (!(i >= 0 && i < arr.length)) {
    throw new Error("File \"belt_Array.ml\", line 41, characters 6-12");
  }
  return arr[i];
}
...
function setExn(arr, i, v) {
  if (!(i >= 0 && i < arr.length)) {
    throw new Error("File \"belt_Array.ml\", line 47, characters 4-10");
  }
  arr[i] = v;
  return /* () */0;
}
...
function reverse(xs) {
  var len = xs.length;
  var result = len > 0 ? new Array(len) : /* array */[];
  for(var i = 0 ,i_finish = len - 1 | 0; i <= i_finish; ++i){
...
function makeByU(l, f) {
  if (l <= 0) {
    ...
    var res = l > 0 ? (f(0), new Array(l)) : /* array */[];
    for(var i = 0 ,i_finish = l - 1 | 0; i <= i_finish; ++i){
      res[i] = f(i);
    }
    return res;
...
function zip(xs, ys) {
  ...
  var s = len > 0 ? new Array(len) : /* array */[];
  for(var i = 0 ,i_finish = len - 1 | 0; i <= i_finish; ++i){
    s[i] = /* tuple */[
      xs[i],
      ys[i]
...
function zipByU(xs, ys, f) {
  ...
  var leny = ys.length;
  var len = lenx < leny ? lenx : leny;
  var s = len > 0 ? (f(xs[0], ys[0]), new Array(len)) : /* array */[];
  for(var i = 0 ,i_finish = len - 1 | 0; i <= i_finish; ++i){
    s[i] = f(xs[i], ys[i]);
  }
  return s;
...
function concat(a1, a2) {
  var l1 = a1.length;
  var l2 = a2.length;
  var a1a2 = l1 > 0 ? new Array(l1 + l2 | 0) : /* array */[];
...
function concatMany(arrs) {
  ...
  var totalLen = 0;
  var firstArrWithLengthMoreThanZero = /* None */0;
  for(var i = 0 ,i_finish = lenArrs - 1 | 0; i <= i_finish; ++i){
    var len = arrs[i].length;
    totalLen = totalLen + len | 0;
    if (len > 0 && firstArrWithLengthMoreThanZero === /* None */0) {
      firstArrWithLengthMoreThanZero = /* Some */[arrs[i]];
    }
    
  }
  var match = firstArrWithLengthMoreThanZero;
  if (match) {
...
function slice(a, offset, len) {
  if (len <= 0) {
    ...
    if (copyLength <= 0) {
      ...
      var result = lena > 0 ? new Array(copyLength) : /* array */[];
...
function mapU(a, f) {
  var l = a.length;
  var r = l > 0 ? (f(a[0]), new Array(l)) : /* array */[];
  for(var i = 0 ,i_finish = l - 1 | 0; i <= i_finish; ++i){
    r[i] = f(a[i]);
  }
  return r;
...
function mapWithIndexU(a, f) {
  var l = a.length;
  var r = l > 0 ? (f(0, a[0]), new Array(l)) : /* array */[];
  for(var i = 0 ,i_finish = l - 1 | 0; i <= i_finish; ++i){
    r[i] = f(i, a[i]);
  }
  return r;
...
function partitionU(a, f) {
  ...
  var i = 0;
  var j = 0;
  var a1 = l > 0 ? new Array(l) : /* array */[];
  var a2 = l > 0 ? new Array(l) : /* array */[];
