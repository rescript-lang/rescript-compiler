...
var Caml_sys_fs              = require("./caml_sys_fs.js");
...
function open_out_gen(mode, perm, name) {
  return Caml_sys_fs.caml_ml_open_descriptor_out(Caml_sys_fs.caml_sys_open(name, mode, perm));
}
...
function open_in_gen(mode, perm, name) {
  return Caml_sys_fs.caml_ml_open_descriptor_in(Caml_sys_fs.caml_sys_open(name, mode, perm));
}
...
function input(ic, s, ofs, len) {
  if (ofs < 0 || len < 0 || ofs > (s.length - len | 0)) {
    ...
    return Caml_sys_fs.caml_ml_input(ic, s, ofs, len);
  }
}
...
function unsafe_really_input(ic, s, _ofs, _len) {
  while(true) {
    ...
    if (len <= 0) {
      ...
      var r = Caml_sys_fs.caml_ml_input(ic, s, ofs, len);
      if (r) {
        _len = len - r | 0;
        _ofs = ofs + r | 0;
        continue ;
...
function input_line(chan) {
  ...
  var scan = function (_accu, _len) {
    while(true) {
      var len = _len;
      var accu = _accu;
      var n = Caml_sys_fs.caml_ml_input_scan_line(chan);
      if (n) {
        if (n > 0) {
          var res = Caml_string.caml_create_string(n - 1 | 0);
          Caml_sys_fs.caml_ml_input(chan, res, 0, n - 1 | 0);
          Caml_sys_fs.caml_ml_input_char(chan);
          if (accu) {
          ...
          Caml_sys_fs.caml_ml_input(chan, beg, 0, -n | 0);
          _len = len - n | 0;
          _accu = /* :: */[
            beg,
            accu
...
function close_in_noerr(ic) {
  try {
    return Caml_sys_fs.caml_ml_close_channel(ic);
  }
  catch (exn){
    return /* () */0;
  }
...
var input_char = Caml_sys_fs.caml_ml_input_char;

var input_byte = Caml_sys_fs.caml_ml_input_char;
...
var close_in = Caml_sys_fs.caml_ml_close_channel;
