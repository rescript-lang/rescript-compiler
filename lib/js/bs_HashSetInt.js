...
function copyBucket(h_buckets, ndata_tail, _, _old_bucket) {
  while(true) {
...
function resize(h) {
  ...
  if (nsize >= osize) {
    ...
    for(var i = 0 ,i_finish = osize - 1 | 0; i <= i_finish; ++i){
      copyBucket(h_buckets, ndata_tail, h, odata[i]);
    }
...
var make = Bs_internalBucketsType.make;

var clear = Bs_internalBucketsType.clear;

var copy = Bs_internalSetBuckets.copy;

var forEach = Bs_internalSetBuckets.forEach0;

var reduce = Bs_internalSetBuckets.reduce0;
