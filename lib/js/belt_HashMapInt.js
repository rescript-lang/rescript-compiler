...
let Caml_hash_primitive = require("./caml_hash_primitive.js");
...
function copyBucketReHash(h_buckets, ndata_tail, _old_bucket) {
  while(true) {
    ...
    let nidx = Caml_hash_primitive.hash_final_mix(Caml_hash_primitive.hash_mix_int(0, old_bucket.key)) & (h_buckets.length - 1 | 0);
...
function set(h, key, value) {
  ...
  let i = Caml_hash_primitive.hash_final_mix(Caml_hash_primitive.hash_mix_int(0, key)) & (buckets_len - 1 | 0);
  let l = h_buckets[i];
  if (l !== undefined) {
...
function remove(h, key) {
  let h_buckets = h.buckets;
  let i = Caml_hash_primitive.hash_final_mix(Caml_hash_primitive.hash_mix_int(0, key)) & (h_buckets.length - 1 | 0);
...
function get(h, key) {
  let h_buckets = h.buckets;
  let nid = Caml_hash_primitive.hash_final_mix(Caml_hash_primitive.hash_mix_int(0, key)) & (h_buckets.length - 1 | 0);
...
let forEachU = Belt_internalBuckets.forEachU;

let forEach = Belt_internalBuckets.forEach;

let reduceU = Belt_internalBuckets.reduceU;

let reduce = Belt_internalBuckets.reduce;

let keepMapInPlaceU = Belt_internalBuckets.keepMapInPlaceU;
