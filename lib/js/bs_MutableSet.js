...
function split(d, key) {
  ...
  var cmp = d.cmp;
  var i = Bs_SortArray.binarySearchByU(arr, key, cmp);
  var len = arr.length;
  if (i < 0) {
    var next = (-i | 0) - 1 | 0;
    return /* tuple */[
  ...
  if (match !== null) {
    if (match$1 !== null) {
      ...
      if (cmp(tmp[sizea - 1 | 0], tmp[sizea]) < 0 || cmp(tmp[totalSize - 1 | 0], tmp[0]) < 0) {
        ...
        var k = Bs_SortArray.intersectU(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0, cmp);
        return {
                cmp: cmp,
  ...
  if (dataa !== null) {
    if (match !== null) {
      ...
      if (cmp(tmp[sizea - 1 | 0], tmp[sizea]) < 0 || cmp(tmp[totalSize - 1 | 0], tmp[0]) < 0) {
        ...
        var k = Bs_SortArray.diffU(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0, cmp);
        return {
                cmp: cmp,
                data: Bs_internalAVLset.ofSortedArrayAux(tmp2, 0, k)
              };
  ...
  if (dataa !== null) {
    if (datab !== null) {
      ...
      if (cmp(tmp[sizea - 1 | 0], tmp[sizea]) < 0) {
        ...
        var k = Bs_SortArray.unionU(tmp, 0, sizea, tmp, sizea, sizeb, tmp2, 0, cmp);
        return {
                cmp: cmp,
                data: Bs_internalAVLset.ofSortedArrayAux(tmp2, 0, k)
              };
