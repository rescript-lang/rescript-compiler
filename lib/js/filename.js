...
function quote(x) {
  ...
  for(let i = 0; i < l; ++i){
    if (Caml_string.get(x, i) === /* '\'' */39) {
      Buffer.add_string(b, quotequote);
    } else {
      Buffer.add_char(b, Caml_string.get(x, i));
    }
  }
  Buffer.add_char(b, /* '\'' */39);
...
function basename(x) {
  return generic_basename(is_dir_sep, current_dir_name, x);
}

function dirname(x) {
  return generic_dirname(is_dir_sep, current_dir_name, x);
}
...
function check_suffix$1(name, suff) {
  ...
  return $$String.lowercase_ascii(s) === $$String.lowercase_ascii(suff);
}
...
function basename$2(x) {
  return generic_basename(is_dir_sep$1, current_dir_name$2, x);
}

function dirname$2(x) {
  return generic_dirname(is_dir_sep$1, current_dir_name$2, x);
}
...
function concat(dirname, filename) {
  ...
  if (l === 0 || is_dir_sep$2(dirname, l - 1 | 0)) {
...
function extension_len(name) {
  ...
  while(true) {
    ...
    if (Caml_string.get(name, i) === /* '.' */46) {
      ...
      while(true) {
        let i$1 = _i$1;
        if (i$1 < 0 || is_dir_sep$2(name, i$1)) {
          return 0;
        }
