...
function binarySearchByU(sorted, key, cmp) {
  ...
  let lo = sorted[0];
  let c = Curry._2(cmp, key, lo);
  if (c < 0) {
    return -1;
  }
  let hi = sorted[len - 1 | 0];
  let c2 = Curry._2(cmp, key, hi);
  if (c2 > 0) {
    return -(len + 1 | 0) | 0;
  } else {
    let _lo = 0;
    ...
    while(true) {
      ...
      let c$1 = Curry._2(cmp, key, midVal);
      if (c$1 === 0) {
        return mid;
      }
      if (c$1 < 0) {
        if (hi$1 === mid) {
          if (Curry._2(cmp, sorted[lo$1], key) === 0) {
            return lo$1;
          } else {
            return -(hi$1 + 1 | 0) | 0;
          }
      ...
      if (lo$1 === mid) {
        if (Curry._2(cmp, sorted[hi$1], key) === 0) {
          return hi$1;
        } else {
          return -(hi$1 + 1 | 0) | 0;
        }
