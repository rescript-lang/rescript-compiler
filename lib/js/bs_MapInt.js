...
function merge(f, s1, s2) {
  ...
  if (s1 !== null) {
    if (s1.h >= Bs_internalAVLtree.height(s2)) {
      ...
      return Bs_internalAVLtree.concatOrJoin(merge(f, l1, match[0]), v1, f(v1, /* Some */[d1], match[1]), merge(f, r1, match[2]));
    } else {
      exit = 1;
    }
  } else if (s2 !== null) {
  ...
  if (exit === 1) {
    if (s2 !== null) {
      ...
      return Bs_internalAVLtree.concatOrJoin(merge(f, match$1[0], l2), v2, f(v2, match$1[1], /* Some */[d2]), merge(f, match$1[2], r2));
    } else {
