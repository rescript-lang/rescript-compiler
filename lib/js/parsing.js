        ...
        case 2 : 
            ...
            try {
              match$1 = /* tuple */[
                ...
                Curry._1(Caml_array.caml_array_safe_get(tables[/* actions */0], env[/* rule_number */12]), env)
              ];
            }
            catch (exn){
              if (exn === Parse_error) {
    ...
    if (exn$1[0] === YYexit) {
      ...
      current_lookahead_fun[0] = function (tok) {
        if (tok.length !== undefined) {
          return +(Caml_array.caml_array_safe_get(tables[/* transl_block */2], tok.tag | 0) === curr_char);
        }
        else {
          return +(Caml_array.caml_array_safe_get(tables[/* transl_const */1], tok) === curr_char);
        }
      };
      throw exn$1;
    }
...
function peek_val(env, n) {
  return Caml_array.caml_array_safe_get(env[/* v_stack */1], env[/* asp */10] - n | 0);
}
...
function symbol_start_pos() {
  ...
  while(true) {
    var i = _i;
    if (i <= 0) {
      return Caml_array.caml_array_safe_get(env[/* symb_end_stack */3], env[/* asp */10]);
    }
    else {
      var st = Caml_array.caml_array_safe_get(env[/* symb_start_stack */2], (env[/* asp */10] - i | 0) + 1 | 0);
      var en = Caml_array.caml_array_safe_get(env[/* symb_end_stack */3], (env[/* asp */10] - i | 0) + 1 | 0);
...
function rhs_start_pos(n) {
  return Caml_array.caml_array_safe_get(env[/* symb_start_stack */2], env[/* asp */10] - (env[/* rule_len */11] - n | 0) | 0);
}

function rhs_end_pos(n) {
  return Caml_array.caml_array_safe_get(env[/* symb_end_stack */3], env[/* asp */10] - (env[/* rule_len */11] - n | 0) | 0);
}
