      ...
      if (typeof match === "number") {
        if (match !== 25) {
          ...
          if ($$String.contains(name$1, /* "/" */47)) {
            ...
            var tmp;
            try {
              tmp = Caml_missing_polyfill.not_implemented("caml_sys_unsafe_getenv");
            }
            catch (exn$1){
...
function map_file(fd, $staropt$star, kind, layout, shared, dims) {
  ...
  return Caml_missing_polyfill.not_implemented("caml_unix_map_file_bytecode");
}
...
function file_descr_not_standard(_fd) {
  while(true) {
    ...
    if (fd >= 3) {
      ...
      _fd = Caml_missing_polyfill.not_implemented("unix_dup");
      continue ;
    }
  };
}
...
function open_process_in(cmd) {
  ...
  var inchan = Caml_io.caml_ml_open_descriptor_in(match[0]);
  try {
...
function open_process_out(cmd) {
  ...
  var outchan = Caml_io.caml_ml_open_descriptor_out(match[1]);
  try {
...
function open_process(cmd) {
  ...
  var match$1;
  try {
    match$1 = Caml_missing_polyfill.not_implemented("unix_pipe");
  }
  catch (e){
    Caml_missing_polyfill.not_implemented("unix_close");
    Caml_missing_polyfill.not_implemented("unix_close");
...
function open_process_full(cmd, env) {
  ...
  var match$2;
  try {
    match$2 = Caml_missing_polyfill.not_implemented("unix_pipe");
  }
  catch (e$1){
    Caml_missing_polyfill.not_implemented("unix_close");
    Caml_missing_polyfill.not_implemented("unix_close");
...
function unsafe_environment(prim) {
  return Caml_missing_polyfill.not_implemented("unix_environment_unsafe");
}

var getenv = Caml_sys.caml_sys_getenv;

function unsafe_getenv(prim) {
  return Caml_missing_polyfill.not_implemented("caml_sys_unsafe_getenv");
}
...
function has_symlink(prim) {
  return Caml_missing_polyfill.not_implemented("unix_has_symlink");
}
