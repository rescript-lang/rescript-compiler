...
function from_function(f) {
  let partial_arg = Caml_bytes.create(512);
  return {
          refill_buff: (function (param) {
              let read = Curry._2(f, partial_arg, partial_arg.length);
              let n = read > 0 ? read : (param.lex_eof_reached = true, 0);
              if ((param.lex_buffer_len + n | 0) > param.lex_buffer.length) {
                if (((param.lex_buffer_len - param.lex_start_pos | 0) + n | 0) <= param.lex_buffer.length) {
                  ...
                  let newlen = (param.lex_buffer.length << 1);
