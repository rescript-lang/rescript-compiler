...
function heightUpdateMutate(t) {
  var hlt = Bs_internalAVLset.height(t.left);
  var hrt = Bs_internalAVLset.height(t.right);
  t.h = (
    hlt > hrt ? hlt : hrt
  ) + 1 | 0;
  return t;
...
function balMutate(nt) {
  var l = nt.left;
  var r = nt.right;
  var hl = Bs_internalAVLset.height(l);
  var hr = Bs_internalAVLset.height(r);
  if (hl > (2 + hr | 0)) {
    var ll = l.left;
    var lr = l.right;
    if (Bs_internalAVLset.height(ll) >= Bs_internalAVLset.height(lr)) {
    ...
    var rl = r.left;
    var rr = r.right;
    if (Bs_internalAVLset.height(rr) >= Bs_internalAVLset.height(rl)) {
...
function removeMutateAux(nt, x) {
  ...
  if (x === k) {
    ...
    if (l !== null) {
      if (r !== null) {
        nt.key = Bs_internalAVLset.min0Aux(r);
