...
function updateU(t, newK, f, cmp) {
  if (t !== undefined) {
    ...
    let c = Curry._2(cmp, newK, k);
    if (c === 0) {
      let newD = Curry._1(f, Caml_option.some(t.v));
      if (newD !== undefined) {
  ...
  let newD$1 = Curry._1(f, undefined);
  if (newD$1 !== undefined) {
...
function mergeU(s1, s2, f, cmp) {
  if (s1 === undefined) {
    if (s2 !== undefined) {
      return Belt_internalAVLtree.keepMapU(s2, (function (k, v) {
        return Curry._3(f, k, undefined, Caml_option.some(v));
      }));
    } else {
      return;
    }
  ...
  if (s2 === undefined) {
    return Belt_internalAVLtree.keepMapU(s1, (function (k, v) {
      return Curry._3(f, k, Caml_option.some(v), undefined);
    }));
  }
  if (s1.h >= s2.h) {
    let v1 = s1.k;
    ...
    let newLeft = mergeU(l1, match[0], f, cmp);
    let newD = Curry._3(f, v1, Caml_option.some(d1), d2$1);
    let newRight = mergeU(r1, match[1], f, cmp);
  ...
  let newLeft$1 = mergeU(match$1[0], l2, f, cmp);
  let newD$1 = Curry._3(f, v2, d1$2, Caml_option.some(d2$2));
  let newRight$1 = mergeU(match$1[1], r2, f, cmp);
...
let cmpU = Belt_internalAVLtree.cmpU;

let cmp = Belt_internalAVLtree.cmp;

let eqU = Belt_internalAVLtree.eqU;
