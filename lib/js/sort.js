  ...
  if (l1) {
    ...
    if (l2) {
      ...
      if (Curry._2(order, h1, h2)) {
        return /* Nested :: */[
                h1,merge(order, l1[1], l2)
              ];
      }
      else {
        return /* Nested :: */[
                h2,merge(order, l1, l2[1])
              ];
      }
    }
    else {
...
function list(order, l) {
  ...
  var merge2 = function (x) {
    if (x) {
      ...
      if (match) {
        return /* Nested :: */[
                merge(order, x[0], match[0]),merge2(match[1])
              ];
      }
      else {
        return x;
    ...
    while(true) {
      ...
      if ((hi - lo | 0) >= 6) {
        ...
        if (!Curry._2(cmp, pivot, arr[hi]) || !Curry._2(cmp, arr[lo], pivot)) {
          throw [
                Caml_builtin_exceptions.invalid_argument,"Sort.array"
              ];
        }
        while(i < j) {
