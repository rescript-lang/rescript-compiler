...
function add(t, x, data) {
  if (t !== null) {
    ...
    return Bs_internalAVLtree.singleton(x, data);
  }
}
...
function mergeU(s1, s2, f) {
  ...
  if (s1 !== null) {
    if (s1.h >= (
      ...
      return Bs_internalAVLtree.concatOrJoin(mergeU(l1, match[0], f), v1, f(v1, /* Some */[d1], match[1]), mergeU(r1, match[2], f));
    } else {
      exit = 1;
    }
  } else if (s2 !== null) {
  ...
  if (exit === 1) {
    if (s2 !== null) {
      ...
      return Bs_internalAVLtree.concatOrJoin(mergeU(match$1[0], l2, f), v2, f(v2, match$1[1], /* Some */[d2]), mergeU(match$1[2], r2, f));
    } else {
...
function cmpU(s1, s2, cmp) {
  var len1 = Bs_internalAVLtree.size(s1);
  var len2 = Bs_internalAVLtree.size(s2);
  if (len1 === len2) {
    return compareAux(Bs_internalAVLtree.stackAllLeft(s1, /* [] */0), Bs_internalAVLtree.stackAllLeft(s2, /* [] */0), cmp);
  } else if (len1 < len2) {
    return -1;
  } else {
    return 1;
...
function eqU(s1, s2, eq) {
  ...
  var len2 = Bs_internalAVLtree.size(s2);
  if (len1 === len2) {
    return eqAux(Bs_internalAVLtree.stackAllLeft(s1, /* [] */0), Bs_internalAVLtree.stackAllLeft(s2, /* [] */0), eq);
  } else {
    return /* false */0;
  }
}
...
function ofArray(xs) {
  var len = xs.length;
  if (len) {
    var next = Bs_SortArray.strictlySortedLengthU(xs, (function (param, param$1) {
