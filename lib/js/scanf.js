...
function kscanf(ib, ef, param) {
  ...
  var k = function (readers, f) {
    ...
    try {
      ...
      var exc$1 = Js_exn.internalToOCamlException(exc);
      if (exc$1[0] === Scan_failure) {
        match = /* Exc */Block.__(1, [exc$1]);
      } else if (exc$1[0] === Caml_builtin_exceptions.failure) {
        match = /* Exc */Block.__(1, [exc$1]);
      } else if (exc$1 === Caml_builtin_exceptions.end_of_file) {
        match = /* Exc */Block.__(1, [exc$1]);
      } else if (exc$1[0] === Caml_builtin_exceptions.invalid_argument) {
        var s = exc$1[1] + (" in format \"" + ($$String.escaped(str) + "\""));
        throw [
              Caml_builtin_exceptions.invalid_argument,
              s
            ];
