...
function sortedLengthAuxMore(xs, _prec, _acc, len, cmp) {
  while(true) {
    var acc = _acc;
    var prec = _prec;
    if (acc >= len) {
...
function strictlySortedLength(xs, cmp) {
  var len = xs.length;
  if (len === 0 || len === 1) {
    return len;
  } else {
    ...
    if (c < 0) {
      ...
      return -sortedLengthAuxMore(xs, x1, 2, len, cmp) | 0;
    } else {
      return 1;
    }
  }
  ...
  while(true) {
    ...
    if (c < 0) {
      ...
      if (i2$1 < src2r) {
        ...
        Caml_array.caml_array_blit(src, i1, dst, d$2, src1r - i1 | 0);
      ...
      if (i1$2 < src1r && i2$2 < src2r) {
        ...
        Caml_array.caml_array_blit(src2, i2$2, dst, d$3, src2r - i2$2 | 0);
        return (d$3 + src2r | 0) - i2$2 | 0;
      } else {
        Caml_array.caml_array_blit(src, i1$2, dst, d$3, src1r - i1$2 | 0);
...
function inter(src, src1ofs, src1len, src2, src2ofs, src2len, dst, dstofs, cmp) {
  var src1r = src1ofs + src1len | 0;
  var src2r = src2ofs + src2len | 0;
  var _i1 = src1ofs;
  var _s1 = src[src1ofs];
...
function binSearch(sorted, key, cmp) {
